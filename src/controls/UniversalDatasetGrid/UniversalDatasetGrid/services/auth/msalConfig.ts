...
export const loginRequest = {
   ...
   *
   * SDAP OAuth Architecture (OBO Flow):
   * 1. PCF requests ONLY BFF API scope (user_impersonation)
   * 2. PCF sends token (Token A) to BFF API
   * 3. BFF API exchanges Token A for Graph token (Token B) via OBO with .default scope
   * 4. Token B includes all admin-consented Graph permissions (FileStorageContainer.Selected, Files.Read.All, etc.)
   *
