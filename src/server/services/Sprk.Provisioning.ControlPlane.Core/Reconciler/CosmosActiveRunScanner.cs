...
public sealed class CosmosActiveRunScanner : IActiveRunScanner
{
    ...
    // enum name ("Running" / "WaitingOnGate" / etc.). The Cosmos SDK's DEFAULT
    // serializer (CosmosModule.cs — no custom STJ CosmosSerializer registered)
    // is Newtonsoft-based, so this string form is produced by the Newtonsoft
    // [JsonConverter(typeof(Newtonsoft.Json.Converters.StringEnumConverter))]
    // attribute on RunStatus, NOT by the STJ JsonStringEnumConverter attribute
    // (which is silently ignored on the actual write path — task 106 /
    // DS-5 §C4.5). Without the Newtonsoft attribute, Newtonsoft writes the
    // enum as an integer and this filter permanently returns zero rows. This
    // filter is index-friendly (composite range index on `status` per
    // cosmos-provisioning.bicep §6.2 index policy).
