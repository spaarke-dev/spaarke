...
public static class ComposeModule
{
    public static IServiceCollection AddComposeModule(this IServiceCollection services)
    {
        ...
        services.AddSingleton<ComposeShadowPatchEngine>();                      // FR-04/FR-11/D5 (task 030) — the SINGLE unified byte-author of the Shadow Document Architecture: applies an ID-anchored ComposeOperationLog (paraId+runIndex+run-local-offset) surgically onto retained OOXML (byte[]->byte[]), emitting native w:ins/w:del/w:comment with ZERO write-path text-search (I-7). Migrates the EDGE-1..4 wisdom from DocxAnnotationWriter (comment-before-trackchange ordering; w:delText; monotonic revision-id seeding). Consolidates the two retiring writers (DocxAnnotationWriter + ComposeParagraphRedlineSynthesizer) — text/mark/comment ops + core resolve-split-emit; structural paragraph ops are task 031. NOT yet routed through ComposeService.SaveAsync (task 032). No external NuGet (DocumentFormat.OpenXml already referenced; task-005 A/B rejected Docxodus). Pure — no AI internals (ADR-013 Tier-1), no Microsoft.Graph (ADR-007); thread-safe stateless singleton (ADR-010); registered UNCONDITIONALLY (symmetric per bff-extensions.md §F.1)
