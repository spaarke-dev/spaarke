...
public static class ComposeModule
{
    public static IServiceCollection AddComposeModule(this IServiceCollection services)
    {
        ...
        services.AddSingleton<ComposeParagraphSpliceService>();                 // FR-02 (task 020, E1) — pure paraId-keyed edited-paragraph rebuild+splice (byte[]->byte[]); thread-safe stateless singleton (ADR-010). Consumes ComposeParaIdSpliceMap (task 012, FR-12); produces the spliced-edited doc task 021's WmlComparer diffs. Registered UNCONDITIONALLY (no feature gate — symmetric per bff-extensions.md §F.1)
        services.AddSingleton<ComposeRedlineComparerService>();                 // FR-03/FR-05 (task 021, E1) — thin Docxodus WmlComparer adapter (byte[]->byte[]); synthesizes minimal w:ins/w:del + rPr/pPrChange between the retained original and the task-020 spliced-edited doc. Diff path = WmlComparer only (no HtmlToWml/FormattingAssembler — SkiaSharp exclusion). Thread-safe stateless singleton (ADR-010); consumed by task 022 SaveAsync. Registered UNCONDITIONALLY (symmetric per bff-extensions.md §F.1)
