...
public static class ComposeModule
{
    public static IServiceCollection AddComposeModule(this IServiceCollection services)
    {
        ...
        services.AddSingleton<ComposeTemplatePartMergeEngine>();                // FR-05 (spaarkeai-compose-r6 task 030) — house-style chrome engine: merges the editor's rendered docx BODY into a firm/matter .dotx via DIRECT OOXML part-merge (template-as-base; styles/numbering/theme/headers/footers/sectPr from the template, body from the editor; numId remap preserves NumberingComputationEngine identity; NOT altChunk per FR-05). byte[]-in/byte[]-out (ADR-007), no AI internals (ADR-013), no dispatch (ADR-039). No external NuGet (DocumentFormat.OpenXml 3.5.1 already referenced). Consumed by the task-032 template-merge endpoint; thread-safe stateless singleton (ADR-010); registered UNCONDITIONALLY (symmetric per bff-extensions.md §F.1)
        services.AddSingleton<ComposePdfModelProjector>();                    // FR-06 (spaarkeai-compose-r6 task 040) — PDF intake projector: maps the PublicContracts DocumentLayout (Azure DI prebuilt-layout via the IComposePdfIntakeSource facade) into the ONE canonical ComposeContentModel with counted pdf-intake-* degradations (honest lossiness — never a construct hard-fail). Pure deterministic mapping — no AI internals (ADR-013; consumes only the neutral facade DTO), no Graph (ADR-007), no dispatch (ADR-039). Consumed by ComposeService.LoadAsync's PDF branch; thread-safe stateless singleton (ADR-010); registered UNCONDITIONALLY (symmetric per bff-extensions.md §F.1 — the gated piece is the intake FACADE, not this projector)
