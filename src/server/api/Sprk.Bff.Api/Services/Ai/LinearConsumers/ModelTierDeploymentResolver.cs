...
/// <summary>
/// Resolves an <see cref="AiModelTier"/> (the catalog vocabulary, ADR-039) to a concrete Azure OpenAI
/// deployment name (the runtime routing surface — config, not catalog per ADR-039). Completes the
...
/// </para>
/// <para>
/// <b>Single routing surface (ADR-039)</b>: this is the ONLY tier→deployment mapping on the
...
public static class ModelTierDeploymentResolver
{
    ...
    /// tier (the platform default per ADR-039). Reasoning falls back to
