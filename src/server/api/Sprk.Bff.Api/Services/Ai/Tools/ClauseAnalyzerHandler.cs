...
public sealed class ClauseAnalyzerHandler : IAnalysisToolHandler
{
    ...
    /// <summary>
    /// Executes clause analysis using the JPS-rendered prompt via PromptSchemaRenderer,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation(
                "Starting JPS-based clause analysis for analysis {AnalysisId}, document {DocumentId}",
                context.AnalysisId, context.Document.DocumentId);
            ...
            _logger.LogDebug(
                "Executing JPS clause analysis, format: {Format}, structuredOutput: {StructuredOutput}, estimated tokens: {Tokens}",
            ...
            if (useStructuredOutput)
            {
                ...
                var schemaName = rendered.SchemaName ?? "clause_analysis";
            ...
            _logger.LogInformation(
                "JPS clause analysis complete for {AnalysisId} in {Duration}ms",
        ...
        {
            _logger.LogWarning("JPS clause analysis cancelled for analysis {AnalysisId}", context.AnalysisId);
            return ToolResult.Error(
                HandlerId,
                tool.Id,
                tool.Name,
        ...
        {
            _logger.LogError(ex, "JPS clause analysis failed for analysis {AnalysisId}", context.AnalysisId);
            return ToolResult.Error(
                HandlerId,
                tool.Id,
                tool.Name,
