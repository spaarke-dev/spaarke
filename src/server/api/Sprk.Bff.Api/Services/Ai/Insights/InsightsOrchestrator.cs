...
public sealed class InsightsOrchestrator : IInsightsAi
{
    ...
    /// goes through the <c>insights-ask</c> Binding rows (FR-P3-01 hard cutover; ADR-039
    /// single routing surface) — the same catalog the <c>/api/insights/ask</c> endpoint
    /// uses. Deployment MUST seed an enabled <c>sprk_playbookconsumer</c> row
    /// (consumerType <c>insights-ask</c>, <c>sprk_consumercode</c> = this name) whose
    /// <c>sprk_playbook</c> lookup targets the Dataverse <c>sprk_analysisplaybook</c> row
    /// generated by <c>Deploy-Playbook.ps1</c> in the target environment.
    /// </summary>
    /// <remarks>
    /// The former snake_case_only constraint (<c>universal_ingest_v1</c>) was a POSIX
    /// env-var artifact of the deleted config map. <c>sprk_consumercode</c> is an
    /// unconstrained Dataverse text column, so the consumer code now matches the playbook's
    /// operational identity (<c>sprk_name</c>) verbatim.
    /// </remarks>
    ...
    private async IAsyncEnumerable<AssistantQueryChunk> StreamPlaybookPathAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        // Same resolution + error path as the single-shot handler (FR-P3-01: insights-ask
        // Binding rows, EXACT consumer-code match, 'default' row for a null hint). On
        // unresolved it throws InvalidOperationException with the canonical "Default
        // playbook" message token; the endpoint catches and maps to 503 with
        ...
        var (canonicalName, playbookId) = await _assistantHandler
            .ResolveInsightsPlaybookAsync(routing.ClassifierPlaybookHint, cancellationToken)
        ...
        CancellationToken cancellationToken)
    {
        ...
        //
        // FR-P3-01 (task 040): the playbook Guid is resolved at invocation time through the
        // insights-ask Binding rows (ADR-039 single routing surface — the config map is
        // gone). The canonical name `universal-ingest@v1` is the operational identity per
        // D-P15-02 "ONE canonical universal-ingest playbook"; the per-env Guid is the
        // sprk_playbook lookup on the Binding row seeded after Deploy-Playbook.ps1.
        //
        // EXACT-code check: ResolveBindingAsync falls back to the 'default' insights-ask
        // row on a miss — that row targets the Assistant default playbook, NOT
        // universal-ingest, so a code mismatch is treated as "not registered" and fails
        // honestly rather than running the wrong playbook.
        var ingestBinding = await _consumerRouting.ResolveBindingAsync(
            ConsumerTypes.InsightsAsk,
            consumerCode: UniversalIngestPlaybookCanonicalName,
            cancellationToken: cancellationToken).ConfigureAwait(false);

        if (ingestBinding is null
            || !string.Equals(ingestBinding.ConsumerCode, UniversalIngestPlaybookCanonicalName, StringComparison.OrdinalIgnoreCase)
            || ingestBinding.PlaybookId is not { } playbookId
            || playbookId == Guid.Empty)
        {
            _logger.Log(
                LogLevel.Error,
                IngestPlaybookFailedEvent,
                "InsightsOrchestrator.RunIngestAsync: universal-ingest@v1 has no enabled sprk_playbookconsumer row (consumerType 'insights-ask', consumerCode '{CanonicalName}') with a sprk_playbook target in this environment. Seed it after Deploy-Playbook.ps1 (FR-P3-01 removed the config map). documentId={DocumentId} matterId={MatterId} tenantId={TenantId}",
                UniversalIngestPlaybookCanonicalName, request.DocumentId, request.MatterId, request.TenantId);
            throw new InvalidOperationException(
                $"universal-ingest@v1 has no enabled sprk_playbookconsumer row (consumerType " +
                $"'insights-ask', consumerCode '{UniversalIngestPlaybookCanonicalName}'). Deploy the " +
                "playbook via scripts/Deploy-Playbook.ps1, then seed the insights-ask Binding row " +
                "targeting its sprk_analysisplaybook Guid (FR-P3-01 removed the config map).");
        }
