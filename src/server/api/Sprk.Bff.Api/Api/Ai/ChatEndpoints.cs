...
public static class ChatEndpoints
{
        ...
        ILogger<SprkChatAgentFactory> logger)
    {
        ...
        try
        {
            ...
            {
                // === R7 Wave 12.3 Phase 12.3a (2026-07-03): explicit-keyword auto-dispatch ===
                // Per operator direction: for NL messages, only auto-dispatch when we have 100%
                // confidence via explicit-keyword match. Anything less → candidate options (PhaseB).
                //
                // Keyword match here means: message contains a whole-word intent keyword that
                // deterministically identifies a single Linear consumer. Currently only
                // "summarize" / "summary" → chat-summarize. As we add more Linear consumers
                // (Phase 12.4 slash-menu schema), this list becomes data-driven from
                // sprk_analysisplaybook.sprk_intenttriggers.
                //
                // Client handles linear_dispatch by invoking the endpoint immediately (like
                ...
                if (explicitConsumerType is not null)
                {
                    ...
                    if (session.UploadedFiles is { Count: > 0 })
                    {
                        for (var i = 0; i < request.Attachments.Count && i < session.UploadedFiles.Count; i++)
                        {
                            ...
                            {
                                sessionAttachmentIdsForDispatch.Add(fileId);
                            }
                        }
                    }

                    var dispatchUrl = explicitConsumerType switch
                    {
                        Sprk.Bff.Api.Services.Ai.PublicContracts.ConsumerTypes.ChatSummarize
                            => $"/api/ai/chat/sessions/{sessionId}/summarize",
                        _ => null,
                    };
                    ...
                    if (dispatchUrl is not null && sessionAttachmentIdsForDispatch.Count > 0)
                    {
                        var requestBodyJson = JsonSerializer.Serialize(
                            new
                            {
                                fileIds = sessionAttachmentIdsForDispatch,
                                style = (string?)null,
                            },
                        ...
                        var linearDispatchData = new Services.Ai.Chat.SseEventTypes.LinearDispatchSseEventData(
                            ConsumerType: explicitConsumerType,
                            DispatchUrl: dispatchUrl,
                            RequestBody: requestBodyJson,
                            Reason: $"explicit-keyword-match:{explicitConsumerType}",
                            SessionAttachmentIds: sessionAttachmentIdsForDispatch);

                        var linearDispatchEvent = Services.Ai.Chat.SseEventTypes.ChatSseEventFactory
                            .CreateLinearDispatchEvent(linearDispatchData);
                        await WriteChatSSEAsync(response, linearDispatchEvent, cancellationToken);
                        await WriteChatSSEAsync(response, new ChatSseEvent("done", null), cancellationToken);

                        // Persist user message so history reflects the turn.
                        var seqBaseForDispatch = session.Messages.Count;
                        var linearDispatchUserMessage = new DvChatMessage(
                            MessageId: Guid.NewGuid().ToString("N"),
                            SessionId: sessionId,
                            Role: ChatMessageRole.User,
                            Content: request.Message,
                        ...
                        await historyManager.AddMessageAsync(session, linearDispatchUserMessage, CancellationToken.None);

                        logger.LogInformation(
                            "Wave 12.3 linear_dispatch emitted — session={SessionId}, consumerType={ConsumerType}, attachmentCount={AttachmentCount}, sessionFileIdCount={SessionFileIdCount}",
                            sessionId, explicitConsumerType, request.Attachments.Count, sessionAttachmentIdsForDispatch.Count);

                        return;
                    }
                }
                // === End Wave 12.3 Phase 12.3a ===
    ...
    /// <summary>
    /// R7 Wave 12.3 Phase 12.3a — explicit-keyword intent detection for auto-dispatch to
    /// Linear consumers. Returns the matched <c>ConsumerTypes.*</c> constant if the message
    /// contains an unambiguous whole-word keyword identifying a single Linear consumer,
    /// else null. Case-insensitive; whole-word to prevent substring false positives
    /// (e.g., "summarize" matches "summarize this" but not "consummate").
    /// </summary>
    /// <remarks>
    /// <para>
    /// This is the deterministic path — 100% confidence per operator direction. Anything
    /// less than exact keyword match falls through to the ambiguous-intent path
    /// (<c>RunPhaseBVectorMatchAsync</c> → <c>playbook_options</c>).
    /// </para>
    /// <para>
    /// <b>Phase 12.4 evolution</b>: replace this hardcoded map with a lookup against
    /// <c>sprk_analysisplaybook.sprk_intenttriggers</c> so makers can add trigger phrases
    /// for new consumers without code changes (per plan D-13 revised + Phase 12.4 scope).
    /// </para>
    /// </remarks>
    private static string? TryDetectExplicitConsumerType(string? message)
    {
        ...
        // Whole-word case-insensitive match. Regex is compiled + shared across requests.
        if (SummarizeKeywordRegex.IsMatch(message))
        {
            return Sprk.Bff.Api.Services.Ai.PublicContracts.ConsumerTypes.ChatSummarize;
        }
