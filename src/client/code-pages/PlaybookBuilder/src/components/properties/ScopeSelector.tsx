...
export const ScopeSelector = memo(function ScopeSelector({
      ...
      <div className={styles.checkboxList}>
        {skills.map((skill) => (
          <Checkbox
            ...
            onChange={(_, data) =>
              handleSkillToggle(skill.id, data.checked === true)
            }
          />
        ))}
      </div>
  ...
  if (showKnowledge) {
    ...
    if (knowledge.length === 0) {
      return (
        <Text className={styles.empty}>No knowledge sources available.</Text>
      );
    }
    return (
      <div className={styles.checkboxList}>
        {knowledge.map((k) => (
          <Checkbox
            ...
            onChange={(_, data) =>
              handleKnowledgeToggle(k.id, data.checked === true)
            }
          />
        ))}
      </div>
  ...
  if (showTools) {
    ...
    return (
      ...
      >
        ...
        {tools.map((tool) => (
          <Radio key={tool.id} value={tool.id} label={tool.name} />
        ))}
      </RadioGroup>
    );
  }
