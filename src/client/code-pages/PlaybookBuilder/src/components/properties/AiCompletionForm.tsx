...
function parseConfig(json: string): AiCompletionConfig {
  try {
    ...
    return {
      ...
      userPromptTemplate:
        typeof parsed.userPromptTemplate === 'string' ? parsed.userPromptTemplate : DEFAULT_CONFIG.userPromptTemplate,
      modelDeploymentId:
        typeof parsed.modelDeploymentId === 'string' ? parsed.modelDeploymentId : DEFAULT_CONFIG.modelDeploymentId,
      temperature:
        typeof parsed.temperature === 'number' && parsed.temperature >= 0 && parsed.temperature <= 2
          ? parsed.temperature
          : DEFAULT_CONFIG.temperature,
      maxTokens:
        typeof parsed.maxTokens === 'number' && parsed.maxTokens >= 100 && parsed.maxTokens <= 32000
          ? parsed.maxTokens
...
export const AiCompletionForm = memo(function AiCompletionForm({
  ...
  const handleSkillToggle = useCallback(
    (skillId: string) => (_e: React.ChangeEvent<HTMLInputElement>, data: CheckboxOnChangeData) => {
      const newIds = data.checked ? [...config.skillIds, skillId] : config.skillIds.filter(id => id !== skillId);
    ...
    <div className={styles.form}>
      ...
      <div className={styles.field}>
        ...
        <Textarea
          ...
          resize="vertical"
        />
        <Text className={styles.fieldHint}>Instructions that define the AI assistant's behavior and role</Text>
      </div>
      ...
      <div className={styles.field}>
        ...
        <Text className={styles.fieldHint}>AI model deployment to use for this completion</Text>
      </div>
      ...
      <div className={styles.parameterRow}>
        <div className={styles.field}>
          ...
          <SpinButton
            ...
            step={0.1}
          />
          <Text className={styles.fieldHint}>0.0 (focused) to 2.0 (creative)</Text>
        </div>
        ...
        <div className={styles.field}>
          ...
          <SpinButton
            ...
            step={100}
          />
          <Text className={styles.fieldHint}>100-32,000 tokens</Text>
        </div>
      </div>
