...
import type { DocumentSearchResult, RecordSearchResult, GraphClusterBy, SearchDomain } from '../types';
...
export function getScore(result: SearchResult): number {
  return isDocumentResult(result) ? result.combinedScore : result.confidenceScore;
}

/** Get the unique ID for a result. */
export function getResultId(result: SearchResult): string {
  return isDocumentResult(result) ? (result.documentId ?? '') : result.recordId;
}

/** Get the display name for a result. */
export function getResultName(result: SearchResult): string {
  return isDocumentResult(result) ? (result.name ?? 'Untitled') : result.recordName;
}
...
export function getResultDate(
  ...
  if (isDocumentResult(result)) {
    ...
    if (field === 'updatedAt' || field === 'modifiedAt') return result.updatedAt;
  } else {
    if (field === 'createdAt') return result.createdAt;
    if (field === 'modifiedAt' || field === 'updatedAt') return result.modifiedAt;
  }
  return undefined;
}
 ...
 */
export function extractClusterKey(result: SearchResult, clusterBy: GraphClusterBy): string {
  if (isDocumentResult(result)) {
    switch (clusterBy) {
      case 'DocumentType':
 ...
 */
export function groupResults(results: SearchResult[], groupBy: GraphClusterBy): ResultGroup[] {
...
export function filterAndSortResults(
  ...
  const filtered = minSimilarity > 0 ? results.filter(r => getScore(r) * 100 >= minSimilarity) : results;

  return [...filtered].sort((a, b) => getScore(b) - getScore(a)).slice(0, maxResults);
}
