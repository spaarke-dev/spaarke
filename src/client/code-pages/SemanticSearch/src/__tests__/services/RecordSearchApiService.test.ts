  ...
  describe('search()', () => {
    ...
    describe('request construction', () => {
      ...
      it('should serialize request with all three record types', async () => {
        ...
        const [, init] = mockFetch.mock.calls[0];
        const parsedBody = JSON.parse(init?.body as string);
        expect(parsedBody.recordTypes).toEqual(['sprk_matter', 'sprk_project', 'sprk_invoice']);
      });
    ...
    describe('network and auth failures', () => {
      ...
      it('should throw if MSAL popup is cancelled', async () => {
        mockGetAuthHeader.mockRejectedValue(new Error('Authentication cancelled by user'));

        await expect(search(sampleRequest)).rejects.toThrow('Authentication cancelled by user');
      });
