...
function createMockResponse(status: number, body: unknown, ok?: boolean, statusText?: string): Response {
...
function createMockResponseWithJsonError(status: number, statusText: string): Response {
  return {
    ok: false,
    status,
    statusText,
...
describe('apiBase', () => {
  ...
  describe('buildAuthHeaders()', () => {
    ...
    it('should throw if getAuthHeader() throws (MSAL not initialized)', async () => {
      ...
      await expect(buildAuthHeaders()).rejects.toThrow('MSAL not initialized. Call initialize() first.');
    });

    it('should throw if getAuthHeader() throws (token acquisition failure)', async () => {
      mockGetAuthHeader.mockRejectedValue(new Error('Failed to acquire token: interaction_required'));

      await expect(buildAuthHeaders()).rejects.toThrow('Failed to acquire token: interaction_required');
    });
  });
