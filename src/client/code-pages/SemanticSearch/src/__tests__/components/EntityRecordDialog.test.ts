...
describe('EntityRecordDialog', () => {
  ...
  describe('domain-to-entity mapping', () => {
    ...
    ])("maps domain '%s' to entity '%s'", (domain, expectedEntity) => {
  ...
  describe('when Xrm is not available', () => {
    ...
    it('logs a warning when Xrm.Navigation is not available', () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Xrm.Navigation not available'));
      warnSpy.mockRestore();
    });
  });
  ...
  describe('error handling', () => {
    it('catches and logs error when navigateTo throws', () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to open record dialog'), expect.any(Error));
