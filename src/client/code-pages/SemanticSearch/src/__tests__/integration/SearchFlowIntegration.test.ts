...
function makeRecordResponse(results: RecordSearchResult[], totalCount: number, searchTime = 150): RecordSearchResponse {
  return {
    results,
    metadata: {
      totalCount,
...
function createErrorFetchResponse(status: number, body: unknown, statusText = 'Error'): Response {
...
describe('SearchFlowIntegration', () => {
  ...
  describe('Document search flow (useSemanticSearch → SemanticSearchApiService → fetch)', () => {
    it('should call POST /api/ai/search with correct query and default options', async () => {
      const response = makeDocResponse([makeDocResult('doc-1', 'Lease Agreement', 0.95)], 1);
  ...
  describe('Record search flow (useRecordSearch → RecordSearchApiService → fetch)', () => {
    it('should call POST /api/ai/search/records with correct recordTypes', async () => {
      const response = makeRecordResponse([makeRecordResult('matter-1', 'Johnson v. Smith', 'sprk_matter', 0.91)], 1);
      mockFetch.mockResolvedValue(createFetchResponse(response));
  ...
  describe('Error handling when API fails', () => {
    ...
    it('should show permission message for 401 errors', async () => {
      mockFetch.mockResolvedValue(createErrorFetchResponse(401, { title: 'Unauthorized' }, 'Unauthorized'));
    ...
    it('should show permission message for 403 errors', async () => {
      mockFetch.mockResolvedValue(createErrorFetchResponse(403, { title: 'Forbidden' }, 'Forbidden'));
    ...
    it('should show rate-limit message for 429 errors', async () => {
      mockFetch.mockResolvedValue(createErrorFetchResponse(429, { title: 'Too Many Requests' }, 'Too Many Requests'));
    ...
    it('should clear error when a new successful search is executed', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createErrorFetchResponse(500, { title: 'Server Error' }));
  ...
  describe('Pagination via loadMore', () => {
    it('should append results from second page with correct offset (document search)', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createFetchResponse(makeDocResponse(page1Results, 50)));
      ...
      mockFetch.mockResolvedValueOnce(createFetchResponse(makeDocResponse(page2Results, 50)));
    ...
    it('should append results from second page with correct offset (record search)', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createFetchResponse(makeRecordResponse(page1Results, 50)));
      ...
      mockFetch.mockResolvedValueOnce(createFetchResponse(makeRecordResponse(page2Results, 50)));
    ...
    it('should not call API when hasMore is false', async () => {
      ...
      mockFetch.mockResolvedValueOnce(createFetchResponse(makeDocResponse([makeDocResult('doc-1', 'Doc 1', 0.9)], 1)));
    ...
    it('should transition through loadingMore state', async () => {
      const page1 = Array.from({ length: 20 }, (_, i) => makeDocResult(`doc-${i}`, `Doc ${i}`, 0.9));
      mockFetch.mockResolvedValueOnce(createFetchResponse(makeDocResponse(page1, 50)));
  ...
  describe('New search replaces previous results', () => {
    it('should clear document results when starting a new search', async () => {
      mockFetch.mockResolvedValueOnce(
        createFetchResponse(makeDocResponse([makeDocResult('doc-old', 'Old Result', 0.9)], 1))
      );
      ...
      mockFetch.mockResolvedValueOnce(
        createFetchResponse(makeDocResponse([makeDocResult('doc-new', 'New Result', 0.95)], 1))
      );
    ...
    it('should clear record results when starting a new search', async () => {
      mockFetch.mockResolvedValueOnce(
        createFetchResponse(makeRecordResponse([makeRecordResult('old-1', 'Old Matter', 'sprk_matter', 0.8)], 1))
      );
      ...
      mockFetch.mockResolvedValueOnce(
        createFetchResponse(makeRecordResponse([makeRecordResult('new-1', 'New Matter', 'sprk_matter', 0.95)], 1))
      );
