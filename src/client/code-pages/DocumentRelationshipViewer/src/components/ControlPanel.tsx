...
const useStyles = makeStyles({
    ...
    sliderValue: { fontWeight: tokens.fontWeightSemibold, color: tokens.colorBrandForeground1, fontSize: tokens.fontSizeBase200 },
    slider: { width: "100%" },
    sliderBlock: { marginBottom: tokens.spacingVerticalS },
    checkboxGroup: { display: "flex", flexDirection: "column", gap: tokens.spacingVerticalXXS },
    ...
    return (
        <Card className={styles.container}>
            ...
            <div className={styles.header}>
                <Text className={styles.headerTitle} size={300}>Visualization Settings</Text>
            </div>
            ...
            <div className={styles.infoSection}>
                ...
                {showInfo && (
                    <div className={styles.infoContent}>
                        <div>
                            <Text className={styles.infoTitle} size={200}>Similarity Score</Text>
                            <Text className={styles.infoText}>
                                The percentage indicates how closely a document's content matches the source. Higher = more relevant.
                            </Text>
                        </div>
                        <div>
                            <Text className={styles.infoTitle} size={200}>Relationship Types</Text>
                            <Text className={styles.infoText}>
                                Semantic: AI-detected content similarity. Same Matter/Project: shared parent record. Same Email/Thread: communication linkage.
                            </Text>
                        </div>
                        <div>
                            <Text className={styles.infoTitle} size={200}>Depth Levels</Text>
                            <Text className={styles.infoText}>
                                Level 1 shows directly related documents. Level 2+ shows documents related to those, expanding discovery.
                            </Text>
                        </div>
                    </div>
                )}
            ...
            <div className={styles.section}>
                <div className={styles.sliderBlock}>
                    <div className={styles.sliderRow}>
                        <Label htmlFor="similarity-slider" size="small">Minimum Similarity</Label>
                        <Text className={styles.sliderValue}>{Math.round(settings.similarityThreshold * 100)}%</Text>
                    </div>
                    <Slider id="similarity-slider" className={styles.slider} min={50} max={95} step={5} size="small" value={Math.round(settings.similarityThreshold * 100)} onChange={handleSimilarityChange} />
                </div>
                ...
                {viewMode === "graph" && (
                    <div className={styles.sliderBlock}>
                        <div className={styles.sliderRow}>
                            <Label htmlFor="depth-slider" size="small">Levels</Label>
                            <Text className={styles.sliderValue}>{settings.depthLimit}</Text>
                        </div>
                        <Slider id="depth-slider" className={styles.slider} min={1} max={3} step={1} size="small" value={settings.depthLimit} onChange={handleDepthChange} />
                    </div>
                )}
                ...
                <div className={styles.sliderBlock}>
                    <div className={styles.sliderRow}>
                        <Label htmlFor="maxnodes-slider" size="small">Nodes per Level</Label>
                        <Text className={styles.sliderValue}>{settings.maxNodesPerLevel}</Text>
                    </div>
                    <Slider id="maxnodes-slider" className={styles.slider} min={10} max={50} step={5} size="small" value={settings.maxNodesPerLevel} onChange={handleMaxNodesChange} />
                </div>
            </div>
