...
const RELATIONSHIP_COLORS: Record<string, { bg: string; border: string }> = {
    semantic: { bg: tokens.colorBrandBackground2, border: tokens.colorBrandStroke1 },
    same_matter: { bg: tokens.colorPaletteGreenBackground2, border: tokens.colorPaletteGreenBorder2 },
    same_project: { bg: tokens.colorPaletteGreenBackground2, border: tokens.colorPaletteGreenBorder2 },
    same_email: { bg: tokens.colorPaletteYellowBackground2, border: tokens.colorPaletteYellowBorder2 },
    same_thread: { bg: tokens.colorPaletteYellowBackground2, border: tokens.colorPaletteYellowBorder2 },
    same_invoice: { bg: tokens.colorPaletteRedBackground2, border: tokens.colorPaletteRedBorder2 },
};

function getPointColor(data: DocumentNodeData): { bg: string; border: string } {
    if (data.isSource) return { bg: tokens.colorBrandBackground, border: tokens.colorBrandStroke1 };
    ...
    return { bg: tokens.colorNeutralBackground3, border: tokens.colorNeutralStroke1 };
}
...
export const RelationshipTimeline: React.FC<RelationshipTimelineProps> = ({
    ...
    useEffect(() => { setZoomDomain(null); }, [inputNodes]);
    ...
    const chartHeight = Math.max(0, dimensions.height - MARGIN.top - MARGIN.bottom - (hasUndated ? NO_DATE_STRIP_HEIGHT + NO_DATE_GAP : 0));
    ...
    const scaleY = useCallback((score: number): number => {
    ...
    const handlePointMouseLeave = useCallback(() => { setHoveredId(null); }, []);

    const handlePointClick = useCallback((e: React.MouseEvent, point: TimelinePoint) => {
        e.stopPropagation();
    ...
    const legendEntries = useMemo(() => [
        { key: "Source", bg: tokens.colorBrandBackground, border: tokens.colorBrandStroke1 },
        { key: "Semantic", bg: tokens.colorBrandBackground2, border: tokens.colorBrandStroke1 },
        { key: "Same Matter", bg: tokens.colorPaletteGreenBackground2, border: tokens.colorPaletteGreenBorder2 },
        { key: "Same Email", bg: tokens.colorPaletteYellowBackground2, border: tokens.colorPaletteYellowBorder2 },
    ], []);
        ...
        return (
            <div className={styles.container} ref={containerCallbackRef}>
                <div className={styles.centerMessage}>
                    <Text size={400} weight="semibold">No documents to visualize</Text>
        ...
        <div className={styles.container} ref={containerCallbackRef}>
            ...
            >
                ...
                {xAxisTicks.map((tick, i) => (
                    <g key={`x-tick-${i}`}>
                        <line x1={tick.x} y1={MARGIN.top + chartHeight} x2={tick.x} y2={MARGIN.top + chartHeight + 6} stroke={tokens.colorNeutralStroke1} strokeWidth={1} />
                        <text x={tick.x} y={MARGIN.top + chartHeight + 20} textAnchor="middle" fill={tokens.colorNeutralForeground3} fontSize={11}>
                ...
                {/* X-Axis label */}
                <text x={MARGIN.left + chartWidth / 2} y={dimensions.height - 8} textAnchor="middle" fill={tokens.colorNeutralForeground2} fontSize={12}>
                    Modified Date
                </text>

                {/* Y-Axis line */}
                <line x1={MARGIN.left} y1={MARGIN.top} x2={MARGIN.left} y2={MARGIN.top + chartHeight} stroke={tokens.colorNeutralStroke1} strokeWidth={1} />

                {/* Y-Axis labels + grid */}
                {yAxisLabels.map((label) => (
                    <g key={`y-${label.pct}`}>
                        <line x1={MARGIN.left - 4} y1={label.y} x2={MARGIN.left} y2={label.y} stroke={tokens.colorNeutralStroke1} strokeWidth={1} />
                        <text x={MARGIN.left - 8} y={label.y + 4} textAnchor="end" fill={tokens.colorNeutralForeground3} fontSize={11}>
                            {label.pct}%
                        </text>
                        {label.pct > 0 && label.pct < 100 && (
                            <line x1={MARGIN.left + 1} y1={label.y} x2={MARGIN.left + chartWidth} y2={label.y} stroke={tokens.colorNeutralStroke2} strokeWidth={0.5} strokeDasharray="4 4" />
                        )}
                    </g>
                ))}
                ...
                {/* Y-Axis label */}
                <text
                    x={14} y={MARGIN.top + chartHeight / 2}
                    textAnchor="middle" fill={tokens.colorNeutralForeground2} fontSize={12}
                ...
                {visibleDated.map((point) => {
                    ...
                    return (
                        <circle
                            ...
                            fill={point.color} stroke={point.borderColor}
                ...
                {hasUndated && (
                    <>
                        <line
                            x1={MARGIN.left} y1={noDateStripY - NO_DATE_GAP / 2}
                            x2={MARGIN.left + chartWidth} y2={noDateStripY - NO_DATE_GAP / 2}
                            stroke={tokens.colorNeutralStroke2} strokeWidth={1} strokeDasharray="6 3"
                        />
                        <text
                            x={MARGIN.left - 8} y={noDateStripY + NO_DATE_STRIP_HEIGHT / 2 + 4}
                            textAnchor="end" fill={tokens.colorNeutralForeground3} fontSize={11} fontStyle="italic"
                        >
                            No Date
                        </text>
            ...
            {hoveredPoint && (
                <div className={styles.tooltip} style={{ left: tooltipPos.x, top: tooltipPos.y }}>
                    ...
                    {hoveredPoint.date && (
                        <div className={styles.tooltipDetail}>Date: {formatDate(hoveredPoint.date)}</div>
                    )}
                    ...
                    {hoveredPoint.data.isSource && (
                        <div className={styles.tooltipDetail} style={{ fontStyle: "italic" }}>Source document</div>
                    )}
                </div>
            )}
        </div>
