...
import type { DocumentNode, DocumentEdge, DocumentNodeData } from "../types/graph";
...
const DEFAULT_COLOR = { bg: tokens.colorNeutralBackground3, border: tokens.colorNeutralStroke1 };
...
export const RelationshipNetwork: React.FC<RelationshipNetworkProps> = ({
    ...
    const { simNodeData, simEdgeData } = useMemo(() => {
        ...
        const docNodes = inputNodes.filter((n) => {
            ...
            return t !== "matter" && t !== "project" && t !== "invoice" && t !== "email";
        });
    ...
    useEffect(() => {
        ...
        if (width > 0 && height > 0) {
            ...
            minX -= VIEWPORT_PADDING; maxX += VIEWPORT_PADDING;
            minY -= VIEWPORT_PADDING; maxY += VIEWPORT_PADDING;
            const w = maxX - minX, h = maxY - minY;
            const cx = (minX + maxX) / 2, cy = (minY + maxY) / 2;
            const fitScale = Math.min(width / w, height / h) * 0.9;
            const cs = Math.max(MIN_SCALE, Math.min(MAX_SCALE, fitScale));
            setViewTransform({ x: width / 2 - cx * cs, y: height / 2 - cy * cs, scale: cs });
        }

        setTickCount((t) => t + 1);

        sim.alpha(0.005).alphaDecay(0.02).on("tick", () => setTickCount((t) => t + 1)).restart();
    ...
    useEffect(() => {
        ...
        const cs = Math.max(MIN_SCALE, Math.min(MAX_SCALE, fitScale));
        setViewTransform({ x: dimensions.width / 2 - cx * cs, y: dimensions.height / 2 - cy * cs, scale: cs });
    ...
    const handleNodePointerDown = useCallback((e: React.PointerEvent, nodeId: string) => {
        ...
        const onUp = (ev: PointerEvent) => {
            ...
            if (Math.abs(dx) < CLICK_THRESHOLD && Math.abs(dy) < CLICK_THRESHOLD && node) {
    ...
    const handleSvgMouseUp = useCallback(() => { setIsPanning(false); panStart.current = null; }, []);
    const handleSvgMouseLeave = useCallback(() => { setIsPanning(false); panStart.current = null; setHoveredId(null); }, []);
    ...
    const handleWheel = useCallback((e: React.WheelEvent<SVGSVGElement>) => {
        ...
        setViewTransform((prev) => {
            ...
            return { x: cursorX - (cursorX - prev.x) * ratio, y: cursorY - (cursorY - prev.y) * ratio, scale: newScale };
        });
    }, []);
    ...
    const hoveredConnectionCount = useMemo(() => {
        ...
        return simEdgeData.filter((e) => e.source === hoveredId || e.target === hoveredId).length;
        ...
        return (
            <div className={styles.container} ref={containerCallbackRef}>
                <div className={styles.centerMessage}>
                    ...
                    <Text size={200}>Load a document with AI embeddings to see the network graph</Text>
                </div>
            </div>
        );
    }
                        ...
                        return (
                            <g key={`edge-${i}`}>
                                <line
                                    ...
                                    style={{
                                        opacity: hoveredId ? (isConnected ? Math.min(baseOpacity * 3, 0.7) : baseOpacity * 0.3) : baseOpacity,
                ...
                {legendEntries.map((entry) => (
                    <div key={entry.key} className={styles.legendItem}>
                        <svg width="16" height="16">
                            <circle cx={8} cy={8} r={7} fill={entry.bg} stroke={entry.border} strokeWidth={1} />
                        </svg>
            ...
            {hoveredNode && !draggedNodeId.current && (
                <div className={styles.tooltip} style={{ left: tooltipPos.x, top: tooltipPos.y }}>
                    ...
                    {hoveredNode.data.documentType && (
                        <div className={styles.tooltipDetail}>Type: {hoveredNode.data.documentType}</div>
                    )}
                    {hoveredNode.data.relationshipLabel && (
                        <div className={styles.tooltipDetail}>Relationship: {hoveredNode.data.relationshipLabel}</div>
                    )}
