...
const DEFAULT_SOURCE_RATIO = 0.30;
...
function computeEffectiveRatios(
  baseRatios: PanelRatios,
  visibility: PanelVisibility
): PanelRatios {
  const visibleSum =
    baseRatios.editor +
    (visibility.source ? baseRatios.source : 0) +
    (visibility.chat ? baseRatios.chat : 0);
...
export function usePanelLayout(options: UsePanelLayoutOptions = {}): UsePanelLayoutResult {
  ...
  const [ratios, setRatios] = useState<PanelRatios>(() =>
    loadPersistedRatios(defaultRatios)
  );
