...
describe('DiffReviewPanel', () => {
  ...
  describe('rendering', () => {
    it('render_IsOpenTrue_ShowsPanelWithContent', () => {
      ...
      expect(screen.getByTestId('diff-review-panel-backdrop')).toBeInTheDocument();
    ...
    it('render_IsOpenFalse_BackdropHiddenPointerEventsNone', () => {
      ...
      expect(screen.queryByTestId('mock-diff-compare-view')).not.toBeInTheDocument();
    });
  ...
  describe('accept action', () => {
    it('accept_ClickAcceptButton_CallsOnAcceptWithProposedText', () => {
      ...
      renderWithProvider(<DiffReviewPanel {...DEFAULT_PROPS} onAccept={mockOnAccept} />);

      fireEvent.click(screen.getByTestId('diff-accept-button'));

      expect(mockOnAccept).toHaveBeenCalledTimes(1);
      expect(mockOnAccept).toHaveBeenCalledWith('<p>Revised analysis content with improvements</p>');
    });
  });
  ...
  describe('reject action', () => {
    it('reject_ClickRejectButton_CallsOnReject', () => {
      ...
      renderWithProvider(<DiffReviewPanel {...DEFAULT_PROPS} onReject={mockOnReject} />);
  ...
  describe('keyboard interactions', () => {
    ...
    it('escape_WhenClosed_DoesNotCallOnReject', () => {
      const mockOnReject = jest.fn();
      renderWithProvider(<DiffReviewPanel {...DEFAULT_PROPS} isOpen={false} onReject={mockOnReject} />);
  ...
  describe('DiffCompareView props', () => {
    it('diffCompareView_ReceivesOriginalAndProposedText', () => {
      ...
      expect(diffView).toHaveAttribute('data-original', '<p>Original analysis content</p>');
      expect(diffView).toHaveAttribute('data-proposed', '<p>Revised analysis content with improvements</p>');
    });
  });
});
