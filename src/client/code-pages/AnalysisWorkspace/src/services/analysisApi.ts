...
function mapDocumentResponse(response: any): DocumentMetadata {
  ...
  return {
    ...
    fileExtension: extractExtension(
      doc.sprk_name ?? doc.sprk_filename ?? doc.name,
    ),
...
export async function fetchAnalysis(
  ...
  if (!response.ok) {
    console.error(
      `${LOG_PREFIX} Dataverse read failed: ${response.status} ${response.statusText}`,
    );
    throw {
      errorCode: `HTTP_${response.status}`,
...
export async function fetchDocumentMetadata(
  ...
  console.log(
    `${LOG_PREFIX} Document metadata loaded: "${data.name}" (${data.mimeType})`,
  );
  return data;
}
...
export async function getDocumentViewUrl(
  ...
  const response = await fetch(
    `${API_BASE_URL}/documents/${documentId}/preview-url`,
    {
      method: "GET",
      headers: buildHeaders(token),
      signal: controller.signal,
...
export async function saveAnalysisContent(
  ...
  console.log(
    `${LOG_PREFIX} Saving analysis content: ${analysisId} (${content.length} chars)`,
  );
...
export async function executeAnalysis(
  ...
  console.log(
    `${LOG_PREFIX} Execute response: ${response.status} ${response.statusText}`,
  );
...
export async function exportAnalysis(
  ...
  const response = await fetch(
    `${API_BASE_URL}/ai/analysis/${analysisId}/export`,
    {
      method: "POST",
      headers: buildHeaders(token),
  ...
  console.log(
    `${LOG_PREFIX} Analysis exported successfully (${blob.size} bytes)`,
  );
  return blob;
}
