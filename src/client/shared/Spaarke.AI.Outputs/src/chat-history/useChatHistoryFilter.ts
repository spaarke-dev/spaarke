 ...
 * ```
 */
export function useChatHistoryFilter(sessions: ChatSession[], searchQuery: string): ChatSession[] {
  ...
  return sessions.filter(session => {
    ...
    const previewMatch =
      session.lastMessagePreview !== undefined && session.lastMessagePreview.toLowerCase().includes(lower);
