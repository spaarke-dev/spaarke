...
import { makeStyles, mergeClasses, tokens, SearchBox, Spinner, Text } from '@fluentui/react-components';
...
export function ChatHistoryPanel({
  ...
  const handleSearchChange = React.useCallback((_event: SearchBoxChangeEvent, data: InputOnChangeData): void => {
    ...
    if (isLoading) {
      return (
        <div className={styles.centeredState}>
          <Spinner size="medium" label="Loading conversations..." labelPosition="below" />
        </div>
      );
    }
    ...
    if (filteredSessions.length === 0) {
      ...
      return (
        <div className={styles.centeredState}>
          ...
          <Text className={styles.emptyTitle}>{hasQuery ? 'No matching conversations' : 'No conversations yet'}</Text>
          <Text className={styles.emptySubtitle}>
            {hasQuery ? 'Try a different search term.' : 'Start a new AI conversation to see your history here.'}
          </Text>
        </div>
      );
    }
      ...
      <div className={styles.listArea} role="list">
        {filteredSessions.map(session => (
          <div key={session.id} role="listitem">
            <ChatSessionCard session={session} onResume={onResume} onDelete={onDelete} />
          </div>
        ))}
      </div>
    );
