...
import { renderWithTheme, webLightTheme, webDarkTheme, mockLegalLibraryProps } from '../../__tests__/test-utils';

describe('LegalLibraryWidget', () => {
  const props = mockLegalLibraryProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<LegalLibraryWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<LegalLibraryWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders the citation title', () => {
    const { getByText } = renderWithTheme(<LegalLibraryWidget {...props} />, webLightTheme);
    expect(getByText('Brown v. Board of Education, 347 U.S. 483 (1954)')).toBeTruthy();
  });
  ...
  it('renders loading state without error', () => {
    const { container } = renderWithTheme(<LegalLibraryWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
