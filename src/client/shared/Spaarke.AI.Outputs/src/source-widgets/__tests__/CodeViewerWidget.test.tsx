...
import { renderWithTheme, webLightTheme, webDarkTheme, mockCodeViewerProps } from '../../__tests__/test-utils';

describe('CodeViewerWidget', () => {
  const props = mockCodeViewerProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<CodeViewerWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<CodeViewerWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders language badge', () => {
    const { getByText } = renderWithTheme(<CodeViewerWidget {...props} />, webLightTheme);
  ...
  it('renders Copy button', () => {
    const { getByLabelText } = renderWithTheme(<CodeViewerWidget {...props} />, webLightTheme);
    expect(getByLabelText('Copy code to clipboard')).toBeTruthy();
  });

  it('renders loading state without error', () => {
    const { container } = renderWithTheme(<CodeViewerWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
