...
import { renderWithTheme, webLightTheme, webDarkTheme, mockDocumentCompareProps } from '../../__tests__/test-utils';

describe('DocumentCompareWidget', () => {
  const props = mockDocumentCompareProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<DocumentCompareWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<DocumentCompareWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders document labels', () => {
    const { getAllByText } = renderWithTheme(<DocumentCompareWidget {...props} />, webLightTheme);
  ...
  it('renders loading state in dark theme without error', () => {
    const { container } = renderWithTheme(<DocumentCompareWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
