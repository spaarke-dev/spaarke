...
import { renderWithTheme, webLightTheme, webDarkTheme, mockSearchResultsProps } from '../../__tests__/test-utils';

describe('SearchResultsWidget', () => {
  const props = mockSearchResultsProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<SearchResultsWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<SearchResultsWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders the search query text', () => {
    const { getByText } = renderWithTheme(<SearchResultsWidget {...props} />, webLightTheme);
  ...
  it('renders loading state in dark theme without error', () => {
    const { container } = renderWithTheme(<SearchResultsWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
