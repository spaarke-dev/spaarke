...
import { renderWithTheme, webLightTheme, webDarkTheme, mockBudgetDashboardProps } from '../../__tests__/test-utils';

describe('BudgetDashboardWidget', () => {
  const props = mockBudgetDashboardProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<BudgetDashboardWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<BudgetDashboardWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders budget title text', () => {
    const { getByText } = renderWithTheme(<BudgetDashboardWidget {...props} />, webLightTheme);
    expect(getByText('Q3 Matter Budget')).toBeTruthy();
  });

  it('renders loading spinner when isLoading is true', () => {
    const { container } = renderWithTheme(<BudgetDashboardWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
