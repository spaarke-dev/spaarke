...
import { renderWithTheme, webLightTheme, webDarkTheme, mockDataTableProps } from '../../__tests__/test-utils';

describe('DataTableWidget', () => {
  const props = mockDataTableProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<DataTableWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<DataTableWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders column headers', () => {
    const { getByText } = renderWithTheme(<DataTableWidget {...props} />, webLightTheme);
  ...
  it('renders loading state in dark theme without error', () => {
    const { container } = renderWithTheme(<DataTableWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
