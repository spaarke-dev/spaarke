...
import { renderWithTheme, webLightTheme, webDarkTheme, mockTimelineProps } from '../../__tests__/test-utils';

describe('TimelineWidget', () => {
  const props = mockTimelineProps();

  it('renders in light theme without error', () => {
    const { container } = renderWithTheme(<TimelineWidget {...props} />, webLightTheme);
    expect(container.firstChild).toBeTruthy();
  });

  it('renders in dark theme (webDarkTheme) without error', () => {
    const { container } = renderWithTheme(<TimelineWidget {...props} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it('renders event labels', () => {
    const { getByText } = renderWithTheme(<TimelineWidget {...props} />, webLightTheme);
  ...
  it('renders loading state in dark theme without error', () => {
    const { container } = renderWithTheme(<TimelineWidget {...props} isLoading={true} />, webDarkTheme);
    expect(container.firstChild).toBeTruthy();
  });
});
