...
const WIDGET_DIRS = [path.resolve(__dirname, '../output-widgets'), path.resolve(__dirname, '../source-widgets')];
...
const HARDCODED_COLOR_PATTERNS = [
  ...
  {
    regex:
      /:\s*["'](red|blue|green|yellow|black|white|gray|grey|orange|purple|pink|brown|cyan|magenta|lime|indigo|violet|teal|coral|salmon|gold|silver|navy|maroon|olive|aqua|fuchsia|transparent)["']/i,
  ...
  for (const dir of WIDGET_DIRS) {
    ...
    for (const filePath of files) {
      ...
      lines.forEach((lineText, zeroBasedIndex) => {
        ...
        const isSafe = SAFE_LINE_PATTERNS.some(pattern => pattern.test(lineText));
        if (isSafe) return;
...
describe('ADR-021 static color scan — no hard-coded colors in widget sources', () => {
  ...
  it('finds no hard-coded color values in widget source files (ADR-021)', () => {
    ...
    const report = violations
      .map(v => `  VIOLATION [${v.pattern}] at ${v.file}:${v.line}\n    > ${v.lineText}`)
      .join('\n');
