...
import { dispatchCrossPaneLink, subscribeToCrossPaneLinks } from './cross-pane-events';
 ...
 */
export function useDispatchCrossPaneLink(): (event: CrossPaneLinkEvent) => void {
 ...
 * });
 */
export function useCrossPaneSubscription(handler: (event: CrossPaneLinkEvent) => void): void {
  ...
  useEffect((): (() => void) => {
    const unsubscribe = subscribeToCrossPaneLinks((event: CrossPaneLinkEvent): void => {
