 ...
 * Priority x Effort scoring grid.
 *
 * R5 FR-01 / task 002 — hoisted host-agnostic from
 * `src/solutions/LegalWorkspace/src/components/SmartToDo/TodoAISummaryDialog.tsx`.
 * Cross-folder imports re-homed: the `todoScoringTypes` type import + re-export
 * now resolve from the package's `../../types/todoScoringTypes`. Sibling cards
 * (`./PriorityScoreCard`, `./EffortScoreCard`) stay folder-local.
 *
 * Layout:
 *   DialogTitle: SparkleRegular + "AI Summary" + subtitle + dismiss button
 *   DialogContent: scoring grid (PriorityScoreCard | EffortScoreCard), analysis
 *     card, suggested-actions list
 *   DialogActions: Close button
 *
 * Design constraints:
 *   - ALL colours from Fluent UI v9 semantic tokens — zero hardcoded hex/rgb
 *   - Icon-only buttons MUST have aria-label
 *   - Fluent v9 Dialog components only; @fluentui/react-icons only
 *
 * Wire-up:
 *   The consuming component owns the scoring data hook and passes props. The
 *   dialog is purely presentational — all fetch logic lives in the host.
 */
