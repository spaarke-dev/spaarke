/**
 * DismissedSection — Collapsible section for dismissed to-do items.
 *
 * R5 FR-01 / task 002 — hoisted host-agnostic from
 * `src/solutions/LegalWorkspace/src/components/SmartToDo/DismissedSection.tsx`.
 * Cross-folder imports re-homed to package sources: `ITodo`/`PriorityLevel`/
 * `EffortLevel` from `../../types/entities`; `computeDueLabel`/`parseDueDate`/
 * `DueUrgency` from the LOCKED `../../utils/todoScoring` (was the LW-local
 * `types/entities`, `types/enums`, `utils/dueLabelUtils`).
 *
 * Layout:
 *   [ChevronDown/Up icon] [Dismissed label] [count Badge]
 *   When expanded: list of dismissed items with a restore button on each row.
 *
 * Design constraints:
...
import { ChevronDownRegular, ChevronUpRegular, ArrowUndoRegular } from '@fluentui/react-icons';
import type { ITodo, PriorityLevel, EffortLevel } from '../../types/entities';
import { computeDueLabel, parseDueDate, type DueUrgency } from '../../utils/todoScoring';
...
const DismissedItem: React.FC<IDismissedItemProps> = React.memo(({ todo, onRestore, isRestoring }) => {
  ...
  return (
    <div className={styles.itemRow} role="listitem" aria-label={rowAriaLabel}>
      {/* Title + context */}
...
export const DismissedSection: React.FC<IDismissedSectionProps> = React.memo(
  ({ items, onRestore, restoringIds = new Set() }) => {
    ...
    return (
      <div className={styles.section} role="region" aria-label="Dismissed to-do items">
        {/* Toggle header */}
        <button
        ...
        >
          ...
          <Badge appearance="filled" color="subtle" size="small" aria-label={countLabel}>
            {items.length}
          </Badge>
        </button>
