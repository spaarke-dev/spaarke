 ...
 * Cross-folder imports re-homed to package sources: `ITodo`/`PriorityLevel`/
 * `EffortLevel` from `../../types/entities`; `computeTodoScore`/
 * `ITodoScoreBreakdown`/`computeDueLabel`/`parseDueDate`/`DueUrgency` from the
 * LOCKED `../../utils/todoScoring`. `createXrmNavigationService` stays imported
 * from the `@spaarke/ui-components` package barrel (sanctioned host-agnostic
 * navigation adapter, ADR-012).
 *
 * Layout:
...
import { makeStyles, tokens, Text, Textarea, Button, Divider } from '@fluentui/react-components';
import { EditRegular, MailRegular, ChatRegular, SparkleRegular } from '@fluentui/react-icons';
import type { ITodo, PriorityLevel, EffortLevel } from '../../types/entities';
import {
  computeTodoScore,
  computeDueLabel,
  parseDueDate,
...
export const TodoDetailPane: React.FC<ITodoDetailPaneProps> = React.memo(({ todo, onSaveDescription }) => {
  ...
  const handleEdit = React.useCallback(() => {
    ...
    createXrmNavigationService()
      .openRecord('sprk_todo', todo.sprk_todoid)
      .catch(err => {
    ...
    <div className={styles.pane}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.section}>
          ...
          />
          {isDirty && (
            <Button appearance="primary" size="small" onClick={handleSave} disabled={isSaving}>
        ...
        <div className={styles.section}>
          ...
          {priorityLevel && (
            <div className={styles.detailRow}>
              ...
              <InlineBadge style={PRIORITY_BADGE_STYLE[priorityLevel]} ariaLabel={`Priority: ${priorityLevel}`}>
                {priorityLevel}
              </InlineBadge>
            </div>
          )}
          {effortLevel && (
            <div className={styles.detailRow}>
              ...
              <InlineBadge style={EFFORT_BADGE_STYLE[effortLevel]} ariaLabel={`Effort: ${effortLevel}`}>
                {effortLevel}
              </InlineBadge>
            </div>
          )}
          {dueLabel.urgency !== 'none' && (
            <div className={styles.detailRow}>
              ...
              <InlineBadge style={DUE_BADGE_STYLE[dueLabel.urgency]} ariaLabel={`Due: ${dueLabel.label}`}>
                {dueLabel.label}
              </InlineBadge>
            </div>
          )}
        ...
        <div className={styles.section}>
          ...
          <div className={styles.actionsRow}>
            <Button appearance="subtle" size="small" icon={<EditRegular />} onClick={handleEdit}>
              Edit
            </Button>
            <Button appearance="subtle" size="small" icon={<MailRegular />} onClick={handleEmail}>
              Email
            </Button>
            <Button appearance="subtle" size="small" icon={<ChatRegular />} onClick={handleTeams}>
              Teams
            </Button>
            <Button appearance="subtle" size="small" icon={<SparkleRegular />} onClick={handleAiSummary}>
              AI
            </Button>
          </div>
        </div>
