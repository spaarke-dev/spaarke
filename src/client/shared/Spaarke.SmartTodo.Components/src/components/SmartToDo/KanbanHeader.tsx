/**
 * KanbanHeader — Header bar above the Kanban board (Smart To Do).
 *
 * R5 FR-01 / task 002 — hoisted host-agnostic from
 * `src/solutions/LegalWorkspace/src/components/SmartToDo/KanbanHeader.tsx` into
 * `@spaarke/smart-todo-components`. Only host-agnostic imports:
 * `MicrosoftToDoIcon` via the `@spaarke/ui-components` package barrel and the
 * folder-local `./AddTodoBar`.
 *
 ...
 *
 * Behaviour:
 *   - Compact right-aligned action buttons only in embedded mode
...
export interface IKanbanHeaderProps {
  ...
  onSettingsOpen: () => void;
  /** When true, hides the full header (matching existing SmartToDo embedded pattern). */
  embedded?: boolean;
}
        ...
        <div className={styles.header} role="banner" style={{ justifyContent: 'flex-end' }}>
          <div className={styles.rightGroup}>
            <Button
              ...
              title="Recalculate — reassign unpinned items by score"
            />
            <Button
              appearance="subtle"
              size="small"
