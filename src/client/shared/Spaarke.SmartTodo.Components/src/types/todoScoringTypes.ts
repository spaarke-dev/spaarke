/**
 * todoScoringTypes.ts — Shared type definitions for the To-Do AI Summary dialog
 * and its sub-cards (PriorityScoreCard, EffortScoreCard).
 *
 * R5 FR-01 / task 002 — hoisted host-agnostic from
 * `src/solutions/LegalWorkspace/src/components/SmartToDo/todoScoringTypes.ts`
 * into `@spaarke/smart-todo-components`. PURE TYPES only (zero runtime, zero
 * platform coupling). Reconciled against `./todo.ts` (widget data shapes) —
 * NO overlap / duplication with those types.
 *
 * These describe the *presentation* payload the AI Summary dialog renders; the
 * data path (BFF/mock) lives in the host. They are distinct from the composite
 * score contract in `../utils/todoScoring.ts` (which stays the locked scoring
 * source of truth — not re-implemented here).
 */
