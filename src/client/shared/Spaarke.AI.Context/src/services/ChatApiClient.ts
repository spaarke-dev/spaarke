...
export class ChatApiClient {
  ...
  async getSessionHistory(sessionId: string): Promise<IChatMessage[]> {
    const url = buildBffApiUrl(
      this.bffBaseUrl,
      `/ai/chat/sessions/${encodeURIComponent(sessionId)}/history`
    );
  ...
  async switchContext(sessionId: string, request: SwitchContextRequest): Promise<void> {
    const url = buildBffApiUrl(
      this.bffBaseUrl,
      `/ai/chat/sessions/${encodeURIComponent(sessionId)}/context`
    );
  ...
  async getPlaybooks(nameFilter?: string): Promise<IPlaybookOption[]> {
    const path = nameFilter
      ? `/ai/chat/playbooks?nameFilter=${encodeURIComponent(nameFilter)}`
      : '/ai/chat/playbooks';
  ...
  async getAnalysisContextMapping(analysisId: string): Promise<IAnalysisChatContextResponse | null> {
    const url = buildBffApiUrl(
      this.bffBaseUrl,
      `/ai/chat/context-mappings/analysis/${encodeURIComponent(analysisId)}`
    );
