...
export async function authenticatedFetch(url, init) {
    ...
    throw new ApiError(problemDetails?.detail ?? problemDetails?.title ?? `HTTP ${lastResponse.status}`, lastResponse.status, problemDetails);
}
/**
...
async function tryParseProblemDetails(response) {
    try {
...
//# sourceMappingURL=authenticatedFetch.js.map