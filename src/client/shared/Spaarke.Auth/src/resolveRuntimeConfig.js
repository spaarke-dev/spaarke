...
function saveToLocalStorage(config) {
    try {
        const entry = { ...config, _ts: Date.now() };
...
function loadFromLocalStorage() {
    try {
        ...
        if (!entry._ts || Date.now() - entry._ts > LS_TTL_MS)
            return null;
        if (!entry.bffBaseUrl || !entry.bffOAuthScope || !entry.msalClientId)
            return null;
        return {
 ...
 */
async function queryEnvironmentVariables(clientUrl, schemaNames) {
    ...
    // 1. Fetch definitions
    const defResponse = await fetch(`${apiBase}/environmentvariabledefinitions?$filter=${filter}&$select=environmentvariabledefinitionid,schemaname,defaultvalue`, {
        method: 'GET',
        headers: {
    ...
    if (!defResponse.ok) {
        throw new Error(`[Spaarke.RuntimeConfig] Failed to query environment variable definitions: ` +
    ...
    const valResponse = await fetch(`${apiBase}/environmentvariablevalues?$filter=${valueFilter}&$select=_environmentvariabledefinitionid_value,value`, {
        method: 'GET',
        headers: {
            Accept: 'application/json',
            'OData-MaxVersion': '4.0',
...
export async function resolveRuntimeConfig() {
    ...
    if (!xrmContext) {
        ...
        if (lsCached) {
            console.warn('[Spaarke.RuntimeConfig] Xrm not available — using localStorage cache. ' + `bffBaseUrl=${lsCached.bffBaseUrl}`);
            cachedConfig = lsCached;
            cacheTimestamp = Date.now();
            return lsCached;
        }
        throw new Error('[Spaarke.RuntimeConfig] Xrm.Utility.getGlobalContext() not available. ' +
            'resolveRuntimeConfig() must be called from within a Dataverse web resource.');
    }
    const clientUrl = xrmContext.getClientUrl();
    if (!clientUrl) {
        throw new Error('[Spaarke.RuntimeConfig] Xrm.Utility.getGlobalContext().getClientUrl() returned empty. ' +
    ...
    if (!bffBaseUrl) {
        throw new Error(`[Spaarke.RuntimeConfig] Environment variable '${ENV_VAR_NAMES.BFF_BASE_URL}' not found in Dataverse. ` +
    ...
    if (!bffAppId) {
        throw new Error(`[Spaarke.RuntimeConfig] Environment variable '${ENV_VAR_NAMES.BFF_APP_ID}' not found in Dataverse. ` +
            'Ensure the SpaarkeCore solution is imported and the variable has a value.');
    }
    ...
    if (!msalClientId) {
        throw new Error(`[Spaarke.RuntimeConfig] MSAL client ID not available. ` +
    ...
    console.log(`[Spaarke.RuntimeConfig] Resolved: bffBaseUrl=${config.bffBaseUrl}, ` +
