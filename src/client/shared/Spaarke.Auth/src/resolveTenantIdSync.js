 ...
 */
export function resolveTenantIdSync() {
    ...
    if (typeof window !== 'undefined') {
        ...
        for (const frame of frames) {
            try {
                const tenantId = frame.Xrm?.Utility?.getGlobalContext?.()?.organizationSettings?.tenantId;
                if (tenantId)
                    return tenantId;
            }
            catch { /* cross-origin */ }
...
//# sourceMappingURL=resolveTenantIdSync.js.map