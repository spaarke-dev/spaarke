...
export const PreferencesDropdown: React.FC<PreferencesDropdownProps> = ({ preferences, onUpdatePreferences }) => {
  ...
  const [pendingPrefs, setPendingPrefs] = React.useState<DailyDigestPreferences>(preferences);
  ...
  React.useEffect(() => {
    setPendingPrefs(prev => (JSON.stringify(prev) === JSON.stringify(preferences) ? prev : preferences));
  ...
  const handleChannelToggle = React.useCallback((category: NotificationCategory, checked: boolean) => {
  ...
  const handleDueWindowChange = React.useCallback((_event: unknown, data: { optionValue?: string }) => {
    if (!data.optionValue) return;
    const value = Number(data.optionValue) as DueWindowDays;
    setPendingPrefs(prev => ({ ...prev, dueWithinDays: value }));
  }, []);

  const handleTimeWindowChange = React.useCallback((_event: unknown, data: { optionValue?: string }) => {
    if (!data.optionValue) return;
  ...
  const handleSave = React.useCallback(async () => {
    ...
    const allKeys = Object.keys(pendingPrefs) as Array<keyof DailyDigestPreferences>;
  ...
  return (
    <Popover positioning="below-end" trapFocus>
      <PopoverTrigger disableButtonEnhancement>
        <Button appearance="subtle" icon={<Settings20Regular />} aria-label="Preferences" />
      </PopoverTrigger>
      ...
      <PopoverSurface className={styles.surface}>
        ...
        <div className={styles.section}>
          ...
          {TOGGLEABLE_CHANNELS.map(channel => {
            const isEnabled = !pendingPrefs.disabledChannels.includes(channel.category);
            return (
              <div key={channel.category} className={styles.channelRow}>
                <Body1>{channel.label}</Body1>
                <Switch
                  checked={isEnabled}
                  onChange={(_ev, data) => handleChannelToggle(channel.category, data.checked)}
        ...
        <div className={styles.section}>
          <Caption1 className={styles.sectionTitle}>Display Parameters</Caption1>
          ...
          <div className={styles.parameterRow}>
            ...
            <Dropdown
              ...
              value={DUE_WINDOW_OPTIONS.find(o => o.value === pendingPrefs.dueWithinDays)?.label ?? ''}
          ...
          <div className={styles.parameterRow}>
            ...
            <Dropdown
              ...
              className={styles.dropdown}
              value={TIME_WINDOW_OPTIONS.find(o => o.value === pendingPrefs.timeWindow)?.label ?? ''}
        ...
        <div className={styles.footerRow}>
          <span
            className={`${styles.statusMessage} ${
              status === 'saved' ? styles.statusSaved : status === 'error' ? styles.statusError : styles.statusSaving
            }`}
            role="status"
            aria-live="polite"
          >
            ...
            {status === 'idle' && isDirty && <Caption1>Unsaved changes</Caption1>}
          </span>
          <Button
            appearance="primary"
            size="small"
