...
function makeSelection(text: string, isCollapsed: boolean, anchorNode: Node | null = null): Selection {
...
describe('useInlineAiToolbar', () => {
  ...
  it('visible_IsInitiallyFalse', () => {
    ...
    const { result } = renderHook(() => useInlineAiToolbar({ editorContainerRef }));
  ...
  it('cleanup_RemovesSelectionChangeListener_OnUnmount', () => {
    ...
    const { unmount } = renderHook(() => useInlineAiToolbar({ editorContainerRef }));
  ...
  it('actions_UsesCustomActions_WhenProvided', () => {
    ...
    const { result } = renderHook(() => useInlineAiToolbar({ editorContainerRef, actions: customActions }));
