...
import { useWriteMode, resolveAutoMode, WRITE_MODE_COMMANDS } from '../useWriteMode';
...
describe('useWriteMode', () => {
  describe('initial state', () => {
    ...
    it('mode_CustomDefault_RespectsDefaultModeOption', () => {
      const { result } = renderHook(() => useWriteMode({ defaultMode: 'diff' }));
  ...
  describe('sessionStorage persistence', () => {
    it('setMode_WithSessionId_PersistsWithCorrectKey', () => {
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-42' }));

      act(() => {
        result.current.setMode('diff');
      });

      expect(sessionStorageMock.setItem).toHaveBeenCalledWith('sprk-write-mode-override:sess-42', 'diff');
    });
    ...
    it('setMode_WithoutSessionId_UsesDefaultKey', () => {
      ...
      expect(sessionStorageMock.setItem).toHaveBeenCalledWith('sprk-write-mode-override', 'stream');
    });

    it('resetToAuto_RemovesFromSessionStorage', () => {
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-99' }));
      ...
      expect(sessionStorageMock.removeItem).toHaveBeenCalledWith('sprk-write-mode-override:sess-99');
    });
    ...
    it('mount_WithExistingOverride_RestoresFromSessionStorage', () => {
      ...
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-restore' }));
    ...
    it('mount_WithoutOverride_DefaultsToAuto', () => {
      ...
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-clean' }));
    ...
    it('mount_SessionStorageUnavailable_DefaultsToAuto', () => {
      ...
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-blocked' }));
    ...
    it('mount_InvalidStoredValue_DefaultsToAuto', () => {
      ...
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-bad' }));
  ...
  describe('session change', () => {
    it('sessionChange_NewSession_ReReadsOverride', () => {
      ...
      const { result, rerender } = renderHook(({ sessionId }: { sessionId: string }) => useWriteMode({ sessionId }), {
  ...
  describe('WRITE_MODE_COMMANDS', () => {
    ...
    it('commands_StreamEntry_HasCorrectShape', () => {
      const stream = WRITE_MODE_COMMANDS.find(c => c.id === 'mode_stream');
  ...
  describe('action menu command integration', () => {
    ...
    it('modeCommand_Stream_SetsStreamOverride', () => {
      const { result } = renderHook(() => useWriteMode({ sessionId: 'sess-cmd' }));
