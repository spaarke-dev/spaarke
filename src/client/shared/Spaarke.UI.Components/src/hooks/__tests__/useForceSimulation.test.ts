...
import { useForceSimulation, type ForceNode, type ForceEdge, type ForceSimulationOptions } from '../useForceSimulation';
...
describe('useForceSimulation', () => {
  ...
  describe('hub-spoke mode', () => {
    ...
    it('uses first node as hub when no isSource is set', () => {
      const nodes: ForceNode[] = [{ id: 'x' }, { id: 'y' }, { id: 'z' }];
      const edges: ForceEdge[] = [
        { source: 'x', target: 'y' },
        { source: 'x', target: 'z' },
      ];
  ...
  describe('edge cases', () => {
    ...
    it('preserves extensible properties on edges', () => {
      const nodes: ForceNode[] = [{ id: 'a', isSource: true }, { id: 'b' }];
      const edges: ForceEdge[] = [{ source: 'a', target: 'b', weight: 0.9, customEdgeProp: 42 }];
      const { result } = runHook(nodes, edges, { mode: 'hub-spoke' });
  ...
  describe('memoization', () => {
    it('returns the same reference for identical inputs', () => {
      ...
      const { result, rerender } = renderHook(() => useForceSimulation(nodes, edges, options));
