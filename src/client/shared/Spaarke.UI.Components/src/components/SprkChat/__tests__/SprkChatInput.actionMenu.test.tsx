...
describe('SprkChatInput - Action Menu Integration', () => {
  ...
  describe('Trigger Detection', () => {
    it("should open action menu when '/' is typed as first character", async () => {
      ...
      renderWithProviders(
        <SprkChatInput onSend={mockOnSend} actions={testActions} onActionSelected={mockOnActionSelected} />
      );
  ...
  describe('Keyboard Delegation', () => {
    it('should navigate menu down with ArrowDown when menu is open', async () => {
      ...
      expect(screen.getByTestId('action-menu-item-run-playbook').getAttribute('aria-selected')).toBe('true');

      // ArrowDown should move to second item
      await user.keyboard('{ArrowDown}');

      await waitFor(() => {
        expect(screen.getByTestId('action-menu-item-summarize').getAttribute('aria-selected')).toBe('true');
      });
    });
    ...
    it('should navigate menu up with ArrowUp when menu is open', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('action-menu-item-toggle-theme').getAttribute('aria-selected')).toBe('true');
      });
    });
    ...
    it('should select active action on Enter when menu is open (not send message)', async () => {
      ...
      expect(mockOnActionSelected).toHaveBeenCalledWith(expect.objectContaining({ id: 'run-playbook' }));
  ...
  describe('Menu Lifecycle', () => {
    ...
    it('should gracefully handle action selection without onActionSelected callback', async () => {
      ...
      renderWithProviders(<SprkChatInput onSend={mockOnSend} actions={testActions} />);
  ...
  describe('Backward Compatibility', () => {
    ...
    it('should not render action menu when actions prop is an empty array', async () => {
      const user = userEvent.setup();
      renderWithProviders(<SprkChatInput onSend={mockOnSend} actions={[]} />);
