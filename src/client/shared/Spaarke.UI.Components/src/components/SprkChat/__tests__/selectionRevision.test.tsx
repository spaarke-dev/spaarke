...
function makeCrossPaneSelection(text: string, overrides?: Partial<ICrossPaneSelection>): ICrossPaneSelection {
  return {
    text,
    fullText: text,
...
describe('Selection-Based Revision: Cross-Pane Selection', () => {
  ...
  it('crossPaneSelection_Present_ShowsCrossPaneToolbar', () => {
    ...
    renderWithProviders(<CrossPaneTestWrapper onRefine={mockOnRefine} crossPaneSelection={selection} />);

    expect(screen.getByTestId('highlight-refine-toolbar-cross-pane')).toBeInTheDocument();
  });
  ...
  it('crossPaneSelection_Null_HidesCrossPaneToolbar', () => {
    renderWithProviders(<CrossPaneTestWrapper onRefine={mockOnRefine} crossPaneSelection={null} />);

    expect(screen.queryByTestId('highlight-refine-toolbar-cross-pane')).not.toBeInTheDocument();
  });
...
describe('Selection-Based Revision: Refine Submission', () => {
  ...
  it('refineSubmit_EnterInstruction_CallsOnRefineWithTextAndInstruction', async () => {
    ...
    expect(mockOnRefine).toHaveBeenCalledWith('Editor selected text', 'Make this more formal');
  });
  ...
  it('refineSubmit_EnterKey_SubmitsRefinement', async () => {
    ...
    expect(mockOnRefine).toHaveBeenCalledWith('Text for enter-key test', 'Simplify');
  });
  ...
  it('refineSubmit_IsRefiningTrue_DisablesRefineButton', () => {
    ...
    renderWithProviders(
      <CrossPaneTestWrapper onRefine={mockOnRefine} crossPaneSelection={selection} isRefining={true} />
    );
...
describe('Selection-Based Revision: Quick Actions', () => {
  ...
  it('quickAction_ClickSimplify_AutoSubmitsWithSimplifyInstruction', async () => {
    ...
    expect(mockOnRefine).toHaveBeenCalledWith('Complex text to simplify', 'Simplify this text');
  });
  ...
  it('quickAction_ClickExpand_AutoSubmitsWithExpandInstruction', async () => {
    ...
    expect(mockOnRefine).toHaveBeenCalledWith('Brief text', 'Expand this text with more detail');
  });
  ...
  it('quickAction_ClickMakeConcise_AutoSubmits', async () => {
    ...
    expect(mockOnRefine).toHaveBeenCalledWith('Verbose text that is too long', 'Make this text more concise');
  });
  ...
  it('quickAction_ClickMakeFormal_AutoSubmits', async () => {
    ...
    expect(mockOnRefine).toHaveBeenCalledWith('Casual text here', 'Rewrite this text in a more formal tone');
  });
...
describe('Selection-Based Revision: Edge Cases', () => {
  it('edgeCase_LargeSelection_TextTruncatedForPreview', () => {
    ...
    renderWithProviders(<CrossPaneTestWrapper onRefine={jest.fn()} crossPaneSelection={selection} />);
  ...
  it('edgeCase_DismissButton_ClosesToolbar', async () => {
    ...
    const { rerender } = renderWithProviders(
      <CrossPaneTestWrapper onRefine={mockOnRefine} crossPaneSelection={selection} />
    );
...
describe('Selection-Based Revision: Bridge Event Routing', () => {
  ...
  it('bridgeRoute_CompleteDiffSequence_EndToEnd', () => {
    ...
    bridge.subscribe('document_stream_start', p => events.push({ type: 'start', payload: p }));
    bridge.subscribe('document_stream_token', p => events.push({ type: 'token', payload: p }));
    bridge.subscribe('document_stream_end', p => events.push({ type: 'end', payload: p }));
