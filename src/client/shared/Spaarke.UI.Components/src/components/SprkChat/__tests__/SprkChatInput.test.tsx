...
describe('SprkChatInput', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should show custom placeholder text', () => {
      renderWithProviders(<SprkChatInput onSend={mockOnSend} placeholder="Ask a question..." />);
    ...
    it('should display custom max character count', () => {
      renderWithProviders(<SprkChatInput onSend={mockOnSend} maxCharCount={500} />);
  ...
  describe('Character Count', () => {
    it('should update character count as user types', async () => {
      const user = userEvent.setup();
      renderWithProviders(<SprkChatInput onSend={mockOnSend} maxCharCount={100} />);
    ...
    it('should disable send button when over character limit', async () => {
      const user = userEvent.setup();
      renderWithProviders(<SprkChatInput onSend={mockOnSend} maxCharCount={5} />);
  ...
  // ── Task 082 (UX fix #1): cursor focus restoration after streaming ───────
  describe('Focus restoration on streaming completion', () => {
    it('should restore focus to textarea when disabled flips from true to false', async () => {
      ...
      // Re-render with disabled=false (stream complete).
      rerender(<SprkChatInput onSend={mockOnSend} disabled={false} />);
