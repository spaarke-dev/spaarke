 ...
 * Thin wrapper around CreateRecordWizard for "Create New Invoice".
 *
 * Provides:
 *   - Entity-specific form step (CreateInvoiceStep)
 *   - Finish handler (InvoiceService.createInvoice + follow-on record creation)
 *   - A custom Next Steps card set — Send Email / Add To Do / Assign Work
 *     (design.md §5.9) — via `config.followOnCards` (task 030 addition to
 *     `ICreateRecordWizardConfig`), NOT the built-in Assign Work/Create
 *     Event/Send Email default. This wizard owns all follow-on state itself
 *     (mirroring the established `WorkAssignmentWizardDialog` /
 *     `SummarizeFilesDialog` pattern of ref-backed closures), since
 *     `IFinishContext.followOn` is populated from CreateRecordWizard's OWN
 *     internal state for the DEFAULT card set only.
 *
 * All other generic wizard mechanics (file upload, step navigation, dialog
 * chrome) are handled by the shared CreateRecordWizard component.
 *
 ...
 * optimization (same pattern as CreateEventWizard).
 *
 * @see IDataService — high-level data access abstraction
 * @see INavigationService — navigation abstraction
 * @see CreateEventWizard/CreateEventWizard.tsx — closest structural template
 *   (associateToStep + lockAssociation wiring)
...
/**
 * Determines the `associateToStep` config for CreateInvoiceWizard.
 *
 * Mirrors `resolveEventAssociateToStepConfig` (CreateEventWizard.tsx) exactly:
 * gated on `navigationService` being present AND either `initialAssociation`
 * being supplied or `lockAssociation` being `true` — so a hypothetical future
 * standalone entry point that passes only `navigationService` (no seed/lock
 * signal) does not suddenly show an unlocked Associate-To step.
 *
 * Uses `INVOICE_REGARDING_TARGETS` (Matter + Project only — narrower than
 * Event's 9-entity list — per spec FR-09 / design §5.6 / the Phase-0
 * field manifest).
 */
export function resolveInvoiceAssociateToStepConfig(
  ...
  return {
    entityTypes: INVOICE_REGARDING_TARGETS.slice(),
    navigationService,
    initialAssociation,
    lockAssociation,
  };
...
export interface ICreateInvoiceWizardProps {
  ...
  /**
   * AAD tenant ID. Forwarded to `EntityCreationService.indexUploadedFiles()`.
   ...
   * task 012/030). When supplied, the AssociateToStep starts pre-selected with
...
const CreateInvoiceWizard: React.FC<ICreateInvoiceWizardProps> = ({
  open,
  onClose,
  dataService,
  navigationService,
  ...
  const [formValues, setFormValues] = React.useState<ICreateInvoiceFormState>(buildEmptyInvoiceForm());
  ...
  React.useEffect(() => {
    if (open) {
      setFormValid(false);
      setFormValues(buildEmptyInvoiceForm());
      setAssignWorkName('');
  ...
  // (design.md §5.9 — replaces the built-in Assign Work/Create Event/Send Email
  // default via ICreateRecordWizardConfig.followOnCards, task 030.)
  const followOnCards: FollowOnCardConfig[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        description: 'Create a to do linked to this invoice.',
        renderStep: () => (
          <AddTodoFollowOnStep
      ...
      },
      {
        ...
        description: 'Create a work assignment with resources linked to this invoice.',
        renderStep: () => {
  ...
  const config: ICreateRecordWizardConfig = React.useMemo(
    () => ({
      ...
      entityLabel: 'invoice',
      filesStepSubtitle: 'Upload documents to associate with this invoice, or click Next to skip.',
      finishingLabel: 'Creating invoice…',

      // task 030 — see resolveInvoiceAssociateToStepConfig doc comment.
      associateToStep: resolveInvoiceAssociateToStepConfig(navigationService, initialAssociation, lockAssociation),

      infoStep: {
        id: 'create-record',
        label: 'Invoice Details',
        canAdvance: () => formValid,
        renderContent: uploadedFiles => (
          <CreateInvoiceStep
            dataService={dataService}
      ...
      // task 030: custom Send Email/Add To Do/Assign Work set — see followOnCards above.
      followOnCards,
      ...
      onFinish: async (context: IFinishContext): Promise<IWizardSuccessConfig> => {
        ...
        const invoiceName = result.invoiceName!;
        const warnings: string[] = [...result.warnings];

        // -- Follow-on: Assign Work (sprk_workassignment regarding the new Invoice) --
        if (context.selectedActions.includes('assign-work') && assignWorkNameRef.current.trim()) {
          try {
            ...
            if (waResult.status === 'error') {
              warnings.push(
                `Work assignment could not be created (${waResult.errorMessage ?? 'Unknown error'}). ` +
                  'You can create it manually from the invoice record.'
              );
            ...
            warnings.push(
              `Work assignment could not be created (${message}). You can create it manually from the invoice record.`
            );
          }
        }
        ...
        if (context.selectedActions.includes('send-email') && emailToRef.current.trim() && authFetch && bffBaseUrl) {
          ...
          const emailResult = await emailService.sendEmail({
            ...
            associations: [{ entityType: 'sprk_invoice', entityId: invoiceId, entityName: invoiceName }],
          });
        ...
        const viewInvoice = () => {
          if (navigationService) {
            navigationService.openRecord('sprk_invoice', invoiceId);
          }
          onClose();
        };
        ...
        return {
          ...
          title: hasWarnings ? 'Invoice created with warnings' : 'Invoice created!',
          body: (
            <Text size={300} style={{ color: tokens.colorNeutralForeground2 }}>
              <span style={{ color: tokens.colorBrandForeground1, fontWeight: 600 }}>&ldquo;{invoiceName}&rdquo;</span>{' '}
              has been created
              {hasWarnings
          ...
          actions: (
            <>
              <Button appearance="primary" onClick={viewInvoice} aria-label={`View invoice: ${invoiceName}`}>
                View Invoice
              </Button>
