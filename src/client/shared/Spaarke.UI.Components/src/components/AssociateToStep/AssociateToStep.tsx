...
export const AssociateToStep: React.FC<AssociateToStepProps> = ({
  ...
  // ── Derived values ───────────────────────────────────────────────────────
  const selectedTypeDef: EntityTypeOption | undefined = entityTypes.find(et => et.entityType === selectedEntityType);
  ...
  const handleSelectRecord = React.useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to open record lookup. Please try again.');
    } finally {
