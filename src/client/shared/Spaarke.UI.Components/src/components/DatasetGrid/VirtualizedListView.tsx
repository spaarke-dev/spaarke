...
export const VirtualizedListView: React.FC<VirtualizedListViewProps> = (
  ...
  const primaryColumn =
    props.columns.find((c) => c.isPrimary) ?? props.columns[0];
  const secondaryColumn = props.columns.find(
    (c) => !c.isPrimary && c.name !== primaryColumn?.name,
  );
  ...
  const renderRow = React.useCallback(
    ({ index, style }: { index: number; style: React.CSSProperties }) => {
      ...
      const primaryValue = primaryRenderer(
      ...
      if (secondaryColumn) {
        const secondaryRenderer =
          ColumnRendererService.getRenderer(secondaryColumn);
        secondaryValue = secondaryRenderer(
          record[secondaryColumn.name],
          record,
          secondaryColumn,
        );
      }
