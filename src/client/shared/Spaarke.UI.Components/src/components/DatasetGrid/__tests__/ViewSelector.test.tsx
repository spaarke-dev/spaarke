...
describe('ViewSelector', () => {
  ...
  describe('loading state', () => {
    ...
    it('should show default view name while loading', () => {
      ...
      renderWithProvider(<ViewSelector {...defaultProps} defaultViewName="My View" />);
  ...
  describe('loaded state', () => {
    ...
    it('should call ViewService.getViews with correct entity', async () => {
      renderWithProvider(<ViewSelector {...defaultProps} entityLogicalName="contact" />);
    ...
    it('should include personal views when requested', async () => {
      renderWithProvider(<ViewSelector {...defaultProps} includePersonalViews />);
  ...
  describe('selection', () => {
    it('should display selected view name', async () => {
      renderWithProvider(<ViewSelector {...defaultProps} selectedViewId="view-2" />);
    ...
    it('should call onViewChange when selection changes', async () => {
      ...
      renderWithProvider(<ViewSelector {...defaultProps} selectedViewId="view-1" onViewChange={onViewChange} />);
      ...
      expect(onViewChange).toHaveBeenCalledWith(expect.objectContaining({ id: 'view-2', name: 'All Records' }));
    });

    it('should auto-select default view when no selection provided', async () => {
      const onViewChange = jest.fn();

      renderWithProvider(<ViewSelector {...defaultProps} onViewChange={onViewChange} />);

      await waitFor(() => {
        expect(onViewChange).toHaveBeenCalledWith(expect.objectContaining({ id: 'view-1', isDefault: true }));
      });
    });
  });
  ...
  describe('accessibility', () => {
    it('should have proper aria-label', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('combobox')).toHaveAttribute('aria-label', 'Select view for contact');
      });
    });
  });
});
