...
function makeStreamToken(operationId: string, token: string, index: number): DocumentStreamTokenPayload {
  return { operationId, token, index };
}

/** Creates a stream end payload */
function makeStreamEnd(operationId: string, totalTokens: number, cancelled = false): DocumentStreamEndPayload {
...
describe('SprkChatBridge Integration Tests', () => {
  ...
  describe('streaming sequence ordering', () => {
    ...
    it('emit_50TokenStreamingSequence_AllEventsReceivedInExactOrder', () => {
      ...
      for (let i = 0; i < TOKEN_COUNT; i++) {
        sender.emit('document_stream_token', makeStreamToken(operationId, `token-${i}`, i));
      }

      sender.emit('document_stream_end', makeStreamEnd(operationId, TOKEN_COUNT));
      ...
      expect((allEvents[0].payload as DocumentStreamStartPayload).operationId).toBe(operationId);
      ...
      expect((endEvent.payload as DocumentStreamEndPayload).totalTokens).toBe(TOKEN_COUNT);
    ...
    it('emit_100TokenStreamingSequence_NoDroppedTokens', () => {
      ...
      // Act
      for (let i = 0; i < TOKEN_COUNT; i++) {
        sender.emit('document_stream_token', makeStreamToken(operationId, `w${i}`, i));
      }
    ...
    it('emit_StreamTokensWithMixedEvents_EventTypesDoNotInterfere', () => {
      ...
      sender.emit('document_stream_token', makeStreamToken('op-mix', 'first', 0));
      sender.emit('selection_changed', {
        text: 'sel1',
        startOffset: 0,
        endOffset: 4,
      });
      sender.emit('document_stream_token', makeStreamToken('op-mix', 'second', 1));
      sender.emit('selection_changed', {
        text: 'sel2',
        startOffset: 10,
        endOffset: 14,
      });
      sender.emit('document_stream_token', makeStreamToken('op-mix', 'third', 2));
  ...
  describe('cross-pane message flow (postMessage fallback)', () => {
    ...
    function dispatchPostMessage(channelName: string, event: SprkChatBridgeEventName, payload: unknown): void {
    ...
    it('emit_SidePaneSendsViaPostMessage_WorkspacePaneReceivesPayload', () => {
      ...
      dispatchPostMessage('sprk-workspace-integ-pm-flow', 'document_stream_start', payload);
    ...
    it('emit_BidirectionalPostMessage_BothPanesReceiveEvents', () => {
      ...
      dispatchPostMessage(channelName, 'document_stream_start', makeStreamStart('op-pm-bidir'));
    ...
    it('emit_50TokenStreamViaPostMessage_AllTokensReceivedInOrder', () => {
      ...
      dispatchPostMessage(channelName, 'document_stream_start', makeStreamStart(operationId));
      for (let i = 0; i < TOKEN_COUNT; i++) {
        dispatchPostMessage(channelName, 'document_stream_token', makeStreamToken(operationId, `pm-tok-${i}`, i));
      }
      dispatchPostMessage(channelName, 'document_stream_end', makeStreamEnd(operationId, TOKEN_COUNT));
      ...
      for (let i = 0; i < TOKEN_COUNT; i++) {
        ...
        expect((event.payload as DocumentStreamTokenPayload).token).toBe(`pm-tok-${i}`);
      }
  ...
  describe('channel isolation', () => {
    it('emit_DifferentChannels_MessagesDoNotCrossTalk', () => {
      ...
      bridgeA.subscribe('document_stream_start', p => bridgeAReceived.push(p));
      bridgeB.subscribe('document_stream_start', p => bridgeBReceived.push(p));
      bridgeC.subscribe('document_stream_start', p => bridgeCReceived.push(p));
    ...
    it('emit_MultipleChannelsInParallel_EachChannelIndependent', () => {
      ...
      pairA_receiver.subscribe('document_stream_token', p => pairATokens.push(p.token));
      pairB_receiver.subscribe('document_stream_token', p => pairBTokens.push(p.token));

      // Act — interleave tokens between the two pairs
      pairA_sender.emit('document_stream_token', makeStreamToken('op-a', 'alpha-0', 0));
      pairB_sender.emit('document_stream_token', makeStreamToken('op-b', 'beta-0', 0));
      pairA_sender.emit('document_stream_token', makeStreamToken('op-a', 'alpha-1', 1));
      pairB_sender.emit('document_stream_token', makeStreamToken('op-b', 'beta-1', 1));
  ...
  describe('disconnect behavior', () => {
    it('disconnect_ReceiverDisconnects_NoEventsDeliveredToDisconnectedBridge', () => {
      ...
      receiver.disconnect();
      sender.emit('document_stream_token', makeStreamToken('op-disc', 'orphan', 0));
    ...
    it('disconnect_SenderDisconnects_EmitThrowsError', () => {
      ...
      expect(() => {
        sender.emit('document_stream_token', makeStreamToken('op-disc-send', 'fail', 0));
    ...
    it('disconnect_DisconnectedSenderDoesNotCrashReceiver_ReceiverRemainsOperational', () => {
      ...
      bridgeA.disconnect();
      bridgeC.emit('document_stream_token', makeStreamToken('op-c-sends', 'still-alive', 0));
    ...
    it('disconnect_MidStream_RemainingTokensNotDelivered', () => {
      ...
      for (let i = 0; i < 5; i++) {
        sender.emit('document_stream_token', makeStreamToken('op-mid', `tok-${i}`, i));
      }
  ...
  describe('cleanup completeness', () => {
    it('disconnect_BroadcastChannelBridge_ClosesUnderlyingChannel', () => {
      ...
      const instancesBefore = MockBroadcastChannel.instances.filter(i => i.name === 'sprk-workspace-integ-cleanup-bc');
      ...
      const instancesAfter = MockBroadcastChannel.instances.filter(i => i.name === 'sprk-workspace-integ-cleanup-bc');
    ...
    it('disconnect_MultipleSubscriptions_AllHandlersCleared', () => {
      ...
      sender.emit('document_stream_token', makeStreamToken('op-cleanup', 'x', 0));
