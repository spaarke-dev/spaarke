 ...
 */
export function buildInitialShellState(steps) {
    const shellSteps = steps.map((config, index) => ({
        id: config.id,
        label: config.label,
 ...
 */
function rebuildStatuses(steps, activeIndex) {
 ...
 */
export function wizardShellReducer(state, action) {
    switch (action.type) {
        // ----- NEXT_STEP --------------------------------------------------------
        case 'NEXT_STEP': {
        ...
        case 'REMOVE_DYNAMIC_STEP': {
            ...
            if (removeIndex === -1)
                return state; // step not found — no-op
...
//# sourceMappingURL=wizardShellReducer.js.map