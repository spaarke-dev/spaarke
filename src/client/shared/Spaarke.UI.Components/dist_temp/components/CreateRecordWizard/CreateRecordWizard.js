...
import { NextStepsStep, FOLLOW_ON_STEP_ID_MAP, FOLLOW_ON_STEP_LABEL_MAP, FOLLOW_ON_CANONICAL_ORDER, } from './FollowOnSteps';
...
// ---------------------------------------------------------------------------
const EMPTY_SEARCH = () => Promise.resolve([]);
// ---------------------------------------------------------------------------
// CreateRecordWizard
// ---------------------------------------------------------------------------
export const CreateRecordWizard = ({ open, onClose, config, embedded }) => {
    ...
    const [assignWorkPriority, setAssignWorkPriority] = React.useState(WORK_ASSIGNMENT_PRIORITY.Normal);
                ...
                const dynamicConfig = {
                    ...
                    renderContent: () => {
                        if (stepId === 'followon-assign-counsel') {
                            const mtVal = assignWorkMatterTypeIdRef.current
                                ? { id: assignWorkMatterTypeIdRef.current, name: assignWorkMatterTypeNameRef.current }
                                : null;
                            const paVal = assignWorkPracticeAreaIdRef.current
                                ? { id: assignWorkPracticeAreaIdRef.current, name: assignWorkPracticeAreaNameRef.current }
                                : null;
                            const attVal = assignedAttorneyIdRef.current
                                ? { id: assignedAttorneyIdRef.current, name: assignedAttorneyNameRef.current }
                                : null;
                            const paraVal = assignedParalegalIdRef.current
                                ? { id: assignedParalegalIdRef.current, name: assignedParalegalNameRef.current }
                                : null;
                            const ocVal = assignedOutsideCounselIdRef.current
                                ? { id: assignedOutsideCounselIdRef.current, name: assignedOutsideCounselNameRef.current }
                                : null;
                            return (React.createElement(AssignWorkFollowOnStep, { nameValue: assignWorkNameRef.current, onNameChange: setAssignWorkName, descriptionValue: assignWorkDescriptionRef.current, onDescriptionChange: setAssignWorkDescription, matterTypeValue: mtVal, onMatterTypeChange: handleMatterTypeChange, onSearchMatterTypes: searchMatterTypes, practiceAreaValue: paVal, onPracticeAreaChange: handlePracticeAreaChange, onSearchPracticeAreas: searchPracticeAreas, priorityValue: assignWorkPriorityRef.current, onPriorityChange: setAssignWorkPriority, responseDueDateValue: assignWorkResponseDueDateRef.current, onResponseDueDateChange: setAssignWorkResponseDueDate, attorneyValue: attVal, onAttorneyChange: handleAttorneyChange, onSearchAttorneys: searchContacts, paralegalValue: paraVal, onParalegalChange: handleParalegalChange, onSearchParalegals: searchContacts, outsideCounselValue: ocVal, onOutsideCounselChange: handleOutsideCounselChange, onSearchOutsideCounsel: searchOrganizations }));
                        ...
                        if (stepId === 'followon-create-event') {
                            if (!config.eventDataService) {
                                ...
                                return (React.createElement(Text, { size: 300, style: { color: 'inherit' } }, "Event creation is not configured for this wizard."));
                            }
                            ...
                            return (React.createElement(CreateEventFollowOnStep, { dataService: config.eventDataService, formValues: currentFormValues, onFormValues: (vals) => {
                        ...
                        if (stepId === 'followon-send-email') {
                            return (React.createElement(SendEmailStep, { emailTo: emailToRef.current, onEmailToChange: setEmailTo, emailSubject: emailSubjectRef.current, onEmailSubjectChange: setEmailSubject, emailBody: emailBodyRef.current, onEmailBodyChange: setEmailBody, onSearchUsers: searchUsers }));
                        }
    ...
    const stepConfigs = React.useMemo(() => {
        const addFilesStep = {
            ...
            renderContent: () => (React.createElement(React.Fragment, null,
                React.createElement("div", null,
                    React.createElement(Text, { as: "h2", size: 500, weight: "semibold", className: styles.stepTitle }, "Add file(s)"),
                    React.createElement(Text, { size: 200, className: styles.stepSubtitle }, filesStepSubtitle)),
        ...
        const nextStepsConfig = {
            ...
            renderContent: () => (React.createElement(NextStepsStep, { selectedActions: selectedActions, onSelectionChange: setSelectedActions, entityLabel: config.entityLabel })),
        };
        if (config.associateToStep) {
            const { entityTypes, navigationService } = config.associateToStep;
            const associateStepConfig = {
                id: 'associate-to',
                label: 'Associate To',
                canAdvance: () => Boolean(associationRef.current), // requires a record selection; Skip button bypasses this
                isSkippable: true,
                renderContent: (handle) => (React.createElement(AssociateToStep, { entityTypes: entityTypes, navigationService: navigationService, value: associationRef.current, onChange: setAssociation, onSkip: () => {
                        setAssociation(null);
                        handle.nextStep();
                    } })),
            };
    ...
    // -- Render --
    return (React.createElement(WizardShell, { ref: shellRef, open: open, embedded: embedded, hideTitle: embedded, title: config.title, ariaLabel: config.title, steps: stepConfigs, onClose: onClose, onFinish: handleFinish, finishingLabel: config.finishingLabel ?? 'Creating\u2026', finishLabel: "Finish" }));
};
export default CreateRecordWizard;
//# sourceMappingURL=CreateRecordWizard.js.map