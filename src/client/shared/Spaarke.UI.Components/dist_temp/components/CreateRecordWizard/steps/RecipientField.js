...
function extractEmailFromName(name) {
...
// Component
// ---------------------------------------------------------------------------
export const RecipientField = ({ label, placeholder, recipients, onRecipientsChange, onSearch, minSearchLength = 2, }) => {
    ...
    const handleSelectContact = React.useCallback((item) => {
    ...
    const handleAddManualEmail = React.useCallback(() => {
        ...
        if (!EMAIL_REGEX.test(email))
            return;
        if (recipients.some(r => r.email === email || r.key === email))
            return;
        const newRecipient = {
            key: email,
            displayName: email,
    ...
    return (React.createElement("div", { className: styles.wrapper, ref: wrapperRef },
        React.createElement(Text, { size: 300, weight: "semibold", className: styles.label }, label),
        recipients.length > 0 && (React.createElement("div", { className: styles.chipList }, recipients.map(r => (React.createElement("span", { key: r.key, className: mergeClasses(styles.chip, r.isManual && styles.chipManual) },
            React.createElement("span", { className: mergeClasses(styles.chipIcon, r.isManual && styles.chipIconManual) }, r.isManual ? React.createElement(MailRegular, { fontSize: 14 }) : React.createElement(PersonRegular, { fontSize: 14 })),
            React.createElement(Text, { size: 200, className: mergeClasses(styles.chipText, r.isManual && styles.chipTextManual), title: r.email ? `${r.displayName} (${r.email})` : r.displayName }, r.displayName),
            React.createElement(Button, { appearance: "subtle", size: "small", icon: React.createElement(DismissRegular, { fontSize: 12 }), onClick: () => handleRemove(r.key), "aria-label": `Remove ${r.displayName}` })))))),
        React.createElement("div", { className: styles.inputRow },
            React.createElement(Input, { value: inputValue, onChange: e => setInputValue(e.target.value), onKeyDown: handleKeyDown, onFocus: handleFocus, placeholder: placeholder ?? 'Search contacts or type email...', contentBefore: React.createElement(SearchRegular, { "aria-hidden": "true" }), "aria-label": label, autoComplete: "off", style: { width: '100%' } }),
            loading && (React.createElement("div", { className: styles.spinnerRow },
                React.createElement(Spinner, { size: "tiny", label: "Searching..." }))),
            showResults && (React.createElement("div", { className: styles.resultsList, role: "listbox", "aria-label": `${label} search results` }, results.map((item, index) => (React.createElement("div", { key: item.id, className: mergeClasses(styles.resultItem, index === highlightedIndex ? styles.resultItemHighlighted : undefined), role: "option", "aria-selected": index === highlightedIndex, onClick: () => handleSelectContact(item), onKeyDown: e => {
                    if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault();
                        handleSelectContact(item);
                    }
                }, tabIndex: 0 },
                React.createElement("span", { className: styles.resultIcon },
                    React.createElement(PersonRegular, { fontSize: 16 })),
                React.createElement(Text, { size: 200 }, item.name))))))),
        React.createElement(Text, { size: 100, className: styles.hintText }, "Search contacts by name, or type an email address and press Enter.")));
};
//# sourceMappingURL=RecipientField.js.map