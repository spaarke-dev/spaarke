...
export const UniversalDatasetGrid = props => {
    ...
    const headlessResult = useHeadlessMode({
        webAPI: props.headlessConfig?.webAPI || {},
    ...
    React.useEffect(() => {
        if (props.dataset) {
            ...
            PrivilegeService.getEntityPrivileges(props.headlessConfig.webAPI, props.headlessConfig.entityName).then(setPrivileges);
        }
    ...
    // Handle record click
    const handleRecordClick = React.useCallback((record) => {
    ...
    const commandContext = React.useMemo(() => {
        return {
            ...
            webAPI: props.headlessConfig?.webAPI || props.context?.webAPI || {},
            navigation: props.context?.navigation || {},
            refresh: refresh,
    ...
    return (React.createElement(FluentProvider, { theme: theme },
        React.createElement("div", { className: styles.root },
            finalConfig.showToolbar && (React.createElement(CommandToolbar, { commands: commands, context: commandContext, compact: finalConfig.compactToolbar, showOverflow: finalConfig.toolbarShowOverflow, onCommandExecuted: key => {
                    console.log(`Command executed: ${key}`);
                } })),
            React.createElement("div", { className: styles.content }, error ? (React.createElement("div", { className: styles.error },
                "Error: ",
                error)) : (React.createElement(ViewComponent, { records: records, columns: columns, selectedRecordIds: props.selectedRecordIds, onSelectionChange: props.onSelectionChange, onRecordClick: handleRecordClick, enableVirtualization: finalConfig.enableVirtualization, rowHeight: finalConfig.rowHeight, scrollBehavior: finalConfig.scrollBehavior, loading: loading, hasNextPage: hasNextPage, loadNextPage: loadNextPage }))))));
};
//# sourceMappingURL=UniversalDatasetGrid.js.map