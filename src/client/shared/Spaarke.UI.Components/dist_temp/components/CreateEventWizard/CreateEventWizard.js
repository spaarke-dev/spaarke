...
// CreateEventWizard
// ---------------------------------------------------------------------------
const CreateEventWizard = ({ open, onClose, dataService, navigationService, embedded, authenticatedFetch: authFetch, bffBaseUrl, resolveSpeContainerId, }) => {
    // -- Entity-specific form state --------------------------------------------
    const [formValid, setFormValid] = React.useState(false);
    const [formValues, setFormValues] = React.useState(EMPTY_EVENT_FORM);
    ...
    const config = React.useMemo(() => ({
        ...
        infoStep: {
            ...
            renderContent: (_wizardFiles) => (React.createElement(CreateEventStep, { dataService: dataService, onValidChange: setFormValid, onFormValues: setFormValues, initialFormValues: formValues })),
        },
        searchContacts: handleSearchContacts,
        searchOrganizations: handleSearchOrganizations,
        searchUsers: handleSearchUsers,
        resolveSpeContainerId,
        buildEmailSubject: (entityName) => `New Event: ${entityName}`,
        ...
        onFinish: async (context) => {
            ...
            if (context.uploadedFiles.length > 0 && context.speContainerId && authFetch && bffBaseUrl) {
                try {
                    ...
                    if (uploadResult.uploadedFiles.length > 0) {
                        const docResult = await entityService.createDocumentRecords('sprk_events', eventId, 'sprk_Event', uploadResult.uploadedFiles, {
            ...
            return {
                ...
                actions: (React.createElement(React.Fragment, null,
                    React.createElement(Button, { appearance: "primary", onClick: viewEvent, "aria-label": `View event: ${eventName}` }, "View Event"),
...
//# sourceMappingURL=CreateEventWizard.js.map