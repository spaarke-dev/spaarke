...
export class EventService {
     ...
     */
    async searchEventTypes(nameFilter) {
        if (!nameFilter || nameFilter.trim().length < 1)
            return [];
        const safeFilter = nameFilter.trim().replace(/'/g, "''");
        const query = `?$select=sprk_eventtype_refid,sprk_name` +
     ...
     */
    async createEvent(formValues, regardingEntityName) {
