 ...
 */
function getPrimaryRelationshipType(nodeId, edges) {
    let best;
...
// ─── Component ───────────────────────────────────────────────────────
export const MiniGraph = ({ nodes, edges, width, height, onClick }) => {
    ...
    const forceNodes = React.useMemo(() => nodes.map(n => ({
        id: n.id,
        label: n.label,
    ...
    const forceEdges = React.useMemo(() => edges.map(e => ({
        source: e.source,
        target: e.target,
    ...
    if (nodes.length === 0) {
        return React.createElement("div", { ref: containerRef, style: { width: '100%', height: '100%', minHeight: '80px' } });
    }
    return (React.createElement("div", { ref: containerRef, style: { width: '100%', height: '100%', minHeight: '80px' } },
        React.createElement("svg", { width: "100%", height: "100%", viewBox: `0 0 ${effectiveWidth} ${effectiveHeight}`, preserveAspectRatio: "xMidYMid meet", onClick: onClick, style: {
                cursor: onClick ? 'pointer' : 'default',
                display: 'block',
            }, role: "img", "aria-label": `Relationship graph preview with ${nodes.length} documents` },
            normalized.edges.map((e, i) => {
                const relType = e.relationshipType;
                const similarity = e.weight ?? 0.5;
                return (React.createElement("line", { key: `e-${i}`, x1: e.px1, y1: e.py1, x2: e.px2, y2: e.py2, stroke: getRelationshipStroke(relType), strokeWidth: 1.5, opacity: 0.3 + similarity * 0.4 }));
            }),
            normalized.nodes.map(n => {
                ...
                return (React.createElement("circle", { key: n.id, cx: n.px, cy: n.py, r: r, fill: fill, stroke: isSource ? tokens.colorBrandStroke1 : tokens.colorNeutralStroke2, strokeWidth: isSource ? 2 : 1 }));
            }))));
};
export default MiniGraph;
//# sourceMappingURL=MiniGraph.js.map