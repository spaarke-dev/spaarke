 ...
 * ```
 */
export function computeHtmlDiff(originalHtml, proposedHtml, options) {
    ...
    for (const change of changes) {
        const segType = change.added ? 'added' : change.removed ? 'removed' : 'unchanged';
 ...
 */
function buildAnnotatedHtml(html, segments, side) {
 ...
 */
function annotateTextWithDiff(text, segments, startOffset, flatText, side) {
    if (!text.trim()) {
        ...
        if (textChar === diffChar) {
            ...
            if (segment) {
                ...
                while (ti < text.length && di < flatText.length) {
                    ...
                    if (seg2.segment !== segment)
                        break;
                    if (text[ti] !== flatText[di])
                        break;
                    run += text[ti];
                    ti++;
                    di++;
 ...
 */
function findSegmentAtOffset(segments, offset) {
    let runningOffset = 0;
 ...
 * ```
 */
export function detectBlockChanges(originalHtml, proposedHtml) {
    ...
    while (oi < originalBlocks.length || pi < proposedBlocks.length) {
        if (li < lcs.length) {
            ...
            const changeType = origBlock.text === propBlock.text ? 'unchanged' : 'modified';
            changes.push({
                type: changeType,
 ...
 */
function computeBlockLcs(original, proposed) {
