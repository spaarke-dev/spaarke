...
async function _discoverNavProps(entityLogicalName) {
    ...
    try {
        ...
        console.info(`[ProjectService] Nav-props for ${entityLogicalName}:`, entries.map((e) => `${e.columnName} → ${e.navPropName} (→ ${e.referencedEntity})`));
 ...
 */
function _findNavProp(entries, referencedEntity, columnHint) {
...
export class ProjectService {
     ...
     */
    async searchProjectTypes(nameFilter) {
        if (!nameFilter || nameFilter.trim().length < 1) {
            return [];
        }
        const safeFilter = nameFilter.trim().replace(/'/g, "''");
        const query = `?$select=sprk_projecttype_refid,sprk_name` +
     ...
     */
    async searchPracticeAreas(nameFilter) {
        if (!nameFilter || nameFilter.trim().length < 1) {
            return [];
        }
        const safeFilter = nameFilter.trim().replace(/'/g, "''");
     ...
     */
    async searchContacts(nameFilter) {
     ...
     */
    async searchOrganizations(nameFilter) {
        if (!nameFilter || nameFilter.trim().length < 2) {
            return [];
        }
        const safeFilter = nameFilter.trim().replace(/'/g, "''");
        const query = `?$select=sprk_organizationid,sprk_name` +
            `&$filter=contains(sprk_name,'${safeFilter}')` +
            `&$orderby=sprk_name asc` +
            `&$top=10`;
     ...
     */
    async createProject(formValues) {
