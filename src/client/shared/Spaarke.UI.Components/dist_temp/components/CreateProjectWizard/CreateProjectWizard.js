 ...
 */
async function associateProjectWithMatter(projectId, matterId) {
 ...
 */
async function associateProjectWithAccount(dataService, projectId, accountId) {
    ...
    try {
        const orgUrl = window.location.origin;
        const resp = await fetch(`${orgUrl}/api/data/v9.0/EntityDefinitions(LogicalName='sprk_project')/ManyToOneRelationships` +
            `?$select=ReferencingAttribute,ReferencingEntityNavigationPropertyName,ReferencedEntity`, { credentials: 'include' });
        if (resp.ok) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const json = await resp.json();
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const rels = json.value ?? [];
            const accountRel = rels.find((r) => r.ReferencedEntity === 'account');
...
// ---------------------------------------------------------------------------
async function searchContactsAsLookup(dataService, query) {
    if (!query || query.trim().length < 2)
        return [];
    const safeFilter = query.trim().replace(/'/g, "''");
    const options = `?$select=contactid,fullname,emailaddress1` +
    ...
    return result.entities.map((e) => ({
        id: e['contactid'],
        name: e['fullname'] + (e['emailaddress1'] ? ` (${e['emailaddress1']})` : ''),
    }));
}
async function searchOrganizationsAsLookup(dataService, query) {
    if (!query || query.trim().length < 2)
        return [];
    const safeFilter = query.trim().replace(/'/g, "''");
    const options = `?$select=sprk_organizationid,sprk_name` +
...
async function searchUsersAsLookup(dataService, query) {
    if (!query || query.trim().length < 2)
        return [];
    const safeFilter = query.trim().replace(/'/g, "''");
    const options = `?$select=systemuserid,fullname,internalemailaddress` +
...
async function searchMatterTypesAsLookup(dataService, query) {
    if (!query || query.trim().length < 1)
        return [];
    const safeFilter = query.trim().replace(/'/g, "''");
    const options = `?$select=sprk_mattertype_refid,sprk_mattertypename` +
...
async function searchPracticeAreasAsLookup(dataService, query) {
    if (!query || query.trim().length < 1)
        return [];
    const safeFilter = query.trim().replace(/'/g, "''");
    const options = `?$select=sprk_practicearea_refid,sprk_practiceareaname` +
...
// ---------------------------------------------------------------------------
function buildWebApiAdapter(dataService) {
    return {
        retrieveMultipleRecords: async (entityLogicalName, options, _maxPageSize) => {
            const result = await dataService.retrieveMultipleRecords(entityLogicalName, options);
            return { entities: result.entities };
        },
        retrieveRecord: (entityLogicalName, id, options) => dataService.retrieveRecord(entityLogicalName, id, options),
...
// CreateProjectWizard
// ---------------------------------------------------------------------------
const CreateProjectWizard = ({ open, onClose, dataService, uploadService: _uploadService, navigationService, embedded, authenticatedFetch: authFetch, bffBaseUrl, resolveSpeContainerId, }) => {
    // ── Entity-specific form state ──────────────────────────────────────────
    const [formValid, setFormValid] = React.useState(false);
    const [formValues, setFormValues] = React.useState(EMPTY_PROJECT_FORM);
    ...
    const handleSearchMatterTypes = React.useCallback((query) => searchMatterTypesAsLookup(dataService, query), [dataService]);
    const handleSearchPracticeAreas = React.useCallback((query) => searchPracticeAreasAsLookup(dataService, query), [dataService]);
    ...
    const config = React.useMemo(() => ({
        ...
        entityLabel: 'project',
        filesStepSubtitle: 'Upload documents to associate with this project. The AI will extract key information to assist with project setup.',
        ...
        infoStep: {
            ...
            renderContent: (wizardFiles) => (React.createElement(CreateProjectStep, { dataService: dataService, onValidChange: setFormValid, onFormValues: setFormValues, uploadedFiles: wizardFiles, initialFormValues: formValues, authenticatedFetch: authFetch, bffBaseUrl: bffBaseUrl, navigationService: navigationService })),
        },
        ...
        buildEmailSubject: (entityName) => `New Project: ${entityName}`,
        ...
        onFinish: async (context) => {
            ...
            const mergedFormValues = { ...currentFormValues };
            ...
            if (context.selectedActions.includes('assign-counsel') && context.followOn.assignWorkName.trim()) {
                try {
                    ...
                    warnings.push(`Work assignment could not be created (${message}). ` +
            ...
            if (context.selectedActions.includes('create-event') && context.followOn.createEventName.trim()) {
                try {
                    ...
                    if (eventResult.success) {
                        ...
                        warnings.push(`Event could not be created (${eventResult.errorMessage ?? 'unknown error'}). ` +
                            'You can create it manually from the project record.');
                    }
                }
                catch (err) {
                    const message = err instanceof Error ? err.message : 'Unknown error';
                    warnings.push(`Event could not be created (${message}). ` +
                        'You can create it manually from the project record.');
                }
            }
            ...
            if (association?.recordId) {
                try {
                    ...
                    warnings.push(`Association with "${association.recordName}" could not be created: ${message} — ` +
            ...
            if (context.uploadedFiles.length > 0 && context.speContainerId && authFetch && bffBaseUrl) {
                try {
                    ...
                    if (uploadResult.uploadedFiles.length > 0) {
                        const docResult = await entityService.createDocumentRecords('sprk_projects', projectId, 'sprk_Project', uploadResult.uploadedFiles, {
            ...
            if (provisioningWarning) {
                warnings.push(`Secure Project provisioning failed: ${provisioningWarning} — ` +
            ...
            return {
                ...
                actions: (React.createElement(React.Fragment, null,
                    React.createElement(Button, { appearance: "primary", onClick: viewProject, "aria-label": `View project: ${projectName}` }, "View Project"),
...
//# sourceMappingURL=CreateProjectWizard.js.map