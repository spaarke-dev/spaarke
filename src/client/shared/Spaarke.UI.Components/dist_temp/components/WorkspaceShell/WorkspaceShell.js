...
const renderSectionBody = (section) => {
    switch (section.type) {
        case "action-cards": {
            const s = section;
            return React.createElement(ActionCardRow, { cards: s.cards, onCardClick: s.onCardClick, disabledCards: s.disabledCards, maxVisible: s.maxVisible });
        }
        case "metric-cards": {
            const s = section;
            return React.createElement(MetricCardRow, { cards: s.cards });
        }
        case "content": {
            const s = section;
...
const SectionPanelWrapper = ({ section, paddedClassName, }) => {
    ...
    return (React.createElement(SectionPanel, { title: section.title, titleContent: section.titleContent, badgeCount: section.badgeCount, toolbar: section.toolbar, className: section.className, style: section.style }, needsPadding ? (React.createElement("div", { className: paddedClassName }, body)) : (body)));
};
// ---------------------------------------------------------------------------
// Component
// ---------------------------------------------------------------------------
 ...
 * ```
 */
export const WorkspaceShell = ({ config, className, style, }) => {
    ...
    const renderSection = React.useCallback((section) => (React.createElement(SectionPanelWrapper, { key: section.id, section: section, paddedClassName: contentPaddingStyles.padded })), [contentPaddingStyles.padded]);
    // -------------------------------------------------------------------------
    // Single-column layout
    // -------------------------------------------------------------------------
    if (config.layout === "single-column") {
        return (React.createElement("div", { className: `${shellStyles.shell}${className ? ` ${className}` : ""}`, style: style }, config.sections.map(renderSection)));
    }
    // -------------------------------------------------------------------------
    // Rows layout
    // -------------------------------------------------------------------------
    const rows = config.rows ?? [];
    return (React.createElement("div", { className: `${shellStyles.shell}${className ? ` ${className}` : ""}`, style: style }, rows.map((row) => {
        ...
        return (React.createElement("div", { key: row.id, className: shellStyles.row, style: {
            ...
            }, "data-columns-small": columnsSmall }, rowSections.map(renderSection)));
    })));
};
