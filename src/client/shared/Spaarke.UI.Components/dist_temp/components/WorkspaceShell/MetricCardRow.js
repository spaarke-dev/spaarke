 ...
 * ```
 */
export const MetricCardRow = ({ cards, className, }) => {
    const styles = useStyles();
    return (React.createElement("div", { className: `${styles.grid}${className ? ` ${className}` : ""}`, role: "group", "aria-label": "Summary metrics" }, cards.map((config) => (React.createElement(MetricCard, { key: config.id, label: config.label, icon: config.icon, ariaLabel: config.ariaLabel, value: config.value, isLoading: config.isLoading, trend: config.trend, badgeVariant: config.badgeVariant, badgeCount: config.badgeCount, onClick: config.onClick })))));
};
MetricCardRow.displayName = "MetricCardRow";
//# sourceMappingURL=MetricCardRow.js.map