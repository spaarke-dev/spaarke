 ...
 */
async function extractTextFromFiles(files, config, signal) {
 ...
 */
async function searchDocuments(query, config, signal) {
    const bffBaseUrl = config.getBffBaseUrl();
 ...
 */
async function searchRecords(query, recordTypes, config, signal) {
    const bffBaseUrl = config.getBffBaseUrl();
 ...
 */
export async function runFindSimilar(files, config, signal) {
    // Step 1: Extract text
...
//# sourceMappingURL=findSimilarService.js.map