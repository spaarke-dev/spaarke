 ...
 */
async function getBusinessUnitContainerId(dataService) {
    ...
    for (const frame of frames) {
        try {
            ...
            if (xrm?.Utility?.getUserId) {
                userId = xrm.Utility.getUserId().replace(/^\{|\}$/g, '');
                if (userId)
                    break;
            }
        }
    ...
    const buId = userRecord['_businessunitid_value'];
    if (!buId) {
    ...
    const containerId = buRecord['sprk_containerid'];
    if (!containerId) {
        throw new Error(`${LOG_PREFIX} Business unit ${buId} does not have a container configured (sprk_containerid is empty). ` +
 ...
 */
async function createDocumentRecord(authenticatedFetch, bffBaseUrl, fileName, containerId) {
    ...
    const documentId = json?.data?.sprk_documentid ??
        json?.data?.id ??
        json?.id;
    if (!documentId) {
 ...
 */
async function createDocumentRecords(authenticatedFetch, bffBaseUrl, files, containerId) {
    const settled = await Promise.allSettled(files.map((f) => createDocumentRecord(authenticatedFetch, bffBaseUrl, f.name, containerId)
    ...
    return settled.map((result) => result.status === 'fulfilled'
        ? result.value
        : { fileName: 'unknown', error: result.reason?.message ?? 'Unknown error' });
}
// ---------------------------------------------------------------------------
// Styles
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
export const SummarizeAnalysisStep = ({ dataService, navigationService, uploadedFiles = [], authenticatedFetch, bffBaseUrl, }) => {
    const styles = useStyles();
    // ── Playbook loading state ─────────────────────────────────────────────
    const [playbooks, setPlaybooks] = React.useState([]);
    const [isLoading, setIsLoading] = React.useState(true);
    const [selectedId, setSelectedId] = React.useState();
    const [launchStatus, setLaunchStatus] = React.useState('idle');
    ...
    const handleSelect = React.useCallback(async (playbook) => {
        ...
        try {
            ...
            if (successfulIds.length === 0) {
                setErrorMessage('All document records failed to create. ' +
            ...
            await navigationService.openDialog(PLAYBOOK_LIBRARY_WEBRESOURCE, data, { width: { value: 85, unit: '%' }, height: { value: 85, unit: '%' } });
    ...
    return (React.createElement("div", { className: styles.root },
        React.createElement("div", { className: styles.headerText },
            React.createElement(Text, { as: "h2", size: 500, weight: "semibold", className: styles.stepTitle }, "Work on Analysis"),
            React.createElement(Text, { size: 200, className: styles.stepSubtitle }, "Choose a playbook to run analysis on the uploaded files. Document records will be created automatically, then PlaybookLibrary will open.")),
        warningMessages.length > 0 && (React.createElement(MessageBar, { intent: "warning" },
            React.createElement(MessageBarBody, null,
                React.createElement("div", null, "Some files could not be saved as documents:"),
                warningMessages.map((msg, i) => (React.createElement("div", { key: i }, msg)))))),
        errorMessage && (React.createElement(MessageBar, { intent: "warning" },
            React.createElement(MessageBarBody, null, errorMessage))),
        launchStatus === 'launching' ? (React.createElement("div", { className: styles.statusContainer },
            React.createElement(Spinner, { size: "large", label: launchStatusLabel, labelPosition: "below" }))) : (React.createElement(PlaybookCardGrid, { playbooks: playbooks, selectedId: selectedId, onSelect: handleSelect, isLoading: isLoading }))));
};
//# sourceMappingURL=SummarizeAnalysisStep.js.map