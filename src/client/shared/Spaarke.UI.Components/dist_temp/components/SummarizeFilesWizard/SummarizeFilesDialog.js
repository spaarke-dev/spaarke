...
import { SummaryNextStepsStep, FOLLOW_ON_STEP_ID_MAP, FOLLOW_ON_STEP_LABEL_MAP, FOLLOW_ON_CANONICAL_ORDER, } from './SummaryNextStepsStep';
...
// ---------------------------------------------------------------------------
export const SummarizeFilesDialog = ({ open, onClose, dataService, navigationService, authenticatedFetch, bffBaseUrl, embedded, }) => {
    ...
    // ── Analysis state ────────────────────────────────────────────────────
    const [summarizeStatus, setSummarizeStatus] = React.useState('idle');
    const [summarizeResult, setSummarizeResult] = React.useState(null);
    const [summarizeError, setSummarizeError] = React.useState(null);
    ...
    const [projectFormValues, setProjectFormValues] = React.useState({ ...EMPTY_PROJECT_FORM });
    ...
    const handleSearchUsers = React.useCallback((query) => dataService ? searchUsersAsLookup(dataService, query) : Promise.resolve([]), [dataService]);
    // ── Run analysis ──────────────────────────────────────────────────────
    const runAnalysis = React.useCallback(async () => {
        ...
        setCompletedStepIds([]);
        try {
            const result = await streamSummarize(fileState.uploadedFiles, {
                onProgress: (stepId) => {
            ...
            }, controller.signal, authenticatedFetch, bffBaseUrl);
            if (!controller.signal.aborted) {
    ...
    const handleSkipAction = React.useCallback((actionId) => {
        ...
        next.forEach((actionId) => {
            if (!prev.includes(actionId)) {
                ...
                const dynamicConfig = {
                    ...
                    footerActions: (React.createElement(Button, { appearance: "subtle", onClick: () => handleSkipAction(actionId) }, "Skip")),
                    renderContent: () => {
                        if (stepId === 'followon-send-email') {
                            return (React.createElement(SummarizeSendEmailStep, { emailTo: emailToRef.current, onEmailToChange: setEmailTo, emailSubject: emailSubjectRef.current, onEmailSubjectChange: setEmailSubject, emailBody: emailBodyRef.current, onEmailBodyChange: setEmailBody, onSearchUsers: handleSearchUsers, includeShortSummary: includeShortSummaryRef.current, onIncludeShortSummaryChange: setIncludeShortSummary }));
                        }
                        if (stepId === 'followon-create-project') {
                            return (React.createElement(SummarizeCreateProjectStep, { dataService: dataService, uploadedFiles: fileStateRef.current.uploadedFiles, onValidChange: setProjectFormValid, onFormValues: setProjectFormValues, initialFormValues: projectFormValuesRef.current }));
                        }
                        if (stepId === 'followon-work-on-analysis') {
                            return (React.createElement(SummarizeAnalysisStep, { dataService: dataService, navigationService: navigationService, uploadedFiles: fileStateRef.current.uploadedFiles, authenticatedFetch: authenticatedFetch, bffBaseUrl: bffBaseUrl }));
                        }
                        return React.createElement(Text, { size: 300 }, stepLabel);
                    },
                };
    ...
    const handleFinish = React.useCallback(async () => {
        ...
        if (currentSelectedActions.includes('send-email') && currentEmailTo.trim()) {
            try {
                ...
                const response = await fetchFn(`${baseUrl}/api/communications/send`, {
                    ...
                    body: JSON.stringify({
                        to: currentEmailTo.split(/[;,]/).map((a) => a.trim()).filter(Boolean),
        ...
        return {
            ...
            body: (React.createElement(Text, { size: 300, style: { color: tokens.colorNeutralForeground2 } },
                "Your file summary is ready. ",
                actionSummary,
                ".")),
            actions: (React.createElement(React.Fragment, null,
                viewProject && (React.createElement(Button, { appearance: "primary", onClick: viewProject, "aria-label": `View project: ${createdProjectName}` }, "View Project")),
    ...
    const stepConfigs = React.useMemo(() => [
        // Step 0: Upload file(s)
        {
            ...
            renderContent: () => (React.createElement(React.Fragment, null,
                React.createElement("div", null,
                    ...
                    React.createElement(Text, { size: 200, className: styles.stepSubtitle }, "Upload one or more documents to summarize. The AI will analyze and extract key information from your files.")),
        ...
        // Step 1: Run Analysis
        {
            ...
            renderContent: () => {
                ...
                return (React.createElement(SummaryResultsStep, { status: summarizeStatus, result: summarizeResult, errorMessage: summarizeError, onRetry: runAnalysis, activeStepId: activeStepId, completedStepIds: completedStepIds }));
            },
        },
        // Step 2: Next Steps
        {
            ...
            renderContent: () => (React.createElement(SummaryNextStepsStep, { selectedActions: selectedActions, onSelectionChange: setSelectedActions, includeShortSummary: includeShortSummary, onIncludeShortSummaryChange: setIncludeShortSummary })),
        },
    ], [
    ...
    // ── Render ────────────────────────────────────────────────────────────
    return (React.createElement(WizardShell, { ref: shellRef, open: open, title: "Summarize New File(s)", ariaLabel: "Summarize New File(s)", steps: stepConfigs, onClose: onClose, onFinish: handleFinish, finishingLabel: "Processing\u2026", finishLabel: "Done", embedded: embedded, hideTitle: embedded }));
};
// Default export enables React.lazy() dynamic import for bundle-size optimization.
export default SummarizeFilesDialog;
//# sourceMappingURL=SummarizeFilesDialog.js.map