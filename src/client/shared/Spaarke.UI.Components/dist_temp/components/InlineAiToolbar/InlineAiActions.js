 ...
 * ```
 */
export const InlineAiActions = ({ actions, onAction }) => {
    const styles = useStyles();
    const handleMouseDown = React.useCallback((action) => (event) => {
    ...
    return (React.createElement("div", { className: styles.actionsRow, role: "group", "aria-label": "Inline AI actions" }, actions.map(action => (React.createElement(Tooltip, { key: action.id, content: action.description ?? action.label, relationship: "description", positioning: "below" },
        React.createElement(Button, { appearance: "subtle", size: "small", className: styles.actionButton, icon: React.createElement("span", { className: styles.actionButtonIcon, "aria-hidden": "true" }, action.icon), onMouseDown: handleMouseDown(action), "aria-label": action.description ?? action.label, "data-action-id": action.id, "data-action-type": action.actionType }, action.label))))));
};
//# sourceMappingURL=InlineAiActions.js.map