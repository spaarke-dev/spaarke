 ...
 */
function groupCommandsBySource(commands) {
    const groups = [];
 ...
 */
function buildFlatCommandList(groups) {
    const flat = [];
 ...
 */
function HighlightedLabel({ label, filterText, className, highlightClass, }) {
    if (!filterText) {
    ...
    return (React.createElement("span", { className: className },
        before,
        React.createElement("span", { className: highlightClass }, match),
        after));
}
// ─────────────────────────────────────────────────────────────────────────────
// Component
 ...
 * ```
 */
export const SlashCommandMenu = ({ visible, commands, filterText, onSelect, onDismiss, }) => {
    ...
    const handleItemClick = React.useCallback((command) => {
        onSelect(command);
    }, [onSelect]);
     ...
     */
    const getCategoryIconClass = (source) => {
        switch (source) {
            case 'playbook':
    ...
    const renderItem = (command, absoluteIndex) => (React.createElement("li", { key: command.id, id: `slash-cmd-item-${command.id}`, role: "option", "aria-selected": absoluteIndex === focusedIndex, className: mergeClasses(styles.item, absoluteIndex === focusedIndex && styles.itemFocused), onMouseEnter: () => handleItemMouseEnter(absoluteIndex), onMouseDown: (e) => {
            // Use mousedown to avoid losing input focus
            e.preventDefault();
            handleItemClick(command);
        }, "data-testid": `slash-cmd-item-${command.id}` },
        command.icon && (React.createElement("span", { className: styles.itemIcon, "aria-hidden": "true" }, command.icon)),
        React.createElement("span", { className: styles.itemContent },
            React.createElement(HighlightedLabel, { label: command.label, filterText: filterText, className: styles.itemLabel, highlightClass: styles.highlightMatch }),
            React.createElement(Text, { className: styles.itemTrigger, as: "span" }, command.trigger),
            React.createElement(Text, { className: styles.itemDescription, as: "span", title: command.description }, command.description),
            command.sourceName && (React.createElement(Text, { className: styles.itemSourceSubtitle, as: "span" },
                "From: ",
                command.sourceName)))));
    // Track the running absolute index across groups for keyboard navigation
    let absoluteIndex = 0;
    return (React.createElement("div", { className: mergeClasses(styles.container, !visible && styles.hidden), role: "dialog", "aria-label": "Slash commands", "aria-hidden": !visible, "data-testid": "slash-command-menu" }, flatCommands.length === 0 ? (React.createElement("div", { className: styles.emptyState, role: "status" },
        "No commands match \u201C/",
        filterText,
        "\u201D")) : (React.createElement("ul", { ref: listRef, role: "listbox", "aria-label": "Available commands", "aria-activedescendant": activeDescendantId, className: styles.list, onKeyDown: handleKeyDown, 
        // Make the list focusable so keyboard events register when
        // the parent wires up focus management
        tabIndex: -1 }, groups.map(group => {
        const startIndex = absoluteIndex;
        absoluteIndex += group.commands.length;
        return (React.createElement(React.Fragment, { key: group.config.key },
            showCategoryHeaders && (React.createElement("li", { role: "presentation", className: styles.categoryHeader, "aria-hidden": "true", "data-testid": `slash-cmd-category-${group.config.key}` },
                React.createElement("span", { className: mergeClasses(styles.categoryIcon, getCategoryIconClass(group.config.key)), "aria-hidden": "true" }, group.config.icon),
                group.config.label)),
