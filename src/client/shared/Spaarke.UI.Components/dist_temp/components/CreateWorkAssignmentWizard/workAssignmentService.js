 ...
 */
function _resolveDocNavProp(entries, referencedEntity) {
...
export class WorkAssignmentService {
     ...
     */
    async searchRecordsByType(recordType, nameFilter) {
        ...
        if (!cfg)
            return [];
        const query = `?$select=${cfg.idField},${cfg.nameField}` +
     ...
     */
    async readRecordForPrefill(recordType, recordId) {
        const fieldMaps = {
            matter: {
                entity: 'sprk_matter',
                nameField: 'sprk_mattername',
        ...
        try {
            ...
            if (mapping.descField)
                basicFields.push(mapping.descField);
            if (mapping.priorityField)
                basicFields.push(mapping.priorityField);
            ...
            const nameVal = record[mapping.nameField];
            if (nameVal)
                result.name = nameVal;
            if (mapping.descField) {
                const descVal = record[mapping.descField];
                if (descVal)
                    result.description = descVal;
            }
            if (mapping.priorityField) {
                const prioVal = record[mapping.priorityField];
                if (prioVal)
                    result.priority = prioVal;
            }
        }
        catch (err) {
     ...
     */
    async searchContactsByOrganization(orgId, nameFilter) {
        if (!nameFilter || nameFilter.trim().length < 1)
            return [];
        const safeFilter = nameFilter.trim().replace(/'/g, "''");
     ...
     */
    async createWorkAssignment(form, _linkedDocIds, uploadedFiles, assignWork, onUploadProgress) {
        const warnings = [];
        // -- Step 1: Create Dataverse record -------------------------------------
        let workAssignmentId;
        ...
        const bindLookup = (referencedEntity, entitySet, guid, columnHint) => {
        ...
        if (form.recordId && form.recordType) {
            ...
            if (refMapping) {
                ...
                const polyWebApi = {
                    retrieveMultipleRecords: (entityLogicalName, query) => this._dataService.retrieveMultipleRecords(entityLogicalName, query),
                };
                await applyResolverFields(polyWebApi, entity, navProps, refMapping.refEntity, refMapping.entitySet, form.recordId, form.recordName, refMapping.hint);
            }
        }
        if (form.matterTypeId)
            bindLookup('sprk_mattertype_ref', 'sprk_mattertype_refs', form.matterTypeId, 'mattertype');
        ...
        if (assignWork) {
            if (assignWork.assignedAttorneyId)
                bindLookup('contact', 'contacts', assignWork.assignedAttorneyId, 'attorney');
        ...
        if (uploadedFiles.length > 0 && this._containerId) {
            ...
            if (!uploadResult.success) {
                ...
                try {
                    ...
                    const createResult = await this._entityService.createDocumentRecords('sprk_workassignments', workAssignmentId, waNavProp, uploadResult.uploadedFiles, {
     ...
     */
    async createFollowOnEvent(workAssignmentId, eventState) {
        try {
     ...
     */
    async sendEmail(workAssignmentId, workAssignmentName, to, subject, body, cc) {
...
//# sourceMappingURL=workAssignmentService.js.map