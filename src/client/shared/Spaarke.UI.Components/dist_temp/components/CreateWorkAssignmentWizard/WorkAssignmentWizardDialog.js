...
import { EMPTY_WORK_ASSIGNMENT_FORM, EMPTY_ASSIGN_WORK_STATE, EMPTY_FOLLOW_ON_EVENT_STATE, WA_FOLLOW_ON_STEP_ID_MAP, WA_FOLLOW_ON_STEP_LABEL_MAP, WA_FOLLOW_ON_CANONICAL_ORDER, } from './formTypes';
...
// Component
// ---------------------------------------------------------------------------
const WorkAssignmentWizardDialog = ({ open, onClose, dataService, authenticatedFetch, bffBaseUrl, containerId, navigationService, embedded, resolveSpeContainerId, }) => {
    const shellRef = React.useRef(null);
    // -- Form state ------------------------------------------------------------
    const [formState, setFormState] = React.useState(EMPTY_WORK_ASSIGNMENT_FORM);
    const [uploadedFiles, setUploadedFiles] = React.useState([]);
    const [selectedActions, setSelectedActions] = React.useState([]);
    const [assignWorkState, setAssignWorkState] = React.useState(EMPTY_ASSIGN_WORK_STATE);
    const [followOnEventState, setFollowOnEventState] = React.useState(EMPTY_FOLLOW_ON_EVENT_STATE);
    const [emailTo, setEmailTo] = React.useState('');
    const [emailSubject, setEmailSubject] = React.useState('');
    const [emailBody, setEmailBody] = React.useState('');
    // Pre-fill values from selected record (loaded asynchronously)
    const [prefillValues, setPrefillValues] = React.useState(undefined);
    ...
    const handleSelectWorkValid = React.useCallback((valid) => {
        selectWorkValidRef.current = valid;
        shellRef.current?.requestUpdate();
    }, []);
    const handleSelectWorkValues = React.useCallback((values) => {
    ...
    React.useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const values = await serviceRef.current.readRecordForPrefill(recordType, recordId);
                if (!cancelled) {
    ...
    const handleUploadedFilesChange = React.useCallback((files) => setUploadedFiles(files), []);
    const handleEnterInfoValid = React.useCallback((valid) => {
        enterInfoValidRef.current = valid;
        shellRef.current?.requestUpdate();
    }, []);
    const handleEnterInfoValues = React.useCallback((values) => {
        setFormState((prev) => ({ ...prev, ...values }));
    }, []);
    const handleFollowOnEventValid = React.useCallback((valid) => {
    ...
    const prevSelectedActionsRef = React.useRef([]);
    React.useEffect(() => {
                ...
                const dynamicConfig = {
                    ...
                    renderContent: () => {
                        if (stepId === 'followon-wa-assign-work') {
                            return (React.createElement(AssignWorkStep, { dataService: dataService, authenticatedFetch: authenticatedFetch, bffBaseUrl: bffBaseUrl, containerId: containerId, onFormValues: setAssignWorkState, initialValues: assignWorkStateRef.current }));
                        }
                        if (stepId === 'followon-wa-send-email') {
                            return (React.createElement(SendEmailStep, { title: "Send Email", emailTo: emailToRef.current, onEmailToChange: setEmailTo, emailCc: emailCcRef.current, onEmailCcChange: setEmailCc, emailSubject: emailSubjectRef.current, onEmailSubjectChange: setEmailSubject, emailBody: emailBodyRef.current, onEmailBodyChange: setEmailBody, onSearchUsers: handleSearchUsers }));
                        }
                        if (stepId === 'followon-wa-create-event') {
                            return (React.createElement(CreateFollowOnEventStep, { dataService: dataService, onValidChange: handleFollowOnEventValid, onFormValues: setFollowOnEventState, initialValues: followOnEventStateRef.current }));
                        }
                        return null;
                    },
                };
    ...
    React.useEffect(() => {
        if (selectedActions.includes('send-email') && emailSubject === '' && formStateRef.current.name) {
            ...
            if (f.matterTypeName)
                lines.push(`Matter Type: ${f.matterTypeName}`);
            if (f.practiceAreaName)
                lines.push(`Practice Area: ${f.practiceAreaName}`);
            if (f.responseDueDate)
                lines.push(`Response Due Date: ${f.responseDueDate}`);
    ...
    const steps = React.useMemo(() => [
        {
            ...
            isSkippable: true,
            renderContent: () => (React.createElement(SelectWorkStep, { onValidChange: handleSelectWorkValid, onFormValues: handleSelectWorkValues, navigationService: navigationService })),
        },
        {
            id: 'add-files',
            label: 'Add Files',
            canAdvance: () => true,
            isSkippable: true,
            renderContent: () => (React.createElement(AddFilesStep, { onUploadedFilesChange: handleUploadedFilesChange })),
        },
        {
            id: 'enter-info',
            label: 'Enter Info',
            ...
            renderContent: () => {
                if (isPrefillRef.current) {
                    return (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: tokens.spacingVerticalL, padding: tokens.spacingVerticalXXL } },
                        React.createElement(Spinner, { size: "medium", label: "Loading record details..." })));
                }
                return (React.createElement(EnterInfoStep, { dataService: dataService, onValidChange: handleEnterInfoValid, onFormValues: handleEnterInfoValues, initialValues: prefillRef.current, uploadedFiles: uploadedFilesRef.current, hasInitialValues: !!prefillRef.current, authenticatedFetch: authenticatedFetch, bffBaseUrl: bffBaseUrl }));
            },
        },
        {
            id: 'next-steps',
            ...
            renderContent: () => (React.createElement(NextStepsSelectionStep, { selectedActions: selectedActions, onSelectedActionsChange: setSelectedActions })),
        },
    ...
    const handleFinish = React.useCallback(async () => {
        ...
        const result = await service.createWorkAssignment(form, [], // No linked doc IDs -- docs are on the associated record
        files, actions.includes('assign-work') ? assignWorkStateRef.current : undefined);
        if (result.status === 'error') {
            return {
                icon: React.createElement(CheckmarkCircleFilled, { fontSize: 64, style: { color: tokens.colorPaletteRedForeground1 } }),
                title: 'Error',
                body: (React.createElement(Text, { size: 300, style: { color: tokens.colorNeutralForeground2 } }, result.errorMessage ?? 'An unknown error occurred.')),
                actions: (React.createElement(Button, { appearance: "primary", onClick: onClose }, "Close")),
        ...
        if (actions.includes('send-email')) {
            const emailResult = await service.sendEmail(waId, form.name, emailToRef.current, emailSubjectRef.current, emailBodyRef.current, emailCcRef.current);
        ...
        return {
            ...
            actions: (React.createElement(React.Fragment, null,
                React.createElement(Button, { appearance: "primary", onClick: viewRecord, "aria-label": `View record: ${form.name}` }, "View Record"),
    ...
    // -- Render ----------------------------------------------------------------
    return (React.createElement(WizardShell, { ref: shellRef, open: open, title: "Create Work Assignment", steps: steps, onClose: onClose, onFinish: handleFinish, finishLabel: "Create", finishingLabel: finishingLabel, embedded: embedded, hideTitle: embedded }));
};
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
async function resolveCurrentUserEmail(dataService) {
    try {
        ...
        return result['internalemailaddress'] || null;
    }
    catch {
        return null;
    }
...
//# sourceMappingURL=WorkAssignmentWizardDialog.js.map