 ...
 * ```
 */
export const SprkChatPredefinedPrompts = ({ prompts, onSelect, disabled = false, }) => {
    const styles = useStyles();
    if (prompts.length === 0) {
        return null;
    }
    return (React.createElement("div", { className: styles.root, role: "region", "aria-label": "Suggested prompts" },
        React.createElement("div", { style: {
                display: 'flex',
                alignItems: 'center',
                gap: tokens.spacingHorizontalXS,
            } },
            React.createElement(LightbulbRegular, null),
            React.createElement(Text, { className: styles.heading }, "Try asking")),
        React.createElement("div", { className: styles.promptGrid }, prompts.map(prompt => (React.createElement(Button, { key: prompt.key, appearance: "outline", className: styles.promptButton, onClick: () => onSelect(prompt.prompt), disabled: disabled, "data-testid": `predefined-prompt-${prompt.key}` }, prompt.label))))));
};
export default SprkChatPredefinedPrompts;
//# sourceMappingURL=SprkChatPredefinedPrompts.js.map