 ...
 * ```
 */
export const SprkChatSuggestions = ({ suggestions, onSelect, visible }) => {
    ...
    const handleKeyDown = React.useCallback((event) => {
        ...
        const focusable = Array.from(container.querySelectorAll('[data-suggestion-chip]'));
    ...
    return (React.createElement("div", { ref: containerRef, className: rootClassName, role: "group", "aria-label": "Follow-up suggestions", onKeyDown: handleKeyDown, "data-testid": "sprkchat-suggestions" }, displaySuggestions.map((suggestion, index) => {
        const displayText = truncateText(suggestion, MAX_TEXT_LENGTH);
        const isTruncated = suggestion.length > MAX_TEXT_LENGTH;
        return (React.createElement(InteractionTag, { key: `suggestion-${index}`, className: styles.chip, appearance: "brand", shape: "circular", size: "small" },
            React.createElement(InteractionTagPrimary, { role: "button", "aria-label": isTruncated ? suggestion : undefined, title: isTruncated ? suggestion : undefined, onClick: () => onSelect(suggestion), "data-suggestion-chip": "", "data-testid": `suggestion-chip-${index}` },
                React.createElement("span", { className: styles.chipText }, displayText))));
    })));
};
export default SprkChatSuggestions;
//# sourceMappingURL=SprkChatSuggestions.js.map