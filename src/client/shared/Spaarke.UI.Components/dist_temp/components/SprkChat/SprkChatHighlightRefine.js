...
import { makeStyles, shorthands, tokens, Button, Input, Spinner, Text, Badge, Tooltip, } from '@fluentui/react-components';
 ...
 * ```
 */
export const SprkChatHighlightRefine = ({ contentRef, onRefine, onRefineRequest, isRefining = false, crossPaneSelection, quickActions, onDismiss, }) => {
    ...
    const resolvedQuickActions = quickActions !== undefined ? quickActions : DEFAULT_QUICK_ACTIONS;
    ...
    const activeSource = hasCrossPaneSelection ? 'editor' : 'chat';
     ...
     */
    const submitRefine = React.useCallback((instructionText, quickActionKey) => {
    ...
    const handleInstructionKeyDown = React.useCallback((e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            handleSubmitRefine();
        }
    ...
    const handleInstructionChange = React.useCallback((_event, data) => {
        setInstruction(data.value);
    }, []);
    // Compute the preview text (truncated for display)
    const previewText = activeText.length > PREVIEW_MAX_LENGTH ? activeText.substring(0, PREVIEW_MAX_LENGTH) + '\u2026' : activeText;
    ...
    const renderSourceIcon = () => (React.createElement("span", { className: styles.sourceIcon }, activeSource === 'editor' ? React.createElement(Document20Regular, null) : React.createElement(Chat20Regular, null)));
    /** Renders the quick action chips row */
    const renderQuickActions = () => {
        if (resolvedQuickActions.length === 0 || isRefining) {
            return null;
        }
        return (React.createElement("div", { className: styles.quickActionsRow, "data-testid": "quick-actions-row" }, resolvedQuickActions.map(action => (React.createElement(Button, { key: action.key, appearance: "outline", size: "small", className: styles.quickActionChip, onClick: () => handleQuickAction(action), disabled: !activeFullText || isRefining, "data-testid": `quick-action-${action.key}` }, action.label)))));
    };
    /** Renders the selected text preview, optionally wrapped in a Tooltip */
    const renderPreviewText = () => {
        const textElement = React.createElement(Text, { className: styles.selectedText },
            "\u201C",
            previewText,
            "\u201D");
        if (showTooltip) {
            return (React.createElement(Tooltip, { content: tooltipContent, relationship: "description", positioning: "below" }, textElement));
        }
        return textElement;
    };
    // -----------------------------------------------------------------------
    ...
    if (hasCrossPaneSelection) {
        ...
        return (React.createElement("div", { ref: toolbarRef, className: crossPaneClassName, role: "toolbar", "aria-label": "Refine editor selection", "data-testid": "highlight-refine-toolbar-cross-pane" },
            React.createElement("div", { className: styles.headerRow },
                React.createElement("div", { className: styles.sourceLabel },
                    renderSourceIcon(),
                    React.createElement(Badge, { appearance: "filled", color: "brand", size: "small" }, "Selected in Editor")),
                React.createElement(Button, { appearance: "subtle", icon: React.createElement(Dismiss20Regular, null), onClick: handleAnimatedDismiss, size: "small", "aria-label": "Dismiss", "data-testid": "refine-dismiss-button" })),
            renderPreviewText(),
            !showInput ? (React.createElement(React.Fragment, null,
                React.createElement("div", { className: styles.refineRow },
                    React.createElement(Button, { appearance: "subtle", icon: React.createElement(TextEditStyle20Regular, null), onClick: handleRefineClick, disabled: isRefining, size: "small", "data-testid": "refine-button" }, "Refine")),
                renderQuickActions())) : (React.createElement(React.Fragment, null,
                React.createElement("div", { className: styles.inputRow }, isRefining ? (React.createElement(Spinner, { size: "tiny", label: "Refining..." })) : (React.createElement(React.Fragment, null,
                    React.createElement(Input, { value: instruction, onChange: handleInstructionChange, onKeyDown: handleInstructionKeyDown, placeholder: "How should this be refined?", size: "small", "aria-label": "Refinement instruction", "data-testid": "refine-instruction-input" }),
                    React.createElement(Button, { appearance: "primary", onClick: handleSubmitRefine, disabled: !instruction.trim(), size: "small", "data-testid": "refine-submit-button" }, "Go")))),
                renderQuickActions()))));
    }
    // -----------------------------------------------------------------------
    ...
    return (React.createElement("div", { ref: toolbarRef, className: localClassName, style: {
            top: toolbarPosition.top,
            left: toolbarPosition.left,
        }, role: "toolbar", "aria-label": "Refine selection", "data-testid": "highlight-refine-toolbar" },
        React.createElement("div", { className: styles.headerRow },
            React.createElement("div", { className: styles.sourceLabel },
                renderSourceIcon(),
                React.createElement(Badge, { appearance: "tint", color: "informative", size: "small" }, "Selected in Chat")),
            React.createElement(Button, { appearance: "subtle", icon: React.createElement(Dismiss20Regular, null), onClick: handleAnimatedDismiss, size: "small", "aria-label": "Dismiss", "data-testid": "refine-dismiss-button" })),
        renderPreviewText(),
        !showInput ? (React.createElement(React.Fragment, null,
            React.createElement("div", { className: styles.refineRow },
...
//# sourceMappingURL=SprkChatHighlightRefine.js.map