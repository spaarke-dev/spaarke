...
import { makeStyles, shorthands, tokens, Text, Spinner, MessageBar, MessageBarBody, mergeClasses, } from '@fluentui/react-components';
 ...
 */
function filterActions(actions, filterText) {
    if (!filterText) {
        return actions;
    }
...
const HighlightedText = ({ text, highlight, className }) => {
    ...
    return React.createElement('span', { className }, before, React.createElement('span', { className: styles.highlight }, match), after);
};
// ─────────────────────────────────────────────────────────────────────────────
// Component
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * ```
 */
export const SprkChatActionMenu = React.forwardRef(({ actions, isOpen, onSelect, onDismiss, filterText = '', anchorRef, isLoading = false, errorMessage }, ref) => {
    const styles = useStyles();
    ...
    const findNextEnabledIndex = React.useCallback((currentIndex, direction) => {
    ...
    const handleItemMouseEnter = React.useCallback((action) => {
    ...
    const renderContent = () => {
        ...
        if (errorMessage) {
            return React.createElement('div', { className: styles.errorState, 'data-testid': 'action-menu-error' }, React.createElement(MessageBar, { intent: 'error' }, React.createElement(MessageBarBody, null, errorMessage)));
        }
        if (hasResults) {
            return React.createElement(React.Fragment, null, ...groups.map(group => React.createElement('div', {
                key: group.category,
                role: 'group',
                'aria-label': group.label,
            }, 
            // Category header
            React.createElement('div', { className: styles.categoryHeader }, React.createElement('span', { className: styles.categoryIcon }, group.icon), React.createElement(Text, { size: 100 }, group.label)), 
            // Action items
            group.items.map(action => {
                ...
                return React.createElement('div', {
                    ...
                    className: mergeClasses(styles.menuItem, isActive && styles.menuItemActive, isDisabled && styles.menuItemDisabled),
                    role: 'option',
                ...
                // Label + description
                React.createElement('div', { className: styles.menuItemContent }, React.createElement(HighlightedText, {
                    text: action.label,
        ...
        return React.createElement('div', { className: styles.emptyState, 'data-testid': 'action-menu-empty' }, React.createElement(Text, { size: 200 }, 'No matching actions'));
    };
    return React.createElement('div', {
        ref: menuRef,
        ...
        'aria-activedescendant': hasResults && !isLoading && !errorMessage ? `action-menu-item-${flatList[activeIndex]?.id}` : undefined,
        tabIndex: 0,
...
//# sourceMappingURL=SprkChatActionMenu.js.map