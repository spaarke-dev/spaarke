...
import { SparkleRegular, SearchRegular, DocumentTextRegular, CheckmarkCircleRegular, LightbulbRegular, ArrowSyncRegular, StopRegular, } from '@fluentui/react-icons';
 ...
 */
function mapInlineActionsToChipActions(actions) {
    return actions.map(a => ({
        id: a.id,
        label: a.label,
        icon: mapActionIdToIcon(a.id),
        actionType: (a.actionType === 'diff' ? 'diff' : 'chat'),
        description: a.description,
    }));
}
export const SprkChat = ({ sessionId: initialSessionId, documentId, playbookId, analysisId, apiBaseUrl, accessToken, onSessionCreated, onPlaybookChange, className, documents = [], playbooks = [], predefinedPrompts = [], contentRef: externalContentRef, maxCharCount, hostContext, bridge, onDocumentStreamEvent: onDocumentStreamEventProp, initialMessages, }) => {
    ...
    const quickChipActions = React.useMemo(() => {
        if (!contextMapping || contextMapping.inlineActions.length === 0)
            return [];
    ...
    const allPlaybooks = React.useMemo(() => {
        const seen = new Set(playbooks.map(p => p.id));
    ...
    const { session, messages, isLoading: isSessionLoading, error: sessionError, createSession, loadHistory, switchContext, addMessage, updateLastMessage, updateLastMessageMetadata, updateMessageMetadataAt, } = chatSession;
    ...
    const { content: streamedContent, isDone: streamDone, isStreaming, isTyping, error: streamError, suggestions, citations: streamCitations, pendingPlanId, pendingPlanData, pendingActionEvent, startStream, cancelStream, clearSuggestions, clearPendingActionEvent, setOnDocumentStreamEvent, } = sseStream;
    ...
    const [editorRefineError, setEditorRefineError] = React.useState(null);
    const editorRefineAbortRef = React.useRef(null);
    ...
    const planApproveAbortRef = React.useRef(null);
    ...
    const [documentPersistenceState, setDocumentPersistenceState] = React.useState(new Map());
    ...
    const additionalDocsDebounceRef = React.useRef(null);
     ...
     */
    const handleActionConfirmationEvent = React.useCallback((data) => {
        if (!data.actionId || !data.actionName || !session)
            return;
        const action = {
     ...
     */
    const handleActionSuccessEvent = React.useCallback((data) => {
        dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Action Completed'), React.createElement(ToastBody, null, data.message || 'Action executed successfully.')), { intent: 'success', timeout: 5000 });
    }, [dispatchToast]);
    /**
     * Handle action_error SSE event (action execution failed).
     * Shows a Fluent v9 error toast (ADR-021).
     */
    const handleActionErrorEvent = React.useCallback((data) => {
        dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Action Failed'), React.createElement(ToastBody, null, data.message || 'The action could not be completed.')), { intent: 'error', timeout: 8000 });
    }, [dispatchToast]);
    /**
     ...
     */
    const handleDialogOpenEvent = React.useCallback((data) => {
        if (!data.targetPage) {
     ...
     */
    const handleNavigateEvent = React.useCallback((data) => {
    ...
    const handleActionConfirm = React.useCallback(async (action) => {
        ...
        try {
            ...
            if (result.success) {
                dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Action Completed'), React.createElement(ToastBody, null, result.message)), { intent: 'success', timeout: 5000 });
            }
            else {
                dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Action Failed'), React.createElement(ToastBody, null, result.message)), { intent: 'error', timeout: 8000 });
            }
        }
        finally {
    ...
    const handleSend = React.useCallback((messageText) => {
        ...
        startStream(`${baseUrl}/api/ai/chat/sessions/${session.sessionId}/messages`, { message: messageText, documentId }, accessToken);
    ...
    const handlePromptSelect = React.useCallback((prompt) => {
        handleSend(prompt);
    }, [handleSend]);
    ...
    const extractTenantIdFromToken = React.useCallback((token) => {
        try {
     ...
     */
    const handlePlanProceed = React.useCallback((messageIndex) => {
        if (!session || !pendingPlanId || isPlanApproving) {
            console.warn('[SprkChat] handlePlanProceed: cannot proceed — session:', !!session, ', pendingPlanId:', pendingPlanId, ', isPlanApproving:', isPlanApproving);
            return;
        }
        ...
        console.log('[SprkChat] handlePlanProceed: approving plan —', 'planId:', pendingPlanId, ', pendingPlanData:', JSON.stringify(pendingPlanData), ', sessionId:', session.sessionId);
                        ...
                        for (const line of lines) {
                            ...
                            if (event.type === 'plan_step_start' && event.data?.stepId) {
                                ...
                                updateMessageMetadataAt(messageIndex, current => ({
                                    ...current,
                                    responseType: 'plan_preview',
                                    plan: current?.plan?.map(s => (s.id === stepId ? { ...s, status: 'running' } : s)) ??
                                        current?.plan,
                                }));
                            }
                                ...
                                updateMessageMetadataAt(messageIndex, current => ({
                                    ...current,
                                    responseType: 'plan_preview',
                                    plan: current?.plan?.map(s => s.id === stepId ? { ...s, status: stepStatus, result: stepResult } : s) ?? current?.plan,
                                }));
                            }
                                ...
                                updateMessageMetadataAt(messageIndex, current => ({
                                    ...current,
                                    responseType: 'plan_preview',
                                    plan: current?.plan?.map(s => (s.status === 'completed' ? s : { ...s, status: 'failed' })) ??
                                        current?.plan,
                                }));
     ...
     */
    const handlePlanCancel = React.useCallback((_messageIndex) => {
     ...
     */
    const handlePlanEditMessage = React.useCallback((editMessage) => {
     ...
     * @param messageIndex - Index of the PlanPreviewCard message in the messages array.
     */
    const handleCancelExecution = React.useCallback((messageIndex) => {
     ...
     */
    const handleInsert = React.useCallback((content) => {
        if (!content) {
            return;
        }
     ...
     */
    const handleSaveToMatterFiles = React.useCallback(async (docId) => {
    ...
    const handleRootDragEnter = React.useCallback((e) => {
        e.preventDefault();
    ...
    const handleRootDragOver = React.useCallback((e) => {
        e.preventDefault();
    }, []);
    const handleRootDragLeave = React.useCallback((e) => {
        e.preventDefault();
        dragCounterRef.current -= 1;
    ...
    const handleRootDrop = React.useCallback((e) => {
        e.preventDefault();
        dragCounterRef.current = 0;
     ...
     */
    const handleUploadComplete = React.useCallback((doc) => {
        // Map BFF response status to document processing status
        const processingStatus = doc.status === 'ready' ? 'complete' : 'processing';
        ...
        console.debug('[SprkChat] Document uploaded and added to context:', 'docId:', doc.documentId, ', fileName:', doc.fileName, ', status:', doc.status);
    }, [addMessage]);
    /**
     * Handle document upload error from SprkChatUploadZone.
     * Shows an error toast via the Fluent v9 Toaster (ADR-021).
     */
    const handleUploadError = React.useCallback((errorMsg) => {
        dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Upload Failed'), React.createElement(ToastBody, null, errorMsg)), { intent: 'error', timeout: 8000 });
    ...
    const handleAdditionalDocumentsChange = React.useCallback((newIds) => {
     ...
     */
    const handleEditorRefine = React.useCallback((selectedText, instruction) => {
     ...
     */
    const handleChatRefine = React.useCallback((selectedText, instruction) => {
        if (!session || isStreaming) {
            return;
        }
        ...
        const baseUrl = apiBaseUrl.replace(/\/+$/, '');
        startStream(`${baseUrl}/api/ai/chat/sessions/${session.sessionId}/refine`, { selectedText, instruction }, accessToken);
     ...
     */
    const handleRefine = React.useCallback((selectedText, instruction) => {
     ...
     */
    const handleChipAction = React.useCallback((action) => {
        if (!session || isStreaming)
            return;
    ...
    React.useEffect(() => {
        ...
        const handleMessage = (event) => {
            ...
            console.debug(`[SprkChat] Inline action received — actionId: "${payload.actionId}", actionType: "${payload.actionType}", textLength: ${payload.selectedText.length}`);
        };
    ...
    const handlePlaybookChipClick = React.useCallback((pb) => {
    ...
    const handleExportWordError = React.useCallback((errorMessage) => {
        dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Export to Word failed'), React.createElement(ToastBody, null, errorMessage)), { intent: 'error' });
    }, [dispatchToast]);
    const handleExportWordSuccess = React.useCallback((_wordOnlineUrl) => {
        dispatchToast(React.createElement(Toast, null, React.createElement(ToastTitle, null, 'Document opened in Word Online')), { intent: 'success' });
    }, [dispatchToast]);
    ...
    return (React.createElement("div", { ref: rootContainerRef, className: className ? `${styles.root} ${className}` : styles.root, "data-testid": "sprkchat-root", onDragEnter: handleRootDragEnter, onDragOver: handleRootDragOver, onDragLeave: handleRootDragLeave, onDrop: handleRootDrop },
        (documents.length > 0 || allPlaybooks.length > 0) && (React.createElement(SprkChatContextSelector, { selectedDocumentId: documentId, selectedPlaybookId: playbookId, documents: documents, playbooks: allPlaybooks, onDocumentChange: handleDocumentChange, onPlaybookChange: handlePlaybookChange, disabled: isStreaming || isEditorRefining, additionalDocumentIds: additionalDocumentIds, onAdditionalDocumentsChange: handleAdditionalDocumentsChange })),
        displayError && (React.createElement("div", { className: styles.errorBanner, role: "alert", "data-testid": "chat-error-banner" }, displayError.message)),
        React.createElement("div", { className: styles.messageList, ref: messageListRef, role: "list", "aria-label": "Chat messages", "data-testid": "chat-message-list" },
            isSessionLoading && messages.length === 0 && (React.createElement("div", { className: styles.loadingContainer },
                React.createElement(Spinner, { label: "Loading chat..." }))),
            showPlaybookChips && (React.createElement("div", { className: styles.playbookChips, "data-testid": "playbook-chips" }, allPlaybooks
                .filter(pb => pb.id !== playbookId)
                .map(pb => (React.createElement(Button, { key: pb.id, appearance: "outline", size: "small", onClick: () => handlePlaybookChipClick(pb), disabled: isStreaming || !session, title: pb.description || pb.name }, pb.name))))),
            showPredefinedPrompts && (React.createElement(SprkChatPredefinedPrompts, { prompts: predefinedPrompts, onSelect: handlePromptSelect, disabled: isStreaming || !session })),
            !isSessionLoading && messages.length === 0 && !showPredefinedPrompts && (React.createElement("div", { className: styles.emptyState },
                React.createElement(Text, { size: 300 }, "No messages yet"),
                React.createElement(Text, { size: 200 }, "Send a message to start the conversation"))),
            messages.map((msg, index) => {
                ...
                return React.createElement(SprkChatMessage, { key: `msg-${index}`, ...msgProps });
            }),
            isTyping && !streamedContent && React.createElement(SprkChatTypingIndicator, null),
            suggestions.length > 0 && (React.createElement(SprkChatSuggestions, { suggestions: suggestions, onSelect: handleSuggestionSelect, visible: !isStreaming && suggestions.length > 0 })),
            React.createElement(SprkChatHighlightRefine, { contentRef: highlightContainerRef, onRefine: handleRefine, isRefining: isStreaming || isEditorRefining, crossPaneSelection: crossPaneSelection }),
            isDragging && session && (React.createElement(SprkChatUploadZone, { sessionId: session.sessionId, apiBaseUrl: apiBaseUrl, accessToken: accessToken, onUploadComplete: handleUploadComplete, onUploadError: handleUploadError, disabled: !session }))),
        quickChipActions.length > 0 && (React.createElement(QuickActionChips, { actions: quickChipActions, onChipClick: handleChipAction, containerRef: rootContainerRef, disabled: isStreaming || !session || isSessionLoading })),
        isStreaming && (React.createElement("div", { className: styles.stopGeneratingContainer, "data-testid": "stop-generating-container" },
            React.createElement(Button, { appearance: "subtle", size: "small", icon: React.createElement(StopRegular), onClick: cancelStream, "data-testid": "stop-generating-button" }, "Stop generating"))),
        React.createElement("div", { className: styles.inputToolbar, "data-testid": "chat-input-toolbar" },
            React.createElement(SprkChatExportWord, { sessionId: session?.sessionId ?? null, messages: messages, apiBaseUrl: apiBaseUrl, accessToken: accessToken, onError: handleExportWordError, onSuccess: handleExportWordSuccess })),
        React.createElement(SprkChatInput, { onSend: handleSend, disabled: isStreaming || !session || isSessionLoading, maxCharCount: maxCharCount, dynamicSlashCommands: dynamicSlashCommands }),
        React.createElement(ActionConfirmationDialog, { pendingAction: pendingAction, onConfirm: handleActionConfirm, onCancel: handleActionCancel, isConfirming: isConfirmingAction }),
        React.createElement(Toaster, { toasterId: toasterId, position: "top-end" })));
};
export default SprkChat;
//# sourceMappingURL=SprkChat.js.map