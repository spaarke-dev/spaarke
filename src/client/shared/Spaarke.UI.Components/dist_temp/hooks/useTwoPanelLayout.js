...
function loadPersistedState(storageKey, defaults) {
    try {
        ...
        if (stored !== null) {
            ...
            return {
                detailWidth: typeof parsed.detailWidth === 'number' && parsed.detailWidth > 0
                    ? parsed.detailWidth
                    : defaults.detailWidth,
                isDetailVisible: typeof parsed.isDetailVisible === 'boolean'
...
// Hook
// ---------------------------------------------------------------------------
export function useTwoPanelLayout(options = {}) {
    const { defaultDetailWidth = DEFAULT_DETAIL_WIDTH_PX, minPrimaryWidth = DEFAULT_MIN_PRIMARY_WIDTH_PX, minDetailWidth = DEFAULT_MIN_DETAIL_WIDTH_PX, storageKey = DEFAULT_STORAGE_KEY, } = options;
    const defaultState = {
    ...
    // State
    // -----------------------------------------------------------------------
    const [detailWidthPx, setDetailWidthPx] = useState(() => {
     ...
     */
    const clampDetailWidth = useCallback((width) => {
        const container = containerRef.current;
        if (!container)
            return Math.max(width, minDetailWidth);
...
//# sourceMappingURL=useTwoPanelLayout.js.map