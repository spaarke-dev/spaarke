 ...
 * ```
 */
export function createXrmDataService() {
    /**
    ...
    function getWebApi() {
        const xrm = getXrm();
        if (!xrm?.WebApi) {
            throw new Error('Xrm.WebApi is not available. Ensure this adapter is used within a Dataverse-hosted context (PCF control or Code Page).');
        }
        return xrm.WebApi;
    }
    return {
        ...
        async retrieveRecord(entityName, id, options) {
            const webApi = getWebApi();
            return webApi.retrieveRecord(entityName, id, options);
        },
        async retrieveMultipleRecords(entityName, options) {
            const webApi = getWebApi();
...
//# sourceMappingURL=xrmDataServiceAdapter.js.map