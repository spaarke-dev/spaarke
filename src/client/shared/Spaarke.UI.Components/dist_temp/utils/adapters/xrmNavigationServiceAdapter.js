 ...
 */
function normaliseDimension(dim) {
    if (dim === undefined) {
        return undefined;
    }
 ...
 * ```
 */
export function createXrmNavigationService() {
    /**
    ...
    function getNavigation() {
        const xrm = getXrm();
        if (!xrm?.Navigation) {
            throw new Error('Xrm.Navigation is not available. Ensure this adapter is used within a Dataverse-hosted context (PCF control or Code Page).');
        }
    ...
    return {
        ...
        closeDialog(result) {
            ...
            for (const frame of frames) {
                try {
                    const closeBtn = frame?.document?.querySelector('[data-id="dialogCloseIconButton"]')
                        ?? frame?.document?.querySelector('.ms-Dialog-button--close');
                    if (closeBtn) {
                        closeBtn.click();
                        return;
                    }
        ...
        async openLookup(options) {
            const xrm = getXrm();
            if (!xrm?.Utility) {
                throw new Error('Xrm.Utility is not available. Ensure this adapter is used within a Dataverse-hosted context (PCF control or Code Page).');
            }
            ...
            try {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const xrmUtility = xrm.Utility;
                // Xrm.Utility.lookupObjects returns Promise<LookupValue[]>
                // where LookupValue has: id, name, entityType
                const results = await xrmUtility.lookupObjects(lookupObjectsOptions);
...
//# sourceMappingURL=xrmNavigationServiceAdapter.js.map