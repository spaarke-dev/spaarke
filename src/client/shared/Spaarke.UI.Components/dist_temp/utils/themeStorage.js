 ...
 */
export function setUserThemePreference(theme) {
 ...
 */
export function detectDarkModeFromUrl() {
    try {
 ...
 */
export function detectDarkModeFromNavbar() {
    try {
 ...
 */
export function getEffectiveDarkMode(context) {
 ...
 */
export function resolveThemeWithUserPreference(context) {
 ...
 */
export function applyMdaTheme(theme) {
    try {
 ...
 * ```
 */
export function resolveCodePageTheme() {
    try {
        ...
        if (urlDark !== null)
            return urlDark ? webDarkTheme : webLightTheme;
        // 3. Navbar DOM color detection
        const navbarDark = detectDarkModeFromNavbar();
        if (navbarDark !== null)
            return navbarDark ? webDarkTheme : webLightTheme;
 ...
 */
export function setupThemeListener(onChange, context) {
 ...
 * ```
 */
export function setupCodePageThemeListener(onChange) {
 ...
 */
export async function syncThemeFromDataverse(webApi, userId) {
    try {
        ...
        if (result.entities.length > 0) {
            const dvTheme = result.entities[0].sprk_preferencevalue;
 ...
 */
export async function persistThemeToDataverse(webApi, userId, theme) {
    try {
        ...
        if (result.entities.length > 0) {
            ...
            const id = result.entities[0].sprk_userpreferenceid;
