 ...
 */
import { AccessRights } from '../types/CommandTypes';
/**
...
export class PrivilegeService {
     ...
     */
    static async getEntityPrivileges(webAPI, entityLogicalName, recordId) {
        try {
     ...
     */
    static async getRecordPrivileges(webAPI, entityLogicalName, recordId) {
        try {
     ...
     */
    static async executeBoundFunction(webAPI, functionUrl) {
        ...
        const context = webAPI._context || webAPI.context;
     ...
     */
    static async executeWhoAmI(webAPI) {
        try {
     ...
     */
    static async getUserSecurityRoles(webAPI, userId) {
        try {
            const response = await webAPI.retrieveMultipleRecords('systemuserroles', `?$select=roleid&$filter=systemuserid eq ${userId}`);
     ...
     */
    static async getEntityPrivilegesFromRoles(webAPI, entityLogicalName, roleIds) {
        if (roleIds.length === 0) {
            return this.getNoPrivileges();
        }
        try {
            // Step 1: Get entity metadata to find privileges associated with this entity
            const entityMetadata = await webAPI.retrieveMultipleRecords('entitydefinition', `?$select=logicalname,objecttypecode&$filter=logicalname eq '${entityLogicalName}'`);
            ...
            const rolePrivileges = await webAPI.retrieveMultipleRecords('roleprivileges', `?$select=privilegedepthmask&$expand=privilegeid($select=name,accessright)&$filter=(${privilegeFilter})`);
            // Step 3: Aggregate privileges across all roles
            let aggregateRights = AccessRights.None;
            rolePrivileges.entities.forEach((rolePrivilege) => {
     ...
     */
    static parseAccessRights(accessRightsValue) {
        let rights = 0;
        if (typeof accessRightsValue === 'string') {
            ...
            rightsArray.forEach(right => {
                if (AccessRights[right] !== undefined) {
                    rights |= AccessRights[right];
                }
            });
        }
     ...
     */
    static getPrivilegesFromDataset(dataset) {
