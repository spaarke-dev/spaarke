...
export class ConfigurationService {
     ...
     */
    async getConfigurations(entityLogicalName) {
        ...
        try {
            ...
            const result = await this.xrm.WebApi.retrieveMultipleRecords('sprk_gridconfiguration', `?$select=${select}&$filter=${filter}&$orderby=sprk_sortorder`);
            this.entityExists = true;
            const configs = result.entities.map(record => this.mapRecordToConfiguration(record));
            // Cache results
     ...
     */
    async getDefaultConfiguration(entityLogicalName) {
    ...
    async getConfigurationById(configurationId) {
        ...
        try {
            ...
            const record = await this.xrm.WebApi.retrieveRecord('sprk_gridconfiguration', configurationId, `?$select=${select}`);
            this.entityExists = true;
            return this.mapRecordToConfiguration(record);
        }
        catch (error) {
            if (this.isEntityNotFoundError(error)) {
                this.entityExists = false;
    ...
    async checkEntityExists() {
        ...
        try {
            ...
            await this.xrm.WebApi.retrieveMultipleRecords('sprk_gridconfiguration', '?$top=1&$select=sprk_gridconfigurationid');
            this.entityExists = true;
            return true;
        }
        catch (error) {
     ...
     */
    mapRecordToConfiguration(record) {
        let configJson;
        if (record.sprk_configjson) {
            try {
                configJson = JSON.parse(record.sprk_configjson);
            }
            catch {
