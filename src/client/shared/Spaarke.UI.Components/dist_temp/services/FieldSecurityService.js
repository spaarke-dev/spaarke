...
export class FieldSecurityService {
     ...
     */
    static async getFieldSecurityPermissions(webAPI, entityLogicalName, fieldNames) {
        const fieldSecurityMap = new Map();
        try {
     ...
     */
    static async getCurrentUserId(webAPI) {
     ...
     */
    static async getUserFieldSecurityProfiles(webAPI, userId) {
        try {
            // Query systemuserprofiles association
            const response = await webAPI.retrieveMultipleRecords('systemuserprofiles', `?$select=fieldsecurityprofileid&$filter=systemuserid eq ${userId}`);
     ...
     */
    static async getFieldSecurityMetadata(webAPI, entityLogicalName, fieldNames) {
        const fieldSecurityMap = new Map();
        try {
            // Query attribute metadata to check IsSecured property
            const filter = fieldNames.map(f => `LogicalName eq '${f}'`).join(' or ');
            const attributes = await webAPI.retrieveMultipleRecords('attributedefinition', `?$select=logicalname,issecured&$filter=EntityLogicalName eq '${entityLogicalName}' and (${filter})`);
            attributes.entities.forEach((attr) => {
     ...
     */
    static async getFieldPermissions(webAPI, entityLogicalName, fieldName, profileIds) {
        try {
            // Step 1: Check if field is secured
            const attributeMetadata = await webAPI.retrieveMultipleRecords('attributedefinition', `?$select=logicalname,issecured&$filter=EntityLogicalName eq '${entityLogicalName}' and LogicalName eq '${fieldName}'`);
            ...
            const permissions = await webAPI.retrieveMultipleRecords('fieldpermission', `?$select=canread,canupdate,cancreate,canreadunmasked&$filter=_attributeid_value eq ${attributeId} and (${profileFilter})`);
            ...
            permissions.entities.forEach((perm) => {
                // Check for value 4 (Allowed) per Microsoft field_security_permission_type choice
                if (perm.canread === 4 || perm.canread === true)
                    canRead = true;
                if (perm.canupdate === 4 || perm.canupdate === true)
                    canUpdate = true;
                if (perm.cancreate === 4 || perm.cancreate === true)
                    canCreate = true;
     ...
     */
    static getFieldSecurityFromDataset(dataset) {
        const fieldSecurityMap = new Map();
        // PCF dataset columns have security information
        dataset.columns.forEach(column => {
            const security = column.security;
            if (security) {
...
//# sourceMappingURL=FieldSecurityService.js.map