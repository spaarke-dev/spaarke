 ...
 */
import { MappingErrorCode, FieldType, CompatibilityMode, SyncMode, MappingDirection, CompatibilityLevel, STRICT_TYPE_COMPATIBILITY, } from '../types/FieldMappingTypes';
/**
...
export class FieldMappingService {
     ...
     */
    async getProfileForEntityPair(sourceEntity, targetEntity) {
     ...
     */
    async getProfilesForSource(sourceEntity) {
     ...
     */
    async getSourceValues(sourceEntity, recordId, fields) {
        if (fields.length === 0) {
            return {};
        }
     ...
     */
    async applyMappings(sourceRecordId, targetRecord, profile, options = {}) {
        ...
        const firstPassResult = await this.executePass(rules, sourceValues, targetRecord, profile, 1, skipValidation, dryRun);
        ...
        if (maxPasses > 1) {
            ...
            if (cascadingRules.length > 0) {
                ...
                const secondPassResult = await this.executePass(cascadingRules, cascadingSourceValues, targetRecord, profile, 2, skipValidation, dryRun);
        ...
        result.success = !result.errors.some(e => e.code === MappingErrorCode.RequiredFieldEmpty || e.code === MappingErrorCode.TypeMismatch);
        return result;
    }
    /**
     ...
     * @returns Pass result
     */
    async executePass(rules, sourceValues, targetRecord, profile, passNumber, skipValidation, dryRun) {
        const result = {
            success: true,
            appliedRules: 0,
            skippedRules: 0,
        ...
        for (const rule of rules) {
            ...
            if (!skipValidation) {
                const compatibility = this.validateTypeCompatibility(rule.sourceFieldType, rule.targetFieldType, rule.compatibilityMode);
     ...
     */
    getCascadingRules(allRules, mappedFields) {
     ...
     */
    validateTypeCompatibility(sourceType, targetType, mode = CompatibilityMode.Strict) {
        const result = {
            isCompatible: false,
        ...
        result.errors.push(`Cannot convert ${FieldType[sourceType]} to ${FieldType[targetType]} in Strict mode. ` +
     ...
     */
    getCompatibleTargetTypes(sourceType) {
     ...
     */
    convertValue(value, targetType, sourceType) {
        ...
        switch (targetType) {
            ...
            case FieldType.DateTime:
                ...
                if (typeof value === 'string')
                    return value; // Assume ISO string
                return null;
     ...
     */
    convertToText(value, sourceType) {
        if (value === null || value === undefined) {
            return '';
        }
        if (typeof value === 'string') {
    ...
    mapProfileEntity(entity) {
        return {
            ...
            mappingDirection: entity.sprk_mappingdirection ?? MappingDirection.ParentToChild,
            syncMode: entity.sprk_syncmode ?? SyncMode.OneTime,
     ...
     */
    mapRuleEntity(entity, profileId) {
        return {
            id: String(entity.sprk_fieldmappingruleid ?? ''),
            profileId,
            name: entity.sprk_name,
            sourceField: String(entity.sprk_sourcefield ?? ''),
            sourceFieldType: entity.sprk_sourcefieldtype ?? FieldType.Text,
            targetField: String(entity.sprk_targetfield ?? ''),
            targetFieldType: entity.sprk_targetfieldtype ?? FieldType.Text,
            compatibilityMode: entity.sprk_compatibilitymode ?? CompatibilityMode.Strict,
            isRequired: Boolean(entity.sprk_isrequired),
            defaultValue: entity.sprk_defaultvalue,
            isCascadingSource: Boolean(entity.sprk_iscascadingsource),
            executionOrder: entity.sprk_executionorder ?? 0,
            isActive: Boolean(entity.sprk_isactive),
        };
    }
    /**
     ...
     */
    async loadRulesForProfiles(profiles) {
