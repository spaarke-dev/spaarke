...
import { AddRegular, DeleteRegular, ArrowSyncRegular, OpenRegular, ArrowUploadRegular } from '@fluentui/react-icons';
...
export class CommandRegistry {
     ...
     */
    static getCommands(keys, privileges) {
        const commands = keys.map(key => this.getCommand(key)).filter((cmd) => cmd !== undefined);
     ...
     */
    static getCommandsWithCustom(keys, entityLogicalName, privileges) {
        const commands = [];
        keys.forEach(key => {
     ...
     */
    static hasRequiredPrivilege(command, privileges) {
