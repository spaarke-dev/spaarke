 ...
 * ```
 */
export class ODataDataverseClient {
    constructor(options) {
    ...
    async createRecord(entityLogicalName, data) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to create ${entityLogicalName} record: HTTP ${response.status} ${response.statusText}. ${errorBody}`);
        }
    ...
    async updateRecord(entityLogicalName, id, data) {
        ...
        if (!response.ok) {
            const errorBody = await this.tryReadErrorBody(response);
            throw new Error(`Failed to update ${entityLogicalName} record ${sanitizedId}: HTTP ${response.status} ${response.statusText}. ${errorBody}`);
        }
     ...
     */
    async extractRecordId(response, entityLogicalName) {
     ...
     */
    async tryReadErrorBody(response) {
        try {
...
//# sourceMappingURL=ODataDataverseClient.js.map