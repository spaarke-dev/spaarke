...
export class MultiFileUploadService {
    constructor(fileUploadService, logger) {
     ...
     */
    async uploadFiles(request, onProgress) {
        ...
        const uploadResults = await Promise.allSettled(files.map(file => this.fileUploadService.uploadFile({ file, driveId: containerId })));
        // Process results
        ...
        this.logger.info('MultiFileUploadService', `Upload complete: ${result.successCount}/${result.totalFiles} successful`);
        return result;
    }
}
//# sourceMappingURL=MultiFileUploadService.js.map