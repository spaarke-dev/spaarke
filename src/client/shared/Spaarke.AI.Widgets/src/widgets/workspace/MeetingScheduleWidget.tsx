/**
 * @spaarke/ai-widgets — MeetingScheduleWidget
 *
 * Workspace widget that opens the Analysis Builder (Playbook Library Code Page)
 * pre-configured for the `meeting-schedule` intent (FR-19: Schedule Meeting).
 *
 ...
 *       data: "intent=meeting-schedule[&bffBaseUrl=...]"
 *     },
 ...
 *
 * See EmailComposeWidget.tsx for the complete design rationale — this widget
 * is its meeting-schedule sibling. Identical mechanics, different intent + icon.
 *
 * Design notes:
 ...
 *
 * Task: 044 (FR-19 — Schedule Meeting)
 *
...
import { CalendarAddRegular, OpenRegular } from '@fluentui/react-icons';
...
/** Canonical intent identifier for the schedule-meeting flow (FR-19). */
const INTENT = 'meeting-schedule';

/** Display label shown in the launching status and dialog title. */
const DISPLAY_NAME = 'Schedule Meeting';
 ...
 * `meeting-schedule` intent pre-configured.
 *
 ...
 * `meeting-schedule` intent. The tab itself shows a launching status while the
 * dialog is in flight; once the dialog closes, the user can either close the
 * tab or relaunch.
 */
const MeetingScheduleWidget: React.FC<WorkspaceWidgetProps<MeetingScheduleData>> = ({
  data,
  isLoading,
  error,
  className,
  ...
  if (error) {
    return (
      <div className={mergeClasses(styles.root, className)}>
        <CalendarAddRegular className={styles.icon} aria-hidden />
    ...
    return (
      <div className={mergeClasses(styles.root, className)}>
        ...
        <Text size={300} className={styles.subtitle}>
          Analysis Builder is only available inside a Dataverse host. Open this page from within Power Apps to schedule
          a meeting.
        </Text>
      </div>
    );
    ...
    return (
      <div className={mergeClasses(styles.root, className)}>
        ...
        <Button
          ...
          data-testid="meeting-schedule-retry"
        >
          Retry
        </Button>
      </div>
    ...
    return (
      <div className={mergeClasses(styles.root, className)}>
        ...
        <Button
          ...
          data-testid="meeting-schedule-relaunch"
        >
          Reopen
        </Button>
      </div>
 ...
 */
export function serializeMeetingScheduleState(): WidgetState<MeetingScheduleData> {
  return {
    widgetType: 'meeting-schedule',
    version: 1,
    queryParams: {},
...
export default MeetingScheduleWidget;
