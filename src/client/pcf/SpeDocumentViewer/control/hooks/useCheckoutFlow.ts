...
export function useCheckoutFlow({
  ...
  const checkout = useCallback(async (): Promise<CheckoutResponse | null> => {
    ...
    try {
      const response = await bffClient.current.checkout(documentId, correlationId);
  ...
  const checkIn = useCallback(
    async (comment?: string): Promise<boolean> => {
      ...
      try {
        const response = await bffClient.current.checkIn(documentId, correlationId, comment);
  ...
  const discard = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const response = await bffClient.current.discard(documentId, correlationId);
