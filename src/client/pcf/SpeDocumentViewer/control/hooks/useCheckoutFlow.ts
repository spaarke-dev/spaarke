...
export interface UseCheckoutFlowResult
  extends UseCheckoutFlowState, UseCheckoutFlowActions {}
...
export function useCheckoutFlow({
  ...
  const checkout = useCallback(async (): Promise<CheckoutResponse | null> => {
    ...
    console.log(
      "[useCheckoutFlow] Starting checkout for document:",
      documentId,
    );
  ...
  const checkIn = useCallback(
    async (comment?: string): Promise<boolean> => {
      ...
      console.log(
        "[useCheckoutFlow] Starting check-in for document:",
        documentId,
        "comment:",
        comment,
      );
  ...
  const updateCheckoutStatus = useCallback(
    (status: CheckoutStatus | null) => {
      ...
      if (
        ...
        console.log(
          "[useCheckoutFlow] Checkout lost to another user, reverting to preview",
        );
        setViewMode(ViewMode.Preview);
        setEditUrl(null);
      }
