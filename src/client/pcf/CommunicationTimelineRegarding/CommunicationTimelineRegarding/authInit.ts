/**
 * Auth bootstrap for the CommunicationTimelineRegarding PCF (ADR-028).
 *
 ...
 * `CommunicationTimeline/authInit.ts` (task 021, itself copied from
 * `CommunicationMessageActions/authInit.ts`, task 062) — no behavior difference for
 * this control. `@spaarke/auth` has no React dependency, so it is safe for React 16
 * PCF controls. Config values are resolved at runtime from manifest inputs /
