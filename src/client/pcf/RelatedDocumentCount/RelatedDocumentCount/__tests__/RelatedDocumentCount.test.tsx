...
jest.mock(
  "@spaarke/ui-components/dist/components/RelationshipCountCard",
  () => ({
    RelationshipCountCard: (props: Record<string, unknown>) => {
      ...
      return (
        <div data-testid="relationship-count-card">
          ...
          {props.error && (
            <span data-testid="card-error">{props.error as string}</span>
          )}
          <button
...
describe("RelatedDocumentCount", () => {
  ...
  describe("FindSimilarDialog", () => {
    ...
    it("opens dialog when card onOpen is triggered", () => {
      ...
      fireEvent.click(
        document.querySelector("[data-testid='card-open-button']")!,
      );
