...
function detectDarkModeFromUrl(): boolean | null {
  try {
    ...
    try {
      ...
      if (parentUrl && (parentUrl.includes('themeOption%3Ddarkmode') || parentUrl.includes('themeOption=darkmode'))) {
        return true;
      }
    } catch {
    ...
    try {
      ...
      if (topUrl && (topUrl.includes('themeOption%3Ddarkmode') || topUrl.includes('themeOption=darkmode'))) {
        return true;
      }
    } catch {
      // Cross-origin access blocked
...
export class UniversalDocumentUpload implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  private isCustomPageMode = false;
  ...
  private validateParentEntity(): void {
    ...
    if (!isEntitySupported(entityName)) {
      throw new Error(
        `Unsupported parent entity: "${entityName}". ` + `Supported entities: ${getSupportedEntities().join(', ')}`
      );
    }
