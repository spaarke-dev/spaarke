...
function detectDarkModeFromUrl(): boolean | null {
  try {
    ...
    try {
      ...
      if (
        topUrl &&
        (topUrl.includes("themeOption%3Ddarkmode") ||
...
function detectDarkModeFromNavbar(): boolean | null {
  try {
    ...
    let navbar: Element | null = document.querySelector(
    ...
    if (!navbar) {
      try {
        const parentNavbar = window.parent?.document?.querySelector(
          '[data-id="navbar-container"]',
        );
    ...
    if (!navbar) {
      try {
        const topNavbar = window.top?.document?.querySelector(
          '[data-id="navbar-container"]',
        );
...
import {
  getEntityDocumentConfig,
  isEntitySupported,
  getSupportedEntities,
} from "./config/EntityDocumentConfig";
 ...
 */
export class UniversalDocumentUpload implements ComponentFramework.StandardControl<
  IInputs,
  IOutputs
> {
  ...
  private detectHostingContext(
    ...
    if (contextAny.page && contextAny.page.type === "custom") {
      logInfo("UniversalDocumentUpload", "Detected Custom Page context", {
        ...
        hasNavigationClose: !!(
          contextAny.navigation && contextAny.navigation.close
        ),
      });
      return true;
    }
  ...
  public init(
    ...
    logInfo(
      ...
      "Initializing PCF control v3.4.0 (Keywords + Entity extraction)",
    );
   ...
   */
  private loadParentContext(
    context: ComponentFramework.Context<IInputs>,
  ): void {
    logInfo(
      "UniversalDocumentUpload",
      "Loading parent context from parameters",
    );
    ...
    if (!parentEntityName || !parentRecordId || !containerId) {
      logWarn(
        "UniversalDocumentUpload",
        "Parameters not yet populated - waiting for user input",
      );
  ...
  private async initializeMsalAsync(): Promise<void> {
    try {
      ...
      throw new Error(
        "Authentication initialization failed. Please refresh and try again.",
      );
    }
  }
  ...
  private initializeServices(
    ...
    // Get API base URL
    const rawApiUrl =
      context.parameters.sdapApiBaseUrl?.raw ||
    ...
    logInfo("UniversalDocumentUpload", "Initializing services (Phase 7)", {
      apiBaseUrl,
      navMapBaseUrl,
    });
    ...
    logInfo(
      "UniversalDocumentUpload",
      "Services initialized (including NavMapClient)",
    );
  }
  ...
  private renderReactComponent(): void {
    ...
    if (
      ...
      !this.documentRecordService
    ) {
      logError(
        "UniversalDocumentUpload",
        "Cannot render: missing dependencies",
        {
  ...
  private closeDialog(): void {
    logInfo(
      "UniversalDocumentUpload",
      "closeDialog() called after successful upload",
      {
    ...
    if (this.isCustomPageMode) {
      ...
      logInfo(
        "UniversalDocumentUpload",
        "shouldClose set to true - Custom Page Timer will close dialog",
      );
    } else {
      ...
      logInfo(
        "UniversalDocumentUpload",
        "Quick Create Form mode - not closing dialog (form handles close on save)",
      );
    }
  }
  ...
  private handleClose(): void {
    logInfo(
      "UniversalDocumentUpload",
      "Dialog closed by user (Quick Create Form mode)",
    );
    ...
    if (typeof Xrm !== "undefined" && Xrm.Navigation) {
      ...
      try {
        ...
        logWarn(
          "UniversalDocumentUpload",
          "Could not refresh Documents subgrid",
          error,
        );
      }
  ...
  public updateView(context: ComponentFramework.Context<IInputs>): void {
    ...
    if (!this._initialized) {
      if (parentEntityName && parentRecordId && containerId) {
        ...
        logInfo(
          "UniversalDocumentUpload",
          "Waiting for parameters to hydrate",
          {
