...
export const RecordMatchSuggestions: React.FC<RecordMatchSuggestionsProps> = ({
  ...
  return (
    <div className={mergeClasses(styles.container, className)}>
      <div className={styles.header}>
        <LinkRegular className={styles.headerIcon} />
        <span className={styles.headerText}>Match Suggestions ({suggestions.length})</span>
      </div>
      <div className={styles.suggestionsList}>
        {suggestions.map(suggestion => (
          <Card
            key={suggestion.recordId}
            className={mergeClasses(styles.suggestionCard, isAssociating && styles.suggestionCardDisabled)}
          ...
          >
            <div className={styles.suggestionContent}>
              <span className={styles.suggestionIcon}>{getRecordTypeIcon(suggestion.recordType)}</span>
              <div className={styles.suggestionDetails}>
                <div className={styles.suggestionHeader}>
                  <Text className={styles.suggestionName} title={suggestion.recordName}>
                    {suggestion.recordName}
                  </Text>
                  <Tooltip content={`${Math.round(suggestion.confidenceScore * 100)}% confidence`} relationship="label">
                    <Badge
                      appearance="filled"
                ...
                <Text className={styles.recordType}>{getRecordTypeDisplayName(suggestion.recordType)}</Text>
                {suggestion.matchReasons.length > 0 && (
                  <div className={styles.matchReasons}>
                    ...
                    {suggestion.matchReasons.length > 3 && (
                      <Tooltip content={suggestion.matchReasons.slice(3).join(', ')} relationship="description">
                        <span className={styles.matchReasonTag}>+{suggestion.matchReasons.length - 3} more</span>
                      </Tooltip>
                    )}
                  </div>
                )}
