...
describe('useSseStream', () => {
  ...
  describe('streaming', () => {
    ...
    it('should call onChunk callback for each chunk', async () => {
      ...
      const { result } = renderHook(() => useSseStream({ ...defaultOptions, onChunk }));
    ...
    it('should call onComplete with final data', async () => {
      const chunks = ['data: {"content": "Final"}\n\n', 'data: {"done": true}\n\n'];
      global.fetch = jest.fn().mockResolvedValue(createMockResponse(chunks));

      const onComplete = jest.fn();
      const { result } = renderHook(() => useSseStream({ ...defaultOptions, onComplete }));
  ...
  describe('SSE parsing', () => {
    ...
    it('should handle plain text content after data: prefix', async () => {
      const chunks = ['data: plain text content\n\n', 'data: {"done": true}\n\n'];
      global.fetch = jest.fn().mockResolvedValue(createMockResponse(chunks));
  ...
  describe('error handling', () => {
    it('should set error status on HTTP error', async () => {
      ...
      const { result } = renderHook(() => useSseStream({ ...defaultOptions, onError }));
    ...
    it('should handle error in SSE data', async () => {
      const chunks = ['data: {"error": "Processing failed"}\n\n'];
      global.fetch = jest.fn().mockResolvedValue(createMockResponse(chunks));
  ...
  describe('authorization', () => {
    it('should include Authorization header when token provided', async () => {
      ...
      const { result } = renderHook(() => useSseStream({ ...defaultOptions, token: 'test-token' }));
  ...
  describe('request body', () => {
    it('should stringify body in request', async () => {
      ...
      const { result } = renderHook(() => useSseStream({ url: '/api/stream', body }));
