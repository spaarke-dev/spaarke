...
describe('DaysUntilDueBadge', () => {
  describe('display value rendering with daysUntilDue prop', () => {
    it('displays "Today" when daysUntilDue is 0', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={0} isOverdue={false} />);

      expect(screen.getByText('Today')).toBeInTheDocument();
    });

    it('displays numeric value for future dates', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={5} isOverdue={false} />);

      expect(screen.getByText('5')).toBeInTheDocument();
    });

    it('displays "+N" format for overdue dates', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={3} isOverdue={true} />);

      expect(screen.getByText('+3')).toBeInTheDocument();
    });

    it('handles single day overdue', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={1} isOverdue={true} />);

      expect(screen.getByText('+1')).toBeInTheDocument();
    });

    it('handles large numbers', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={365} isOverdue={false} />);
  ...
  describe('urgency-based coloring', () => {
    it('applies overdue styling for overdue events', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={5} isOverdue={true} />);
    ...
    it('applies critical styling for 0-1 days', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={0} isOverdue={false} />);
    ...
    it('applies urgent styling for 2-3 days', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={3} isOverdue={false} />);
    ...
    it('applies warning styling for 4-7 days', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={5} isOverdue={false} />);
    ...
    it('applies normal styling for 8+ days', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={10} isOverdue={false} />);
  ...
  describe('urgency override', () => {
    it('uses urgencyOverride when provided', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={10} isOverdue={false} urgencyOverride="critical" />);
    ...
    it('supports all urgency override values', () => {
      ...
      urgencyLevels.forEach(urgency => {
        const { unmount } = renderWithProvider(
          <DaysUntilDueBadge daysUntilDue={5} isOverdue={false} urgencyOverride={urgency} />
        );
  ...
  describe('size variants', () => {
    ...
    it('renders small size when specified', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={5} isOverdue={false} size="small" />);
    ...
    it('renders medium size when explicitly specified', () => {
      const { container } = renderWithProvider(<DaysUntilDueBadge daysUntilDue={5} isOverdue={false} size="medium" />);
  ...
  describe('accessibility', () => {
    ...
    it('has accessible label for future date', () => {
      ...
      expect(screen.getByRole('status')).toHaveAttribute('aria-label', 'Due in 5 days');
    });

    it('has accessible label for today', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={0} isOverdue={false} />);

      expect(screen.getByRole('status')).toHaveAttribute('aria-label', 'Due today');
    });

    it('has accessible label for overdue', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={3} isOverdue={true} />);

      expect(screen.getByRole('status')).toHaveAttribute('aria-label', '3 days overdue');
    });

    it('uses custom aria-label when provided', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={5} isOverdue={false} ariaLabel="Custom label" />);
  ...
  describe('edge cases', () => {
    it('handles 1 day until due', () => {
      renderWithProvider(<DaysUntilDueBadge daysUntilDue={1} isOverdue={false} />);

      expect(screen.getByText('1')).toBeInTheDocument();
      expect(screen.getByRole('status')).toHaveAttribute('aria-label', 'Due in 1 day');
    });
    ...
    it('handles 1 day overdue', () => {
      ...
      expect(screen.getByText('+1')).toBeInTheDocument();
      expect(screen.getByRole('status')).toHaveAttribute('aria-label', '1 day overdue');
    });
