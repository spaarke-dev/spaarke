...
const renderControlPanel = (settings: FilterSettings = DEFAULT_FILTER_SETTINGS, onSettingsChange = jest.fn()) => {
  return render(
    <TestWrapper>
...
describe('ControlPanel', () => {
  ...
  describe('Similarity Threshold Slider', () => {
    it('displays current similarity value', () => {
      const settings = { ...DEFAULT_FILTER_SETTINGS, similarityThreshold: 0.8 };
  ...
  describe('Slider Hints', () => {
    it('shows similarity slider hint text', () => {
      ...
      expect(screen.getByText('Higher values show more similar documents only')).toBeInTheDocument();
    });

    it('shows depth slider hint text', () => {
      renderControlPanel();

      expect(screen.getByText('How many levels of related documents to show')).toBeInTheDocument();
    });

    it('shows max nodes slider hint text', () => {
      renderControlPanel();

      expect(screen.getByText('Maximum documents shown at each depth level')).toBeInTheDocument();
    });
  });
});
