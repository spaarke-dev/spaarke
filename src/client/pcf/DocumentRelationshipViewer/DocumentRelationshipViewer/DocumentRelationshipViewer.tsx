 ...
 *
 * MIGRATION NOTE: authProvider is no longer needed. Authentication is handled
 * transparently by @spaarke/auth (initialized in index.ts via authInit.ts).
 */
...
export const DocumentRelationshipViewer: React.FC<IDocumentRelationshipViewerProps> = ({
  ...
  const checkedItems = React.useMemo(() => {
    const items: Record<string, string[]> = {
      filter: selectedRelationshipTypes,
    };
    return items;
      ...
      <div className={styles.container}>
        ...
        <div className={styles.graphContainer} ref={containerRef}>
          {isLoading ? (
            ...
            <div className={styles.placeholder}>
              <DocumentFlowchart24Regular style={{ fontSize: 48 }} />
              <Text>Select a document to view its relationships</Text>
              <Text size={200}>Graph visualization will appear here</Text>
            </div>
          ) : !hasDimensions ? (
            <div className={styles.placeholder}>
