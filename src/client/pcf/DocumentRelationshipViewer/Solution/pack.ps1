...
try {
    ...
    $controlsDir = Join-Path $PSScriptRoot 'Controls\sprk_Spaarke.Controls.DocumentRelationshipViewer'
    if (Test-Path $controlsDir) {
        Get-ChildItem -Path $controlsDir -File | ForEach-Object {
            $entryName = 'Controls/sprk_Spaarke.Controls.DocumentRelationshipViewer/' + $_.Name
