/**
 * Auth bootstrap for the CommunicationActions PCF (ADR-028).
 *
 * Uses `@spaarke/auth` (`initAuth` + `authenticatedFetch`) — the single shared MSAL
 * provider every Spaarke surface uses. Adapted from SemanticSearchControl/authInit.ts.
 * `@spaarke/auth` has no React dependency, so it is safe for React 16 PCF controls.
 * Config values are resolved at runtime from manifest inputs / Dataverse env vars —
 * no hardcoded client id, tenant, or BFF URL.
 */
