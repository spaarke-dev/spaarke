...
export class BffClient {
  ...
  public async getPreviewUrl(
    ...
    try {
      ...
      console.log(
        `[BffClient] Preview URL acquired for document: ${data.documentInfo.name}`,
      );
  ...
  public async getOpenLinks(
    ...
    try {
      ...
      console.log(
        `[BffClient] Desktop URL available: ${data.desktopUrl ? "Yes" : "No"}`,
      );
  ...
  public async getViewUrl(
    ...
    try {
      ...
      console.log(
        `[BffClient] View URL acquired for document: ${data.documentInfo.name}`,
      );
      if (data.checkoutStatus?.isCheckedOut) {
        console.log(
          `[BffClient] Document is checked out by: ${data.checkoutStatus.checkedOutBy?.name}`,
        );
      }
      ...
      console.error("[BffClient] Request failed:", error);
      throw new Error(
        `Failed to get view URL: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private handleApiError(error: ApiError, correlationId: string): never {
    ...
    const errorCode = (problemDetails as Record<string, unknown>)?.extensions
    ...
    switch (errorCode) {
      ...
      case "mapping_missing_item":
        throw new Error(
          "This file is still initializing. Please try again in a moment.",
        );
      case "storage_not_found":
        throw new Error(
          "File has been removed from storage. Contact your administrator.",
        );
      ...
      default:
        switch (error.status) {
          ...
          case 409:
            throw new Error(
              "File is not ready for preview. Please try again shortly.",
            );
          case 500:
          case 502:
          case 503:
          case 504:
            throw new Error(
              `Server error (${error.status}). Please try again later. Correlation ID: ${correlationId}`,
            );
          default:
