...
export class AnalysisBuilder implements ComponentFramework.StandardControl<
  IInputs,
  IOutputs
> {
  ...
  public init(
    ...
    this.loadConfiguration().then(() => {
      logInfo(
        "AnalysisBuilder",
        `Configuration loaded, API URL: ${this._apiBaseUrl}`,
      );
    });
  ...
  private navigateToWorkspace(analysisId: string): void {
    try {
      ...
      if (xrm?.Navigation?.navigateTo) {
        logInfo(
          "AnalysisBuilder",
          `Navigating to Analysis record: ${analysisId}`,
        );
        ...
        xrm.Navigation.navigateTo(
          ...
          () => {
            logInfo(
              "AnalysisBuilder",
              "Navigation to Analysis record succeeded",
            );
          },
          (err: unknown) => {
            logError(
              "AnalysisBuilder",
              "Navigation to Analysis record failed",
              err,
            );
        ...
        logInfo(
          "AnalysisBuilder",
          "Xrm.Navigation not available, falling back to close",
        );
        this.closeDialog();
      }
    } catch (err) {
