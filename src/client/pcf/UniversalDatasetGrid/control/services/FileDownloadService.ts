...
export class FileDownloadService {
   ...
   */
  async downloadFile(driveId: string, itemId: string, fileName: string): Promise<ServiceResult> {
    try {
  ...
  async downloadMultipleFiles(
    ...
    for (const file of files) {
      const result = await this.downloadFile(file.driveId, file.itemId, file.fileName);
