...
describe('ActionEditor', () => {
  it('renders the textarea with a label', () => {
    renderWithProvider(<ActionEditor value="" onChange={jest.fn()} />);
  ...
  it('renders correct character count for provided value', () => {
    ...
    renderWithProvider(<ActionEditor value={text} onChange={jest.fn()} />);
    const charCount = screen.getByTestId('char-count');
  ...
  it('calls onChange when textarea value changes', () => {
    ...
    renderWithProvider(<ActionEditor value="" onChange={onChange} />);
  ...
  it('does not show warning for short prompts', () => {
    renderWithProvider(<ActionEditor value="Short prompt" onChange={jest.fn()} />);
  ...
  it('shows warning when token count approaches 2000 threshold', () => {
    ...
    renderWithProvider(<ActionEditor value={longText} onChange={jest.fn()} />);
    expect(screen.getByTestId('token-warning')).toBeInTheDocument();
    expect(screen.getByTestId('token-warning')).toHaveTextContent(/approaching token limit/i);
  });
  ...
  it('shows error warning when token count exceeds 4000 threshold', () => {
    ...
    renderWithProvider(<ActionEditor value={veryLongText} onChange={jest.fn()} />);
    expect(screen.getByTestId('token-warning')).toBeInTheDocument();
    expect(screen.getByTestId('token-warning')).toHaveTextContent(/may exceed model context limits/i);
  });

  it('textarea is disabled when readOnly is true', () => {
    renderWithProvider(<ActionEditor value="Some text" onChange={jest.fn()} readOnly={true} />);
    expect(screen.getByLabelText(/system prompt editor/i)).toBeDisabled();
  });

  it('textarea is enabled when readOnly is false', () => {
    renderWithProvider(<ActionEditor value="Some text" onChange={jest.fn()} readOnly={false} />);
