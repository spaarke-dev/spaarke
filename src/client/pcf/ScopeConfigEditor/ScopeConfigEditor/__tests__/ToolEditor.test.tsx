...
function makeHandlersResponse(
  handlers: { handlerClass: string; displayName?: string }[],
) {
...
describe("ToolEditor — JSON validation", () => {
  ...
  it("shows JSON valid indicator for valid JSON", () => {
    ...
    expect(screen.getByTestId("json-valid-indicator")).toHaveTextContent(
      /valid json/i,
    );
  });
  ...
  it("shows JSON error indicator for invalid JSON", () => {
    ...
    expect(screen.getByTestId("json-error-indicator")).toHaveTextContent(
      /json error/i,
    );
  });
  ...
  it("does not show valid indicator for whitespace-only value", () => {
    ...
    expect(
      screen.queryByTestId("json-valid-indicator"),
