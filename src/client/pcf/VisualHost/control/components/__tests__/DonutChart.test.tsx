...
describe('DonutChart', () => {
  describe('rendering', () => {
    ...
    it('renders with title', () => {
      renderWithTheme(<DonutChart data={mockData} title="Status Distribution" />);
    ...
    it('renders as donut with default innerRadius', () => {
      renderWithTheme(<DonutChart data={mockData} innerRadius={0.5} />);

      expect(document.querySelector('svg')).toBeInTheDocument();
    });

    it('renders as pie chart with innerRadius of 0', () => {
      renderWithTheme(<DonutChart data={mockData} innerRadius={0} />);

      expect(document.querySelector('svg')).toBeInTheDocument();
    });

    it('shows center value by default', () => {
      renderWithTheme(<DonutChart data={mockData} showCenterValue={true} />);
    ...
    it('shows custom center label when provided', () => {
      renderWithTheme(<DonutChart data={mockData} centerLabel="Total: 100" showCenterValue={true} />);
  ...
  describe('interactions', () => {
    it('accepts onDrillInteraction callback', () => {
      ...
      renderWithTheme(<DonutChart data={mockData} onDrillInteraction={mockDrill} drillField="status" />);
  ...
  describe('theme support', () => {
    it('renders correctly in dark theme', () => {
      renderWithTheme(<DonutChart data={mockData} title="Dark Theme Donut" />, webDarkTheme);
  ...
  describe('props', () => {
    it('respects showLegend prop', () => {
      renderWithTheme(<DonutChart data={mockData} showLegend={true} />);

      expect(document.querySelector('svg')).toBeInTheDocument();
    });

    it('respects height prop', () => {
      const { container } = renderWithTheme(<DonutChart data={mockData} height={250} />);

      expect(container.firstChild).toBeInTheDocument();
    });

    it('hides center value when showCenterValue is false', () => {
      renderWithTheme(<DonutChart data={mockData} showCenterValue={false} />);
