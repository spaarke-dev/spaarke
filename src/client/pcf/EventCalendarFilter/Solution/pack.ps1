# Pack EventCalendarFilter Solution
...
try {
    ...
    $controlsDir = Join-Path $PSScriptRoot 'Controls\sprk_Spaarke.Controls.EventCalendarFilter'
    if (Test-Path $controlsDir) {
        Get-ChildItem -Path $controlsDir -File | ForEach-Object {
            ...
            $entryName = 'Controls/sprk_Spaarke.Controls.EventCalendarFilter/' + $_.Name
            [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
                $zip,
                $_.FullName,
                $entryName,
