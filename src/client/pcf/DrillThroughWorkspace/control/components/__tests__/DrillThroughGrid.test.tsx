 ...
 * Create mock column
 */
function createMockColumn(name: string, displayName: string): ComponentFramework.PropertyHelper.DataSetApi.Column {
  return {
    name,
    displayName,
...
const TestWrapper: React.FC<{
  ...
  return <FilterStateProvider dataset={dataset}>{children}</FilterStateProvider>;
};
...
describe('DrillThroughGrid - Selection', () => {
  it('calls onSelectionChange when selection changes', () => {
    ...
    render(
      <TestWrapper dataset={mockDataset}>
        <DrillThroughGrid dataset={mockDataset} onSelectionChange={handleSelectionChange} />
      </TestWrapper>
    );
