...
/**
 * Map sprk_todosource string to the source category for icon selection.
 * Defaults to 'User' when unknown.
 */
function deriveSourceType(source: string | undefined): TodoSource {
  switch (source) {
...
const SourceIcon: React.FC<ISourceIconProps> = ({ source }) => {
  switch (source) {
    ...
    case "Flagged":
      return <FlagRegular fontSize={14} aria-hidden="true" />;
    case "User":
    default:
...
function getSourceLabel(source: TodoSource): string {
  switch (source) {
    ...
    case "Flagged": return "Flagged from feed";
    case "User":
...
export const TodoItem: React.FC<ITodoItemProps> = React.memo(
  ({ event, onToggleComplete, onDismiss, isDismissing = false }) => {
    ...
    const isCompleted = event.sprk_todostatus === 1; // 1 = Completed
      ...
      >
        ...
        <div className={styles.checkboxWrapper}>
          <Checkbox
            ...
            aria-label={`Mark "${event.sprk_subject}" as ${isCompleted ? "open" : "complete"}`}
          />
        </div>
        ...
        {onDismiss && (
          <div className={styles.actionsColumn}>
            <Button
              ...
              aria-label={`Dismiss "${event.sprk_subject}"`}
