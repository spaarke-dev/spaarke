 ...
 *
 * The Dataverse option set stores integers; we map them to a 1-4 rank for sort.
 * If the Dataverse priority int is already 1-4 we can use it directly;
 * otherwise we fall back to sprk_priorityscore.
 */
function getPriorityRank(event: IEvent): number {
  // sprk_priority stores the Dataverse option set integer.
  // Treat it as a 1-4 level where higher == more critical.
  if (event.sprk_priority !== undefined && event.sprk_priority !== null) {
    return event.sprk_priority;
  }
  // Fall back: convert priorityscore (0-100) to a 1-4 bucket
  const score = event.sprk_priorityscore ?? 0;
  if (score >= 75) return 4; // Critical
