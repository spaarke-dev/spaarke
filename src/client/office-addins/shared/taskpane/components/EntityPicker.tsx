...
export const EntityPicker = forwardRef<HTMLInputElement, EntityPickerProps>(function EntityPicker(props, ref) {
    ...
    return (
      ...
      >
        <div className={styles.optionContent}>
          ...
          {entity.displayInfo && <span className={styles.optionInfo}>{entity.displayInfo}</span>}
        </div>
      </Option>
    );
  };
  ...
  return (
    <div className={mergeClasses(styles.container, className)}>
      ...
      {/* Type Filter Chips */}
      {showTypeFilter && (
        <div className={styles.filterChips} role="group" aria-label="Filter by entity type">
          {effectiveAllowedTypes.map(type => {
            const isActive = typeFilter.length === 0 || typeFilter.includes(type);
            return (
              <Badge
                key={type}
                className={mergeClasses(styles.filterChip, isActive && styles.filterChipActive)}
              ...
              >
                {ENTITY_ICONS[type]}
                <span style={{ marginLeft: tokens.spacingHorizontalXXS }}>{type}</span>
              </Badge>
            );
          })}
        </div>
      ...
      <div className={styles.comboboxWrapper}>
        ...
        >
          ...
          {showCreateOption && (showResultsSection || showEmptyState) && <Divider />}
