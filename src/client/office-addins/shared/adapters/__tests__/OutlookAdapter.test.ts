    ...
    getAsync: jest.fn(
      (
        ...
        callback({
          status: Office.AsyncResultStatus.Succeeded,
          value: [
            { emailAddress: "to@example.com", displayName: "To Recipient" },
          ],
          error: null,
...
describe("OutlookAdapter", () => {
  ...
  beforeEach(() => {
    ...
    global.Office.onReady = jest
      .fn()
      .mockImplementation(
        (callback: (info: { host: Office.HostType }) => void) => {
          callback({
  ...
  describe("read mode operations", () => {
    beforeEach(async () => {
      ...
      (global.Office.context.mailbox as unknown as { item: unknown }).item =
        mockReadItem;
      await adapter.initialize();
    });
    ...
    describe("getAttachmentContent", () => {
      ...
      it("should throw when attachment not found", async () => {
        await expect(
          adapter.getAttachmentContent("non-existent"),
  ...
  describe("compose mode operations", () => {
    ...
    describe("insertLink", () => {
      it("should insert link successfully", async () => {
        const result = await adapter.insertLink(
          "https://example.com",
          "Example Link",
        );
      ...
      it("should escape HTML in URL and display text", async () => {
        await adapter.insertLink(
          "https://example.com?a=1&b=2",
          "Test <script>",
        );

        const callArgs =
          mockComposeItem.body.setSelectedDataAsync.mock.calls[0];
    ...
    describe("attachFile", () => {
      it("should attach file successfully", async () => {
        const result = await adapter.attachFile(
          "dGVzdA==",
          "test.txt",
          "text/plain",
        );
        ...
        expect(
          mockComposeItem.addFileAttachmentFromBase64Async,
        ).toHaveBeenCalled();
      });

      it("should fail when Mailbox 1.8 not supported", async () => {
        global.Office.context.requirements.isSetSupported = jest
          .fn()
