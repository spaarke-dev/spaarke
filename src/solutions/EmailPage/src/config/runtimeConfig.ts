/**
 * EmailPage runtime-config singleton.
 *
 * Thin consumer of the canonical `createRuntimeConfigStore` factory from
 * `@spaarke/auth` (FR-21 / ADR-028) — mirrors
 * `src/solutions/DailyBriefing/src/config/runtimeConfig.ts` exactly. Multiple
...
const store = createRuntimeConfigStore({
  errorLabel: "EmailPage",
  // EmailPage's auth bootstrap awaits waitForConfig() as the
  // createCodePageAuthInitializer `beforeInit` hook (mirrors DailyBriefing).
