/**
 * Spaarke Compose — Three-Pane Coordination Contracts (R1)
 *
 * Project:     spaarkeai-compose-r1
 * Task:        041 — Create six TypeScript data-contract interfaces
 * Authority:   This file is the PRODUCTION promotion of the locked spike
 *              artifact at `projects/spaarkeai-compose-r1/notes/spikes/
 *              spike-2-prototype/contracts.ts` (Spike #2). Six interfaces, one
 *              per flow per design.md §5. Receivers may be stubs in R1; the
 *              contract IS the deliverable per CLAUDE.md (Implementation Notes).
 *
 * Source-of-truth (BINDING — byte-for-byte preservation required):
 *   - design.md §5 (Six coordinated flows)
 *   - design.md §14 row 2 (HostContext non-extension)
 *   - design.md §11 (component reuse map — PaneEventBus)
 *   - spec.md FR-20 (data contracts ship FIRST; receivers can be stubs)
 *   - Spike #2 report §10.3 (BINDING items for promotion)
 *
 * Promotion-target decision (task 041 / Spike #2 §11 open item #2):
 *   Path A — solution-local types file (this file).
 *   Rationale per CLAUDE.md §11 component justification:
 *     - Existing: no overlap — `src/solutions/SpaarkeAi/src/types/` did not
 *       exist; PaneEventTypes.ts (in `@spaarke/ai-widgets`) covers the FOUR-
 *       channel bus discriminants for non-Compose use cases.
 *     - Extension: technically possible (additive fields on existing
 *       `WorkspacePaneEvent` / `ContextPaneEvent` / `ConversationPaneEvent`),
 *       but those base interfaces already carry 20+ optional fields each;
 *       injecting Compose's 6 new discriminants + ~10 new fields bloats them
 *       further and conflates Compose-specific concerns with the shared bus.
 *     - Cost-of-doing-nothing: Tasks 042/043/045 ship with `any`-typed
 *       payloads; FR-20 fails; no compile-time validation; silent runtime
 *       drift; smoke test cannot assert payload shape.
 *   The shared-lib additive merge (Path B) is DEFERRED to a future task if/
 *   when a second consumer needs these types. R1 ships solution-local + the
 *   spike file remains as design rationale (per Spike #2 §15 handoff note 6).
 *
 ...
 *     NOT log these fields. Mirrors the existing `selectionText` privacy
 *     contract on `WorkspacePaneEvent.user_selection`.
 *  5. ADR-030 additive-types rule: the six new event-type discriminants are
 *     ADDITIVE on existing four PaneEventBus channels. Existing subscribers
 *     tolerate unknown event.type values per the additive contract.
 *  6. R1 deliverable: contracts defined + Flows 1, 2, 5 wired with stub
 ...
 *
 * @see projects/spaarkeai-compose-r1/notes/spikes/spike-two-three-pane-wiring.md
 * @see projects/spaarkeai-compose-r1/notes/spikes/spike-2-prototype/contracts.ts (locked spike artifact — design rationale)
 * @see src/client/shared/Spaarke.AI.Widgets/src/events/PaneEventTypes.ts (existing PaneEventBus contract)
 */
 ...
 * `sprkDocumentId` field is also populated — but `speDriveItemId` remains
...
// ---------------------------------------------------------------------------
// Flow 1 — Workspace → Context (R1 WIRED)
// ---------------------------------------------------------------------------
 ...
 *
 * @binding spec.md FR-20 · design.md §5 flow #1 · Spike #2 §3 flow 1
 */
...
// ---------------------------------------------------------------------------
// Flow 2 — Workspace → Assistant (R1 WIRED)
// ---------------------------------------------------------------------------
 ...
 *
 * @binding spec.md FR-20 · design.md §5 flow #2 · Spike #2 §3 flow 2
 */
 ...
 * selection payload).
 *
 * @binding spec.md FR-20 · design.md §5 flow #3 · Spike #2 §3 flow 3
 */
 ...
 *
 * @binding spec.md FR-20 · design.md §5 flow #4 · Spike #2 §3 flow 4
 */
...
// ---------------------------------------------------------------------------
// Flow 5 — Assistant → Workspace (R1 WIRED with manual-confirm gate)
// ---------------------------------------------------------------------------
 ...
 * insert. The smoke test for actual insertion is R2.
 *
 ...
 *
 * @binding spec.md FR-20 · design.md §5 flow #5 · Spike #2 §3 flow 5
 */
export interface ComposeAssistantToWorkspaceFlow {
   ...
   * R1 DEFAULT (per Spike #2 §10.3 binding item): `true` — avoids
   * auto-injection UX risk pre-R2 actions.
   * Tier 1 safe.
   */
 ...
 *
 * @binding spec.md FR-20 · design.md §5 flow #6 · Spike #2 §3 flow 6
 */
 ...
 *
 * Use when a single subscriber routes ALL Compose flows uniformly (e.g.
 * the R1 stub receivers / smoke harness). Production subscribers should
 * narrow to the channel-specific union (`ComposeWorkspaceEvent` etc.)
 * instead.
 */
...
// ---------------------------------------------------------------------------
// ComposeEventBusContract — discriminated dispatch shapes per channel
// ---------------------------------------------------------------------------

/**
 * Channel-keyed dispatch contract for the Compose event surface.
 *
 * Used by the PaneEventBus dispatcher wiring (tasks 042/043/045) to validate
 * that an event of a given discriminant is dispatched on its correct channel
 * per the channel-routing map (`COMPOSE_FLOW_CHANNEL_MAP`).
 *
 * Each key is a `PaneChannel` value; each value is the discriminated union of
 * Compose flow events that ride that channel.
 *
 * Example (subscriber-side narrowing):
 * ```ts
 * usePaneEvent('workspace', (event) => {
 *   // Existing WorkspacePaneEvent handlers still work — event.type can
 *   // still be 'widget_load' etc.
 *   //
 *   // Compose-specific handling: narrow on the additive discriminants.
 *   if (event.type === 'compose_context_insert' || event.type === 'compose_assistant_insert') {
 *     const composeEvent = event as ComposeEventBusContract['workspace'];
 *     // Now `composeEvent.type` narrows to the two Compose `workspace`
 *     // event types and field access is type-safe.
 *   }
 * });
 * ```
 *
 * @binding Spike #2 §10.2 promotion contract + POML §steps[3] stub-receiver
 *          enablement.
 */
export interface ComposeEventBusContract {
  /** Compose discriminants additive on the `workspace` channel. */
  workspace: ComposeWorkspaceEvent;
  /** Compose discriminants additive on the `context` channel. */
  context: ComposeContextEvent;
  /** Compose discriminants additive on the `conversation` channel. */
  conversation: ComposeConversationEvent;
  /**
   * Compose dispatches NOTHING on the safety channel. Declared here for
   * exhaustiveness — Compose flows never carry safety annotations
   * directly; the safety perimeter operates upstream of Compose dispatch.
   */
  safety: never;
}
 ...
 *
 * The values are constrained to `PaneChannel` so a typo (e.g. dispatching
 * on `'conversations'` instead of `'conversation'`) is a compile error.
 *
 * @binding Spike #2 §10.3 binding item: "Channel mapping per §2" preserved
 *          byte-for-byte from the locked spike artifact.
 */
export const COMPOSE_FLOW_CHANNEL_MAP: Record<string, PaneChannel> = {
 ...
 *
 * Consumed by the Phase 6 smoke test to assert that the receivers it expects
 * to be wired (Flows 1, 2, 5) are flagged `r1Wired: true` here.
 *
 * @binding Spike #2 §8 R1 vs R2 receiver matrix; POML acceptance criterion #4.
 */
 ...
 *     logFlowEvent('flow-5', event as unknown as ComposeAssistantToWorkspaceFlow);
 *   }
 * });
 */
 ...
 * logs to console, returns. Used by the R1 stub subscribers (Phase 4
 * tasks 042 / 043 / 045).
 *
 * **Privacy note**: this function logs `sessionId`, `timestamp`, and
 * `documentRef` (all Tier 1 safe identifiers) but DOES NOT log
 * `selectionText`, `contentHtml`, or `insightText` (Tier 3 user content).
 * This deliberately mirrors the `selectionText` privacy contract on the
 * existing PaneEventBus — Compose flows MUST NOT smuggle user content
 * onto telemetry / log surfaces.
 */
