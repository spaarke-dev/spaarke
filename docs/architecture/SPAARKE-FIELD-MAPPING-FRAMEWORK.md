...
> **Current state vs target (updated 2026-09-25 — ADR-002: no plugins; invariants server-side).** "No plugin" stands — Spaarke ships no Dataverse plugins at all. What changes is *where* the rule is enforced. **Master today**: this client engine is the **sole enforcement** of creation-time mapping, so records created outside a `Create*Wizard` (OOB forms, Office add-ins, imports, flows) get no mapping — a known [ADR-002](../adr/ADR-002-no-heavy-plugins.md) **WP-2** gap, tracked as registry row **I-3** in [`DATAVERSE-WRITE-PATH-ARCHITECTURE.md`](DATAVERSE-WRITE-PATH-ARCHITECTURE.md) (§4.2, §5). **Target (WP-1/WP-2)**: the canonical engine moves server-side — a C# port, `CreateTimeFieldMapping`, invoked by `RecordCreationService`, exists on branch `work/spaarkeai-word-add-in-r1` (Matter/Project; **not yet on master**). Once wizards create through the BFF, this client engine becomes **preview-only**, and the `/push` endpoint's `ApplyMappingRule` (Copy-only) is to be replaced by the same server engine. The description below documents the master-today mechanism.
...
| No Dataverse plugin / form script, ever | *(Original decision)* The engine is called only from `Create*Wizard` service `onFinish`/create methods. *(Updated 2026-09-25 — ADR-002: no plugins; invariants server-side)* "No plugin" stands, but client-only enforcement is **no longer the canonical pattern**: per ADR-002 WP-1/WP-2 the canonical engine is the server-side `CreateTimeFieldMapping` via `RecordCreationService` (on `work/spaarkeai-word-add-in-r1`, not yet on master); the client engine becomes preview-only once wizards create via the BFF, and `/push`'s `ApplyMappingRule` is replaced by the same engine | Original rationale: creation-time mapping only existed for wizard-created entities; anything else fell back to the manual push. That gap (records created outside a wizard get no mapping) is now a registered WP-2 defect — invariant **I-3** in [`DATAVERSE-WRITE-PATH-ARCHITECTURE.md`](DATAVERSE-WRITE-PATH-ARCHITECTURE.md) §4.2/§5 — closed by the server write path plus WP-5 async fix-up, never by a plugin | [ADR-002](../adr/ADR-002-no-heavy-plugins.md) (was: project memory `no-dataverse-plugins`) |
