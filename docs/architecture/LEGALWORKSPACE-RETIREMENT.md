...
2. **Migration of `WorkspaceGrid.tsx:432` navigateTo** — **RESOLVED 2026-05-26 (code)**: Inline ToDo modal landed via R4 task 044 (`044-w6-followup-inline-todo-modal.poml`). The handler at `WorkspaceGrid.tsx#handleOpenTodoDialog` now opens a Fluent v9 `<Dialog>` (`src/solutions/LegalWorkspace/src/components/SmartToDo/SmartToDoDialog.tsx`) rendering `<SmartToDo webApi userId />` inside the existing `FeedTodoSyncProvider` scope — no `Xrm.Navigation.navigateTo` to `sprk_corporateworkspace` remains. Behavior parity verified: same `SmartToDo` Kanban component, same hooks (`useTodoItems` / `useKanbanColumns` / `useUserPreferences`), same filters, same actions, same data shape. No new BFF endpoints or services. Build verified clean on both LegalWorkspace and SpaarkeAi packages; SpaarkeAi bundle delta ~0 KB (modal lives in LegalWorkspace, not SpaarkeAi). UI tests in the task POML are deferred until the next deployment cycle (`<ui-tests>` runtime tests require a deployed env; task 044 was code+build-verify-only per operator guardrail).
