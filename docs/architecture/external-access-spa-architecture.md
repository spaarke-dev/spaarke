...
**Expiry — corrected 2026-09-08 (task 026, review finding M5): expiry IS ENFORCED, on the read paths that confer access.** This line previously said "expiry is NOT ENFORCED anywhere"; that was true until task 007 (2026-08-23) closed finding A-5. As of task 007, an optional `sprk_expiresdate` (Date Only) stored on the grant is read with the predicate `(sprk_expiresdate eq null or sprk_expiresdate ge {today})` on every path that confers access: the per-contact grant set, the organization-grant union, and the display-only participant list (`Infrastructure/ExternalAccess/ExternalParticipationService.cs`). An expired grant is excluded from `/api/v1/external/me` and every downstream authorization check the same way, while the row itself stays `statecode = 0` (Active) until someone revokes it.

The predicate is **deliberately absent** from two paths, and that is by design, not a residual gap: the write-side grant-upsert/revoke-sweep lookup (`ExternalGrantLifecycle.QueryActiveRowsAsync` / `RetrieveRowAsync`) and the project-closure cascade (`ProjectClosureEndpoint.BuildActiveProjectGrantsFilter`) must still be able to SEE and revoke an expired-but-still-Active row — filtering expiry into those queries would make expired grants unrevokable. There is still **no proactive sweep/deactivation job**: an expired grant remains `statecode = 0` in the table indefinitely unless something reads and revokes it; it simply no longer confers access on any read that matters. Live confirmation against a real tenant (that Dataverse actually honours the Date Only comparison) is task 034's obligation, not yet independently re-verified here.
