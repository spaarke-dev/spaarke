...
- **Instances** — the host runs on every instance, but each tick runs once: Redis lease, and the others record `Skipped`. Non-production slots run no ticks (`Scheduling__RunScheduledJobs=false`, slot-sticky, set by `scripts/Deploy-BffApi.ps1 -UseSlotDeploy`). Admin enable/disable and run history below are per instance. (`unified-access-control-r2` task 103.)
