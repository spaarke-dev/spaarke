...
| **Consumer dispatch** — `sprk_playbookconsumer` entity contract, `IConsumerRoutingService.ResolveAsync` semantics (5-min IMemoryCache TTL), `IInvokePlaybookAi.InvokePlaybookAsync` non-streaming facade, Path A / A.5 / B decision matrix, R4 `/narrate` case study | [`ai-architecture-consumer-routing.md`](ai-architecture-consumer-routing.md) |
