...
>
> This document has been updated for Spaarke Auth v2:
> - **OBO flow taxonomy + server-side patterns** (`GraphClientFactory`, endpoint filter inventory, OBO vs app-only decision matrix, CORS preflight) — canonical in v2.
> - **App-only Graph** (Pattern 3, 6) — updated to `DefaultAzureCredential` (managed identity) per ADR-028; `ClientSecretCredential` is local-dev fallback only.
> - **Pattern 7 (token acquisition)** — rewritten to the v2 two-layer model (`InMemoryCache` over pluggable `AuthStrategy`). The retired 6-strategy cascade is documented as "Pre-v2 historical".
> - **Pattern 8 (cross-iframe sharing)** — updated to MSAL `localStorage` (browser SOP) replacing the retired `window.__SPAARKE_BFF_TOKEN__` bridge.
> - **MUST NOT** reference `BridgeStrategy`, `XrmStrategy`, `MsalSilentStrategy`, `MsalRedirectStrategy`, `window.__SPAARKE_BFF_TOKEN__`, `tokenBridge`, `accessToken: string` typed props. Use `useAuth()` + `authenticatedFetch` from `@spaarke/auth`.
>
...
### App-Only Flow (Pattern 3, 6 — Auth v2 / ADR-028 canonical)
...
Every Spaarke-internal browser surface routes through one library: **`@spaarke/auth`** at [`src/client/shared/Spaarke.Auth`](../../src/client/shared/Spaarke.Auth/). This guarantees the MSAL invariants INV-1..INV-8 (localStorage + cookie state + tenant-specific authority + UPN-not-display-name as loginHint + omit authority in initAuth + one PCA via getAuthProvider + bundling reality) are applied uniformly. Direct `PublicClientApplication` use is **forbidden** on internal surfaces (per ADR-028).
...
| Code Pages (`src/solutions/*`, `src/client/code-pages/*`) | 12 | `@spaarke/auth` via `await initAuth({...})` in `main.tsx` (single call; ADR-028) | ✅ Canonical (v2) |
...
| Sync vs async name collisions in consumer authInit.ts | Runtime fails silently with Promise where string expected | Consumer's local `runtimeConfig.ts` exports `getTenantId` (sync) that collide with `initAuth()` param names | Use import aliases: `import { getTenantId as getRuntimeTenantId } from './runtimeConfig'`. See ADR-028 'consumer authInit.ts pattern'. |
| `sprk_TenantId` env var unset in new environment | 401-after-refresh class of bugs | v2 reads `sprk_TenantId` FIRST; Xrm frame-walk is fallback only | Populate `sprk_TenantId` per env (auth-deployment-setup.md §3). Verify console shows authority `https://login.microsoftonline.com/{tenant-guid}/`, not `/common` or `/organizations`. |
...
| New surface uses `@azure/msal-browser` directly | Popup on every tab open for that surface; no token sharing with other Spaarke components | Bypasses `@spaarke/auth` (violates INV-7 "one PCA via getAuthProvider") | Always consume `@spaarke/auth` via `useAuth()` / `authenticatedFetch`. Office Add-ins use `OfficeNaaStrategy` (also exported from `@spaarke/auth`); External SPA is the documented out-of-scope exception (D-AUTH-7). |
