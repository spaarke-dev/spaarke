...
**Issue: "pac pcf push fails with MSB3231: Unable to remove directory"**

**Cause:** Windows file lock on the temporary metadata folder during Solution Packager cleanup phase
...
**Key Insight:** The solution zip file IS generated successfully BEFORE the cleanup error occurs.

**Workaround:**
```bash
# 1. The error occurs during cleanup, but the zip file is already created
# Look for the generated solution in:
ls /c/code_files/spaarke/src/client/pcf/[ControlName]/obj/PowerAppsToolsTemp_[prefix]/bin/Debug/*.zip

# 2. Import the solution directly
pac solution import \
  --path /c/code_files/spaarke/src/client/pcf/[ControlName]/obj/PowerAppsToolsTemp_[prefix]/bin/Debug/PowerAppsToolsTemp_[prefix].zip \
  --async true \
  --publish-changes true

# Example for SpeFileViewer with sprk prefix:
pac solution import \
...
**Prevention:** Clear the temporary folder before running `pac pcf push`:
```bash
rm -rf obj/PowerAppsToolsTemp_*
pac pcf push --publisher-prefix sprk
```
...
| 1.2 | 2025-12-04 | Added MSB3231 file lock workaround for pac pcf push |
...
**Last Verified:** 2025-12-04 (Spaarke Dev 1)
**Verified Controls:** UniversalQuickCreate, UniversalDatasetGrid, SpeFileViewer
