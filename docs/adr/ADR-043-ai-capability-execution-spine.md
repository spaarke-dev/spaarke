...
#### Amendment 1 — declared companion inputs render alongside the operand (2026-08-24, `spaarkeai-compose-r8` task 051, §6.5 Path B)

**The gap.** The operand answers *what content the completion runs over*. It cannot answer *where that content came from*, because it is **single-valued by construction**: the binder returns on the first vocabulary match and builds a one-key object. So a caller holding a deterministic anchor had nowhere to put it. Compose captures a stable `w14:paraId` at selection time, sent it on every dispatch — and it was **accepted and then silently dropped before the prompt**. The model, never given an identifier, could only name its edit target by *quoting the prose back*. Quoting is a **generation** step and generation is lossy; that is the mechanism behind Compose's "wording differs slightly" dead end, and it survived four releases because nothing tied the declared contract to what actually reached the model.

**The amendment.** An Action's other **declared** inputs render into the same `## Input` object alongside the operand. **Declaration is the contract**: a property the Action declares in `sprk_inputschema` and the caller supplies reaches the model; anything else stays accepted-and-ignored. This closes the accepted-and-ignored trap itself, not just its first victim, and keeps capability shape in catalog DATA (ADR-039) — a new companion input is an Action-row edit, not a code change. It is also the same door a future enumerated closed set (e.g. a whole-document paragraph list) walks through, so no second mechanism is owed later.

**Bounds.** Never the operand field; never another vocabulary name (a second *content* field must not enter this way); never the `ledger_resolution` control member; count- and size-capped; an oversized value is **skipped and logged, never truncated** — a half-sent identifier list reads as complete and is worse than an absent one.

**Rejected alternatives.** (1) *A fourth operand-vocabulary entry* — not merely a spine change but **architecturally wrong**: the vocabulary is a pick-one list, so the new name would COMPETE with `selectionText` and one of the two would silently vanish, breaking every selection dispatch. (2) *Nesting identity inside the operand value* (`selectionText: {text, paraId}`) — a type pun: `OperandKind.SelectionText` means the TEXT, tiered as content (ADR-015), while an anchor is a Tier-1 opaque identifier; it would also make `selectionText` a string in most Actions and an object in a few, which is the inconsistency that breeds the next hidden gap.

**Verification obligation.** The amendment ships with a `tests/integration/seam/**` slice asserting all three halves: a declared+supplied input **reaches** the rendered prompt, an **undeclared** arg does not, and the operand is **not displaced**. The declaration must also survive deployment — the seed-to-column mapping is asserted too, because an authored `inputSchema` that the deploy script never writes is inert in every environment and indistinguishable at runtime from never having been declared.
