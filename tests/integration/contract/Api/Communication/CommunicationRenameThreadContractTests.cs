...
/// <summary>
/// Endpoint contract tests for <c>POST /api/communications/threads/{threadId}/rename</c> (task 004 / FR-17).
...
/// <see cref="ThreadResolver"/> (the atomic sprk_name + Edited-marker write) — over three mocked module
/// boundaries only: <see cref="IImpersonatedCommunicationQuery"/> (visibility), <see cref="ICallerSystemUserResolver"/>
/// (caller resolution), and <see cref="IGenericEntityService"/> (the Dataverse write). No
/// <c>Mock&lt;HttpMessageHandler&gt;</c>, no DI-registration test.
/// <para>
/// Covers the closed contract set: 401 (unauth), 400 (blank name), <b>403 (caller cannot see the thread — the
/// negative-auth case, the correctness-critical NFR-01 guard)</b>, and 200 (happy path: sprk_name set AND
/// sprk_nameisautoderived flipped to Edited in ONE write — edit-preserve).
/// </para>
/// </summary>
public class CommunicationRenameThreadContractTests : IClassFixture<CommunicationRenameTestWebAppFactory>
{
    private readonly CommunicationRenameTestWebAppFactory _factory;
    private readonly HttpClient _client;

    private static readonly Guid CallerSystemUserId = Guid.Parse("77777777-7777-7777-7777-777777777777");

    public CommunicationRenameThreadContractTests(CommunicationRenameTestWebAppFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task RenameThread_WithoutAuth_Returns401()
    {
        using var anonFactory = CommunicationRenameTestWebAppFactory.CreateAnonymous();
        var anonClient = anonFactory.CreateClient();

        var response = await anonClient.PostAsJsonAsync(
            $"/api/communications/threads/{Guid.NewGuid()}/rename", new { name = "Case strategy" });
    ...
    [Fact]
    public async Task RenameThread_WhenCallerCannotSeeThread_Returns403AndDoesNotWrite()
    {
        ...
        var response = await _client.PostAsJsonAsync(
            $"/api/communications/threads/{threadId}/rename", new { name = "Case strategy" });

        response.StatusCode.Should().Be(HttpStatusCode.Forbidden);
        // No rename write is ever attempted for a thread the caller cannot see (no marker flip, no name change).
    ...
    [Fact]
    public async Task RenameThread_WhenCallerCanSeeThread_Returns200_SetsNameAndFlipsMarkerToEdited()
    {
        var threadId = Guid.NewGuid();
        _factory.ResolveCaller(CallerSystemUserId);
        ...
        payload.GetProperty("name").GetString().Should().Be("Case strategy"); // trimmed

        // Edit-preserve: sprk_name AND sprk_nameisautoderived=false (Edited) in ONE update.
        written.Should().NotBeNull();
        ((string)written!["sprk_name"]).Should().Be("Case strategy");
        ((bool)written["sprk_nameisautoderived"]).Should().BeFalse("a rename flips the marker to Edited");
        _factory.EntityServiceMock.Verify(e => e.UpdateAsync(
            "sprk_communicationthread", threadId, It.IsAny<Dictionary<string, object>>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
...
/// <summary>
/// WebApplicationFactory for the rename-thread contract tests. Mirrors <c>OfficeCommunicationsTestWebAppFactory</c>'s
/// host-config baseline; additionally swaps the <see cref="IImpersonatedCommunicationQuery"/> (visibility) and
/// <see cref="ICallerSystemUserResolver"/> (caller resolution) boundaries so tests can drive the 403/200 paths, and
/// exposes the <see cref="IGenericEntityService"/> mock to assert the atomic rename write.
/// </summary>
public sealed class CommunicationRenameTestWebAppFactory : WebApplicationFactory<Program>
{
    ...
    public CommunicationRenameTestWebAppFactory() : this(disableAuth: false) { }

    private CommunicationRenameTestWebAppFactory(bool disableAuth) => _disableAuth = disableAuth;

    public static CommunicationRenameTestWebAppFactory CreateAnonymous() => new(disableAuth: true);
        ...
        builder.ConfigureTestServices(services =>
        {
            ...
            if (_disableAuth)
            {
                services.AddAuthentication("Test")
                    .AddScheme<AuthenticationSchemeOptions, CommRenameDenyAllAuthHandler>("Test", _ => { });
            }
            else
            {
                services.AddAuthentication("Test")
                    .AddScheme<AuthenticationSchemeOptions, CommRenameAllowAllAuthHandler>("Test", _ => { });
            }
            ...
            // Swap the three module boundaries the rename path crosses.
...
internal sealed class CommRenameAllowAllAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    public CommRenameAllowAllAuthHandler(
...
internal sealed class CommRenameDenyAllAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    public CommRenameDenyAllAuthHandler(
        IOptionsMonitor<AuthenticationSchemeOptions> options,
        ILoggerFactory logger,
        System.Text.Encodings.Web.UrlEncoder encoder)
        : base(options, logger, encoder) { }
