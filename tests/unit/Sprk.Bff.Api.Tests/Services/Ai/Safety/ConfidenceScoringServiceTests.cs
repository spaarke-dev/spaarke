...
public class ConfidenceScoringServiceTests
{
    ...
    [Fact]
    public void Score_ReturnsMedium_WhenTwoSourcesAndOneUngroundedSegment()
    {
        // Arrange
        // 2 sources  → source_score = 2/5 = 0.4
        // 1 ungrounded out of ~2 segments → groundedness_ratio = (2-1)/2 = 0.5
        // raw_score = (0.5 * 0.6) + (0.4 * 0.4) = 0.30 + 0.16 = 0.46 → Medium
        ...
        result.Score.Should().BeInRange(0.40f, 0.74f);
        result.Rationale.Should().NotBeNullOrWhiteSpace();
    }
    ...
    [Fact]
    public void Score_ReturnsMedium_WhenFiveSourcesAndHalfUngrounded()
    {
        // Arrange
        // 5 sources → source_score = 1.0
        // 1 ungrounded of 2 segments → groundedness_ratio = (2-1)/2 = 0.5
        // raw_score = (0.5 * 0.6) + (1.0 * 0.4) = 0.30 + 0.40 = 0.70 → Medium
        var groundedness = GroundednessResult.Ungrounded(
        ...
        // Assert
        result.Level.Should().Be(ConfidenceLevel.Medium);
        result.Score.Should().BeApproximately(0.70f, precision: 0.001f);
        result.Rationale.Should().NotBeNullOrWhiteSpace();
    }
