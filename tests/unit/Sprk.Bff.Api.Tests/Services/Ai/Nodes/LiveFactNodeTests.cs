...
public sealed class LiveFactNodeTests
{
    ...
    [Fact]
    public async Task ExecuteAsync_MatterSubject_RoutesToMatterResolver()
    {
        // Three distinct mocks let us verify the dispatcher routes to the right one.
        var matterMock = new Mock<ILiveFactResolver>();
        var projectMock = new Mock<ILiveFactResolver>();
        var invoiceMock = new Mock<ILiveFactResolver>();

        var fact = BuildFact(MatterSubject, "attorney", 0.0);
        matterMock.Setup(r => r.ResolveAsync(MatterSubject, "attorney", It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(fact);

        var resolvers = new Dictionary<string, ILiveFactResolver>(StringComparer.OrdinalIgnoreCase)
        {
            ["matter"] = matterMock.Object,
            ["project"] = projectMock.Object,
            ["invoice"] = invoiceMock.Object
        };
        ...
        result.Success.Should().BeTrue();
        matterMock.Verify(r => r.ResolveAsync(MatterSubject, "attorney", It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);
        projectMock.VerifyNoOtherCalls();
        invoiceMock.VerifyNoOtherCalls();
    }

    [Fact]
    public async Task ExecuteAsync_ProjectSubject_RoutesToProjectResolver()
    {
        var matterMock = new Mock<ILiveFactResolver>();
        var projectMock = new Mock<ILiveFactResolver>();
        var invoiceMock = new Mock<ILiveFactResolver>();

        var fact = BuildFact(ProjectSubject, "projectName", 0.0);
        projectMock.Setup(r => r.ResolveAsync(ProjectSubject, "projectName", It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(fact);
        ...
        var context = InsightsNodeTestHelpers.CreateContext(
            ActionType.LiveFact,
            $$"""{ "subject": "{{ProjectSubject}}", "predicate": "projectName" }""");

        var result = await node.ExecuteAsync(context, CancellationToken.None);

        result.Success.Should().BeTrue();
        projectMock.Verify(r => r.ResolveAsync(ProjectSubject, "projectName", It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);
        matterMock.VerifyNoOtherCalls();
        invoiceMock.VerifyNoOtherCalls();
    }

    [Fact]
    public async Task ExecuteAsync_InvoiceSubject_RoutesToInvoiceResolver()
    {
        var matterMock = new Mock<ILiveFactResolver>();
        var projectMock = new Mock<ILiveFactResolver>();
        var invoiceMock = new Mock<ILiveFactResolver>();

        var fact = BuildFact(InvoiceSubject, "invoiceNumber", 0.0);
        invoiceMock.Setup(r => r.ResolveAsync(InvoiceSubject, "invoiceNumber", It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(fact);
        ...
        var context = InsightsNodeTestHelpers.CreateContext(
            ActionType.LiveFact,
            $$"""{ "subject": "{{InvoiceSubject}}", "predicate": "invoiceNumber" }""");

        var result = await node.ExecuteAsync(context, CancellationToken.None);

        result.Success.Should().BeTrue();
        invoiceMock.Verify(r => r.ResolveAsync(InvoiceSubject, "invoiceNumber", It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Once);
        matterMock.VerifyNoOtherCalls();
        projectMock.VerifyNoOtherCalls();
    }
