...
public sealed class SessionFileRetrievalBindingGuardTests
{
    ...
    [Fact]
    public async Task Retrieval_TargetingSparkleSessionFiles_DoesNotTriggerGuard()
    {
        // Arrange — happy path: default chat-domain index is configured. The guard MUST
        // NOT fire; the session-scoped routing branch proceeds to GetSearchClient.
        var service = CreateService(sessionFilesIndexName: "spaarke-session-files");

        // Set up the SearchClient mock to return an empty result set so the search
        // completes cleanly (the guard's positive path passes control to Azure SDK calls).
        var sessionSearchClient = new Mock<SearchClient>(MockBehavior.Loose);
        ...
        sessionSearchClient
            .Setup(c => c.SearchAsync<KnowledgeDocument>(
                ...
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(Response.FromValue(emptyResults, Mock.Of<Response>()));
        sessionSearchClient.SetupGet(c => c.IndexName).Returns("spaarke-session-files");

        _searchIndexClientMock
            .Setup(x => x.GetSearchClient("spaarke-session-files"))
            .Returns(sessionSearchClient.Object);
        ...
        // Act — SHOULD NOT throw; the guard is selective, not a blanket block.
        var response = await service.SearchAsync("test query", options);

        // Assert — happy path lands on the session-files SearchClient (guard passed).
        response.Should().NotBeNull();
        _searchIndexClientMock.Verify(
            x => x.GetSearchClient("spaarke-session-files"),
            Times.Once);
    }

    [Fact]
    public async Task Retrieval_GuardOnlyAppliesUnderSessionScopedRouting_NotKnowledgeIndexPath()
    {
        // Arrange — when SessionId is absent the routing branch is the pre-R5 deployment
        // path (tenant-knowledge-index family). The guard MUST NOT fire on that path
        // because operator-configured SessionFilesIndexName is not consulted there.
        // This documents the scope of the guard (chat-memory session path only).
        var service = CreateService(sessionFilesIndexName: "spaarke-insights-index"); // would trip guard IF SessionId were set

        // Deployment-service returns a mock SearchClient that immediately returns empty.
        var knowledgeSearchClient = new Mock<SearchClient>(MockBehavior.Loose);
        var emptyResults = SearchModelFactory.SearchResults<KnowledgeDocument>(
            values: new List<SearchResult<KnowledgeDocument>>(),
            totalCount: 0,
            facets: null,
        ...
        knowledgeSearchClient
            .Setup(c => c.SearchAsync<KnowledgeDocument>(
                ...
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(Response.FromValue(emptyResults, Mock.Of<Response>()));
        knowledgeSearchClient.SetupGet(c => c.IndexName).Returns("spaarke-knowledge-index-v2");

        _deploymentServiceMock
            .Setup(d => d.GetSearchClientAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(knowledgeSearchClient.Object);

        var options = new RagSearchOptions
        {
            TenantId = "tenant-A"
            // SessionId intentionally NOT set — routes via deployment path
        };

        // Act — SHOULD NOT throw; guard is gated on session-scoped routing branch.
        var response = await service.SearchAsync("test query", options);

        // Assert — knowledge path was taken; the misconfigured SessionFilesIndexName was
        // never consulted, so the guard never fired.
        response.Should().NotBeNull();
        _searchIndexClientMock.Verify(
            x => x.GetSearchClient(It.IsAny<string>()),
            Times.Never);
        _deploymentServiceMock.Verify(
            x => x.GetSearchClientAsync("tenant-A", It.IsAny<CancellationToken>()),
            Times.Once);
    }
}
