...
public class SafetyPipelineMiddlewareTests
{
    ...
    [Fact]
    public async Task SendMessageAsync_PostLlmCheckThrows_ResponseDelivered_DegradedAnnotation()
    {
        // Arrange — groundedness service throws.
        var groundedness = new Mock<IGroundednessCheckService>();
        groundedness
            .Setup(g => g.CheckAsync(It.IsAny<GroundednessRequest>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new HttpRequestException("Content Safety API down"));
        ...
        confidence
            .Setup(c => c.Score(It.IsAny<ConfidenceScoringRequest>()))
            .Returns(new ConfidenceScoringResult(ConfidenceLevel.High, 0.8f, "Degraded but OK."));
        ...
        var updates = await DrainAsync(
            middleware.SendMessageAsync("What is grounding?", [], CancellationToken.None));

        // Assert — response tokens still delivered.
        string.Concat(updates.Select(u => u.Text))
              .Should().Be("Response despite groundedness failure.",
                  "a post-LLM check failure must never suppress the AI response");

        // Assert — safety_annotation still emitted (degraded).
        emitted.Should().ContainSingle(e => e.Type == "safety_annotation");

        // Assert — audit was still written.
        audit.Verify(
            a => a.LogInteractionAsync(It.IsAny<AuditEntry>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
