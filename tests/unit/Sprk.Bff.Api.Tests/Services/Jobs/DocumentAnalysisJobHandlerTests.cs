...
public class DocumentAnalysisJobHandlerTests
{
    private readonly Mock<IDocumentIntelligenceService> _summarizeServiceMock;
    private readonly Mock<IIdempotencyService> _idempotencyServiceMock;
    private readonly Mock<IDataverseService> _dataverseServiceMock;
    private readonly Mock<ILogger<DocumentAnalysisJobHandler>> _loggerMock;
    private readonly DocumentAnalysisJobHandler _handler;

    public DocumentAnalysisJobHandlerTests()
    {
        _summarizeServiceMock = new Mock<IDocumentIntelligenceService>(MockBehavior.Strict);
        _idempotencyServiceMock = new Mock<IIdempotencyService>(MockBehavior.Strict);
        _dataverseServiceMock = new Mock<IDataverseService>();
        _loggerMock = new Mock<ILogger<DocumentAnalysisJobHandler>>();

        _handler = new DocumentAnalysisJobHandler(
    ...
    {
        _handler.JobType.Should().Be("ai-analyze");
    }
    ...
    public async Task ProcessAsync_WhenSummarizationSucceeds_ReturnsSuccessOutcome()
    {
        ...
        var expectedKey = $"analyze:{documentId}:{job.JobId}";
        ...
        _summarizeServiceMock
            .Setup(x => x.AnalyzeAsync(It.IsAny<DocumentAnalysisRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(result);
        ...
        outcome.JobType.Should().Be("ai-analyze");

        _summarizeServiceMock.Verify(
            x => x.AnalyzeAsync(
                It.Is<DocumentAnalysisRequest>(r => r.DocumentId == documentId),
    ...
    public async Task ProcessAsync_WhenSuccessful_MarksEventAsProcessed()
    {
        ...
        var result = AnalysisResult.Succeeded("Summary", 100, TextExtractionMethod.Native);
        _summarizeServiceMock
            .Setup(x => x.AnalyzeAsync(It.IsAny<DocumentAnalysisRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(result);
    ...
    public async Task ProcessAsync_WhenAlreadyProcessed_SkipsProcessingAndReturnsSuccess()
    {
        ...
        _summarizeServiceMock.Verify(
            x => x.AnalyzeAsync(It.IsAny<DocumentAnalysisRequest>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
    ...
    public async Task ProcessAsync_WhenSummarizationFails_StillReturnsSuccess()
    {
        ...
        var result = AnalysisResult.Failed("File not found");
        _summarizeServiceMock
            .Setup(x => x.AnalyzeAsync(It.IsAny<DocumentAnalysisRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(result);
    ...
    public async Task ProcessAsync_WhenInvalidPayload_ThrowsArgumentException()
    {
        ...
        var expectedKey = $"analyze:{job.SubjectId}:{job.JobId}";
        SetupIdempotencyForProcessing(expectedKey, isProcessed: false, lockAcquired: true);
    ...
    {
        var payload = new DocumentAnalysisJobPayload(documentId, driveId, itemId);
...
public class DocumentAnalysisJobPayloadTests
{
    [Fact]
    public void ToRequest_CreatesCorrectDocumentAnalysisRequest()
    {
        // Arrange
        var documentId = Guid.NewGuid();
    ...
    {
        // Arrange
        var original = new DocumentAnalysisJobPayload(Guid.NewGuid(), "drive-id", "item-id");

        // Act
        var json = JsonSerializer.Serialize(original);
        var deserialized = JsonSerializer.Deserialize<DocumentAnalysisJobPayload>(json, new JsonSerializerOptions
        {
