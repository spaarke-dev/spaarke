...
4. **Verify publish-size impact** on EVERY BFF-touching task (not just NuGet adds — per R4 NFR-01 / F-3, strengthened 2026-05-26).
...
| Dataverse write path (no plugins — ADR-002) | `src/server/api/Sprk.Bff.Api/Services/Dataverse/CoreAncestorResolver.cs` | Reference server-side invariant owner; see `docs/architecture/DATAVERSE-WRITE-PATH-ARCHITECTURE.md` |
...
| **Document profiling + the 3 AI execution models (READ before changing the file/Document create → profile → index pipeline)** | [`docs/architecture/DOCUMENT-PROFILE-AND-AI-EXECUTION-MODELS.md`](docs/architecture/DOCUMENT-PROFILE-AND-AI-EXECUTION-MODELS.md) — the authoritative map of the **three ways the BFF runs AI** (node-based playbook · direct Action/linear ADR-043 · legacy sequential-tool) and how a saved `sprk_document` gets profiled through **three divergent entry points** (wizard + Compose = direct Action; **Outlook/app-only = node playbook**). Documents the confirmed failure mode (Part 4): the "Document Profile" playbook's **Update Record** node uses the Playbook-Builder **wrapper format** (JSON-inside-a-JSON-string); the single-level JSON-aware Layer-1 renderer escapes only the outer level, so `ParseConfig`'s re-parse of the nested string throws `0x0A invalid at $.fieldMappings[0].value` → `filesummarystatus=Failed`. Cross-ref [`.claude/FAILURE-MODES.md` AP-10](.claude/FAILURE-MODES.md#ap-10-a-json-aware-renderer-that-escapes-one-nesting-level-over-a-config-re-parsed-deeper). email-communication-intelligence-r2 (2026-09-01, GitHub #919). |
...
| **Dataverse write path — no plugins; invariants live on the server (READ before adding any rule a record must satisfy on save, or any create/update path)** | [`docs/architecture/DATAVERSE-WRITE-PATH-ARCHITECTURE.md`](docs/architecture/DATAVERSE-WRITE-PATH-ARCHITECTURE.md) + [ADR-002](.claude/adr/ADR-002-thin-plugins.md) (reviewed 2026-09-25, §6.5 Path C) — Spaarke ships **no Dataverse plugins**. Each record invariant (field-mapping defaults, core-ancestor stamp, secure isolation, owner, container/index default) has **ONE BFF server-side owner** (WP-1); client wizards **preview only** (WP-2); invariant-bearing tables are written **via BFF, not `Xrm.WebApi`** (WP-3); security + on-load UX applied **inline** (WP-4); writes outside the product → async fix-up + reconciliation (WP-5); **security fails closed** (WP-6). Invariant registry §5 lists owner + gap per invariant. Root cause the review found: invariants enforced only in client wizards, skipped by every other write path incl. Office add-ins. |
