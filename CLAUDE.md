...
| **God-class ratchet (server file-size gate)** | [`.claude/patterns/testing/god-class-ratchet.md`](.claude/patterns/testing/god-class-ratchet.md) — `GodClassGuardTests` (ArchTest/CI). **No NEW `src/server/**/*.cs` file > 2,000 lines; existing oversized files are frozen at their LOC (+100 grace).** READ THIS before growing a large server file — when the gate fails, **decompose** (preferred) or **re-baseline the file's waiver with a PR reason** (never silently). 13 frozen files (2026-08-16; `DataverseWebApiService` graduated below 2,000 via RED-4 B dead-code deletion); worst offenders have decomposition seeds in `projects/code-quality-and-assurance-r3/notes/red-item-analyses/RED-1,2,4`. code-quality-and-assurance-r3 task 040 + followups. |
