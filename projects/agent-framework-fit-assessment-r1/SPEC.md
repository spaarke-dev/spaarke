...
| S5 | **Foundry Agent Service overlap** — durable / HITL / A2A multi-agent work | **Bimodal** (per task 002 inventory, 2026-06-03): (a) in-BFF wrapper SHIPPED under `Services/Ai/Foundry/` using `Azure.AI.Projects.AgentsClient` (default-OFF kill switch per ADR-018, consumed via `AgentServiceRoutingMiddleware` + JPS `AgentServiceNodeExecutor`); (b) canonical durable / HITL legal workflows curated-only, no Spaarke code yet | `src/server/api/Sprk.Bff.Api/Services/Ai/Foundry/` (shipped wrapper) + `knowledge/foundry-agent-service/` (curated canonical surface) |
