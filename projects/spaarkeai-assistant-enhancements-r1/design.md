...
The grounding worry — *"if the check can pick the 'best' candidates, aren't we back at auto-detecting from the full set?"* — is resolved in code by keeping grounding **binary, not a ranking**. `AgentToolProjection.PreFilter` is documented as *"a pure predicate — no scoring, no classification, no utterance inspection"* (ADR-039 permits exactly one scale aid: deterministic pre-filtering, **never** a decision-maker). Today it branches **only on `Surface` + the Binding's `sprk_surfaces`** (plus `sprk_matchconditions`). Structural session facts are *available* on the filter context — `HasSessionFiles`, `HasActiveDocument`, `HasAnalysisBinding`, and the host record (`ChatHostContext.EntityType`/`EntityId`) — but are **declared-but-not-yet-consumed** (the code reserves them for future catalog columns). Grounding **removes the impossible; it never picks the best.** To hide "Create matter" when already inside a matter, the guard is **one new catalog column** (e.g. `requires-no-attached-record`) **plus a new `PreFilter` branch** that tests it against `ChatHostContext.EntityId == null` — a predicate, no model call. (Slightly more than "add a column," but still additive and cheap — and exactly the sanctioned extension point.) The owner's instinct (a pre-narrowed, human-curated successor list makes this cheap and high-quality) is exactly the sanctioned pattern: a small curated set + a boolean truth-filter, not an AI relevance-scorer over the whole catalog.
...
**One reader (shipped seam, currently empty):** implement `IOrganizationalContextProvider` (the ADR-032 null-object at `Services/Ai/PublicContracts/`, registered in `AnalysisServicesModule.cs` as `NullOrganizationalContextProvider` — banner "Work IQ provider deferred") to project *stated profile + membership/role* into the agent context, alongside the User-scope memory fragment `ContextBinder` already injects. **Confirmed still unimplemented post-pull (2026-07-15).** **Wiring this seam is most of the User Model's runtime value** — the store work is small; the injection is the point.

> **Note on preferences (resolved 2026-07-15):** a stated-preferences EAV table exists, and the **live/deployed entity is the plural `sprk_userpreferences`** (PK `sprk_userpreferencesid`). The DailyBriefing client (`preferencesService.ts`) references a *singular* `sprk_userpreference` with **no matching deployed entity — a real client/schema mismatch bug, not just doc drift.** If R1 reuses this table for stated prefs, target the plural entity and fix (or route around) the client bug; otherwise prefer the dedicated `sprk_userprofile` entity.
...
**Assessment (retrofit, not re-architect — to confirm in a spike):** the interaction needs one new thing — a **wizard entry-payload contract**: a standard envelope (files, seed field values, source metadata) any caller can open a wizard with, plus letting the AI pre-fill consume a *handed-in* file rather than only one uploaded *inside* the wizard. The optimistic part: the wizards **already have a creation-time pre-fill seam** (the Field Mapping Framework, wired into all seven `Create*Wizard` services). The Assistant's pre-fill feeds *Assistant-supplied seed values* into that existing seam. Bracketed by two existing seams — **PaneEventBus (`@spaarke/ai-widgets`, `Spaarke.AI.Widgets/src/events/PaneEventTypes.ts`) carries the launch in via `WorkspaceWidgetLoadEvent` (whose `widgetData` is typed `unknown`, so it can already carry anything), the Field Mapping pre-fill applies it** — the only genuinely new piece is the *typed* entry-payload envelope in between.
...
- **`IOrganizationalContextProvider`** (`Services/Ai/PublicContracts/`, ADR-032 null-object) — the shipped-but-empty injection seam (FR-B-11); implement it, don't invent a new context path. Confirmed still unimplemented post-pull.
...
2. **Stated-profile store shape + the feeds-the-decider-never-grants-capability invariant.** Learned/role slices reuse `MemoryItem` + `sprk_userentityassociation` (§6). The open call: the **new stated profile** as a typed `sprk_userprofile` entity (recommended) vs a new type on the existing **plural `sprk_userpreferences`** (note: the DailyBriefing client's singular `sprk_userpreference` is a mismatch bug — see §6). Plus the binding rule: preference ≠ permission; grounding still gates.
...
4. **Stated-profile shape** (§12.2) — largely resolved by the 2026-07-15 investigation (learned → `MemoryItem`; role → membership/BU; live prefs entity = **plural `sprk_userpreferences`**). Remaining call: new `sprk_userprofile` entity (recommended) vs a new type on `sprk_userpreferences` — and whether to also fix the DailyBriefing client's singular-name mismatch bug or route around it.
