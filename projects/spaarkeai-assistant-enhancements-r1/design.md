...
|---|---|---|---|
| `sprk_primaryrole` | Primary Role | Choice — **existing GLOBAL role option set** (owner decision 2026-07-15) | Reuse the org-wide role choice set (the profile serves more than the AI). Values incl. Senior Partner … Associate General Counsel (14 roles, 100000000-based). The AI reader reads the *selected label*; role is not a create-flow constrained field, so no resolver dependency. *Capture the global set's logical name when wiring FR-E2.* |
| Practice Areas | Practice Areas | **N:N relationship** `sprk_userprofile` ↔ **`sprk_practicearea_ref`** (owner decision 2026-07-15) — surfaced as a **subgrid** on the profile (reusable beyond AI) | Reuse the existing **Practice Area** ref table (`sprk_practiceareaname`); the reader (FR-E2) queries the related rows and renders their names. **Consistency win:** `sprk_practicearea_ref` is the same governed closed set the FR-B1 resolver matches the *matter* practice-area lookup against (UC-3) — one source of truth. *(Replaces the earlier multi-select `sprk_practiceareas` choice column.)* |
...
> **Registry obligation (Fable review 2026-07-15):** this project is **not yet in [`projects/INDEX.md`](../INDEX.md)** — the §H/root-§17 active-project registry consumed by `/conflict-check`. It MUST be registered (13 of 17 active worktrees touch BFF). *Note: `ai-architecture-redesign-r2` is complete/merged/deployed (verified 2026-07-15) — no concurrent-edit dependency on it; R1 builds on its shipped `ContextBinder`/dispatch-spine state (§13).*
...
| **ai-architecture-redesign-r1 / r2** | Charter/platform baseline for the Action + Binding + session-dispatch surface this project builds on. **Status (verified 2026-07-15): r2 is COMPLETE, merged, and deployed** (its `current-task.md`: "all substantive r2 work complete + merged + deployed"; create-matter seeded live). **No cross-project coordination — R1 builds on r2's shipped state on `master`, not concurrent edits.** ADR-043 is still labeled *Proposed* only because the status-flip is r2's wrap-up bookkeeping; the **spine is merged**, so R1 complies with the as-merged code. **R1 self-owns the dispatcher-side invariants** (ack-or-honest-failure FR-C1, no-collateral-teardown FR-C2) rather than depending on r2 for them (owner decision 2026-07-15: do it here, no cross-project coordination). Any Compose-*surface* root fix (UC-4/UC-5 tab lifecycle) stays compose/governance territory but is not an R1 dependency. |
