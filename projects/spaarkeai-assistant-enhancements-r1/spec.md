...
1. **FR-A1** (P2/P3): Draft-in-chat → the dispatcher **launches the correct pre-seeded create surface** for the intended entity and that surface owns the gated write. The fix targets the R2-UAT **multi-turn field-by-field elicitation dead-end** (chat asked for each required field, then failed a closed-set lookup and created nothing) — R1 stops collecting constrained fields in chat and pops the surface, pre-seeded with already-collected inputs (esp. the uploaded file) so the user is never re-asked. **Surface routing (design §10.1):** matter → **Create Matter wizard**; Event-Task → **Event wizard (subtype preset = Task)**; To Do → **OOB `sprk_todo` create form in a modal** (`Xrm.Navigation.navigateTo`). — Acceptance: "create a matter / event(-task) / to-do" opens the routed surface, pre-seeded (file attached, drafts defaulted), produces a real record of the **right entity**; **no multi-turn field-by-field elicitation; no dead-end**; the user is not re-asked for an already-provided file.
2. **FR-A2** (P4): Intent is modeled to the **real entity + its real create surface**, not one generic capability (design §10.1). The shipped `create-task` (writes `sprk_event`(Task) via **direct `dataverse.create_record`**) is **repointed to the Event wizard (subtype=Task)**; a distinct **To Do** intent routes to the **OOB `sprk_todo` form modal**. — Acceptance: explicit "To Do" opens the `sprk_todo` OOB form (creates `sprk_todo`); "task"/"event" opens the Event wizard preset to Task (creates `sprk_event`); the **Event-Task-vs-To-Do** ambiguity resolves by high-confidence inference or one-tap pick, never text negotiation (§5).
3. **FR-A3**: A typed **entry-payload envelope** (files + resolved/proposed values + source metadata + **target surface**) carries the launch; for custom wizards it rides `wizard_step` `set-field` (not `applyFieldMappings`, which is record-sourced) — unify the 5-of-7 embedded wrappers + close the 2 gaps; for To Do it opens the **OOB form in a modal** pre-seeded via the OOB form's default-value/createFromEntity params. — Acceptance: a handed-in file + seed values pre-fill the launched wizard **and** the OOB To Do form modal.
...
10. **FR-C1**: Every action claim is **ack-gated** on a client acknowledgment referencing the emitted action, or **fails honestly**. **R1 owns this end-to-end** (adopts + completes the D-F3 ack contract for all Follow-Through actions — no dependency on redesign-r2, which is merged; owner decision 2026-07-15). — Acceptance: a UI action that does not complete yields an honest failure message, never a fabricated success (UC-5 regression test).
...
14. **FR-E1**: Typed columns on `sprk_userprofile` (**created 2026-07-15**): `sprk_focusareas`, `sprk_officelocation`, `sprk_assistantpreferences`, `sprk_profilecompletedon`, `sprk_profileversion`; **`sprk_primaryrole`** bound to the **existing GLOBAL role choice set** (reusable beyond AI — owner decision); **Practice Areas** modeled as an **N:N relationship `sprk_userprofile` ↔ `sprk_practicearea_ref`** surfaced as a subgrid (owner decision — replaces the multi-select choice). **Relationship to user:** `sprk_systemuser` lookup on `sprk_userprofile` → `systemuser` **+ alternate key** (option B — keyed upsert, 1:1 enforced, no OOB dependency). — Acceptance: columns/relationship/alt-key exist; global role set bound; N:N practice-area subgrid present.
15. **FR-E2**: A **User-scope stated-profile producer** renders a fragment composed into `ContextBinder.userFragment` (sibling to the `MemoryItem` fragment), soft-fail-to-null. Read path: `CallerSystemUserResolver` (`oid`→`systemuserid`) → keyed retrieve of `sprk_userprofile` by `sprk_systemuser`; renders the `sprk_primaryrole` **label**, the **related `sprk_practicearea_ref` names** (N:N), plus focus/office/preferences. Write path: **keyed upsert** by `sprk_systemuser` (no find-then-create race); practice areas written as N:N associates. — Acceptance: a profiled user's turn carries the stated profile; unprofiled/absent degrades to no fragment, never fails the bind.
...
- **ADR-043** (Input-Resolution, **Proposed** — spine merged; label-flip is r2 wrap-up) — `ContextBinder` is the one input-resolution model; named engine owner + intake path for dispatch-spine changes. **redesign-r2 is complete/merged/deployed — R1 builds on the shipped spine, no cross-project coordination.**
...
| **ADR-043 (Proposed)** | "Named engine owner + intake; no ownerless execution-wiring change" | R1 modifies `ContextBinder` + dispatch spine | **C (comply)** | **redesign-r2 is COMPLETE/merged/deployed (verified 2026-07-15)** — R1 builds on the *shipped* spine, not concurrent edits; the "Proposed" label is r2 wrap-up bookkeeping. Register in `projects/INDEX.md`; `tests/integration/seam/**` DoD on FR-D/FR-H. **No cross-project coordination.** |
...
| Create-flow shape (task-002 conflict) | As-built `create-task`→`sprk_event`(Task) via direct `create_record`; no wizard launch; spec assumed `sprk_todo` + wizard | **Wizard/form-launch hand-off is the fix (not deferred). Route by intent: matter→Create Matter wizard; Event-Task→Event wizard (subtype=Task); To Do→OOB `sprk_todo` form modal. Repoint shipped `create-matter`/`create-task` off direct-`create_record`.** (owner 2026-07-15) | Reshapes FR-A1/A2/A3 + tasks 002/012; genuinely-new mechanism = "capability opens surface X pre-seeded" (design §10.1) |
