...
| **Next Action** | **050 — SCOPED + APPROVED, ready to build.** Owner approved (2026-07-22) the approach: capability-only, Event-type Task, assigned-to-me + open, in a Workspace grid tab. Full build plan: [`notes/050-list-tasks-build-plan.md`](notes/050-list-tasks-build-plan.md). **Key: NO BFF code change** — it's catalog DATA (`list-tasks` Binding + `LIST-TASKS@v1` Action + schemas, live via MCP) + Dataverse DATA (grid config + `sprk_event` saved query with `ownerid eq-userid`) + CLIENT code (a `my-tasks-list` widget registration + a `handleSurfaceLaunch` branch). Reuses the existing `DataverseEntityViewWidget`/`<DataGrid>` + generic `surface_launch` emit. Owner reviews the toolDescription content (FR-J1) before ✅. Then **051** (evals, NFR-06; depends on 050) → **054** (deploy verify, DEV ONLY) → **090** (wrap-up). Rule: `/worktree-sync` before any BFF deploy (050 needs none). |
