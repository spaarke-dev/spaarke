...
- **(c) ✅ ROOT-CAUSE FIX + CONFIRMED WORKING 2026-07-17.** After (a)+(b) shipped, the wizard still didn't open. Diagnosis (App Insights confirmed healthy + a temp client diag): `launchSurface` returned `launched:false`. Root cause: the terminal chunk's `consumerType` was sourced from the ledger **UcId** (`sprk_ucid ?? sprk_consumertype`). The live create-matter binding has a **distinct `sprk_ucid = "UC-B-6"`** ≠ `sprk_consumertype = "create-matter"`, so the client received `"UC-B-6"` — no entry in `surfaceLaunchRegistry.ts` (keyed by `sprk_consumertype`) → registry miss → silent no-launch. **Broke BOTH text + chip paths.** Fix (`SessionDispatchOrchestrator` emits `binding.ConsumerType`; `BindingCapabilityTool` routes on `_binding.ConsumerType`) + seam-test regression lock (divergent ucid; 18/18 pass). Committed `999b72fb6` → rebased/merged to master `86621a00e`. BFF redeployed to dev. **UAT-CONFIRMED: the Create Matter wizard now opens on typed "create a matter".** Remaining cleanup: the deployed SpaarkeAi client still carries the temporary `⚠️ [diag]` instrumentation (source already reverted on master; only surfaces on failure) — redeploy a clean client bundle to remove it.

## 🔴 P0.1 — Create-Matter wizard pre-seed gaps (UAT round 2, 2026-07-17, after wizard-open fix)

Wizard now opens + name/description pre-fill correctly. Two remaining gaps:

| # | Symptom | Diagnosis | Fix location |
|---|---|---|---|
| W-2 | **"Add file(s)" step does not show the session file** | The **file leg of the hand-off is unimplemented**: (a) neither client caller (`ConversationPane.handleSurfaceLaunch` text path NOR `useConsumerChips` chip path) passes `fileIds` to `launchSurface` — always omitted; (b) `handoffSeedMapping.ts` maps only name/description/type/area — **no `fileIds` mapping**; (c) the wizard's Add-file step doesn't pre-populate from a hand-off; (d) the create-write has nothing to attach. | client (ConversationPane + useConsumerChips) + shared-lib wizard (handoffSeedMapping + Add-file step + create-write attach) |
| W-5 | **Created matter record has no file attached** | Same root as W-2 — file never reaches the wizard, so nothing is linked to `sprk_matter`. | (same) |
| W-3 | **Matter Type + Practice Area dropdowns empty** (name/description DID pre-fill) | **Mostly by design.** The Action DOES draft `practice_area_suggestion` + `matter_type_suggestion` (confirmed in App Insights — `Create_Matter_for_Chat` schema requires them). The wizard pre-selects a dropdown ONLY on `confidence: 'high'` from the deterministic resolver (ADR-039 — never auto-pick an unsure closed-set value). The earlier "Commercial" test pre-filled (high-confidence match); this **patent** matter's labels (IP / Patent Prosecution) did not high-confidence-match the practice-area/matter-type option records → left for the user. **Independent of the missing file** (the draft ran fine). Open question: is this working-as-designed (unmatched label), a DATA gap (no patent option records), or a resolver-matching gap? Needs a focused resolver-confidence + option-record check. | data/config investigation → possibly resolver tuning |
| W-4 | "no file → pre-fill not running" (user hypothesis) | **Not accurate** — pre-fill (draft) runs regardless of the file-leg gap; name/description prove it. W-3 (type/area) is a separate confidence-gating mechanism, not caused by W-2/W-5. | (n/a — clarification) |

**Sequencing**: W-2/W-5 (file leg) is real implementation work (client + wizard + create-write); it's the higher-value gap. W-3 needs a data/resolver investigation first (may be working-as-designed). W-4 is a clarification.
