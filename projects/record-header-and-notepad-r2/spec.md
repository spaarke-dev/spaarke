...
- **Inline lookup editing** reproducing the OOB form control, with **Advanced** escalating to `Xrm.Utility.lookupObjects` (FR-15a — amended 2026-08-27; the picker-only design shipped as v1.1.8 and was reversed after UAT)
...
15a. **FR-15a** *(added 2026-08-25; **AMENDED 2026-08-27** after UAT round 5 — see [design.md §6.5a](design.md))* — **Editable lookups use an inline type-ahead that reproduces the OOB form control, with Advanced escalating to the OOB dialog.**

     The original text required the OOB `Xrm.Utility.lookupObjects` picker as the *default* surface and the deletion of the OData search builder. That shipped as v1.1.8 and was reversed on evidence: `lookupObjects` opens the **side pane**, whereas OOB renders a lookup **on a form** as an inline list under the field and reserves the dialog for **Advanced**. Hosting the platform's own inline control is not possible (`ComponentFramework.Factory` has no way to instantiate it), so the shape is reproduced with supported primitives.

     *Acceptance*: an editable lookup cell renders an inline search-as-you-type list; a right-side browse button opens the full list without typing; the list scrolls independently with a pinned, right-aligned **Advanced** action that opens `Xrm.Utility.lookupObjects({ entityTypes: [Targets[0]], allowMultiSelect: false })`; there is **no "+ New"** (owner decision — taxonomy targets users cannot add to); selection from either surface stages to the form buffer; read-only **and target-less** lookups still render via the display-only `fields/LookupField`. The target's `primaryNameAttribute` MUST be read from metadata, never inferred — the convention is non-uniform across taxonomy tables.
...
26. **FR-26** — Ship `Spaarke.Records.RecordHeader` in a new `RecordHeaderPcf` solution; re-bind the Matter form; verify parity vs v1.0.20; then retire `MatterHeaderPcf` in two ordered steps (remove all form references + publish, then delete the CustomControl). *Acceptance*: Matter renders identically; the old control is gone on delivery. ⚠️ The "**except that lookup cells now open the OOB picker**" carve-out is **WITHDRAWN 2026-08-27** — both controls now render the same shared inline `components/LookupField`, so parity is unqualified. (Matter's lookups do gain the right-side browse button and modern scrollbar on its next rebuild, since the shared component was upgraded — that is a shared-library change, not a RecordHeader divergence.)
...
15. [ ] Matter renders identically to `MatterHeaderPcf` v1.0.20 — *Verify*: screenshot diff, light and dark, against the pre-change baseline. ⚠️ The lookup carve-out is **WITHDRAWN 2026-08-27**: both controls render the same shared inline `components/LookupField`, so the lookup interaction no longer needs excluding from the diff. Rebuild `MatterHeaderPcf` before baselining — the shared component gained a browse button and a modern scrollbar, which both controls now show
