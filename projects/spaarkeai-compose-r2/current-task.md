...
> **Last Updated**: 2026-07-12 — ROUND-3 DEPLOYED to spaarkedev1 (BFF + code page + catalog seed). Branch HEAD = `3b380fa87` + master-merge commit; **0 behind master**. Master merge HELD until owner UAT passes.
> **Protocol**: [Context Recovery](../../docs/procedures/context-recovery.md)

---

## 🔴 ACTIVE — UAT ROUND-3 (2026-07-12) — DEPLOYED, AWAITING OWNER RE-UAT — READ FIRST

**Round-2 DONE + live (round-2 block below).** Round-3 (DEF-10..DEF-19 cluster) is **code-complete AND deployed to spaarkedev1**. Now awaiting owner re-UAT on the live surface; **branch→master merge is intentionally HELD until UAT passes** (keeps un-UAT'd code off master; merge is the only hard-to-reverse step).

| Field | Value |
|---|---|
| **Branch** | `work/spaarkeai-compose-r2`; HEAD = `3b380fa87` + clean master-merge (46 ahead / 0 behind). Post-merge gate GREEN: BFF 0-err + **1006** band; Compose.Components tsc + **132**; SpaarkeAi build + conversation **298**. |
| **Committed round-3** | Wave 1: `c5c832a94` (editor UX DEF-15/16/17) + `7eb47a483` (DEF-14 Save-lock 423/412). Wave 2: `7cb764115` (DEF-10 doc-text→Assistant + DEF-19 session keys). Wave 3: `f96105913` (DEF-12 accept/reject→Assistant + DEF-13 comment-on-edit). Wave 4: **`24df659c0`** (DEF-11 `compose-revise-document`, dual-channel edits+comments, 4 intents, disp 100000006, doc-session routing, `materializeMany`, two-session wire-body e2e). |
| **✅ DEPLOYED 2026-07-12** | **BFF** `spaarke-bff-dev` — 46.61 MB, 4/4 SHA-256 hash-verified, /healthz Healthy. **Code page** `sprk_spaarkeai` — clean vite build (4940 KB) updated+published. **Catalog seed** `compose-revise-document` to spaarkedev1 — Action `44b1eb8f-367e-f111-ab0e-7ced8ddc4a05` + Binding `b11aaf8b-367e-f111-ab0e-70a8a590c51c` (disp 100000006, surfaces `assistant,compose`, enabled, link OK). Direct-Web-API seed (scratchpad `seed-compose-revise-document.ps1`), matched live sibling field posture; NO version suffix. **BFF 5-min IMemoryCache TTL** → new capability served on next read; no restart. |
| **Re-UAT follow-ups (KNOWN, not in this deploy)** | (a) clickable revision-intent **chips** — currently the disambiguation is text-based (`composeReviseRouting.ts` `/revise <intent>`); owner's confirmed UX (Assistant offers "highlight sections OR provide guidance" + clickable next-step links per intent) needs client-side binding-id resolution for `compose-revise-document`. (b) auto-**open the SOURCE in Compose if not already open** (`handleOpenInCompose` currently only seeds AI-drafted HTML). Both flagged for this-or-next round. |
...
| **NEXT ACTION** | **AWAIT OWNER RE-UAT of the round-3 cluster on the live surface** (give BFF cache ≤5 min for the new revise capability). Test: natural-language "revise this document" in the Assistant → server selects `compose-revise-document` → produces edits (align-clauses/improve-clarity) and/or anchored comments (flag-risks) into the OPEN document's session. **IF UAT passes → merge branch → master (PR like #634) — this is the held step.** IF UAT surfaces defects → new wave, redeploy, re-UAT before merge. **LESSON (binding): build each wave with ONE agent; never let a build agent spawn nested sub-agents (DEF-11 runaway).** |
...
| **Next Action** | **✅ UAT round-2 wrap-up COMPLETE + DEPLOYED (2026-07-11). Compose-r2's critical-path work is DONE.** All 5 cluster fixes + finish items merged to master (**PR #634** `f5e77585a`, CI green) on top of core **#633**, and **compose-r2 ran the combined deploy** (Option 1): BFF `spaarke-bff-dev` (hash-verified, /healthz Healthy, compose+ack routes 401) + SpaarkeAi `sprk_spaarkeai` code page (bundle verified) + **`compose-draft-document` catalog seeded** to spaarkedev1 (Dataverse-verified; Action `86673d89…` + Binding `d5fadd89…` disp 100000006; BFF 5-min cache picks up auto-route). **REMAINING (not compose-r2 code):** (1) **core live activation** — seed create-matter rows + flip PromptShield (`AiSafety:PromptShield:ChatPipelineEnabled=true` + ContentSafety + MI role); (2) **operator consolidated UAT** (core `CONSOLIDATED-UAT-CHECKLIST` A/B/C + DEF-fixed Compose); (3) **056** Word round-trip + **082** flagship browser gate (owner live-verify); (4) **090** wrap-up. **FR-30/063 = OUT of 100%** (memory-governance project, operator scheduling). |

### ✅ DEPLOY DONE 2026-07-11 — what's live + testable
- **DEF-09** inline redline (Draft Alternative redlines IN the document w/ accept-reject; document-session routing) — LIVE.
- **DEF-08** draft→Compose: "Open in Compose" affordance LIVE (client-only); auto-route ("write a letter"→seeded Compose tab) lights up on BFF catalog cache refresh (≤5 min after the compose-draft-document seed).
- **071** content-render honest-ack (no false "opened/pasted it") — LIVE.
- **064** Context trace pane · **014-split** parent-association picker · **DEF-01/02** — LIVE.
- Deploy artifacts: BFF hash-verified 4/4; code page bundle carried compose_content_rendered/sessionIdOverride/compose.draft/"Open in Compose". Coordination notes: `PING-to-core-master-final-ready-to-deploy-2026-07-11.md`, `REPLY-to-core-633-combined-deploy`, `HANDOFF-to-core-session-identity-and-compose-seed`.

### 🔗 UAT round-2 defect cluster + joint-deploy coordination (2026-07-10 night) — READ
**Owner UAT round-2 surfaced a coherent cluster: "AI output → the Compose editor" is broken from two entry points.** Both are registered in `notes/defer-issues.md` with confirmed root causes:
- **DEF-09** (Draft Alternative → no inline redline): **session-scope mismatch** — toolbar dispatch writes the `compose`-disposition ledger entry into the **chat** session (`ConversationPane.dispatchComposeAction` → `getSessionId()→chatSessionId`), but `ComposeWorkspace.materializeComposeDraftFromLedger` reads from its **document** session (`state.sessionId`, keyed DocumentId+MatterId per 062/102). **Owner decision (binding): a Compose edit belongs to the DOCUMENT session — keep TWO sessions, do NOT unify.** Fix = route compose EDIT-action dispatch to the document session (client-only) + confirmation-only Assistant render. Wave-A-committed already: 064/014-split (b9e25c8a3).
- **DEF-08** (chat "write a letter" → Compose): open-tab flow carries **no content payload** (opens blank shell), no editor content-injection tool → "paste it" is **hallucinated** (R2-D false-UI-claim). Owner chose **"both layered"**: (A) new catalog Action `compose-draft-document` (Compose disposition) → open + **seed** a Compose tab from a full-document ledger output; (B) "Open in Compose" affordance; (C) honest-ack the content RENDER (extend 071/D-F3 from tab-open to content-render); + single-tab reuse. Additive `SendWorkspaceArtifactHandler.widgetData` seed with **server-side resolution** (no generic tool-schema change → no hard core dep).
- **Coordination**: core merges **#633** (PromptShield default-off + create-matter code-half) then HOLDS activation deploy; I finish the wrap-up + merge; ONE combined deploy carries both. Notes: `REPLY-to-core-633-merge-and-deploy-2026-07-10.md`, `HANDOFF-to-core-session-identity-and-compose-seed-2026-07-10.md`. Incoming from core: `PING-to-compose-core-completion-deploy-2026-07-10.md`, `HANDOFF-to-compose-r2-fr30-triage-2026-07-10.md`.
- **FR-30 / #629**: core triaged → memory hard-governance project (NOT r2 core); no core deliverable owed; task 063 "ready on delivery". **FR-30 is OUT of compose-r2's "100%"** — operator scheduling call on the governance project.
- **Process lesson still binding**: DEF-08/09 are the SAME "unit-green, E2E-inert" class as 115 — every fix here MUST carry a **wire-body** test (dispatch→ledger→client-materialize→editor), NOT a stored-ledger/shared-single-session shortcut.
