...
| Field | Value |
|---|---|
| **Diagnosis** | 6 read-only investigations → `notes/uat-round3-reuat-diagnosis.md`. Build plan → `notes/round4-active-artifact-build-plan.md`. Core FR-30 anchor answer → `notes/HANDOFF-to-core-fr30-anchor-2026-07-12.md` (core WAITS; single owner = us). |
| **Committed round-4 waves** | W1 `89fbef95d` redline anchor **normalization** (smart-quotes/NBSP/dash fold, both-sides 1:1) + selection fallback (**Test #4** — was a pre-existing brittle exact-match, NOT a regression). W2 `7e1819db0` **session establishment**: BFF sets `session.ActiveDocument` on chat upload (**Test #2**) + browse-upload mints a client `state.sessionId` (**Test #3** CLICK path). W3 `209edb6cc` **Open-in-Compose opens the ARTIFACT** not msg text (**Test #1**, incl. chat-only-upload gap fix) + DocumentSessionId flows server-side (TEXT path) + tab-scoped ActiveDocument. W4 `4577a0608` Context **Execution Trace auto-opens** on Compose activation + `sprk_document` context-mapping allowlist (kills console 400). W6 `3c451e933` **non-docx reference-only** guard (editable=DOCX only; PDF → calm reference state not blank). W5 `d5ae23cca` **Compose additive first-class Direct widget + getVisibleState** (dormant/additive; layout path unchanged, zero regression). |
| **✅ DEPLOYED 2026-07-13** | **BFF** `spaarke-bff-dev` — 46.61 MB, 4/4 SHA-256 hash-verified, /healthz Healthy. **Code page** `sprk_spaarkeai` — clean vite build (4947 KB) updated+published. **NO new catalog this round** (W4 sprk_document is code-only; round-3 `compose-revise-document` seed already live). Changes live immediately (no cache wait). |
| **Consolidated gate** | BFF full suite **8204 passed / 0 failed / 101 skipped**; publish **46 MB** (<60 ceiling); **no new CVE** (a HIGH `Microsoft.Kiota.Abstractions 1.21.2` advisory is PRE-EXISTING — round-4 changed no package; flag for a separate dependabot bump). Compose.Components **146/146**; SpaarkeAi workspace+conversation **398/398** (zero regression). ADR/§10 focused review clean (additive only; no new endpoint/service/DI/package). |
| **Known boundaries (documented, not bugs)** | W5 Direct widget is registered-but-DORMANT (layout path still drives Compose — additive shape for the future, no new runtime behavior). W5 `getVisibleState` does NOT retire `composeActionBridge.registerActiveDocument` (read-only prompt projection vs client→server routing write — different purpose; retiring would regress routing). W3 multi-instance tab_change last-write-wins — unreachable under DEF-08 single-tab reuse; revisit if genuine multiple Compose tabs ship. |
| **NEXT ACTION** | **AWAIT OWNER RE-UAT of the round-4 fixes** (re-UAT checklist maps to Tests #1–4 + trace + non-docx; hard-refresh Ctrl+F5 first). **IF UAT passes → merge branch → master (this is the held step).** IF defects → new wave, redeploy, re-UAT before merge. **LESSON (binding): build each wave with ONE agent; never let a build agent spawn nested sub-agents.** |
