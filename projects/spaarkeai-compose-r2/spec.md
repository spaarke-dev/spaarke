...
25. **FR-24 — Push annotations**: `DocxAnnotationWriter` writes `<w:comment>` + `<w:ins>`/`<w:del>` (correct author/date/id, comments-before-track-changes ordering, paragraph-boundary deletion handling; note `w:del` text uses `DeletedText` not `Text` — Spike 5) via Open XML SDK (already a BFF dep — zero new package); `POST /api/compose/document/{spe-id}/push-annotations` saves to SPE via `ReplaceFileContentAsUserAsync`. **This task MUST ADD an `If-Match`/ETag precondition to the SPE write facade** — per Spike 7, `ReplaceFileContentAsUserAsync` does NOT take `If-Match` today and the current Compose save is a blind `PUT /content` that can silently overwrite a concurrent Word edit; the write handler must also catch `423 Locked`/`412 Precondition Failed` and surface a typed conflict (gap G-1/G-2, feeds FR-27 conflict UX). *Acceptance*: Word for Web renders pushed comments + track changes natively; a stale-ETag push is rejected as a typed conflict, not a silent overwrite.
