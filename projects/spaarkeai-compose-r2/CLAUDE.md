...
- **Compose-disposition ROUTING PROMOTION applied by compose-r2 2026-07-09 — ⚠️ HALF-LANDED, see [ADR-043-impact-review.md](notes/ADR-043-impact-review.md)**: we added `BindingDisposition.Compose` (Binding.cs) + `ToLedgerValue` → `"compose"` + the `OutputRouter` pass-through case (2 of the 3 disposition lists). We did **NOT** widen the **disposition admit-gate** (`SessionDispatchOrchestrator.cs:229`, the 3rd list) — verified 2026-07-09: it still admits only `Informational | WorkProduct`, so a `compose`-disposition dispatch **422s before reaching** the OutputRouter leg we added (dead code on the dispatch path). **The admit-gate fix is core's E-20 (`DispositionRoutability` registry), 🔲 not started.** CORRECTION: the routing promotion does **NOT** unblock end-to-end dispatch — it unblocked the router/ledger leg only (the "31 tests green" are router/unit layer). **draft-alternative (042), 033, 034 are shipped-but-GATED-ON-E-20 for end-to-end** — they 422 through the full `/dispatch` seam today (false-green; re-verify after E-20). The **4 informational actions** (explain/compare/summarize-changes/defined-terms) **DO** dispatch end-to-end now (E-10 fixed their input path). E-20 will delete our 2 hand-added switch entries into the registry → **freeze compose edits to `OutputRouter.cs` + `Binding.cs` until E-20** (merge-collision hazard). The `BindingDisposition.Compose` enum member stays (registry key).
