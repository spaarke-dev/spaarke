...
**FR-30 realization status (2026-07-10, task 063 §6.5 escalation)**: the `derivedInsights → gated MemoryItem` disposition above is **correct and unchanged** — it is the right long-term design. Its realization is **blocked on core (redesign-r2)**, not on Compose: the shipped `memory.write` (core task 057) is **chat-invocation-context-only** and its **untrusted-origin gate was deferred** (operator ruling 2026-07-08 → Tier-1 silent Execute). Compose's insights originate from **dispatched catalog Actions** (Click-path/Playbook context), which 057 rejects, and Compose may not fork a path (ADR-013 facade + charter §3.4). Two core-owned pieces are required and have been handed off — **[issue #629](https://github.com/spaarke-dev/spaarke/issues/629)** / `notes/HANDOFF-to-core-fr30-memory-governance-2026-07-10.md`: **(1)** a dispatched-action / gated-promotion capture facade (`PublicContracts`, provenance-carrying: `source=ai-derived`, `bindingId`, `ledgerRef`, workspace→Record scope), and **(2)** the untrusted-origin gate (the deferred memory-poisoning-prevention this §8 row exists to demand). Compose task 063 builds insight-distillation + capture on delivery. **No interim substitute is claimed**: tasks 061 (action-history over the ledger) and 062 (cross-version persistence) are their own FRs (FR-31/FR-33), not FR-30 — they display action history but are not governed, recallable memory.
...
| **`IDocumentProfileAi` OBO profile facade owned by compose-r2, not core (UAT #7b fix, 2026-07-13)** | ADR-013 facade discipline — the `Services/Ai/PublicContracts/IDocumentProfileAi` contract was nominally slated to be **core-owned** (paused `spaarke-ai-architecture-redesign-r2`) | **Path A — accepted PERMANENT facade (canonical; resolved 2026-07-15, #615 / PE-D4)** | **What deviates**: compose-r2 authors `IDocumentProfileAi` + `DocumentProfileAi` under `Services/Ai/PublicContracts/` itself (interface + impl + DI reg), rather than consuming a core-published contract. **Why**: the round-6 create-on-save profile step enqueued an app-only (MI) `AppOnlyDocumentAnalysis` job that **403s on the user-OBO-written SPE file** (MI is intentionally not on the container type — Pattern 4), so profile fields silently never populated — a document saved from Compose was not a complete DMS record. The fix must download the file under the **same OBO identity that wrote it**, exactly as the RAG indexing step already does inline (`IPostUploadIndexingEnqueuer` → `IFileIndexingService.IndexFileAsync`). Core is paused; blocking a complete-DMS-record on it is a worse outcome than owning the canonical facade here. **ADR-013 stays satisfied**: `ComposeService` (CRUD) injects ONLY the `PublicContracts` facade — never `IAppOnlyAnalysisService`/`IOpenAiClient`/`IPlaybookService`/`IConsumerRoutingService`; `ADR013_ComposeFacadeTests` + `ADR013_AiBoundaryTests` pass. **Reuse (§11)**: the impl runs the SHARED "Document Profiler" Action (ACT-011) on the ADR-043 completion-engine spine (`IActionResolver` → OBO `IDocumentTextSource` download → `IActionRunner` → field-map → `UpdateDocumentFieldsAsync`) — the SAME Action the platform's canonical profiler `AnalysisEndpoints.ExecuteDocumentProfilePipelineAsync` runs; zero profiling/mapping duplication, no forked AI logic. **Resolution (2026-07-15, #615 / PE-D4)**: this facade IS the canonical OBO document-profile contract — owner-blessed as PERMANENT, not a stand-in. If the paused core effort resumes it ADOPTS this facade as-is; there is nothing to reconcile and no competing build. **Alternative rejected**: keeping the MI enqueue (Path C comply) — rejected because it is the silently-failing bug itself; a background MI job cannot read a user-OBO-written SPE item under Pattern 4. |
