...
| **`IDocumentProfileAi` OBO profile facade owned by compose-r2, not core (UAT #7b fix, 2026-07-13)** | ADR-013 facade discipline — the `Services/Ai/PublicContracts/IDocumentProfileAi` contract was nominally slated to be **core-owned** (paused `spaarke-ai-architecture-redesign-r2`) | **Path A — project-scoped exception** | **What deviates**: compose-r2 authors `IDocumentProfileAi` + `DocumentProfileAi` under `Services/Ai/PublicContracts/` itself (interface + impl + DI reg), rather than consuming a core-published contract. **Why**: the round-6 create-on-save profile step enqueued an app-only (MI) `AppOnlyDocumentAnalysis` job that **403s on the user-OBO-written SPE file** (MI is intentionally not on the container type — Pattern 4), so profile fields silently never populated — a document saved from Compose was not a complete DMS record. The fix must download the file under the **same OBO identity that wrote it**, exactly as the RAG indexing step already does inline (`IPostUploadIndexingEnqueuer` → `IFileIndexingService.IndexFileAsync`). Core is paused; blocking a complete-DMS-record on it is a worse outcome than a narrow, documented stand-in. **ADR-013 stays satisfied**: `ComposeService` (CRUD) injects ONLY the `PublicContracts` facade — never `IAppOnlyAnalysisService`/`IOpenAiClient`/`IPlaybookService`/`IConsumerRoutingService`; `ADR013_ComposeFacadeTests` + `ADR013_AiBoundaryTests` pass. **Reuse (§11)**: the impl changes ONLY the download identity to OBO and delegates the extract → classify → summarize → field-map → `UpdateDocumentAsync` pipeline to the existing `IAppOnlyAnalysisService.AnalyzeDocumentFromStreamAsync` — zero profiling/mapping duplication. **Reconciliation**: when core resumes and publishes its canonical `IDocumentProfileAi`, reconcile this facade against it (adopt the core signature or delete this stand-in in favor of the core impl). **Alternative rejected**: keeping the MI enqueue (Path C comply) — rejected because it is the silently-failing bug itself; a background MI job cannot read a user-OBO-written SPE item under Pattern 4. |
