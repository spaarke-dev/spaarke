...
| 063 | FR-30 Record-scope MemoryItems via IComposeMemoryCapture facade (capture+recall; gate deferred) | 6 Memory | ✅ DONE (2026-07-15) | none | RE-SCOPED + BUILT: investigation showed NO blocker (durable store `IMemoryItemStore` + recall already shipped, decoupled from chat). Built the #615-style `IComposeMemoryCapture` PublicContracts facade → shared `IMemoryItemStore`; `ComposeService.SaveAsync` distils session defined-term canon into Record-scope MemoryItems keyed to the promoted sprk_document, best-effort. Untrusted-origin gate DEFERRED to [#629](https://github.com/spaarke-dev/spaarke/issues/629) (TrustLevel inert). adr-check COMPLIANT; code-review MEDIUM fixed. Build 0-err; 37 tests green. AC3 CAPTURE→RECALL ✅◐ E2E-pending (Cosmos+HTTP wire = dev/UAT). | FULL | sonnet | high |
