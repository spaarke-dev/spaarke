...
| Field | Value |
|-------|-------|
| **Status** | **Task 073 UAT round-4 (v1.0.25) DEPLOYED; awaiting owner UAT.** v1.0.24 (all 11 round-3 items) owner-CONFIRMED working (native lookup, "Add Access Permissions", per-row levels, contact links, Standing removed). v1.0.25 = two follow-ups: **#1** Contact dialog was rendering BEHIND Manage Access (Fluent portal z-index) → now HIDE Manage Access while the Contact dialog is open + restore on close + larger dialog (70%×80%); **#2** Access Permission segmented picker got cut off at low res → replaced with a single-value **Dropdown** (Standard default, opens Limited/Restricted) in the shared TrackingFieldTrio core. PCF **v1.0.25** on SPAARKE DEV 1. BFF @ HEAD; external-spa = dual-plane shell (owner-triggered). |
