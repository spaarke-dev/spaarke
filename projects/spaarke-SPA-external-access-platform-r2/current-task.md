...
| **NEXT ACTION** | **v1.0.17 DEPLOYED** (SPAARKE DEV 1, commit 9b4e74e41): punch **#1 (email crash), #2 (standing-grant), #5 (thin-scrollbar standard)** all DONE. REMAINING = **decisions #3, #4, #6, #7** surfaced to owner (see punch list "RESOLVED IN v1.0.17" block). Await owner answers → next PCF version. #1 root cause was a duplicate React 19 bundled via Lexical's `react/jsx-runtime` subpath; fixed with a scoped webpack alias in `src/client/pcf/webpack.config.js` (+ WidgetErrorBoundary safety net). |
