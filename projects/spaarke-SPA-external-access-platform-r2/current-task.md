...
| **NEXT ACTION** | Work the **ordered punch list** in `notes/task-073-uat-punchlist.md` **in order**. **#1 = email icon crashes the PCF (React #31)** — start there. Rebuild/redeploy the PCF per the recipe in that note (bump version 5 places → build:prod → copy bundle → pack.ps1 → pac solution import → commit/push). |
