...
| **Status** | **✅ R2 WRAPPED UP (2026-08-12).** All code work merged to master (0 unmerged; working tree clean): PCF TrackingFieldTrio v1.0.20→v1.0.29, shared-lib modal/grid changes, external-spa 5B login, 4C DataGrid inline-fetchXml hardening, master-red hotfixes (#759/#760), test-suite rewrites. Merges: PR #758 (punch #1–#7 + P2b), #761 (v1.0.29 + 5B + hotfixes), #762 (4C). SPA/Teams interactive gaps (3E record-open, 4B intake wizards, 6A notify, grid columns) → **carried to R3** (`work/spaarke-SPA-external-access-platform-r3`, `projects/spaarke-SPA-external-access-platform-r3/design.md` DRAFT for review). **Owner open (non-code):** trigger `deploy-external-spa.yml --ref master`; review r3 design.md + answer its 5 open Qs; verify SR grid after redeploy (re-save record if configjson stale); re-enable master branch protection (OFF since the CI incident); optionally verify 5A first-assign CIAM email.** Round-3 (v1.0.24 all 11 items) + round-4 (v1.0.25 Contact-stacking + Access-Permission control) + round-5 (v1.0.26 email-modal nonBlocking + toolbar spacing) + round-6 (v1.0.27 Access-Permission **pill** selector) all DEPLOYED to SPAARKE DEV 1 + owner-confirmed. **Test debt fixed**: AccessGrantModal (26/26) + TrackingFieldTrio (27/27) suites rewritten to the shipped UI (were red since v1.0.23). Last commit **fa0fe0b16**, pushed; **~13 ahead / 1 behind** origin/master → **follow-up merge-to-master pending** once SPA UAT settles. BFF @ HEAD; external-spa = dual-plane module-host shell (owner-triggered deploy). **NEXT FOCUS: UAT the external SPA(s).** |
