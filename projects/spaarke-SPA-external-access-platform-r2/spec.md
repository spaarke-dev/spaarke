...
25. **FR-25 (Policy Library + "Review Policy Question")** *(refined 2026-08-06 per owner)* — R2 builds **both**: (i) a **browse/read policy library**, and (ii) **"Review Policy Question"** — a user submits a **policy question and receives an *official human* response from the law department** (NOT an AI answer — that's FR-26). Library home: **SPE (content bytes) + a new unrestricted `sprk_policy` Dataverse entity** (title, category, owner, version, effective/expiration, status, SPE pointer or rich-text) **+ the existing RAG layer** for grounded search. **MUST NOT** use Dataverse `knowledgearticle` (restricted table → D365 Customer Service Enterprise license per author + multiplexing landmine for unlicensed external readers). "Review Policy Question" is a **Front Door request type** (`PolicyQuestion`) that reuses the intake + **FR-24 official-human-response feedback loop** (legal answers in the MDA; the answer returns to the requester in-app + email). Served to SPA users via the broker-only path (app-only). The library also grounds FR-26. Acceptance: an SPA user browses/reads a published policy (app-only, no Power-Apps license); expired/unpublished policies never surface (RAG security-trimmed on `sprk_policy` status/effective-date — negative); a user submits a policy question and receives an official human legal response tracked end-to-end (in-app + email).

26. **FR-26 (Q&A Assistant — "Ask Legal", WORKFORCE-INTERNAL only)** *(refined 2026-08-06 per owner)* — Embed the Spaarke AI Assistant (`SprkChat`, context-agnostic per ADR-012) in the Legal Front Door for **internal workforce users** (NOT external/outside-counsel) so they can ask questions (AI-grounded on the FR-25 policy library + their entitled content — *instant, non-authoritative*, distinct from FR-25's official human answer) and **submit Front Door requests** conversationally. **Scope simplification vs external**: single **workforce-plane, entitlement-scoped** assistant endpoint (no CIAM dual-scheme), an **ADR-039 closed tool-catalog subset** (Q&A + "submit request" only — no core-user tools), and **app-only RAG grounding** (no OBO `RetrievePrincipalAccess`; the core `/api/ai/chat` assumes a core-user Dataverse identity + OBO and cannot be used directly). Reuses the surface-launch spine to route "submit NDA" → the intake wizard. Acceptance: a workforce SPA user gets grounded answers scoped to their entitlements (out-of-entitlement → no grounded leakage — negative); the assistant launches/submits a Front Door request; the tool catalog excludes all core-user capabilities (negative); no OBO on any path; **Ask Legal is not shown to external/outside-counsel users**. **Forward-looking (NOT R2)**: exposing Spaarke AI capabilities to *outside counsel* (external CIAM) in surfaces they access is a future broker-only-external-AI opportunity — recorded, deferred.
...
- **New Dataverse**: `sprk_policy` (unrestricted); additive `sprk_servicerequest` fields (decision/outcome, response text, response-doc linkage, AI-extracted markers); NDA-intake `sprk_playbookconsumer` binding row.
