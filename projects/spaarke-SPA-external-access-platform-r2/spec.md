...
- [x] **teams-app-r1 coordination** — RESOLVED: teams-app-r1 implements **Option A** (principal-agnostic dual-scheme) now, to R2 guardrails, and returns coordination notes; R2 builds directly on their `CallerPrincipalResolver` + dual-schemed collaboration endpoints (P1). Option A is R2 P1 work done early (not throwaway); Option B is rejected. Guardrails + return-notes spec: `notes/teams-app-r1-coordination.md`. Open sub-item R2 needs from them: the **workforce Tier-2 record-scope predicate** (how a workforce user's accessible project set is computed — must not be all-projects, per NFR-08).
