...
### 2026-06-26 — Canonical-truth loop step 4 complete; 5 JPS skills aligned + 6 code-archaeology open questions resolved
- **Reason**: Loop step 4 aligns the JPS skill bodies to the new canonical docs so future authors invoke skills that cite the load-bearing runtime contract + config-home boundary.
- **Skill updates**: `jps-action-create` gains Step 1.5 config-home guard + Step 5.5 MCP verify; `jps-playbook-design` gains Step 1.5 runtime-contract reminder + Step 10 MCP node-row verification; `jps-playbook-audit` gains Check 3.5 repo-vs-deployed reconciliation + orphan-node check; `jps-validate` gains Step 7.5 playbook-definition checks (actionCode lint, NodeType allow-list, anti-pattern checks); `jps-scope-refresh` documents the advisory-not-enforcement boundary + two-authoring-surface distinction (PlaybookBuilder UI vs MDA Node Type OptionSet).
- **Open questions resolved**: §13.1 `CanvasServerMappingDriftTests.cs` lives at `tests/integration/Sprk.Bff.Api.IntegrationTests/Playbooks/`; §13.2 playbook `sprk_configjson` IS read at runtime by `OutputOrchestratorService.cs:319` (outputMapping parse — niche, MVP-flagged); §13.3 `sprk_outputschemajson` IS read at runtime via `PlaybookExecutionEngine.cs:490` (chat-summarize FK chain) — confirmed Home A; §13.5 `IPlaybookExecutionEngine` has 4 methods (Conversational + Batch + DetermineMode + ChatSummarize) — chat-summarize is the dedicated boundary; §13.6 canvas types = 13 (in `canvas.ts:17-30`), server NodeType = 5; §13.8 `AppOnlyAnalysisService.cs:530` fires from `ProfileSummaryJobHandler`, `EmailAnalysisJobHandler`, `AppOnlyDocumentAnalysisJobHandler` (Service Bus job handlers, not scheduler).
- **Next**: step 5 (R4 playbook deploy from canonical foundation).
