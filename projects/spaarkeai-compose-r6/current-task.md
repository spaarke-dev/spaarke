...
| **Status** | 032 server half SCOPED, not yet implemented. Design state: endpoint = POST /api/compose/documents/{documentSpeId}/apply-template {driveId, templateIdOrName, variables?} in ComposeEndpoints.cs (map beside /save at :94); ComposeService.ApplyTemplateAsync = DownloadFileAsUserAsync current bytes (:294 pattern) → IComposeTemplateSource.ResolveAsync → ComposeTemplatePartMergeEngine.Merge → persist via ReplaceFileContentAsUserAsync (:1047 pattern) → return save-shaped result (contentModel re-projection like post-save) so the client re-mounts. **OPEN DESIGN QUESTION before implementing**: how ApplyTemplateAsync acquires the USER's Dataverse token for IComposeTemplateSource.ResolveAsync (sibling template services take caller-supplied dataverseUrl+token from playbook context; Compose has only HttpContext) — investigate how BFF does user-context Dataverse OBO elsewhere (e.g. Office endpoints / IDataverseService user paths) and mirror it; do NOT invent a new auth path (ADR-028). |
