...
| **Status** | in-progress — UAT fix plan. **#1 DONE** (`7807c02a1`), **#4 DONE + wired** (`d9ff4df82`, live e2e pending an inbound email). |
| **Next Action** | **Fix #2** — render `EmailConnectionsReview` INLINE in `ReconciliationWorkspace.tsx:303-310` (r5-contended → `/conflict-check` first). Then #3 (remount), #5 (two new views). #4 needs an operator test-email to confirm triage populates. |
