...
> **Last Updated**: 2026-09-01 (document-profile bug #919 FULLY INVESTIGATED + DOCUMENTED — root cause empirically confirmed, authoritative architecture doc written; FIX not yet applied — owner picks among 3 options)
...
## 🔴 NEXT SESSION — document-profile playbook bug (#919): INVESTIGATED + DOCUMENTED (fix not yet applied)
**Symptom**: every saved Document shows `sprk_filesummarystatus = Failed (100000004)` — the document-profile AI playbook doesn't complete (no summary / AI documenttype). Save/search/association/indexing all WORK; only profiling fails.

**📘 AUTHORITATIVE WRITE-UP (read this first)**: [`docs/architecture/DOCUMENT-PROFILE-AND-AI-EXECUTION-MODELS.md`](../../docs/architecture/DOCUMENT-PROFILE-AND-AI-EXECUTION-MODELS.md) — the three AI execution models, all three document-profile entry paths, the confirmed failure mechanism (Part 4), fix options, change-safety checklist. Cross-ref `.claude/FAILURE-MODES.md` AP-10. This is the durable reference; the summary below is the recovery pointer.
