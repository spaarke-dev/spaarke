...
>
> **NEXT (proceed as planned): 065 then 064.**
> - **065** (E2b/E2c, NO BFF): `FieldUpdateReconcileTab` — self-resolve `getXrmForPicker().Utility.getEntityMetadata(targetEntity,[targetField])` → DateTime→Fluent DatePicker / Picklist/State/Status→Dropdown(options) / Lookup→lookupObjects(Targets) / else text; **E2c** full-width "+ Update other fields" → `navigateToEntityRecordSurfaceAsync` (confirmed record form, self-resolved Xrm), modal-on-modal. Reuse 066's `mockPickerXrm` test pattern. Author `tasks/065-*.poml` then task-execute.
> - **064** (E1b/E1c — BFF §10 + SpaarkeAi hot-path; build LAST): injected `onLaunchCreateRecord` + additive `QuickStartModal.onRecordCreated` (await `launchSurface` outcome) → re-enter `applyRegardingSelection`; **E1c BFF resolver** materializes archived `.eml`(sprk_document) as a **chat-session document → {sessionId,fileId}** (wizard fetch is session-scoped, `CreateMatterWizard/main.tsx:74`) → `QuickStartModal.getFileContext`. **Step 0: find+reuse the Assistant's existing session-document-ingest endpoint.** /conflict-check BFF+SpaarkeAi; §10 governance (placement, size, CVE, seam test).
