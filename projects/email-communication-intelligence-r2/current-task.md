...
| **⚠️ PENDING — owner UAT (round 2)** | **First UAT (2026-09-04) FAILED on category**: test email `Fw: LITG-119896 Monte Rosa…` triaged (priority High, RI-conf 0.75, review Route, rich summary) but `sprk_triagecategory` was STILL `None`. Root-caused deeper (below) + a real fix deployed. **Send a NEW test email → confirm `sprk_triagecategory` now populates.** Query: `sprk_communications?$select=_sprk_triagecategory_value,sprk_triagepriority&$orderby=createdon desc&$top=5`. |
