...
>
> **E1c REMAINING (next focused unit):**
> 1. **BFF seam test** (ADR-038 KEEP path) for `IngestArchiveDocumentAsync` — happy path (archive doc → {sessionId,fileId,fileName} + binary cached) + 404 missing-doc + 422 not-archive + non-fatal manifest. Handler is `private static`; existing tests use WebApplicationFactory (`tests/integration/contract/Api/Ai/ChatDocumentEndpointsContractTests.cs` is the pattern to mirror).
> 2. **Hosts (062):** `sprk_communicationreconciliation` code page + SpaarkeAi `communications-reconciliation` widget — implement `onLaunchCreateRecord` (mount QuickStartModal, bridge promise via `onRecordCreated`, null on cancel) + `resolveEmlSource(row)` (row's archived `.eml` `sprk_document` id) + call the new endpoint → pass `{sessionId,fileIds:[fileId],fileNames}` as `QuickStartModal.getFileContext`. Find the 062 host files under `src/solutions/` (code page) + the SpaarkeAi widget registry.
> 3. **Governance:** publish-size (`dotnet publish -c Release ... -o deploy/api-publish/`, report absolute+delta vs ~49.63 MB, ≤60 MB) + no new HIGH CVE (`dotnet list package --vulnerable --include-transitive`); Step 9.5 (`/code-review` + `/adr-check`); `/conflict-check` BFF+SpaarkeAi; r3 coordination note (r3 COMPLETED — file in `notes/defer-issues.md` that r2 added the from-document ingest on the shared `Services/Ai/Chat` surface); `projects/INDEX.md` still BFF=Y/SpaarkeAi=Y (no change); PR #755 description update.
> - Deploys PAUSED — endpoint ships next BFF release on operator go-ahead.
>
