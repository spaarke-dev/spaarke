...
| **Status** | in-progress — UAT fix plan. **#1,#2,#3,#5 DONE** (`7807c02a1`,`6757a55ae`,`a0e58c9ee`,`97e97e1b9`); **#4 DONE + wired** (`d9ff4df82`, live e2e pending inbound email); **#6 BLOCKED** on clarification (widget already registered — need the specific "library" UI). |
| **Next Action** | **#8 deploy** — rebuild+redeploy the CommunicationReconciliation code page (carries #2/#3/#5). Then **#7** (merge `.eml` archive fix `0026af5e1` to master) + **#9** (cleanup). **Operator TODOs:** (a) send a test email to `mailbox-central@spaarke.com` to confirm #4 triage; (b) point to the "library" UI for #6. |
...
- [~] **#6 — SpaarkeAi widget not in the library — BLOCKED on clarification 2026-08-13.** The premise was wrong: the widget **IS** registered (`communications-reconciliation`, task 062 `0e5d0fafd`, on master, `registerWorkspaceWidgets()` invoked). The `WorkspaceWidgetRegistry` is resolve-by-type with **no enumeration**, so the browsable "library" is a **separate curated catalog** — and none found (LegalWorkspace `sectionMetadataCatalog`+`sectionRegistry`, GetStartedCards, WorkspacePaneMenu) lists reconciliation. **Need the operator to say WHICH library UI they opened** (most likely add a `reconciliation` section to `sectionMetadataCatalog.ts`+`sectionRegistry.ts` mirroring `email` — r5-owned, `/conflict-check` first). Full analysis: `notes/FINDING-reconciliation-widget-library.md`.
