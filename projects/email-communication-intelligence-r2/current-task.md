...
| **NEXT** | **M365 re-register** to pick up 1.0.21 (manifest URL `https://icy-desert-0bfdbb61e.6.azurestaticapps.net/outlook/manifest.json`) — remove+re-add the custom add-in OR update the M365 admin deployment (version bump forces refresh). Then owner **live UAT**: email → Save tab → pick Matter/Project/Invoice → Save → Create To Do tab → fill → Save → confirm a real `sprk_todo` is created regarding that record (this is the §1.7 real-Dataverse smoke — first exercise of the new write path). Open decision: SaveFlow footer gray-"Saved" style (§A.4). |
