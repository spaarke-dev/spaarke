...
| Field | Value |
|-------|-------|
| **Task** | **042 ✅ COMPLETE (all criteria incl. ribbon; Option B).** Drag-to-matter + engine pre-select + ribbon quick-save (FR-B2). 2 commits. Note: `notes/042-engine-predict-dependency-finding.md`. **No gated tail** — the ribbon button is a manifest edit (deploys via 044), not a Dataverse live-env import (POML's `ribbon-edit` ref was wrong for an Outlook add-in). |
| **Step** | **Done.** BFF: `GET /api/office/communications/by-message-id/{id}/suggestions` (reuses evaluate path; no fork; + candidate display-name resolution via shared `RegardingNameFields`; DTO `Names`). Client: `communicationSuggestionsService` (ProvenanceDoc → **shared `derivePrimaryReview`**, folds resolved names into `targetName`); `SaveFlow` renders `EntityPicker` + pre-selects; `quickSave` #234; **manifest `QuickSaveButton`** → `quickSave` action + `Office.actions.associate`. **12/12 client tests**, **24/24 BFF office+resolver tests**, add-in build exit 0, BFF 0-err, publish 48.32 MB (Δ≈0), CVE clean, ADR clean. |
| **Status** | Pillar D complete (030✅ 031✅ 032✅ 034✅). Pillar B: 040🔲(dep 004 gated) · 041✅ · **042 ✅** · 043 🔲 runnable (deps 021✅/024✅). |
| **Next Action** | **Next non-gated code:** **043** (unify user-upload with capture — engine + dedup, BFF, parallel-safe:false → `/conflict-check` before PR). Pillar E (050→…) sequential main-session. **Gated (operator):** 010 HMAC, 004 Entra, 027/028/029 schema, 033 seed, all deploys (017/026/035/044/045/059 — incl. 044 which ships the 042 manifest button). |
