...
| Field | Value |
|-------|-------|
| **Task** | **043 ✅ COMPLETE (2026-08-06). FR-B3 unify user-upload with capture.** New `EmailUploadCaptureService` (`Services/Communication/`) routes a user-saved email through the SAME engine as mailbox capture (association+triage+provenance → `sprk_communication`, not just a `sprk_document`); structural dedup via `CreateCommunicationRaceProofAsync` alt-key (NFR-02); best-effort/non-fatal (NFR-04). Wired into `OfficeService.SaveAsync` (optional dep) + unconditional DI. **Escalation trigger evaluated → did NOT fire** (engine is channel-neutral; reused seams, no fork — the email peer of `MessagingIngestor`). 4 seam tests green + 164/0 Office/Comm regression. Step 9.5 gates CLEAN (0 violations). Publish **48.32 MB / ~0 delta**, no new CVE. Client mechanism-2 already delivered by server re-routing (`useSaveFlow` POSTs `/api/office/save`). Finding+completion: `notes/043-engine-entry-and-placement-finding.md`. ⚠️ `/conflict-check` + commit/push pending. **Next pending code-only Pillar-B: 040 is dep-004-GATED; remaining autonomous candidates limited — report + await operator.**
| ~~was-043~~ | **043 🔄 was in progress. Design done, escalation DID NOT FIRE** — engine entry contract is channel-neutral (`NormalizedMessage` via `IncomingAssociationResolver.ResolveAsync`+`ICommunicationEnrichmentService.EnrichAsync`; `ProcessAsync` is only the Graph adapter, `MessagingIngestor` is the proven non-Graph peer per `ICommunicationChannelIngestor`/ADR-045 rule 4). Upload=3rd peer entry=extension, not fork. Finding: `notes/043-engine-entry-and-placement-finding.md`. Design: new thin `EmailUploadCaptureService` (`Services/Communication/`, §11-justified; ONE deviation from POML `justification OMIT`, documented for PR §6.5 path A) builds `NormalizedMessage` from `EmailMetadata` + `AssociationContext.CallerSuppliedRegarding` from `TargetEntity` (save-pane=rung 0); `CreateCommunicationRaceProofAsync` (dedup free via alt-key, NFR-02) → `ResolveAsync`+`EnrichAsync`, all non-fatal; Office calls best-effort. FR-C3 attach dedup + FR-C2/C4 link ALREADY in code.**
| ~~was~~ | **042 ✅ COMPLETE (checkpointed for a FRESH session).** Unify user-upload ("Save to Spaarke") with the capture pipeline — same engine + dedup (FR-B3). FULL · sonnet·high · deps 021✅/024✅. `parallel-safe:false` (contended `Services/Office` + `Services/Communication`). **`/conflict-check` already run 2026-08-06 → CLEAN** (no open-PR overlap; re-run before PR). **042 ✅ COMPLETE** (2 commits `b5df3a3dc`+`214d6d294`, pushed; note `notes/042-engine-predict-dependency-finding.md`). |
