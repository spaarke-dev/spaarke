...
>
> **E1c HOSTS — DECISION RESOLVED (operator 2026-08-11): full Quick Start menu, NOT just Matter.**
> - **Reuse target CONFIRMED:** `GetStartedCardsWidget` (the card menu QuickStartModal renders) is exported from the SHARED `@spaarke/ai-widgets` barrel (`onCardClick:(cardId:GetStartedCardId)=>void`). Both hosts can use it → present the FULL create menu WITHOUT importing the SpaarkeAi-only QuickStartModal. §11-clean (reuse the shared cards widget; do NOT duplicate QuickStartModal).
> - **BUILD: new shared `CreateRecordChooserModal`** in `@spaarke/ai-widgets` (e.g. `src/widgets/workspace/CreateRecordChooserModal.tsx`, export from barrel). Renders `<SprkModal><GetStartedCardsWidget onCardClick=…/></SprkModal>`. Card→`{consumerType,entityType}` map = `create-matter-wizard→{create-matter,sprk_matter}`, `create-project-wizard→{create-project,sprk_project}`, `assign-work→{create-work-assignment,sprk_workassignment}`. On a record-card click → `launchSurface({consumerType, bffBaseUrl, ...fileArgs})` → on `result.committed` fire `onResult({id:recordId, entityType})`, else `onResult(null)`; dismiss → `onResult(null)`. Fire `onResult` EXACTLY ONCE (avoid the double-resolve race: separate visual `onClose` from the resolving `onResult`). Reuse the RECORD_CARD_ENTITY + outcome→ref logic already in QuickStartModal (factor to a shared util to avoid dup).
> - **Hosts implement `onLaunchCreateRecord(ctx)`** = `buildEmlFileArgs(ctx.emlSource)` (best-effort, NFR-04) → `new Promise(resolve => openChooser({fileArgs, resolve}))`; chooser `onResult`→resolve+close.
>   - **Widget host** (`ReconciliationWorkspaceWidget.tsx`, ai-widgets): session via `useAiSession().chatSessionId` (create via `POST /api/ai/chat/sessions {}` + `setChatSessionId` if null); `bffBaseUrl` from `useAiSession()`.
>   - **Code page** (`CommunicationReconciliation/main.tsx`): create an ephemeral session via `POST /api/ai/chat/sessions {}`.
>   - `buildEmlFileArgs` = ensure session → `POST …/documents/from-document {documentId}` → `{fileIds:[fileId], source:{sessionId}, provenance:{sourceFiles:[fileName]}}`; any failure → `undefined` (wizard opens un-seeded; E1b still confirms).
> - **⚠️ TWO UNKNOWNS to verify at implementation:** (1) the create-session `POST /api/ai/chat/sessions` response shape (assumed `{sessionId}` — confirm vs `ConversationPane.tsx:1941`). (2) **`resolveEmlSource(row)` — which grid-row field carries the archived `.eml` `sprk_document` id?** The needs-review grid query may not select it; check `ReconciliationGrid`/`needs-review.gridconfiguration.json` + how the browse reader resolves the `.eml` (eml-render doc id). If absent, resolveEmlSource returns undefined → E1c pre-seed no-ops (E1b unaffected) → may need a grid-column add.
> - **Build-verify:** rebuild communication-components dist so ai-widgets + code page typecheck against the new `onLaunchCreateRecord`/`resolveEmlSource`/`EmlSource` exports.
>
> **(superseded) prior fork framing:**
...
>
> **E1b WIDGET HOST + FULL-MENU CHOOSER SHIPPED ✅** (`bc87a8d1f`): shared `CreateRecordChooserModal` in `@spaarke/ai-widgets` (reuses `GetStartedCardsWidget` + `launchSurface`; `onResult` exactly-once). `ReconciliationWorkspaceWidget` wires `onLaunchCreateRecord` (full menu → wizard → confirm) + `resolveEmlSource` + `buildEmlFileArgs` (session + from-document, best-effort). Barrels export chooser + `CreatedRecordRef`/`EmlSource`. My files typecheck clean (remaining ai-widgets tsc errors are pre-existing: unbuilt sibling `@spaarke/ai-context`/`@spaarke/ai-outputs`).
>
> **§10 GOVERNANCE DONE ✅:** publish **47.09 MB compressed** (< 60 ceiling; ~0 delta — no new package); **no new HIGH CVE**; placement justified (reuses `IDocumentDataverseService`+`SpeFileStore`+cache — no new service/DI/AI-internal); seam test 3/3; r3 coordination note filed (`COORD-064-01` in `notes/defer-issues.md`).
>
> **064 REMAINING (closeout):**
> 1. **Code-page host** (`CommunicationReconciliation/main.tsx`): wire `onLaunchCreateRecord`. **Dep tradeoff** — it depends on ui-components + communication-components but NOT `@spaarke/ai-widgets`. Add ai-widgets for the full-menu chooser (heavier bundle + pulls unbuilt `@spaarke/ai-context`/`@spaarke/ai-outputs` transitively — the plan's flagged weight risk) OR a THIN `launchSurface`→Create-Matter launcher via ui-components (already a dep; Matter-only). Recommend: thin launcher (code page is the secondary MDA surface; widget has the full menu). `bffBaseUrl` via `getBffBaseUrl()`; session via `POST /api/ai/chat/sessions {}`.
> 2. **Step 9.5:** `/code-review` + `/adr-check` on the 064 diff.
> 3. **PR #755 description** update (add 064 E1b+E1c summary + §10 line).
> - E1c pre-seed activation still needs the grid-column (archived `.eml` `sprk_document` id) follow-on.
>
> **(historical) 064 GOVERNANCE (after hosts):** publish-size (`dotnet publish -c Release src/server/api/Sprk.Bff.Api/ -o deploy/api-publish/`, report absolute+delta vs ~49.63 MB, ≤60 MB) + CVE (`dotnet list package --vulnerable --include-transitive`); Step 9.5 (`/code-review` + `/adr-check`); `/conflict-check` BFF+SpaarkeAi; r3 coordination note in `notes/defer-issues.md` (r3 COMPLETED — r2 added `from-document` ingest on the shared `Services/Ai/Chat` surface, additive/back-compat); PR #755 description update. Deploys PAUSED.
>
