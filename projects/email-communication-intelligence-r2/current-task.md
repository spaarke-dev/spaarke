...
| Field | Value |
|-------|-------|
| **Task** | **042 ✅ COMPLETE (code; Option B — expanded to BFF+client per operator).** Drag-to-matter + engine pre-select + ribbon quick-save (FR-B2). Committed. Note: `notes/042-engine-predict-dependency-finding.md` (escalation → Option B decision → COMPLETION). **One gated tail remains: the ribbon-button solution import (live env) — operator go-ahead.** |
| **Step** | **Done (code).** BFF: `GET /api/office/communications/by-message-id/{id}/suggestions` (reuses evaluate-only path; no fork; new DTO + shared query helper; 8/8 office contract tests). Client: `communicationSuggestionsService` (reconstructs ProvenanceDoc → **shared `derivePrimaryReview`** via webpack-alias/tsconfig/jest to pure `provenance.ts`); `SaveFlow` renders `EntityPicker` + pre-selects predicted; `quickSave` #234 (commands bootstrap + file-to-predicted + no-prediction→open-taskpane, never auto-file a guess). **10/10 client tests**, add-in build exit 0, BFF 0-err, publish **48.32 MB** (Δ≈0), CVE clean, ADR checks clean. |
| **Status** | Pillar D complete (030✅ 031✅ 032✅ 034✅). Pillar B: 040🔲(dep 004 gated) · 041✅ · **042 ✅ (code; ribbon import gated)** · 043 🔲 runnable (deps 021✅/024✅). |
| **Next Action** | **Gated tail for 042:** ribbon quick-save button via `ribbon-edit` (export Communication ribbon solution → add button → **import to spaarkedev1 = live env, operator go-ahead**). **Next non-gated code:** **043** (unify user-upload with capture — engine + dedup, BFF, parallel-safe:false → `/conflict-check` before PR). Pillar E (050→…) sequential main-session. **Gated (operator):** 010 HMAC, 004 Entra, 027/028/029 schema, 033 seed, all deploys (017/026/035/044/045/059) + the 042 ribbon import. |
