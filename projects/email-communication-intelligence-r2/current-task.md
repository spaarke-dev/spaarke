...
| **⚠️ PENDING — owner UAT** | Send a test email through the capture flow → confirm `sprk_triagecategory` now populates (was 100% empty pre-fix). Everything upstream verified: 7 taxonomy names resolve, pluralization fix live. Query: `sprk_communications?$select=_sprk_triagecategory_value,sprk_triagepriority&$orderby=createdon desc&$top=5`. |
| **The triage bug (root cause + fix)** | Category was empty on 100% of captures. `LookupChoicesResolver` derived the OData set name as `logicalName+"s"` → `sprk_triagecategory`→`sprk_triagecategorys` (404) → the 7 taxonomy names never reached the TRIAGE-EMAIL prompt → model emitted free-form category → exact-name match missed. Fixed: `ToEntitySetName` (y→ies) in `LookupChoicesResolver.cs` + tolerant match in `CommunicationEnrichmentService.ResolveTriageCategoryIdAsync`. 2 regression tests. |
