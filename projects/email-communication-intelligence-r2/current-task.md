...
| **Status** | in-progress — UAT fix plan. **#1,#2,#3 DONE** (`7807c02a1`,`6757a55ae`,`a0e58c9ee`); **#4 DONE + wired** (`d9ff4df82`, live e2e pending an inbound email). |
| **Next Action** | **Fix #5** (two new views + view-switcher) → **#6** (register SpaarkeAi reconciliation widget) → **#7** (merge `.eml` archive fix `0026af5e1` to master) → **#8** (deploy: code-page rebuild+redeploy carries #2/#3, + SpaarkeAi + full BFF) → **#9** (cleanup). **Operator TODO:** send a test email to `mailbox-central@spaarke.com` to confirm #4 triage populates. |
