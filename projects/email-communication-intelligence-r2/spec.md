...
- *(Candidate)* Job C create-task apply endpoint.
...
11. **FR-C3 — SPE content dedup (absorb `sdap-file-duplication-detector-r1`).** Indexed `sprk_document.sprk_canonicalhash` (`quickXorHash`) with two-tier detection (Tier 1 exact hash; Tier 2 near-dup reusing `documentVector3072` "Find Similar" at a duplicate-tuned threshold), hooked at all upload paths **including the email-attachment path**; on a hit, notify (never silent), open-canonical / proceed-hash-linked; copy graduates to its own document on content divergence. **Acceptance**: uploading byte-identical content twice (any path) is detected before/at persist and does not create a second canonical document; near-duplicates surface a notify with the canonical; both SPE spikes (NFR-08) are resolved before Tier-1/Tier-2 build.
...
17. **FR-D5 (candidate) — Job C apply endpoint.** Extend a confirm/apply path for `kind:"create-task"` proposals. **Acceptance** *(if included)*: a confirmed create-task proposal calls `IActionSeam.CreateTaskAsync` under audit; otherwise this FR is explicitly parked to r5.
...
- **r5** (`email-communication-solution-r5`) — consumes R2 feeds (drag-to-matter suggestions, dedup-aware lists).
- **Exchange admin** (per enterprise client) — mail-flow rule for FR-A2 addresses.
...
- [ ] **Mail topology per client** — mail-flow rule feasibility for FR-A2 addresses. Blocks: FR-A2 deterministic external-system filing (not the body-footer primary path).
- [ ] **SPE spike outcomes** (quickXorHash timing; Tier-2 robustness). Blocks: FR-C3 Tier-1 gate-before-vs-after-write and Tier-2 build.
- [ ] **Job C apply endpoint (FR-D5)** — in R2 or parked to r5? Blocks: nothing in R2 core; scope decision only.
- [ ] **Body-footer DLP/deliverability** — light check that the transparent footer doesn't trip a client's DLP. Blocks: FR-A1 rollout confidence (low risk; visible content).
