...
> **Created**: 2026-08-03
> **Finalized**: 2026-08-04 (consistency pass); **2026-08-05** (FR-A1 config = operator-managed app setting; Pillar E → grid-first reconciliation model reusing DataGrid + EmailConnectionsReview + SprkModal; NFR-10 association-before-proposals)
...
21. **FR-E4 — Field-update reconcile modal (Job B, `SprkModal` `FormModal`).** Once the record is confirmed, the update-fields row action opens a `FormModal` preset listing that record's Job B proposals — each showing **current value → matched value**, citation, and confidence — with **Accept / Reject / Hold** (Accept → `POST /api/communications/proposals/{reviewLogId}/apply` under audit; Reject → terminal-dismiss; Hold → leave `Proposed`). **Acceptance**: a proposal shows current→proposed with its citation; Accept writes under audit; Dismiss/Hold behave per the state map; the modal also mounts on the email record form.
22. **FR-E5 — Task/deadline reconcile modal (Job C, `SprkModal` `FormModal`).** The create-tasks row action opens a `FormModal` preset rendering `kind:"create-task"` proposals with fields **name · description · base date · due date · final due date · assigned to**, each Accept / Reject / Hold; Accept → the FR-D5 apply endpoint. Requires a **new `create-task` discriminator on the queue-feed** (today only `association-exception` + `pending-proposal` exist — see `QueueFeedItemKinds`). **Acceptance**: a task proposal renders the six fields editable-before-confirm; Confirm creates the task under audit; nothing deadline-bearing auto-finalizes (ADR-015); the modal also mounts on the email record form.
