...
| 044 | Deploy Pillar B add-in (Azure SWA) — **✅ deployed** (live SWA `b64eb1876` = current-master add-in code, verified 2026-09-01; Entra NAA broker-redirect SPA URIs added). Residual = an operator sign-in smoke at a live Office host (non-gating; owner accepted deployed code as UAT-correct 2026-09-07). `word-add-in-r1` owns further add-in enhancements. Runbook: `notes/044-addin-deploy-runbook.md`. | 4·B | ✅ | 040,042 | sonnet·med | ❌ false |
...
| 064 | **E1b/E1c Quick Start "+ New record" + .eml pre-load** — E1b: injected `onLaunchCreateRecord` → host mounts `QuickStartModal` (additive `onRecordCreated` awaiting `launchSurface` outcome) → created ref re-enters `applyRegardingSelection` as confirmed regarding; modal-on-modal. E1c (**BFF resolver, decided**): new BFF endpoint materializes archived `.eml` (sprk_document) as a **chat-session document → `{sessionId,fileId}`** (wizard fetch is session-scoped) → host passes as `QuickStartModal.getFileContext` → wizard AI-pre-populates. §10 governance + **/conflict-check BFF+SpaarkeAi**. Explicit plan **§9**. Build LAST. **✅ SHIPPED as "Option C" (`309e7f674`, on master)** — built a shared `CreateRecordChooserModal` (`@spaarke/ui-components`) instead of mounting `QuickStartModal` cross-solution (better: no cross-solution import). All 4 layers: BFF `from-document` ingest endpoint (`ChatDocumentEndpoints.IngestArchiveDocumentAsync`) + `GetEmailArchiveByCommunicationAsync` resolver + shared-lib `onLaunchCreateRecord`/`CreatedRecordRef`/`EmlSource` + both hosts + `ChatDocumentEndpointsContractTests`. Escalation trigger did NOT fire (raw `.eml`/`message/rfc822` is the correct materialization; wizard re-extracts server-side). Status marker was stale; reconciled 2026-09-07 (task-execute verify). | 5·E-mount | ✅ | 052,062 | **opus·high** | ❌ false (SpaarkeAi+BFF+shared hot-path) |
...
| 090 | Project wrap-up (test-diet, lessons, doc-drift, size) — **✅ COMPLETE 2026-09-07**: `/test-diet` re-run clean (0 del, `notes/test-diet-report.md`), `/code-review` clean (064 endpoint verified: correct object-level authz on the app-only download), all deps ✅ (064 shipped, 044 reconciled), README→Complete flipped. `/devops-project-archive` next. | 6 | ✅ | all | sonnet·high | ❌ false |
| 090-note | wrap-up artifacts DONE 2026-08-31 (test-diet clean/0-del · doc-drift clean · publish ~44 MB≤60 · 0 CVE · lessons · r5 coord · INDEX). **UNBLOCKED 2026-09-07**: 044 reconciled (deployed; operator smoke non-gating per owner); 064 reconciled (shipped `309e7f674`). Project-close re-run of `/test-diet` (covers tests added since 2026-08-31: 064 contract test + E1b RTL + R3-CARD units) + `/code-review` + `/devops-project-archive` in progress; then README→Complete + all-✅. | 6 | 🔄 | 044 | — | — |
