...
| 064 | **E1b/E1c Quick Start "+ New record" wiring** — "+ New record" opens `QuickStartModal` → wizard (surface-launch) → created record added to Related-to candidates → Related-to `Confirm`; modal-on-modal. **E1c**: the reconciled email's **`.eml` auto-loads** into the wizard's AI-pre-populate step via the existing Assistant pass-through (`QuickStartModal fileCtx.fileIds`→`launchSurface`→`SurfaceHandoffEnvelope.fileIds`). **GATED on prototype sign-off.** Reuse: plan §7.5; scope: UX §E E1b/E1c | 5·E-mount | 🔲 | 052,062 | **opus·high** | ❌ false (SpaarkeAi+shared hot-path) |
| 065 | **E2b/E2c Fields tab typed controls + Update-other-fields** — type-correct editors (date picker, option-set dropdown, **lookup → OOB advanced-lookup side pane**) + full-width **"+ Update other fields"** button opening the confirmed Related-to record's real form (OOB `navigateTo` / `RecordNavigationModalShell`); modal-on-modal. **GATED on prototype sign-off.** Reuse: plan §7.5; scope: UX §E E2b/E2c | 5·E-mount | 🔲 | 055,063 | **opus·high** | ❌ false (shared lib) |
| 066 | **E3b New-task Assigned-to OOB advanced-lookup** — the New-task modal `Assigned to` uses the standard OOB advanced-find side-pane lookup (systemuser/team), not a bespoke picker. **GATED on prototype sign-off.** Reuse: plan §7.5; scope: UX §E E3b | 5·E-mount | 🔲 | 056,063 | sonnet·high | ❌ false (shared lib) |
