...
| 061 | **ReconciliationWorkspace composition** — new presentational component composing `ReconciliationGrid` → `ReconciliationBrowseShell` (renderTabs = Related-to/Fields/Tasks) + apply UX **A4/A5/A6** to the shell's two-pane body (SprkModal xl/full · ModalScrollArea thin scroll · PanelSplitter 50/50 drag-resize); NFR-10 gate + citation wiring. **Compose+enhance existing components — no fork, no BrowseModal wrapper** (shell already IS the SprkModal). notes/061 | 5·E-mount | 🔲 | 050–057 | sonnet·high | ❌ false (shared lib) |
| 062 | **Dual host** — new Vite code page `src/solutions/CommunicationReconciliation` → `sprk_communicationreconciliation` (mirror EmailPage: initAuth + IDataverseClient + authenticatedFetch) **and** a SpaarkeAi widget-registry entry; both mount the SAME `ReconciliationWorkspace`. `NEEDS_REVIEW_CONFIG_ID` left as placeholder for 059. Hot-path SpaarkeAi — /conflict-check before PR. No deploy here. notes/062 | 5·E-mount | 🔲 | 061 | sonnet·high | ❌ false (SpaarkeAi hot-path) |
...
| **W5-mount — E mount build** | 060 ✅ → 061 → 062 | 061←050-057; 062←061 | **Sequential, main-session** (shared lib + SpaarkeAi hot-path). 060 done (prototype). 061 composes grid→shell→tabs + applies A4/A5/A6; 062 dual-host. goal-eligible: NO (shared/hot-path, ≤2 build tasks). |
