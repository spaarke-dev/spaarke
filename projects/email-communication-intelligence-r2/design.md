...
**R2 is NOT:** the docketing / email→dated-obligations flagship (parked — see §7, recommended as a standalone R3), a UI project (r5 owns surfaces; R2 supplies feeds + engine), or a re-architecture (every item extends the R1 engine, capture path, or Outlook add-in).
...
- **D5 · (candidate) Job C apply endpoint.** Job C create-task proposals land as `Proposed` rows with no confirm/apply surface. Include only if it fits; otherwise park to r5.

---

## 3. Deep-dive — the three review questions (design detail)
...
- **r5** (`email-communication-solution-r5`) — owns review/reading surfaces; R2 feeds it (drag-to-matter suggestions, dedup-aware lists).
