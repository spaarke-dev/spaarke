...
| A26 | ✅ **applied 2026-08-26 SESSION 12** (operator-executed) | — | Human operator executed the 4-step ceremony per `notes/queue-recreate-runbook-2026-08.md`: (1) pre-check confirmed live state `requiresSession=false, requiresDuplicateDetection=false`; (2) `az servicebus queue delete` on `sprk-provisioning-jobs` (namespace `spaarke-servicebus-dev`, RG `SharePointEmbedded`, sub `484bc857-3802-427f-9ea5-ca47b43db0f0`); (3) `az deployment group create` against `infrastructure/bicep/modules/controlplane-sb-queue.bicep` with `serviceBusNamespaceName=spaarke-servicebus-dev`; (4) verify returned `{"duplicateDetectionHistoryTimeWindow":"PT1H","requiresDuplicateDetection":true,"requiresSession":true}` ✅. RBAC survival check (Step 5) confirmed 4 role assignments intact at namespace scope (2× Data Sender + 2× Data Receiver on principals `38f7693f-...` + `9fd47efb-...`). |
...
| A02 | skill-step-0.5-external-prereqs | Extend `/provision-environment` Step 0.5 to read `prereqs.yaml` + iterate check recipes + HARD STOP on failure | skill | 6 | A01 | ✅ applied 2026-08-26 SESSION 12 (task 203c) — SKILL.md Step 0.5 wired between Step 0 and Step 1 at line 174; consumes `powershell-yaml` module (auto-install idempotent); scopes filter (`once_per_tenant` + `once_per_subscription` + `once_per_env` when `-Environment` known); best-effort output classification against `check_recipe.expect`; HARD STOP with remediation link pointing INTO `docs/guides/PROVISIONING-PREREQUISITES.md`; `-SkipStep0_5` (or batch `skipExternalPrereqs`) opt-in for offline operators. Iteration is DYNAMIC — new prereqs added by future task 202 amendments picked up without SKILL.md edit. | `.claude/skills/provision-environment/SKILL.md` |
| A03 | skill-step-1-batch-mode | Add `--batch` flag + JSON Schema at `scripts/provisioning-prereqs/intake.schema.json` | skill + json-schema | 4 | A02 | ✅ applied 2026-08-26 SESSION 12 (task 203c) — Two parts: (1) authored `scripts/provisioning-prereqs/intake.schema.json` per JSON Schema Draft 2020-12 covering profile enum + customerId + tenantId + tenancyModel + environment + environmentId + optional region/tier/operatorUpn/notes + conditional `tenancyModel × profile` invariant + 2 examples; (2) SKILL.md Step 1.0 batch-mode sub-section wired at line 267+ — accepts `--batch <path.json>`, validates via `ajv-cli` (Draft 2020-12), pre-fills every 1a-1e field, still runs 1f auto-create when `intake.environmentId` null, honors `intake.skipExternalPrereqs`. | `provisioning-run-agent-autonomy-design.md` |
| A04 | skill-step-7-postmortem | Author mandatory Step 7 (writes `lessons-learned.md` per run) | skill | 3 | A05 | ✅ applied 2026-08-26 SESSION 12 (task 203c) — SKILL.md Step 7 wired at line 909 (before Fallback Matrix). Runs UNCONDITIONALLY after Step 6 (Succeeded/Failed/Quarantined/Drifted/manual-abort). Copies 203a `provisioning-runs/_templates/lessons-learned.md` template + prefills `{customerId}/{runId}/{operatorUpn}/{ts}` placeholders + interactive-or-batch postmortem collection + `provisioning-runs/INDEX.md` row append + commit gate. MANDATORY — declining still writes a 3-line acknowledgment. 203a dependency verified: template exists at `provisioning-runs/_templates/lessons-learned.md`. | `provisioning-run-structure-design.md` |
...
| A26 | T108-queue-recreate-ceremony | `sprk-provisioning-jobs` queue: session receiver requires `requiresSession=true` + `requiresDuplicateDetection=true` (creation-time-only). Live queue lacks both. Bicep authored per task 108; live delete-and-recreate DEFERRED. RUN CEREMONY per `notes/queue-recreate-runbook-2026-08.md` | operator runbook | 2 | A13 | ✅ **applied 2026-08-26 SESSION 12** — operator-executed ceremony (4 steps + RBAC survival check); post-recreate verify confirmed `requiresSession=true, requiresDuplicateDetection=true, duplicateDetectionHistoryTimeWindow=PT1H`; 4 role assignments (2× Data Sender + 2× Data Receiver) survived at namespace scope. Last task 186 prerequisite CLEARED. | task-108-deviations, DS-2 |
...
| B11 | staging-slot shadow-worker defect | `Sprk.Provisioning.ControlPlane.Core` + `Sprk.Provisioning.ControlPlane.Worker` are separate projects (split happened); `.Api` project split status unverified | **✅ applied — Path SPLIT — verified already complete via Wave G-1 tasks 100/101/102** — `.Api` Program.cs registers ZERO `AddHostedService`/handler DI (only auth+swagger+cosmos+SB+telemetry+CustomerRunGuard+Rollback + endpoint mappings); `.Worker` Program.cs owns all 21 `IProvisioningHandler` keyed registrations + `AddHostedService<StateReconcilerService>` (via `AddReconcilerModule`) + `AddHostedService<CrashRecoveryStartupService>` + `AddHostedService<ProvisioningHandlerDispatcher>`; `modules/controlplane-worker-app-service.bicep` declares `.Worker` as SLOTLESS `Microsoft.Web/sites` (no `sites/slots` child resource) on the same P1v3 plan — defect structurally closed. Guard: `Sprk.Provisioning.ControlPlane.Tests/Dispatch/ApiHostShadowWorkerGuardTests.cs` (task 204d, new) asserts `.Api` DI graph has zero project-owned `IHostedService` + zero keyed `IProvisioningHandler`. Path FLAGS was NOT chosen (rationale in `notes/task-204d-path-decision.md`: introducing three new `Enabled` flags to a slotless Worker would be pure new-surface with zero behavioral gain — fails CLAUDE.md §11 minimalism). Design.md §4.2a "Split contract" sub-section added by task 204d. NO Bicep change; NO FR-35 pre-check required (no live setting renamed/removed). |
