...
| 4 | Platform KV secret `Sidecar-Shared-Secret` is populated | `az keyvault secret show --vault-name sprk-controlplane-{env}-kv --name Sidecar-Shared-Secret --query value -o tsv` — non-empty (H4 / task 126) |
| 5 | Worker has `keyVaultReferenceIdentity` PATCHed to the UAMI | `az webapp show -n spaarke-provisioning-controlplane-worker-{env} -g rg-spaarke-platform-{env} --query "properties.keyVaultReferenceIdentity" -o tsv` — matches UAMI resource id (H4 / task 125) |
