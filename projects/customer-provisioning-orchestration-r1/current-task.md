...
> **Last Updated**: 2026-08-19 (Task 130 COMPLETE — H3 Graph app-reg port. GraphAppRegistrationProvisioner + GraphAdminConsentVerifier replace the retired shell-out/Null scaffold; Model 1/Model 2 tenancy branch (I6-enforced) + FIC (auth-v4 §3.1) + deferred-KV-write-after-consent ordering (DS-4 §3) + BFF-API-ClientId/Audience/ClientSecret RunParameters.Secrets population (task 129's manifest contract) all landed. **Two documented Path-C scope deviations** (14-AppRoleAssignedTo grants + Dataverse app-user assign both already owned by H10, task 053 — duplicating would be wrong-principal / dead-code respectively) — see task 130 POML `<notes-completion>` for full rationale. L2 tests: 903 → **916/916** (+13, zero regressions). Wave G-3 continues: 131 (H8) + 132 (H9) can now dispatch in parallel.)
...
> **PR**: https://github.com/spaarke-dev/spaarke/pull/779 (DRAFT — DO NOT MERGE — Phase C'' incomplete; Waves G-3..G-7 remain. Wave G-2.5 (customer.bicep completion, tasks 127/128/128b/129) is fully closed. Wave G-3 task 130 (H3) is now complete.)
...
| **Next decision** | Wave G-2.5 (customer.bicep) + task 130 (H3) both complete. Dispatch 131 (H8) + 132 (H9) next — parallel-safe, different handler folders, per DS-4 §4 batching guidance. |
