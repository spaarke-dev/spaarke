...
> **Last Updated**: 2026-08-20 (Task 132 COMPLETE — H9 artifact-based rebuild, handler side. ArtifactManifestVerifier (pure C# manifest hard-block, no shell-out) + BlobArtifactDownloader (Azure.Storage.Blobs UAMI RBAC) + KuduZipDeployer (typed HttpClient POST to Kudu zipdeploy, MI bearer token) + ArmSlotSwapper (WebSiteSlotResource.SwapSlotAsync — implements the EXISTING IAppServiceSlotSwapper interface unchanged) replace the retired DotnetR3GateVerifier / DeployBffApiScriptRunner / AzCliAppServiceSlotSwapper (kept on disk unregistered). H9BffDeployHandler rewritten: buildId run parameter now OPTIONAL (resolves from task 116's latest.json manifest when absent); rollback-re-swap tail is BYTE-IDENTICAL to the pre-task-132 handler (only the swap collaborator's DI registration changed). L2 tests: 916 → **930/930** (+14, zero regressions). Storage-account-doesn't-exist-yet (live-ceremony item #4) documented as a known live-ceremony dependency — fully unit-testable today via fake blob/HTTP transports. Sibling task 131 (H8) ran in parallel in a different folder — no coordination needed. Wave G-3 now fully complete (130 + 131 + 132 all done, pending confirmation of 131's own landing).)
...
> **PR**: https://github.com/spaarke-dev/spaarke/pull/779 (DRAFT — DO NOT MERGE — Phase C'' incomplete; Waves G-4..G-7 remain. Wave G-2.5 (customer.bicep completion) is fully closed. Wave G-3 (130/131/132) is now complete.)
...
| **Wave G-3 status** | Task 130 (H3) + task 132 (H9) COMPLETE. Task 131 (H8) ran in parallel — verify its own landing/commit before declaring Wave G-3 100% closed. |
| **Next decision** | Confirm task 131 (H8) has committed + pushed, then dispatch Wave G-4 (140-144: H5/H6/H7/H10/H11). |
