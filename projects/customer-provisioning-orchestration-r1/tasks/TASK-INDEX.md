...
> **Last Updated**: 2026-08-25 SESSION 10 — **Task 205 sub-phase authored** (6 POMLs 205a–f covering auth-v4 runtime-contract integration rows A38–A44) via 6 parallel background agents; owner sign-off pending before S1∥ mini-wave dispatch (A38 ∥ A42 at Fable/Opus xhigh). Prior: 2026-08-25 SESSION 9 (Fable deep review of auth-v4 change request + all 11 owner decisions Q1-Q11 RESOLVED + S1∥ A36/A37/A40 landed + A35 master merge both directions); 2026-08-25 SESSION 8 (Task 203a COMPLETE); 2026-08-24 SESSION 7 (task 202 COMPLETE + Class-B verification pass + 11 POMLs authored for 203a/b/c/d + 204a/b/c/d/e/f/g); 2026-08-20 (Wave G-7 Batch G-7E TERMINAL — task 186 authoring + framework-level r1 E2E goal proof landed).
> **Status**: Phase Pre-Live-Fire fully authored across SESSION 7 + SESSION 10 — task 202 (COMPLETE) + 203a (COMPLETE) blocks 203b/c/d + 204a-g + 205a-f; together they block task 186 E2E live-fire per owner directive + task 186 pre-check trigger. Task 204c is the ORIGINAL HARD-BLOCKER (H13 real probes — 10 sub-tasks); task 205 sub-phase is the AUTH-V4 hard-blocker (A38+A42→A39 critical path = 11h serial).
> **Task count**: 158 POMLs (78 original across 10 phases + 62 Phase C'' across 8 waves + 18 Pre-Live-Fire tasks: 202 + 203a-d + 204a-g + 205a-f SESSION 10 2026-08-25)
...
| 205a | 🔲 (blocked by nothing — A35 landed) | **Auth-v4 integration — row A38** (H4/H4-shared credential-type seam FR-39; secret-free omit contract): on secret-free envs, OMIT (never sentinel per §9.1) `BFF-API-ClientSecret` + `ServiceBus-ConnectionString` + `AiSearch--AdminKey` manifest entries; positive migration marker = KV tag or provisioning-state field (never credential slot). Partially closes A30 (H4 half + H7/task-142 half in-scope with A44.5 escape). Model 1 (shared KV) vs Model 2 (per-customer × N vaults) fleet-consistency behavior explicit. Deps: A35 (landed). ~4h. | FULL | fable / xhigh | S1∥ mini-wave (with 205b parallel-safe:true) | 202, 203a |
| 205b | 🔲 (blocked by nothing — A35 landed) | **Auth-v4 integration — row A42** (`-FicOnly` consume/reconcile FR-C4): reconcile task-130 C# `CreateFic` (per-profile issuer at :547-557) vs master PS `Register-EntraAppRegistrations.ps1 -FicOnly` estate. Either (a) H3 delegates via subprocess (NEVER dot-source), OR (b) parity dual-path with triple-keyed idempotency + AADSTS70025 exact-match retry + exit 0/1/2 semantics. Port `Assert-SpaarkeFicTenancy` cross-tenant refusal into `CreateFic`. Q9 verified spaarke-hosted → §9.2 does NOT affect 186. Deps: A35 (landed). ~4h. | FULL | fable / xhigh | S1∥ mini-wave (with 205a parallel-safe:true) | 202, 203a |
