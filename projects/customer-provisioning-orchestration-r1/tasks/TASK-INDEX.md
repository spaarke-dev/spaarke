...
> **Last Updated**: 2026-08-20 (**Wave G-7 Batch G-7E TERMINAL COMPLETE** — task 186 authoring + framework-level r1 E2E goal proof landed. `E2EAcceptanceCompositionRootTests.cs` (21 new tests, all pass; L2 total 1481/1 skip/0 fail; +21 vs Batch G-7D baseline) asserts all 15 H13 constituent checks resolve to real Wave-G-7 implementations in the real Worker composition root (zero placeholders remain in the H13 dependency graph). Report at [`notes/phase-c-double-prime-e2e-acceptance-real-run.md`](../notes/phase-c-double-prime-e2e-acceptance-real-run.md) is the binding SC #5 / SC #6 acceptance trail for task 090 wrap-up; live ceremony deferred per task 162 Path C precedent (L2 Worker App Service does not exist on Azure yet). Also flipped stale TASK-INDEX row 176 → ✅ (landing commit `54a348ed8` present since 2026-08-20 early AM). Prior update: 2026-08-19 Wave G-2.5 tasks 127/128/128b/129 closed customer.bicep gaps; spec.md/design.md/CLAUDE.md amended to v3.6 for Model 1/Model 2 Redis split.)
> **Status**: Phase Pre-Live-Fire authored SESSION 5 (2026-08-24) — task 202 (audit + PREREQUISITES + provisioning-run structure design) blocks task 203 (apply); together they block task 186 E2E live-fire per owner directive to codify prereqs + apply lessons BEFORE E2E ceremony.
> **Task count**: 142 POMLs (78 original across 10 phases + 62 Phase C'' across 8 waves + 2 Pre-Live-Fire tasks 202/203 added SESSION 5 2026-08-24)
...
### Phase Pre-Live-Fire — Consolidate lessons + structural gaps before E2E (2 tasks — designed 2026-08-24 SESSION 5)

Owner directive 2026-08-24: BEFORE invoking task 186 E2E ceremony, audit + apply all captured-but-unapplied lessons so live-fire proves the platform's automation rather than iterating through fail-cycles. SESSION 5 attempted manual BFF boot (5 config gates cleared + IActionSeam code fix `e3a15db91`) — owner recognized this was symptom-fixing not platform validation. Corrective sequence: 202 audits + designs + produces punch list → 203 applies (code + skill + docs) → 186 fires cleanly. Also: formalize PROVISIONING-PREREQUISITES.md as codified reference (owner directive: "documented somewhere that can then feed into the process — formalized, file/app/table") + design provisioning-runs/{customerId}-{runId}/ project structure mirroring coding-project pattern (owner directive: "shouldn't there be a project structure within which a new environment provisioning project runs?").

| ID | Status | Title | Rigor | Model / Effort | Parallel Group | Deps |
|---|---|---|---|---|---|---|
| 202 | 🔲 | Pre-live-fire lessons audit + formalize PROVISIONING-PREREQUISITES.md + design provisioning-run structure (blocker for task 186 E2E). Outputs: `docs/guides/PROVISIONING-PREREQUISITES.md` (codified: markdown + sibling YAML at `scripts/provisioning-prereqs/prereqs.yaml`) enumerating ≥7 manual prereqs (SPE container-types, Office add-in apps, Copilot bot apps, Power BI SPs, Azure sub, OpenAI TPM bumps, provider registration) + `notes/provisioning-run-structure-design.md` (7-file per-run folder + INDEX.md + template contents) + `notes/provisioning-run-agent-autonomy-design.md` (--batch flag + JSON intake schema) + `notes/task-202-punch-list.md` (machine-readable unapplied-lesson × landing-spot × effort × blocks-e2e). Audits: `lessons-learned-model1-prod-standup-2026-08-22.md` (870 lines, F1-F20+), 40+ task-XXX-deviations, 4 wave-drift notes, SESSION 5's 4 live lessons. STANDARD rigor (docs + design memos; no .cs). Blocks: 203 + 186. | STANDARD | opus / high | none (parallel-safe:false; touches `.claude/patterns/provisioning/`) | 200, 201 |
| 203 | 🔲 (blocked by 202) | Apply pre-live-fire lessons per task-202-punch-list.md: (a) code fixes for any surfaced ADR-032-class asymmetric-registration bugs beyond IActionSeam (`e3a15db91`); (b) expand H4b `per_env_settings` manifest with full BFF IOptions inventory (~40 modules × ~2-4 settings); (c) extend `/provision-environment` skill with Step 0.5 External Prerequisites Verification (reads `scripts/provisioning-prereqs/prereqs.yaml`) + per-run folder creation + Step 7 mandatory postmortem; (d) create `provisioning-runs/` root + `INDEX.md`; (e) fill `.claude/patterns/provisioning/` scaffolding per 202 design; (f) resolve ADR Tensions surfaced during 202 (specifically SESSION 5 Lesson 3: multi-tenant Dataverse routing at `DataverseServiceClientImpl.cs:39`); (g) update `docs/guides/SPAARKE-CUSTOMER-DEPLOYMENT-GUIDE.md` with cross-refs to PREREQUISITES + provisioning-runs pattern. Rigor auto-determined per lesson; likely FULL for .cs + TEST-MODIFYING for any ArchTest additions. | FULL | opus / xhigh | none (serial after 202) | 202 |
