...
| 110 | 🟡 | Bicep: SB Data Receiver (+Sender) RBAC role assignments for L2 UAMI(s) (C5.5) — authoring complete (new modules/controlplane-sb-rbac.bicep, namespace-scope, cross-RG module mirroring task 108's queue pattern), live grant + role-assignment-list verification deferred to operator/task 113 deploy script. FOLDED-IN: fixed IDENTICAL C5.1 key-shape bugs task 109 flagged in controlplane-worker-app-service.bicep (Cosmos__* rename, ServiceBus__ConnectionString -> FQNS+QueueName MI-only, added ManagedIdentity__ClientId); removed now-fully-dead serviceBusKeyVaultSecretName param from platform-controlplane.bicep. | FULL | sonnet / high | none | 100, 101 |
...
| 113 | 🟡 | Author Deploy-ControlPlane.ps1 (L2 repeatable deploy script, C5.9/C1.7) -- authored at scripts/provisioning/Deploy-ControlPlane.ps1 (path deviation, parity with task 111's sibling script); PSScriptAnalyzer-clean; -Target Api/Worker/Both + -Swap + -WhatIf/-Confirm/-DryRun; also fixed a discovered defect (.Api had no /healthz route -- HealthEndpoints.cs) and documented (not fixed, out of scope) the .Worker Bicep config-key shape gap task 109 filed but left unowned; live full-run + broken-setting smoke test deferred to operator per dispatch instruction -- see notes/task-113-deviations.md | FULL (overridden up from STANDARD -- .cs touch) | sonnet / high | none | 100, 101, 108, 109 |
