...
| 213 | 🟡 | **SESSION 20 SPE topology reconciliation** — 4 of 7 items LANDED 2026-08-30 (commit c7b695678): (1) topology doc cross-worktree copy + provenance ✅; (3) `Create-NewContainerType.ps1` DEPRECATED banner + throw + runbook redirect ✅; (5) `SPAARKE-SPE-TOPOLOGY-SETUP-RUNBOOK.md` authored (8-step operator runbook) ✅; (6) SKILL Step 0.5c topology-verify + Step 0.5b BFF search fix ✅. **Deferred**: (4) `Register-EntraAppRegistrations.ps1` extension (1630-line script, 2-3h careful refactor — next-turn 213.4). **Blocked**: (7) constants population + smoke-verify — awaits operator one-time SPE runbook completion. **Escalated to task 214**: (2) H8 rework — Option A live-test 2026-08-30 empirically verified §R5 (403 accessDenied, third confirmation); H8 requires full rewrite scoped as 214 (8-12h xhigh); see `runs/h8-live-test-2026-08-30.md`. | FULL | sonnet / xhigh | none | 199, 212 |
| 214 | 🔲 | **SESSION 20 H8 handler full rewrite (H8-B semantics)** — Driven by task 213.2 Option A live-test empirical verification of topology doc §R5 for the third time (HTTP 403 accessDenied on `POST /storage/fileStorage/containerTypes` under `client_credentials` regardless of credential shape — grant-type is what matters, not credential shape). Also discovered incidental KV cert drift (`spaarke-spekvcert/spe-app-cert` not registered on any Spaarke app-reg). H8 has been doubly-broken in dev since task 131 Wave G-3 Graph SDK port — never live-fired end-to-end. Scope: (1) delete container-TYPE creation from GraphContainerTypeProvisioner; (2) refactor H8SpeContainerTypeHandler → H8SpeContainerHandler as container-CREATION only (reads containerTypeId from constants); (3) rename SpeContainerType* → SpeContainer* interfaces + namespace; (4) delete T6 trap detection (retained for H13 only); (5) rewrite test surface; (6) update HandlerIds + DagAdvancer; (7) retire KV cert drift (`spe-app-cert` + `spe-app-cert-pass` — not in never-delete list; grep-verify before soft-delete). Can run PARALLEL with 213.4. Full evidence: `runs/h8-live-test-2026-08-30.md`. Blocks task 186 alongside 213.4 + 213.7 + 207 + 208 + 209. | FULL | sonnet / xhigh | none | 199, 212, 213 |
