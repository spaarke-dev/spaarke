...
> **Last Updated**: 2026-08-30 SESSION 21 — **Task 213.4 LANDED + Task 214 LANDED (parallel sub-agent execution)**. Post-compact resume-from-SESSION-20 executed the two remaining parallel work-streams: 213.4 (Register-EntraAppRegistrations.ps1 topology-mode extension — added `-CreateOwningApp/-CreateBffApp/-CustomerName` mode-switches per topology doc §3A; 6 files / +618/-21) and 214 (H8-B full rewrite — delete `Handlers/SpeContainerType/` folder + new `Handlers/SpeContainer/` folder + container-CREATION only per topology doc §6; 28 files touched incl. 8 new + 11 deleted + tests + DI + KV manifest). Full test suite: **1919 pass / 0 fail / 1 skip (pre-existing)**. 213 now 6 of 7 items (213.7 blocked pending operator SPE runbook execution). New follow-on filed: 214-cert-retirement (deprecate live consumers of `spe-app-cert` before soft-deleting from KV). Dispatch sequence: {212 partial ✅} → {213 partial 6/7 ✅ ∥ 206 ✅ ∥ **214 ✅ NEW**} → {213.7 ∥ 207} → 208 → 209 → task 186. Prior: 2026-08-30 SESSION 20 END-2 — Task 213 partially landed (4 of 7 items) + Task 206 completed + Task 214 filed after H8 live-test verified §R5 empirically (HTTP 403 accessDenied under `client_credentials` regardless of credential shape). Prior: 2026-08-30 SESSION 20 mid — Task 212 landed terminology + name-templates + constants-rename; Task 213 filed. Prior: 2026-08-28 SESSION 20 start — Task 212 initial filing after HARD STOP at Step 0.5b constants sanity check. Prior: 2026-08-27 SESSION 14 — **Task 199 SESSION-14 addendum + tasks 206/207/208/209 filed**. First-live-Step-0.5 iteration of `scripts/provisioning-prereqs/prereqs.yaml` during `/provision-environment trial1 --batch` uncovered 3 orthogonal defect classes (18 YAML syntax defects + 32 recipe-contract violations + 31 placeholder-substitution defects) via ultracode workflow (`wf_75aa08a8-13e`: 3 audit + 3 adversarial verify, 499K tokens). SESSION 14 fixed Class A atomically (18 yaml defects + scope-enum defect on line 359) + authored the forcing function (`scripts/provisioning-prereqs/validate.ps1` parser-parity validator using powershell-yaml + `.github/workflows/provisioning-prereqs-validate.yml` always-on-PR gate + `.lintstagedrc.mjs` author-time relief) — validator empirically verified end-to-end (PASS on fix / FAIL exit-1 on regression / PASS on restore). Class B + C + forcing-function architectural gaps filed as tasks 206 (recipe-contract exit-1 remediation, xhigh 4-6h) + 207 (placeholder-substitution + SKILL Step 0.5b substitution block extension, xhigh 3-5h) + 208 (integrate validator into ci-router.yml single-gate per FR-A01) + 209 (restore master branch protection — currently DISABLED per adversarial-verify discovery). Full audit findings preserved: `notes/prereqs-yaml-audit-2026-08-27.md`. **Task 186 batch dispatch now unblocked for Class-A parseability** — Class B + C are pre-existing debt not this dispatch's regression. Prior: 2026-08-26 SESSION 13 (task 199 SESSION-13 addendum + 30-column schema deploy + skill drift fix); 2026-08-26 SESSION 10 (S2∥ wave A38 split + A44.5 landed).
...
> **Task count**: 169 POMLs (78 original across 10 phases + 62 Phase C'' across 8 waves + 21 Pre-Live-Fire tasks: 202 + 203a-d + 204a-g + 205a/b/c/d/e/f/g/h/i SESSION 10 2026-08-25 + 4 SESSION 14 remediation tasks: 206 + 207 + 208 + 209 + 3 SESSION 20 dispatch-readiness tasks: 212 + 213 + 214 + 1 SESSION 21 follow-on: 215)
...
| 206 | 🔲 | **SESSION 14 remediation (Class B)** — Remediate 32 recipe-contract violations in `scripts/provisioning-prereqs/prereqs.yaml` (SESSION 12 exit-1 contract was written but never applied beyond PRQ-E-14). Silent-PASS class includes PRQ-C-02 (OpenAI Deprecated model check — the EXACT recipe SESSION 12 rewrote), PRQ-E-06 (F19/F20 root cause), PRQ-E-09 (F16 KV drift), PRQ-E-12 (T108 SB recreate), PRQ-C-06 (F14 SpaarkeMaster fail), PRQ-C-07 (PowerShell `Select-String` inside `bash -c` wrapper). Full defect list: `notes/prereqs-yaml-audit-2026-08-27.md` § Class B. | FULL | sonnet / xhigh | none | 199 |
| 207 | 🔲 | **SESSION 14 remediation (Class C)** — Resolve 31 placeholder-substitution defects (19 prereqs) in `scripts/provisioning-prereqs/prereqs.yaml` + extend SKILL Step 0.5b substitution block. Tokens: `{containerTypeId}`, `{l2UamiPrincipalId/SpId/ClientId}`, `{subId/sub}`, `{artifactsStorageId/acrId/bffAppServiceId/kvResourceId}`, `{sbNamespace/sbName}`, `{adminDvUrl/dvUrl}`, `{bffAppId/graphAppId}`, `{customerId}`, `{region}`, `{pinnedVer}`. Plus PRQ-E-07 unescaped `$filter` bash-expansion + PRQ-E-13 scope-placement mismatch (once_per_env but references `{customerId}` known only post-intake). Sequenced AFTER 206 (exit-1 guards make placeholder false-fails LOUD). | FULL | sonnet / xhigh | none | 199, 206 |
| 208 | 🔲 | **SESSION 14 remediation (arch)** — Integrate `provisioning-prereqs-validate.yml` into `ci-router.yml` single-gate per FR-A01 (currently runs as parallel advisory). Extend `classify` job with `provisioning_prereqs` output covering `scripts/provisioning-prereqs/**` + `.claude/skills/provision-environment/**`. Route validation as step/sub-job in `ci-tier1-blocking.yml` gated on the new output. Preserve parser parity (powershell-yaml). Retire OR retain standalone workflow per docs-only-path decision. | FULL | sonnet / high | none | 199 |
| 209 | 🔲 | **SESSION 14 remediation (governance)** — Restore master branch protection on `spaarke-dev/spaarke` (currently HTTP 404 — DISABLED per adversarial-verify discovery). Required checks list = exactly `[CI / Router]` per FR-A01. Document admin-bypass posture (blocked, or audit-trail-required). Sequenced AFTER 208 so the checks list is authoritative before flipping enforcement on. | STANDARD | sonnet / medium | none | 208 |
| 212 | 🟡 | **SESSION 20 pre-dispatch readiness — small-scope terminology + name-template + constants-rename** (2026-08-30 partial-land, work-in-progress commit). Scope narrowed after owner alignment on SPAARKE-SPE-CONTAINER-TYPE-TOPOLOGY.md authority (Q1) + confirmation that `Spaarke Trial 1` container-type + owning app-reg don't exist yet (Q2) + H8-B rework decision (Q3) + owner directive to create NEW `Spaarke BFF — Trial 1` (Q4) + scope-split decision (Q5, task 212 stays small + 213 handles topology). **212 LANDED**: (a) ADR-028 line 229 rewrite `multi-tenant BFF` → `single-tenant Spaarke BFF` + line 239 OPEN CLOSED with resolution note; (b) project CLAUDE.md § MUST rule terminology reconciliation; (c) spaarke-constants.yaml `name_templates` corrected against LIVE Azure (`sprk-controlplane-{env}-kv`, `bffAppServiceRg: rg-spaarke-{env}`, `sprkcpartifacts{env}`, `sprkcontrolplane{env}acr`); (d) rename `bffMultiTenantAppId` → `bffApiAppId` in constants + all 4 consumer sites (SKILL Step 0.5b line 346 + Step 5a line 1353 + context-defaults.dev.json + context-defaults.prod.json). **NOT LANDED (moved to 213)**: constants VALUE population (blocked on operator one-time SPE topology setup); H8 rework (Gap E); operator runbook for topology setup. Cross-worktree topology-doc merge pending (from sdap-SPE-admin-app-r2). Full 5-gap audit: `runs/pre-dispatch-readiness-gap-report.md`. Sequence: {212 ✅} → {213 ∥ 206 ∥ 207} → 208 → 209 → task 186 dispatch. | FULL | sonnet / high | none | 199 |
| 213 | 🟡 | **SESSION 20 SPE topology reconciliation** — 6 of 7 items LANDED. From SESSION 20 (commit c7b695678): (1) topology doc cross-worktree copy + provenance ✅; (3) `Create-NewContainerType.ps1` DEPRECATED banner + throw + runbook redirect ✅; (5) `SPAARKE-SPE-TOPOLOGY-SETUP-RUNBOOK.md` authored (8-step operator runbook) ✅; (6) SKILL Step 0.5c topology-verify + Step 0.5b BFF search fix ✅. From SESSION 21 (this session): (4) `Register-EntraAppRegistrations.ps1` extension ✅ — added `-CreateOwningApp/-CreateBffApp/-CustomerName` mode-switches per topology doc §3A. Model 2 owning app = `AzureADMultipleOrgs` (only multi-tenant app-reg in Spaarke topology); all others single-tenant. Idempotent by display name; NO client secrets minted; NO KV writes; parse-check + 4 gate tests + prod-flow no-regression proof all pass. Runbook Steps 1+6 rewritten to invoke; scripts/README.md documents topology mode. From SESSION 20 END-2 escalation: (2) H8 rework LANDED as task 214 ✅ (SESSION 21). **Blocked**: (7) constants population + smoke-verify — awaits operator one-time SPE runbook execution. | FULL | sonnet / xhigh | none | 199, 212 |
| 215 | 🔲 | **SESSION 21 follow-on — Retire drifted SPE cert `spe-app-cert` + `spe-app-cert-pass`** (filed 2026-08-30 during task 214 completion). Grep-verify found LIVE consumers in `config/spaarke-resources.yaml` (declarative dev-env manifest, lines 452-455 + 492-493) + `scripts/Import-And-Register.ps1` (bootstrap script params, lines 6-8) — cert retirement needs its own deprecation pass before KV soft-delete. NOT blocking task 186 dispatch (H8-B works without cert retirement; retirement is hygiene). Scope: 3 items — (1) grep sweep to confirm consumer list, (2) deprecate consumers per each file's convention (config `_meta: status: retired` + script Option A archive), (3) KV soft-delete `spe-app-cert` + `spe-app-cert-pass` (RETAIN `spe-owning-app-secret` per E-1 exception). Sub-agent-safe, ~1-2h medium. Full context: POML 215-cert-retirement-spe-app-cert. | STANDARD | sonnet / medium | none | 214 |
| 214 | ✅ | **SESSION 20 H8 handler full rewrite (H8-B semantics) LANDED 2026-08-30** — Driven by task 213.2 Option A live-test empirical verification of topology doc §R5 (HTTP 403 accessDenied on `POST /storage/fileStorage/containerTypes` under `client_credentials` regardless of credential shape). Delivered: (1) Old `Handlers/SpeContainerType/` folder deleted (11 files); new `Handlers/SpeContainer/` folder with 8 files (`H8SpeContainerHandler.cs` + `GraphContainerProvisioner.cs` + `GraphAppOnlyContainerVerifier.cs` + `SpeConfidentialClientGraphFactory.cs` + `ISpeContainerProvisioner.cs` + `ISpeContainerVerifier.cs` + `SpeContainerOptions.cs` + `SpeContainerRejectionCodes.cs`); (2) New handler reads `containerTypeId` from `IntakeParameters.NonSecretParameters["containerTypeId"]` (populated by SKILL Step 4.0 from constants), calls `Containers.PostAsync` + `Containers[id].Activate.PostAsync` per topology doc §6; (3) Interface + records renamed `SpeContainerType*` → `SpeContainer*`; T6 trap detection removed from H8 (retained in `SpeConfidentialClientGraphFactory.IsDelegatedTokenTrapError` static for H13 verifier); (4) `ISpeContainerIdKvWriter` + `SecretClientSpeContainerIdKvWriter` deleted (no more per-customer KV write); (5) `SPE-ContainerTypeId` KV manifest slot semantics rewritten to `value_source: from-topology-constants`; (6) `HandlerDispatchRegistrationModule` + `Worker/Program.cs` DI updated; `DagAdvancer.cs` + `HandlerIds.cs` comments updated; H8→H3 DAG dep RETAINED (uses `InterStepState.BffAppRegId` as ClientCertificateCredential clientId); (7) 17 new `H8SpeContainerHandlerTests` (all pass). **Build**: 0 errors 0 warnings across Core + Worker + Api + Tests. **Full test suite**: 1919 pass / 0 fail / 1 skip (pre-existing). **Deferred** (out of 214's clean scope): cert retirement (`spe-app-cert` + `spe-app-cert-pass`) has live consumers in `config/spaarke-resources.yaml` + `scripts/Import-And-Register.ps1` — filed as follow-on task 214-cert-retirement (deprecate consumers → grep-verify → KV soft-delete). Full evidence: `runs/h8-live-test-2026-08-30.md`. | FULL | sonnet / xhigh | none | 199, 212, 213 |
...
| 203d | 🔲 (blocked-post-186; A32 SUPERSEDED SESSION 19) | Apply Class-A punch list — sub-phase nice-to-have (POST-186): **A33/A34 only** (h9-workflow cadence runbook + SC #11 env-var checks). **A32 SUPERSEDED** SESSION 19 2026-08-28 commit `e426191eb` (MED#10 H13 Cosmos-first refactor delivered SKILL Step 6a read-first-then-conditionally-PATCH behavior AND handles new Cosmos-first ordering). Gate: `deferred-post-186` per punch list §Sequencing. Reduced ~5h → ~4h. | STANDARD | sonnet / medium | none (parallel-safe:false) | 202, 186 |
