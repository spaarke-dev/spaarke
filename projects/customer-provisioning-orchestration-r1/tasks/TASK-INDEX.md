...
> **Task count**: 167 POMLs (78 original across 10 phases + 62 Phase C'' across 8 waves + 21 Pre-Live-Fire tasks: 202 + 203a-d + 204a-g + 205a/b/c/d/e/f/g/h/i SESSION 10 2026-08-25 + 4 SESSION 14 remediation tasks: 206 + 207 + 208 + 209 + 2 SESSION 20 dispatch-readiness tasks: 212 + 213)
...
| 213 | 🔲 | **SESSION 20 SPE topology reconciliation + H8 rework (H8-B)** — Reconcile r1 provisioning stack against SPAARKE-SPE-CONTAINER-TYPE-TOPOLOGY.md (owner-confirmed authoritative 2026-08-30, currently in `sdap-SPE-admin-app-r2` worktree pending merge). Scope: (1) cross-worktree merge coordination for the topology doc; (2) H8 rework — rename `H8SpeContainerTypeHandler` → `H8SpeContainerHandler`, swap `ContainerTypes.PostAsync` → `Containers.PostAsync + activate` (per §R5 delegated-only for container-TYPE creation, but app-only OK for container creation); (3) deprecate `scripts/Create-NewContainerType.ps1` (documented broken §7 + §8.1); (4) extend `Register-EntraAppRegistrations.ps1` for 6 topology-doc app-regs (3 owning apps + 3 BFF apps per §3A; only `Spaarke SPE Model 2 Owner` gets `AzureADMultipleOrgs`, all others `AzureADMyOrg`); (5) author `docs/guides/SPAARKE-SPE-TOPOLOGY-SETUP-RUNBOOK.md` (8-step operator runbook for one-time SPE topology setup via SPE Admin app / VS Code extension / SharePoint admin center); (6) SKILL Step 0.5c topology-verify addition + Step 0.5b BFF App Service search fix per task 212 Gap C; (7) operator one-time setup for Trial tier + constants population + smoke-verify. Blocks task 186 alongside 206/207. | FULL | sonnet / xhigh | none | 199, 212 |
