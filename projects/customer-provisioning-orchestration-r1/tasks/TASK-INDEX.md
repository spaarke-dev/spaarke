...
> **Last Updated**: 2026-08-25 SESSION 10 — **S1∥ mini-wave: 205b (A42) COMPLETE, 205a (A38) ESCALATED + RE-SCOPED**. 205b landed path (b) contract-parity (Fable/xhigh, 26 parity tests, task-130 I6 preserved, build 0/0, tests 1581 pass, 0 escalations, 0 ADR violations); Assert-SpaarkeFicTenancy ported + triple-keyed idempotency + FicExchangeOutcomeClassifier + FicVerificationState typed exit codes + CrossTenantFicRefusedException. 205a fired `partial-omit-set-discovered` at grep-verify Step 2 (5 live upsert sites beyond manifest — CORRECT trigger disposition per POML contract); owner APPROVED full re-scope split → **205a→A38a (revised, FileKvSecretManifest filter-served + ficOmitSecretNames seam extension) + 205g→A38b (customer.bicep gate) + 205h→A38c (script gates) + 205i→A44.5 (H7/L2 Worker credential seam)**. 4 new/revised POMLs authored via parallel workflow. Prior: 2026-08-25 SESSION 9 (Fable deep review of auth-v4 change request + all 11 owner decisions Q1-Q11 RESOLVED + S1∥ A36/A37/A40 landed + A35 master merge both directions); 2026-08-25 SESSION 8 (Task 203a COMPLETE); 2026-08-24 SESSION 7 (task 202 COMPLETE + Class-B verification pass + 11 POMLs authored for 203a/b/c/d + 204a/b/c/d/e/f/g); 2026-08-20 (Wave G-7 Batch G-7E TERMINAL — task 186 authoring + framework-level r1 E2E goal proof landed).
...
| 203c | 🔲 (blocked by 202) | Apply Class-A punch list — sub-phase skill wiring: A02/A03/A04/A15/A16 (Step 0.5 external prereqs reader + `--batch` flag + JSON schema + Step 7 postmortem + Grant-ControlPlaneIdentity.ps1 + 11 GraphAppRoles null GUIDs). Touches BFF-consumed shared code (A16) — publish-size verification REQUIRED per CLAUDE.md §10. ~15-20h. | FULL | sonnet / xhigh | none (parallel-safe:false; touches `.claude/**` + BFF) | 202 |
...
| 204b | ⏸ (blocked by 202; owner-decision-gate) | Class-B B04 multi-tenant Dataverse routing — ADR tension resolution FIRST per CLAUDE.md §6.5 three-path protocol, then apply chosen path. `DataverseServiceClientImpl.cs:37-39` reads single `Dataverse:ServiceUrl`; contradicts Model 1 per-tenant DV design intent. PAUSES at Step 2 for owner path-choice escalation (Path A ~1h doc / Path B ~16-40h ADR amendment + refactor / Path C scope-uncertain). Gates 204c (H13 probes reference registry client behavior). | FULL | opus / xhigh | none (parallel-safe:false; foundational) | 202 |
| 204c | 🔲 (blocked by 202 + 204b) | **HARD-BLOCKS TASK 186 E2E** — Class-B B07 H13 real probes (10 sub-tasks: T1/T2/T3/T4/T5/T6 traps + I2/I3/I4/I5 invariants — currently returning InfraFault). Sub-sub-phase into 204c-1..204c-10 if any single sub-task exceeds ~8h (escalation trigger `sub-task-effort-exceeded` at Step 0.5). ~40-80h. | FULL | sonnet / xhigh | none (parallel-safe:false; 10 sub-tasks touch same `E2EAcceptanceModule.cs` DI) | 202, 204b |
| 204d | 🔲 (blocked-post-186) | Class-B B11 staging-slot topology split — one-process L2 topology has staging-slot shadow-worker defect (reconciler + crash-recovery + dispatcher run against PRODUCTION Cosmos+SB during pre-production build deploys). Split into `.Core+.Api+.Worker` OR three permanent slot-sticky `Enabled` flags per DS-3 §5. Gate: `deferred-post-186` (accepted risk for first run; MUST-FIX for production traffic). ~16h. | FULL | opus / xhigh | none (parallel-safe:false; architectural) | 202, 186 |
...
| 205a | ⏸ **ESCALATED-REVISED (2026-08-25)** | **Auth-v4 integration — row A38a (revised from A38 post-escalation 2026-08-25)**: ORIGINAL A38 fired `partial-omit-set-discovered` at grep-verify Step 2 — 5 upsert sites verified beyond manifest generator (customer.bicep:705/713, Rotate-Secrets:588/661, Seed-ProductionKeyVault:123/178, Provision-Customer:533, plus H7/task-142 L2 Worker seam). Owner APPROVED full split: **205a→A38a (manifest half via LIVE FileKvSecretManifest — extend existing `ficOmitSecretNames` seam at H4KvSecretsPopulationHandler:179+:366 per task 126; filter served entries, respect FileKvSecretManifest:151 BINDING invariant, do NOT delete yaml row)**. Old POML file `205a-a38-h4-kv-manifest-secret-free-omit.poml` SUPERSEDED (git-rm'd); revised POML at `205a-a38a-h4-kv-manifest-secret-free-omit.poml`. **BINDING SEQUENCING**: A38a MUST commit-bundle with A38b (205g) — A38a alone re-opens false-migrated trap. Deps: A35 (landed); 205g (bundle sibling). ~4h. | FULL | fable / xhigh | S1∥ mini-wave revised (with 205g/h/i) | 202, 203a, 205g |
...
| 205c | 🔲 (blocked by 205a + 205b) | **Auth-v4 integration — row A39** (H4b `per_env_settings` — 8 §10.2 live-contract entries): `Graph__Credentials__Order__0=ManagedIdentityFederated` (sole entry) + `RequireSecretFreeIdentity=true` (fail-fast) + `ManagedIdentity__ClientId` (FromHandlerOutput) + 3 SB `FullyQualifiedNamespace` (NOT conn string) + 2 MI-Enabled flags. **ORDERING GUARD BINDING per §10.2**: identity + FIC + RBAC BEFORE these settings; A42 must land first (else fresh stamp boot-loops exit-134). No load-bearing key `required=false` (H4b:286 silent-skip trap). Tolerate FIC propagation flap (~130s AADSTS70025 window). Deps: A36/A37 (landed 1bc049e4c), A38 (205a), A42 (205b). ~4h. | FULL | sonnet / xhigh | none (parallel-safe:false; ORDERING GUARD dep on 205a+205b) | 202, 203a, 205a, 205b |
| 205d | 🔲 (blocked by nothing — A35 landed; dedupe-first) | **Auth-v4 integration — row A41** (H10 dual DV app-user + Q8 extension): TWO Dataverse app-user rows per env — (1) app-reg row (OBO), (2) UAMI row with `azureactivedirectoryobjectid = UAMI principalId` (NOT clientId — silent-fail trap). Dedupe against Wave-3E-053-H10-AppUser + ds8-uami-dv-appuser output FIRST (may collapse to probe-only, -2h). Extend T2 probe with principalId byte-equality. **Q8 SCOPE EXTENSION 2026-08-25 (+30 min)**: fix design.md:61 D3 wording drift ("Managed Identity" per-customer for Model 1 is stale) + add Naming Standards Model 1 UAMI row for `sprk-{env}-shared-bff-uami`. Fix design.md:153 figure drift (GraphAppRoles.cs source of truth). Deps: A35 (landed). ~3.5h (was 3; +0.5h Q8). | FULL | sonnet / high | groupAuthV4-parallel (with 205e; 205f main-session-only) | 202, 203a |
| 205e | 🔲 (blocked by nothing — A35 landed) | **Auth-v4 integration — row A43** (`Deploy-AllIndexes.ps1` silent-fallback gate — §10.5 trap 2): gate silent admin-key fallback ("KV secret AiSearch--AdminKey not found — falling back to live admin key") when `AiSearch__ManagedIdentity__Enabled=true` OR env carries secret-free marker → FAIL LOUD or use AAD-token path (Get-AzAccessToken `https://search.azure.com/`). Prevents silent Δ2 migration reversal on every re-deploy/rotation. Precedent pattern already in `Deploy-AllIndexes.ps1:617-660` (`-CutoverBffSettings` MI-check refusal). Deps: A35 (landed). ~2h. | FULL | sonnet / high | groupAuthV4-parallel (with 205d; 205f main-session-only) | 202, 203a |
| 205f | 🔲 (blocked by nothing — A35 landed; MAIN SESSION ONLY) | **Auth-v4 integration — row A44** (appsettings.template.json guard + §6.5 EDIT package + doc sweep): (a) consumer guard — H4b/H9 deploy paths skip `appsettings.template.json` `ServiceBus-ConnectionString` KV ref on secret-free envs (auth-v4 obligation 051-E owns template edit itself, deferred 2026-11-23); (b) apply §6.5 EDIT package (Q3 signed 2026-08-25) — `.claude/constraints/provisioning.md` + `spec.md:259/:275` + root CLAUDE.md §17 per `notes/decisions/adr-028-a4-integration-conflict-resolution.md` EDITs 1-4 verbatim; (c) doc sweep — replace stale 280-line `PROVISIONING-CHANGE-REQUEST.md` mirror with 626-line canonical + header naming canonical path; delete `PROVISIONING-CHANGE-REQUEST copy.md`; fix name-based UAMI resolution in 2 script help-texts (SF-1 residual); post-merge grep-confirm oauth-obo-patterns.md + BFF CLAUDE.md corrections present. Deps: A35 (landed); §6.5 owner sign-off (Q3 recorded APPROVED); A38a (205a — secret-free detection convention); A43 (205e — same marker convention). ~2.5h. | FULL | sonnet / high | none (parallel-safe:false; touches `.claude/**` + root CLAUDE.md) | 202, 203a, 205a, 205e |
| 205g | 🔲 (blocked by nothing — A35 landed; **BUNDLE-COMMIT with 205a**) | **Auth-v4 integration — row A38b (NEW, added 2026-08-25 per owner-approved A38 re-scope)**: `customer.bicep` secret-free gate on `kvSecretValues` (closes H2a re-seed vector). Add `requireSecretFreeIdentity` bool param (default false); on true, conditionally OMIT `AiSearch--AdminKey` + `ServiceBus-ConnectionString` from `kvSecretValues` map (Bicep conditional-object-property). Regenerate `customer.json`. Mirror `signalrEnabled` pattern at `customer.bicep:78`; wire H2a → deployment-param via existing `SignalREnabled` RunParameter → `H2aBicepInfraDeployHandler.cs:123+:276-288` template. **BINDING SEQUENCING**: MUST commit-bundle with 205a A38a — A38a manifest omit alone re-opens false-migrated trap if `customer.bicep` still re-seeds. Deps: A35 (landed); 205a (bundle sibling). ~2h. | FULL | sonnet / high | S1∥ mini-wave revised (with 205a/h/i) | 202, 203a, 205a |
| 205h | 🔲 (blocked by nothing — A35 landed) | **Auth-v4 integration — row A38c (NEW, added 2026-08-25 per owner-approved A38 re-scope)**: operator-script marker pre-check gates. Gate `Rotate-Secrets.ps1:588+661` (SB rotation platform + per-customer), `Seed-ProductionKeyVault.ps1:123+178` (SB + admin key), `Provision-Customer.ps1:533` (legacy 13-step SB write). Detection: KV tag `spaarke-secret-free-identity` OR `sprk_dataverseenvironment` state field `sprk_credentialmode=secret-free` (same convention as A38a). Gate behavior: FAIL LOUD (linking A38a omit contract) on marker present; current behavior on no-marker (backwards compat). Mirror A43 fail-loud shape at `Deploy-AllIndexes.ps1:617-660`. Deps: A35 (landed); coord with 205a on marker convention. ~1.5h. | FULL | sonnet / high | S1∥ mini-wave revised (with 205a/g/i parallel-safe:true) | 202, 203a |
| 205i | 🔲 (blocked by nothing — A35 landed; A42 landed via 205b ✅) | **Auth-v4 integration — row A44.5 (NEW, added 2026-08-25 per owner-approved A38 re-scope)**: H7/H6 + L2 Worker credential seam (closes H7/task-142 half of A30 sentinel contract). L2 Worker CONSUMES `BFF-API-ClientSecret` via task-142's unconditional `EnvVarValues__ClientSecret` KV-ref + `EnvVarValuesOptions.Validate` boot fail-fast → on secret-free env the L2 Worker BOOT-LOOPS (or invites §9.1 sentinel). Add FR-39-style ordered credential provider to H7/H6 (MI-FIC first for secret-free; client-secret fallback for backwards compat, mirroring master's `DataverseServiceClientImpl` A35-brought pattern). Make `modules/controlplane-worker-app-service.bicep` KV-ref conditional (mirror A38b pattern). Relax `EnvVarValuesOptions.Validate()` to accept empty ClientSecret under MI-FIC-first mode. Model 1 shared-Worker+shared-UAMI vs Model 2 per-customer-Worker+per-stamp-UAMI both asserted. Deps: A35 (landed); A42 (205b ✅ landed). ~4h. | FULL | fable / xhigh | S1∥ mini-wave revised (with 205a/g/h parallel-safe:true) | 202, 203a, 205b |
