...
> - 2026-08-27 (v3.7: **§4.2a sidecar base image amended** — `mcr.microsoft.com/powershell:7.4-mariner` → `mcr.microsoft.com/dotnet/runtime:10.0-azurelinux3.0` + pwsh 7.6.5 as a pinned `dotnet tool`. Path B (design amendment) per root CLAUDE.md §6.5, owner-approved 2026-08-27: the named image was not a sub-optimal choice to be excepted, it had become **factually wrong** — Mariner 2.0 reached end of life and MCR publishes a `7.4-mariner-2.0-EOL` tag beside the one §4.2a pinned. The image's own header claimed "no .NET runtime"; PowerShell 7 *is* a .NET application, so the base necessarily carried one (.NET 8.0.10, months stale), and that runtime was essentially the image's entire Trivy surface — 28 HIGH/CRITICAL. Bumping within the `powershell:*` line does not fix it: 7.4→.NET 8 (EOL 2026-11-10), 7.5→.NET 9 (STS), 7.6→.NET 10 LTS **unpublished as a base**. Inverting the dependency ends the churn and aligns the sidecar with `dotnet-10-upgrade-r1`. **Size claim now measured, not estimated**: 212 MB compressed (436 MB uncompressed) — the original ≈200–230 MB estimate held; the CI gate previously compared *uncompressed* bytes against a 350 MB proxy for the compressed ceiling and so failed a compliant image, and now measures `docker save \| gzip`. Also in the same change: Trivy `ignore-unfixed: false`→`true` (as written the gate also failed on CVEs with **no patch available** — a red no one could clear by any action; unfixed findings still reach the Security tab via the `if: always()` SARIF upload), and a `pull_request` trigger added so a Dockerfile change is built *before* it merges rather than after. §11.2 sidecar row updated to match. Shipped in PR #836. **Not amended**: the ≤250 MB ceiling, the sitecontainer topology, the auth legs, and the DS-1b §3–4 rejected-topology analysis all stand unchanged.)
...
**EXO sidecar**: one **sitecontainer** on the SAME App Service (Linux sitecontainers, GA — same Plan, same UAMI, same App Insights; zero new Azure resources beyond an ACR repo tag; B2's parity rationale preserved). Image (**amended v3.7, 2026-08-27** — was `mcr.microsoft.com/powershell:7.4-mariner`): `mcr.microsoft.com/dotnet/runtime:10.0-azurelinux3.0` + PowerShell **7.6.5** installed as a pinned `dotnet tool` in a discarded SDK build stage + pinned `ExchangeOnlineManagement` **3.5.1** + `Set-ExchangeApplicationAccessPolicy.ps1` + a ~60-line HTTP listener. **Measured 212 MB compressed / 436 MB uncompressed — inside this section's original ≈200–230 MB estimate and under the 250 MB ceiling; Trivy-gated in CI.** *Why the base inverted*: every published `powershell:*` tag rides a .NET release that is EOL or nearly so — 7.4→.NET 8 (EOL **2026-11-10**, the same deadline that drove `dotnet-10-upgrade-r1`), 7.5→.NET 9 (STS), and 7.6→.NET 10 LTS **has no base image published**. The original pin additionally sat on end-of-life Mariner 2.0 and shipped .NET 8.0.10, which produced 28 HIGH/CRITICAL Trivy findings — essentially the image's entire vulnerability surface — and made the hard gate unsatisfiable by construction (this workflow had never once been green). Basing on the .NET 10 LTS runtime and layering pwsh on top picks the runtime deliberately (supported to **Nov 2028**) and puts the sidecar on the same .NET 10 the rest of the estate migrated to, so there is one runtime story rather than two. A build-time contract check asserts the EXO module still resolves under pwsh 7.6 and that `Connect-ExchangeOnline` still exposes `-Certificate` (the auth mode this sidecar depends on), turning both plausible breakages into a red build instead of a runtime failure mid-provisioning. Revisit if MCR publishes `powershell:7.6-azurelinux-3.0` — that permits a single-stage `FROM`, but is a simplification, not a correctness fix. Contract: `POST http://localhost:8091/apply-policy` `{tenantId, expectedAppIds[], policyScopeGroupId, correlationId: RunId, timeoutSeconds}` → `{outcome: Success|Failure|AlreadyCompliant, policiesApplied[], diagnostic}` — mirrors the script's existing `Write-ResultJson` envelope. The C# `ExchangePolicySidecarClient : IExchangePolicyApplier` maps the envelope onto `HandlerResult` exactly as the shell-out applier mapped exit codes. Auth: (main→sidecar) localhost-only binding + per-boot shared-secret header from platform KV; (sidecar→Exchange) app-only `Connect-ExchangeOnline` with the PFX fetched from platform KV at call time under the same UAMI, passed as `-Certificate` (X509 object — thumbprint mode assumes a Windows cert store a Linux container lacks). No new idempotency layer: the script is get-before-set idempotent; sidecar HTTP failures map connection-refused/timeout → `InfraFault` (Resumable), structured `Failure` → existing H14 classification. Observability: `correlationId = RunId` per request; one structured JSON log line per request → same Log Analytics workspace. Build: same GitHub Actions workflow as the main deploy, monthly rebuild cadence (pwsh + one signed module — a quiet loop, not az CLI's Python tree). Why this residual exists at all, and why it will live years: no Graph API exists for `ApplicationAccessPolicy` **or** its designated successor RBAC-for-Applications (both EXO-PowerShell-only, verified 2026-08-18 — DS-1b §0 with Microsoft Learn cites); the R22 migration is a sidecar-script change behind `IExchangePolicyApplier`, not a handler change.
...
| **NEW: EXO sidecar image + sitecontainer** | ACR repo + `platform-controlplane.bicep` sitecontainer config + CI workflow stage | **NEW (v3.4, §4.2a)** | pwsh 7.6.5 on the .NET 10 LTS runtime base (v3.7) + pinned ExchangeOnlineManagement 3.5.1 + one script + HTTP listener; ≤250 MB ceiling (measured 212 MB compressed), Trivy-gated; monthly rebuild cadence. |
