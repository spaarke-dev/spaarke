...
| Field | Value |
|-------|-------|
| **Task** | Phase 1-4 ✅ (on master). **Phase 5 Round 1 ✅** — 043 (FR-21 privilege/privacy: DTO markers + permitted-recipient display, over-disclosure seam test, gate clean), 040 (FR-24 `sprk_ispinned` field CREATED live in spaarkedev1 — **041 must treat null as unpinned**), 044 (FR-23 search config — PREPARED; operator Maker-Portal steps in notes; cosmetic `description="test"` left on a Quick-Find view for operator cleanup). Verified: BFF build 0 err, shared-UI 157/157, tsc 0. **Round 2 ✅ 042** (attachments — SPE viewer reuse, attach-on-compose 25MB/MIME, access-filtered, no BFF change; 312+77 jest). **Round 3 IN FLIGHT: 041** (pin UI, persists `sprk_ispinned`, null=unpinned). After 041, all unblocked Phase 5 done → only 045 ⛔ (spine-blocked) + 046 ⏸ (deferred) remain before Phase 6 (050 deploy, 090 wrap-up). #675 still blocked. Portfolio 26/31. |
