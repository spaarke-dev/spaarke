...
4. **(Task 051 — OPERATOR-GATED, required for the runtime union to see `true`)** Grant the **BFF Dataverse Application User** field-level-security READ on `contact.sprk_standinggrant`. Because the field is `IsSecured=true` (§2), the platform silently strips it from an app-only retrieve when the reader's identity lacks FLS read — so `ContactStandingGrantReader` would read **every** contact as "no standing grant" (whole-feature-dark) with no error. Fix: add the BFF Application User (the systemuser the App Service managed identity maps to) as a **member of the `Standing Grant Administrators` Field Security Profile** (`f4be217b-b38f-f111-b8db-7ced8ddc4a05`, §2/§5.1 pattern) — read is sufficient; it does not need write. This was NOT applied live by task 051 (the code work is host-independent and ran without confirmed live Dataverse auth); it is a live config step for whichever environment runs the BFF. **Detection aid (task 051 hardening):** if the grant is missing, `ContactStandingGrantReader` now emits a `[WF-STANDING] ... secured attribute 'sprk_standinggrant' absent ...` **WARNING** per read — grep the BFF logs for it to confirm whether this step is outstanding.
