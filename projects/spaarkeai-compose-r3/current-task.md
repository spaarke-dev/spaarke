...
| **Next Action** | **Approach B is now the clear path** (verified 2026-07-17: Docxodus 7.1.0/net10 reproduces BOTH defects identically → version/net10 does NOT fix it; Codeuctivity same lineage → deprioritized). Owner confirms: **adopt design §4.2 Approach B** (graft comparer w:ins/w:del onto retained original — re-scope task 022; task-021 adapter's role narrows to revision-synthesizer). Then re-run `Nfr09RealTemplateHardeningTests` to certify → unblock 022. Meanwhile startable-without-gate: **task 030 (E3, deps 012✅)** or Toolset **040/042/043/044** (client, no deps). |
