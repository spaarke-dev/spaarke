...
| **Next Action** | **Option C VALIDATED (spike passed on real CSA) → recommended for task 022.** Self-synthesized paragraph redline (word-diff + native w:ins/w:del in place on the retained original) preserves 345/345 paraIds + 6/6 tables (WmlComparer: 0 + 5), no Docxodus, net8. Awaiting owner GO on the design pivot (§6.5). If adopted: task 022 builds `ComposeParagraphRedlineSynthesizer` for real + FR-05 format-change handling; **task 021 comparer retired from save path**; **task 001 Docxodus droppable**. Spike: `OptionC_SelfSynthesizedRedlineSpikeTests`. Meanwhile startable-without-gate: **task 030 (E3, deps 012✅)** or Toolset **040/042/043/044**. |
