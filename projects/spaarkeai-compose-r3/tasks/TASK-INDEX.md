...
| 030 | FR-13/FR-16 server-derived `confidence_band` (additive `ComposeDraftPayload`) + paraId/offsets on anchor. 🔄 **FOUNDATION LANDED 2026-07-18 (committed), LIVE-WIRING RESIDUAL → task 031.** Delivered + green: `ComposeConfidenceBand` enum + additive `confidence_band`/`paraId`/`start_offset`/`end_offset` on `ComposeDraftPayload` + pure `DeriveConfidenceBand`(cited-source AND retained-original-support ⇒ high; one ⇒ medium; neither ⇒ low; structurally immune to a spoofed self-report) + `ResolveParaIdAnchor` in `ComposeDraftDisposition.cs`; client-mirror fragment `ComposeDraftConfidenceFields` in `compose-contracts.ts`; 14 unit + 3 seam tests (Compose 465✅); ADR013 NetArchTest 2/2. **§6.5 Path-C (grep-verified):** POML premise wrong — `ComposeService.cs` never references `ComposeDraftPayload`; the live materialization is the read projection `GET /api/ai/chat/sessions/{id}/compose-outputs` (`ChatEndpoints.GetComposeOutputsAsync`), and `OutputRouter` stores the payload opaque (never parses). **RESIDUAL (folded into 031):** derivation is NOT yet invoked live — wire `DeriveConfidenceBand` into `GetComposeOutputsAsync`'s `ComposeDraftOutput` projection + mirror the fields into the REAL client `ComposeDraftPayload` (which lives in `ComposeEditor.tsx`, not `compose-contracts.ts`) + a LIVE seam assertion (band present on the wire). Offset live-resolution (FR-16) needs doc context — resolve with 031/032. | 3 E3 | 🟡 | 012 | 🔄 | FULL | sonnet | high | false (contract mirror) |
| 031 | FR-14 rationale-first, anti-rubber-stamp accept/reject surface (no auto-accept low-band) **+ finish 030's live band-wiring** (server: derive `confidence_band` in `ChatEndpoints.GetComposeOutputsAsync` projection; client: mirror `confidence_band`/`paraId`/offsets into the REAL `ComposeDraftPayload` interface in `ComposeEditor.tsx`; add a LIVE through-the-wire seam assertion per NFR-06/FR-16 — supersede 030's gap-documenting seam) | 3 E3 | 🔴 | 030 | 🔲 | FULL | sonnet | high | true (ComposeEditor UI) |
