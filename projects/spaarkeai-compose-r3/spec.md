...
13. **FR-13 (confidence band)**: Each AI redline carries a coarse `confidence_band` (`high`/`medium`/`low`) **derived deterministically from grounding/verifiability evidence** — cited-source presence + whether `target_text` resolves in the *live* document. **AMENDED 2026-07-18 (§6.5 Path B — supersedes "server-derived"):** the derivation runs **client-side at render**, not server-side. Rationale: the doc-relative grounding signals (does the target still resolve? re-anchor score) only exist where the payload and the live editor doc converge — the client; the band is a derived VIEW of the opaque payload exactly like the redline diff itself; and client derivation keeps the AI ledger path **envelope-only** (ADR-013/ADR-040 — the platform ships the payload opaque and never parses it). A deterministic function over grounding evidence is **not** a model self-report, so ADR-039 is upheld. **No AI catalog Action change** (engine frozen); **no server payload field** for the band/offsets. See [`docs/architecture/COMPOSE-REDLINE-DERIVED-VIEWS.md`](../../docs/architecture/COMPOSE-REDLINE-DERIVED-VIEWS.md). — Acceptance: a grounded suggestion renders `high`, an ungrounded one `low`, recomputed reactively as the doc changes; verified by a **client render test** (not a BFF seam — NFR-06 DoD follows the computation); no catalog row modified; no BFF wire change.
...
16. **FR-16 (paraId + offsets on the anchor)**: The redline anchor carries `paraId` (rides E2); precise character offsets are **resolved client-side against the live editor document** at render. **AMENDED 2026-07-18 (§6.5 Path B):** offsets are client-derived, not a server payload field — only the client holds the current paraId layout, so a server-computed offset would be stale the moment the user edits. `ComposeDraftPayload.paraId` is retained (an optional authored hint); `start_offset`/`end_offset`/`confidence_band` are **not** server fields (see FR-13 + `docs/architecture/COMPOSE-REDLINE-DERIVED-VIEWS.md`). — Acceptance: a redline round-trips to the exact paragraph (`paraId`) + offset, resolved client-side against the live doc.
...
| Confidence | How is the E3 band produced? | **Client-derived at render** (grounding + live-doc resolvability) — §6.5 Path B, supersedes "server-derived" | FR-13; no catalog change (NFR-04); envelope-only preserved (ADR-013/040) |
