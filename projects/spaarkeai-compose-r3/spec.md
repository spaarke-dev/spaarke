...
- **E1 — Retained-original delta save**: dirty saves apply edits as a delta onto the load-time original; untouched content preserved (structural/semantic). Adopt `Docxodus` `WmlComparer`; drop `docx.js` from export.
...
- **Recreating Word**: pagination/page layout, footnote/endnote numbering, complex multi-level numbering *authoring*, cross-reference/TOC field computation, print fidelity, full style *management* → deferred to **Open-in-Word** (already shipped).
- **Multi-user co-editing / CRDT** (R5+).
- **True byte-identity of untouched content** (Approach B splice-back) — MVP accepts WmlComparer's cosmetic re-serialization (Approach A); Approach B deferred unless a hard requirement (OOXML digital signatures) appears.
...
- `src/server/api/Sprk.Bff.Api/Services/Compose/` — NEW: paraId pre-parse + Docxodus splice orchestration; REUSE: `DocxAnnotationWriter.cs`, `DocxAnnotationReader.cs`, `AnnotationReanchorService.cs`.
- `src/server/api/Sprk.Bff.Api/Services/…/SpeFileStore` (facade) — NEW: fetch a driveItem **version's** content by `versionId`.
- `src/server/api/Sprk.Bff.Api/Sprk.Bff.Api.csproj` — add `Docxodus` 7.1.0 (**exclude SkiaSharp assets**); bump `DocumentFormat.OpenXml` 3.4.1 → 3.5.1.
...
1. **FR-01 (baseline inversion)**: On a **dirty** save, the persisted document MUST be derived from the **load-time original OOXML**, NOT a TipTap reconstruction. `docx.js` is removed from the export path. — Acceptance: a save after editing a formatted doc no longer routes through `tipTapJsonToDocxBytes`; a through-the-wire test asserts the saved doc's untouched paragraphs match the original (structurally/semantically).
2. **FR-02 (edited-paragraph rebuild + splice)**: The server rebuilds **only the edited paragraphs'** OOXML (keyed by `w14:paraId`) and splices them into a copy of the retained original; untouched paragraphs are not rebuilt. — Acceptance: given an N-paragraph doc with K edited, exactly K paragraphs differ pre-comparer.
3. **FR-03 (redline synthesis via Docxodus)**: The server runs `Docxodus` `WmlComparer` (7.1.0, MIT) between the retained original and the spliced-edited document to synthesize minimal `w:ins`/`w:del`, and persists the comparison output. — Acceptance: 3 edited paragraphs → minimal ins/del; author attribution set; no exception on nested tables / numbering / delete / split (per S1/S1b).
...
5. **FR-05 (run-level format fidelity — D4)**: Inline run-formatting edits (bold/italic/font) inside a changed paragraph are represented via WmlComparer Format-Change Detection (not delete+re-insert). — Acceptance: bolding a word yields an `rPr`/`pPrChange`, not a full-run del+ins (per S1).
6. **FR-06 (baseline retrieval — S4)**: The baseline is the **load-time SPE version fetched by `versionId`** (captured at Load). `SpeFileStore` is extended to fetch a specific driveItem version's content. Client-retained `state.docxBytes` is a same-session fast-path; a size-capped Redis cache (ADR-009, Tier-3) is the fallback. — Acceptance: a save after a page refresh (client bytes gone) still applies the delta onto the correct load-time version.
7. **FR-07 (fidelity guarantee — structural/semantic)**: Untouched content — paragraph text, `w14:paraId`, styles, numbering, headers/footers, footnotes, tables (incl. nested) — is preserved across a dirty save. WmlComparer's cosmetic re-serialization (BOM/whitespace) is acceptable (Approach A). — Acceptance: the flagship round-trip (Success Criteria) shows all listed structures intact after edit+save+reopen.
...
- **NFR-01 (publish size)**: BFF publish ≤ 60 MB compressed. `Docxodus` net add ≈ 2.44 MB (managed) with **SkiaSharp assets excluded** (validated S3); measure the delta on every BFF-touching task vs the ~49.63 MB baseline; ≥ +5 MB single-task → justify.
...
- ✅ MUST exclude SkiaSharp assets when adding Docxodus; MUST NOT invoke Docxodus `HtmlToWml`/`FormattingAssembler` (would re-pull SkiaSharp).
