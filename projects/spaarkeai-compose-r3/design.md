...
### 4.4 Born-in-editor documents — the SERVER authors the `.docx` (FR-01a, added 2026-07-18)

§4.1–4.3 assume a **retained original** exists. A document **born in the editor** — AI-drafted (the `compose-draft-document` capability seeds `body_html`; `ComposeWorkspace` mounts it via `mountDraftHtml`, `docxBytes=null`), blank, or browse-local — has **no original to delta against**. R2/R1 saved these through the client `docx.js` writer (`tipTapToDocxBytes`), which flattens multi-level numbering, real styles, and firm formatting — so an AI-drafted *legal* document (whose value **is** its numbered-clause structure) was degraded the moment it was first saved. That is the other half of the fidelity defect FR-01 fixes for edits.

**Resolution — the server is the single authority for all `.docx` authoring.** This is not a workaround; it is the industry-correct architecture. Harvey (the leading legal-AI drafter) builds and edits documents with **deterministic server-side OOXML code and confines the LLM to text** ("asking a model to do legal reasoning *and* XML parsing regresses both"); client-side JS exporters are *generators, not fidelity round-trippers*. Two server engines, one authority:

| Save case | Server engine | Fidelity |
|---|---|---|
| Loaded doc, dirty edit | `ComposeParagraphRedlineSynthesizer` — delta onto retained original (§4.2, built) | untouched paragraphs **byte-identical** |
| Loaded doc, clean save | passthrough of the retained original (FR-06a) | byte-identical |
| **Born-in-editor** (AI-draft / blank / browse-local) | **`ComposeDocumentRenderer`** — content model → high-fidelity `.docx` (NEW, task 026) | real styles + **style-linked multi-level numbering** (FR-27) + native tables + minted `w14:paraId` |

**The unified client contract**: the client is a **pure editing surface**. It holds a full paraId-keyed model in `editor.getJSON()` (every paragraph/heading carries a hidden `paraId`) and hands that model to the server — it **never authors `.docx` bytes** (task 027 removes `docx.js`). The server decides the operation by whether a retained original exists. The AI draft the server already holds as `body_html` is the *seed*; the authoritative save-time source is the client's **current** content model (the user may have edited the seed).

**Numbering is authored, not faked (FR-27)**: for a born-in-editor legal doc, the renderer emits a real **style-linked** `NumberingDefinitionsPart` (one multi-level `abstractNum`, `%N` `lvlText` cascades, `isLgl`, per-list `lvlRestart`) referenced via the heading/list styles — *not* a direct paragraph `numId` (which double-numbers) and *not* literal "1. " text runs (the `DocxExportService` shortcut). Numbering must be instance-clean at birth: a malformed `numId` stays invisible until a later tracked-change edit corrupts the redline (it cannot be cheaply retrofit). This makes multi-level numbering *authoring* in-scope for the render path (reconciling the earlier out-of-scope note — see spec Out-of-Scope 2026-07-18 reconciliation); numbering *preservation* on the edit path stays by-construction.

**ADR-039**: server-side rendering-from-a-content-model is **deterministic OOXML authoring, NOT an AI dispatch** — no new dispatch endpoint, no catalog rows, no `Services/Ai` types. It reuses the existing `create-on-save` route (§9). No ADR-039 conflict (§11).
...
- **On save**, the paraId map is the splice key for E1's Option C redline synthesis (`ComposeParagraphRedlineSynthesizer`) — the same substrate serves both.
...
SAVE (dirty):
  baseline = retained ORIGINAL bytes (NOT a TipTap reconstruction)      ◄── E1: drop docx.js export
    ...
    │     └─ ComposeParagraphRedlineSynthesizer (Option C — word-level diff → native w:ins/w:del in place)  ◄── self-synthesized, no external diff engine
    ├─ apply AI redlines + comments via DocxAnnotationWriter             ◄── R2, reused verbatim
    └─ untouched paragraphs preserved byte-identical (same content/       ◄── the fidelity guarantee
       styles/numbering/paraId) — Option C edits only the changed
       paragraphs in place, so the rest is never re-serialized
       (byte-preservation by construction; no Approach A/B distinction).
  ──► SPE ReplaceFileContentAsUserAsync (If-Match, atomic)              ◄── ComposeService.cs:1107 pattern
```

The load-bearing change is a single inversion: **the save baseline becomes the retained original, and edits are deltas onto it** — instead of the baseline being a reconstruction and the original being discarded. Everything else is reused or off-the-shelf: the annotation writer, the accept/reject workflow, and the atomic If-Match write already exist (R2); the paragraph-redline synthesis is `ComposeParagraphRedlineSynthesizer` (Option C — a self-synthesized word-level diff → native `w:ins`/`w:del` in place, no external diff engine). The only genuinely new code is the paraId carry-through (E2) and the save-path orchestration that rebuilds edited paragraphs and drives the synthesizer.
...
| Editor framework | TipTap **MIT base** + R2 custom marks (`insertion`/`deletion`/`commentAnchor`) | + `paraId` block attribute (E2, via MIT `@tiptap/extension-unique-id` 2.27.2 — split-minting built-in, S2; `generateID` → OOXML-shaped); **NO TipTap Pro** |
...
| **Paragraph redline synthesis** (loaded-doc edit) | — | **`ComposeParagraphRedlineSynthesizer` (Option C — pure OpenXML, no external diff engine, no new NuGet)** — self-synthesized word-level diff → native `w:ins`/`w:del` in place. Supersedes the reversed Docxodus `WmlComparer` path. |
| **Born-in-editor document authoring** (no retained original) | `DocxExportService` from-scratch pattern (`WordprocessingDocument.Create`, tables, style catalog) + Compose run/paraId-mint idioms | **`ComposeDocumentRenderer` (Option C creation — task 026, pure OpenXML, no new NuGet)** — content model → high-fidelity `.docx`: real styles + **style-linked multi-level numbering** (FR-27, the net-new surface — zero numbering authoring exists today) + native tables + minted `w14:paraId`. The from-scratch counterpart to the synthesizer; replaces the removed client `docx.js`. |
...
| **Server-side `.docx` rendering from a content model** (`ComposeDocumentRenderer`, born-in-editor, FR-01a) | ADR-039 (frozen AI engine) | **Path C — comply (non-conflict)** | The renderer is **deterministic OOXML authoring**, not an AI dispatch — no new dispatch endpoint, no catalog rows, no `Services/Ai` types, no LLM call. It consumes the client content model on the existing `create-on-save` route and emits bytes (same class as `ComposeParagraphRedlineSynthesizer` / `DocxAnnotationWriter`). ADR-039 governs the AI seam; this is not it. §11 three-question test: *existing* = client `docx.js` (lossy, being removed) + `DocxExportService` (fake numbering, wrong consumer); *extension* = neither authors style-linked multi-level numbering; *cost-of-doing-nothing* = AI-drafted legal docs born as degraded `.docx`. |
| **Multi-level numbering authoring in scope** (FR-27) | spec Out-of-Scope (numbering *authoring* was deferred) | **Path B — supersede the out-of-scope note** | The deferral assumed every doc has a retained original (numbering always preserved, never authored). Born-in-editor rendering (FR-01a) makes numbering *authoring* first-class for legal clauses; research shows it must be correct at birth or later redlines corrupt it. Numbering *preservation* on the edit path stays by-construction. Reconciled in spec Out-of-Scope (2026-07-18). |
...
3. **Publish-size** *(reconciled 2026-07-18 — Docxodus reversed)*: **no new NuGet.** Both Option C engines — `ComposeParagraphRedlineSynthesizer` (edit) and `ComposeDocumentRenderer` (born-in-editor authoring, incl. multi-level numbering) — are pure `DocumentFormat.OpenXml` (3.5.1, already referenced), so there is **~0 MB package delta** and no SkiaSharp/`ExcludeAssets` concern at all. Current baseline (task 022 Increment A): **46.00 MB** compressed incl PDBs, well under the 60 MB ceiling. Measure per BFF-touching task; ≥ +5 MB single-task → justify. *(The prior Docxodus 2.44 MB + SkiaSharp-exclusion analysis is obsolete decision-history — see the §4 amendment banner.)*
