...
│                                                                              │
│   ┌──────────────┐      ┌──────────────┐      ┌──────────────┐              │
│   │   Extract    │      │   Analyze    │      │    Detect    │              │
│   │   Entities   │─────▶│   Clauses    │─────▶│    Risks     │              │
│   │              │      │              │      │              │              │
│   │ ┌──────────┐ │      │ ┌──────────┐ │      │ ┌──────────┐ │              │
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     AI WORKFLOW BUILDER SPECTRUM                             │
│                                                                              │
│  General Purpose ◀──────────────────────────────────────▶ Domain Specific   │
│                                                                              │
│  n8n          Langflow      Flowise       Spaarke Playbook Builder          │
│  (automation) (LLM chains)  (chatbots)    (legal document intelligence)     │
│                                                                              │
│  ────────────────────────────────────────────────────────────────────────── │
│                                                                              │
│  Code-Heavy ◀────────────────────────────────────────────▶ No-Code          │
│                                                                              │
│  LangGraph    CrewAI       Langflow       Spaarke (target: low-code)        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │                  Playbook Graph Engine (BFF API)                        ││
│  │  • Graph validation and compilation                                     ││
│  │  • Execution sequencing                                                 ││
│  │  • Data flow management                                                 ││
│  │  • Scope resolution (Skills, Tools, Knowledge)                          ││
│  │  • Output assembly                                                      ││
│  └─────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
...
│                                                                              │
│  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐ ┌───────────────┐   │
│  │  Tool         │ │  Skill        │ │  Knowledge    │ │  Output       │   │
│  │  Handlers     │ │  Injectors    │ │  Retrievers   │ │  Formatters   │   │
│  │               │ │               │ │               │ │               │   │
│  │ • Summary     │ │ • Prompt      │ │ • RAG Search  │ │ • Markdown    │   │
│  │ • RiskDetect  │ │   Enhancement │ │ • Doc Lookup  │ │ • Structured  │   │
│  │ • ClauseComp  │ │ • Rubrics     │ │ • Standards   │ │ • Chart Data  │   │
│  │ • DateExtract │ │ • Heuristics  │ │               │ │               │   │
│  └───────────────┘ └───────────────┘ └───────────────┘ └───────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ ┌─────────────────────────────────────────────────────────────────────────┐ │
│ │  Playbook: Software License Agreement Analysis            [Save] [Run] │ │
│ └─────────────────────────────────────────────────────────────────────────┘ │
├────────────────┬────────────────────────────────────────────┬───────────────┤
│                │                                            │               │
...
│                │                                            │               │
│ ┌────────────┐ │    ┌──────┐    ┌──────┐    ┌──────┐       │ ┌───────────┐ │
│ │ + Actions  │ │    │Entity│───▶│Clause│───▶│ Risk │       │ │ Detect    │ │
│ │            │ │    │Extract│   │Analyze│   │Detect│       │ │ Risks     │ │
│ │ ○ Extract  │ │    └──────┘    └──────┘    └──┬───┘       │ ├───────────┤ │
│ │ ○ Analyze  │ │                               │           │ │           │ │
│ │ ○ Detect   │ │                               ▼           │ │ SKILLS    │ │
│ │ ○ Compare  │ │    ┌──────┐              ┌──────┐         │ │ [+] Add   │ │
│ │ ○ Generate │ │    │Compare│─────────────▶│Summary│        │ │ ☑ License │ │
│ │ ○ Custom   │ │    │Std   │              │      │         │ │   Risk    │ │
│ │            │ │    └──────┘              └──────┘         │ │   Heurist.│ │
│ ├────────────┤ │                                            │ │           │ │
│ │ + Connectors│ │                                            │ ├───────────┤ │
...
│  │  │                                                               │ │    │
│  │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐        │ │    │
│  │  │  │ 📚 Skills │ │ 🔧 Tools  │ │ 📖 Knowl. │ │ 📄 Output │        │ │    │
│  │  │  │    (2)   │ │    (1)   │ │    (2)   │ │    (1)   │        │ │    │
│  │  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘        │ │    │
│  │  │                                                               │ │    │
│  │  │  Status: Ready                          Est. Time: ~15s      │ │    │
│  │  └───────────────────────────────────────────────────────────────┘ │    │
│  │                                                                     │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
...
│  │  │   "Applies SaaS-specific risk identification patterns"      │   │    │
│  │  │                                                              │   │    │
│  │  │ ☑ Risk Severity Calibration                                  │   │    │
│  │  │   "Maps findings to High/Medium/Low severity levels"         │   │    │
│  │  │                                                              │   │    │
│  │  │ [+ Add Skill]                                                │   │    │
│  │  └─────────────────────────────────────────────────────────────┘   │    │
│  │                                                                     │    │
...
│  │  ┌─────────────────────────────────────────────────────────────┐   │    │
│  │  │ 🔧 Risk Detector                                             │   │    │
│  │  │    Handler: RiskDetectorHandler                              │   │    │
│  │  │    Input: Clause analysis results                            │   │    │
│  │  │    Output: Risk register with severity                       │   │    │
│  │  └─────────────────────────────────────────────────────────────┘   │    │
│  │                                                                     │    │
│  │  KNOWLEDGE                                                          │    │
│  │  ┌─────────────────────────────────────────────────────────────┐   │    │
...
│  │  │    Embedding: Cohere-Legal (R6)                              │   │    │
│  │  │                                                              │   │    │
│  │  │ 📖 Standard License Positions                                │   │    │
│  │  │    Type: RAG Index                                           │   │    │
...
│  │  │ Format:  Table with columns: Risk, Clause, Severity, Notes  │   │    │
│  │  │ Order:   3 (after Clause Analysis, before Summary)          │   │    │
│  │  └─────────────────────────────────────────────────────────────┘   │    │
│  │                                                                     │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
...
│  ┌──────┐    ┌──────┐    ┌──────┐    ┌──────┐                              │
│  │ ✓    │═══▶│ ✓    │═══▶│ ⚠    │═══▶│ ✓    │   ⚠ = Warning (partial)    │
│  │ 2.1s │    │ 8.3s │    │ 4.5s │    │ 3.2s │   ✗ = Error                 │
│  └──────┘    └──────┘    └──────┘    └──────┘                              │
│                                                                              │
│  CLICK ON NODE TO SEE:                                                      │
│  ┌─────────────────────────────────────────────────────────────┐            │
│  │ Execution Details: Detect Risks                             │            │
│  ├─────────────────────────────────────────────────────────────┤            │
...
│  │                                                              │            │
│  │ Input Data:                                                 │            │
│  │   └─ 15 clauses from previous node                         │            │
│  │                                                              │            │
│  │ Output Data:                                                │            │
│  │   └─ 7 risks identified (3 High, 2 Medium, 2 Low)          │            │
│  │                                                              │            │
│  │ Warnings:                                                   │            │
│  │   └─ "Indemnification clause ambiguous - manual review"    │            │
│  │                                                              │            │
│  │ [View Full Output] [Re-run This Node]                       │            │
│  └─────────────────────────────────────────────────────────────┘            │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│  1. COMPILATION                                                             │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │ Canvas JSON → Validate → Topological Sort → Execution Plan      │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                      │                                       │
│                                      ▼                                       │
│  2. EXECUTION (per node)                                                    │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │                                                                 │     │
│     │  ┌──────────────┐   ┌──────────────┐   ┌──────────────┐        │     │
│     │  │ Collect      │   │ Resolve      │   │ Build        │        │     │
│     │  │ Input Data   │──▶│ Scopes       │──▶│ Prompt       │        │     │
│     │  │ (from edges) │   │ (Skills,     │   │              │        │     │
│     │  │              │   │  Knowledge)  │   │              │        │     │
│     │  └──────────────┘   └──────────────┘   └──────────────┘        │     │
│     │         │                  │                  │                 │     │
│     │         ▼                  ▼                  ▼                 │     │
│     │  ┌──────────────┐   ┌──────────────┐   ┌──────────────┐        │     │
│     │  │ Execute      │   │ Format       │   │ Store        │        │     │
│     │  │ Tool Handler │──▶│ Output       │──▶│ Result       │        │     │
│     │  │              │   │ (section)    │   │              │        │     │
│     │  └──────────────┘   └──────────────┘   └──────────────┘        │     │
│     │                                                                 │     │
│     └─────────────────────────────────────────────────────────────────┘     │
...
│  3. ASSEMBLY                                                                │
│     ┌─────────────────────────────────────────────────────────────────┐     │
│     │ Collect all node outputs → Order by OutputSpec → Render Work    │     │
│     │ Product (Markdown/DOCX/PDF)                                     │     │
│     └─────────────────────────────────────────────────────────────────┘     │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│  └──────┘    └──────┘    └──────┘                                          │
│              │                                                               │
│              └─ "Detect Risks: No tool configured"                          │
│                                                                              │
│  CIRCULAR DEPENDENCY:                                                       │
│  ┌──────┐    ┌──────┐                                                       │
│  │ ✗    │◀──▶│ ✗    │     (edges highlighted red)                          │
│  └──────┘    └──────┘                                                       │
│  "Error: Circular dependency detected between Node A and Node B"            │
│                                                                              │
│  ORPHAN NODE:                                                               │
│  ┌──────┐    ┌──────┐    ┌──────┐                                          │
│  │ ✓    │───▶│ ✓    │    │ ⚠    │  (no connections)                        │
│  └──────┘    └──────┘    └──────┘                                          │
│                          │                                                   │
│                          └─ "Warning: Node not connected to workflow"       │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                       EXTENDED DATAVERSE SCHEMA                              │
│                                                                              │
│  sprk_analysisplaybook (existing)                                           │
│  ├── sprk_playbookid (PK)                                                   │
│  ├── sprk_name                                                              │
│  ├── sprk_description                                                       │
│  ├── sprk_documenttypes                                                     │
│  ├── sprk_canvasjson (NEW - stores graph definition)                        │
│  ├── sprk_version (NEW - for versioning)                                    │
│  └── sprk_isvisualmode (NEW - flag for visual vs legacy)                    │
│                                                                              │
│  sprk_actionnode (NEW)                                                      │
│  ├── sprk_actionnodeid (PK)                                                 │
│  ├── sprk_playbookid (FK → sprk_analysisplaybook)                           │
│  ├── sprk_nodetype (Choice: Extract, Analyze, Detect, Compare, Generate)   │
│  ├── sprk_name                                                              │
│  ├── sprk_position_x (canvas position)                                      │
│  ├── sprk_position_y                                                        │
│  ├── sprk_toolid (FK → sprk_analysistools)                                  │
│  ├── sprk_outputsection                                                     │
│  ├── sprk_outputformat (Choice)                                             │
│  ├── sprk_outputorder (int)                                                 │
│  └── sprk_configjson (tool-specific parameters)                             │
│                                                                              │
│  sprk_actionnode_skill (N:N junction)                                       │
│  ├── sprk_actionnodeid (FK)                                                 │
│  └── sprk_skillid (FK → sprk_analysisskills)                                │
│                                                                              │
│  sprk_actionnode_knowledge (N:N junction)                                   │
│  ├── sprk_actionnodeid (FK)                                                 │
│  └── sprk_knowledgeid (FK → sprk_analysisknowledge)                         │
│                                                                              │
│  sprk_nodeedge (NEW - connections between nodes)                            │
│  ├── sprk_edgeid (PK)                                                       │
│  ├── sprk_playbookid (FK)                                                   │
│  ├── sprk_sourcenodeid (FK → sprk_actionnode)                               │
│  ├── sprk_targetnodeid (FK → sprk_actionnode)                               │
│  └── sprk_edgetype (Choice: DataFlow, Conditional)                          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  R4 (Current): Table-Driven Configuration                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ sprk_analysisplaybook                                               │    │
│  │   └── N:N → sprk_analysisactions                                    │    │
│  │              └── N:N → sprk_analysisskills                          │    │
│  │              └── N:N → sprk_analysistools                           │    │
│  │              └── N:N → sprk_analysisknowledge                       │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                      │                                       │
│                              Auto-convert                                    │
│                                      ▼                                       │
│  R6+ (New): Node-Based Visual                                               │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ sprk_analysisplaybook                                               │    │
│  │   ├── sprk_canvasjson (graph)                                       │    │
│  │   └── sprk_actionnode (per action)                                  │    │
│  │          └── N:N → sprk_analysisskills                              │    │
│  │          └── FK → sprk_analysistools                                │    │
│  │          └── N:N → sprk_analysisknowledge                           │    │
│  │          └── OutputSpec embedded                                    │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  COMPATIBILITY:                                                             │
│  • R4 playbooks continue to work (isVisualMode = false)                     │
│  • "Convert to Visual" migrates R4 → R6 format                              │
│  • Both modes use same execution engine                                      │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                              │
│  From R5 Design:                                                            │
│  • Knowledge sources have configurable embedding models                      │
│  • Each Knowledge scope can use different model (Cohere, Legal-BERT, etc.) │
│                                                                              │
│  Visual Builder Shows:                                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │ KNOWLEDGE                                                            │    │
│  │ ┌─────────────────────────────────────────────────────────────────┐ │    │
│  │ │ 📖 Risk Taxonomy                                                 │ │    │
│  │ │    Type: Structured Reference                                    │ │    │
│  │ │    Embedding: Cohere-Legal                    [Configure RAG]   │ │    │
│  │ │              ↑                                                   │ │    │
│  │ │    (From R5 sprk_embeddingmodel field)                          │ │    │
│  │ │                                                                  │ │    │
│  │ │ 📖 Standard License Positions                                    │ │    │
│  │ │    Type: RAG Index                                               │ │    │
│  │ │    Embedding: text-embedding-3-small                            │ │    │
│  │ │    Index: contoso-legal-small                                   │ │    │
│  │ └─────────────────────────────────────────────────────────────────┘ │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
// Playbook compilation (canvas → execution plan)
...
| Deliverable | Description |
|-------------|-------------|
| Compile API | Canvas → execution plan |
