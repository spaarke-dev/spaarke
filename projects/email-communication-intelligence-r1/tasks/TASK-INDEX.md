...
| 031 | Job B apply endpoint → `IActionSeam.UpdateRecordAsync` under **impersonation (`MSCRMCallerID` = confirming user)** + `sprk_emailreviewlog` audit row | P3 | bff-api, communication | FR-10 | 030, 012 | P3 | **false** (shared endpoints) | FULL | opus/high | ✅ (2026-07-29 — Option 2 impersonation; 6 seam tests; code-review SHIP-WITH-FIXES all invariants HOLD; see `notes/031-write-identity-decision.md`) |
...
| 041 | Attachment-grounded action extraction — ground Action on extracted attachment text, gated to action-triggers | P4 | bff-api, catalog | FR-13 | 040 | P4 | **false** (shared) | FULL | opus/high | ✅ (2026-07-29 — Path B machine-verified per-attachment/page locator per owner §6.5; extends extractor+envelope additively, flat AttachmentText byte-unchanged; new `email-attachment-action` step reuses 040 Action+facade+create leg (no fork); cost gate + code-derived page; 12 tests + eval; code-review SHIP-WITH-FIXES, adr-check CLEAN; W1 dup-extraction documented Path A. See `notes/041-attachment-locator-escalation.md`) |
| 042 | Regarding-vs-related intent — classify file/update/new-record; demote referenced identifier on "new filing based on X" (suppress auto-file → Suggested); note cross-ref in triage summary; propose gated create-new-record | P4 | bff-api, catalog, communication | FR-12 | 020, 022 | P4 | **false** (shared) | FULL | opus/xhigh | ✅ (2026-07-30 — **owner dissolved the ADR-024 tension in-session**: ONE relationship = `regarding`, NO related field / second mechanism; cross-refs noted in `sprk_triagesummary`. ADR-024 AMENDED + spec §6.5/FR-12 re-cast. Deterministic `NewRecordIntentDetector` (no LLM Action) → in-rung capture-time confidence cap (0.90→0.65, demote-only, safe-by-construction) + best-effort `email-regarding-intent` enrichment step (summary note + gated create-new Proposed row, human-confirmed). 21 tests (15 detector golden-utterance/precision + 2 rung suppression + 4 seam). Build clean; publish ~47 MB compressed (Δ≈0); no new CVE. code-review SHIP, adr-check CLEAN. See `notes/042-regarding-vs-related-owner-decision.md`) |
...
| 051b | M365-group-mailbox capture — forked pipeline | P5 | bff-api, communication | FR-15 | 050 | P5 | **false** (capture path) | FULL | sonnet/high | ⛔→**DESCOPED** 2026-07-29 (owner; §6.5 path A — central/shared mailbox covered by 051a; group deferred, no forked pipeline / no tenant-wide `Group.Read.All`) |
