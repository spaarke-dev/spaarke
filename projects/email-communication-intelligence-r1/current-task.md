...
| **061 UAT ROUND 3 (2026-07-31) — core-only auto-association** | Owner: "only auto-associate to CORE records (matters, projects); contacts/orgs/invoices/etc. can be suggestions but NEVER auto-associated." Owner-selected core set (AskUserQuestion): **matter + project + service request**. **Diagnosis** (live provenance comm `cfd3f282`): two matters conflicted → Ambiguous (correct), but `sprk_regardingperson`=Ralph was `written:true` (ParticipantCorrelation 0.70 + ThreadContinuity 0.65 + ContactNameMatch 0.68 → 0.966) → r5 showed contact under "Filed automatically". **Fix** (commit `68b612a22`, single chokepoint `AssociationStatusMapper`): config-driven `CoreWritableEntities` gate (`Communication:AutoFile:CoreWritableEntities` default matter/project/service-request; ADR-018, per-tenant override) via `AutoFileGate`; `AddWrites` skips non-core → candidate-only (surfaced in provenance, never written); auto-file keys off `topDetCore`. Resolver unchanged (writes only `RegardingWrites`); outbound `SendAsync` unaffected. Tests: +4 mapper (incl. exact UAT regression) + flipped old "contact still written" assertions in mapper+resolver → **51/51 pass**; build clean; publish ~47 MB (Δ≈0), no new CVE. **DEPLOYING to spaarke-bff-dev** (bg `bzg83ktke`). See `notes/061-uat-round3-core-only-autofile.md`. |
