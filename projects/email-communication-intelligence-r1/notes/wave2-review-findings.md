...
## 023 — triage enrichment trigger — ACCEPTED (two flags for 060/wrap-up)
1. **FR-06 RAG grounding — mechanism-proven but inert (PRE-EXISTING data gap).** 023 wired matter-scoped RAG grounding (`IRagService.SearchAsync` via `BoundInputs.RecordMemoryFragment`) correctly, but the inbound/outbound RAG-INDEXING call sites pass `ParentEntity: null`, so communications are not yet matter-tagged in the index → grounding returns empty in practice until that pre-existing tagging gap closes. NOT 023's file scope. **Action**: fix the ParentEntity tagging at the indexing call sites (candidate for a 060-adjacent task or a fast-follow); until then FR-06 grounding is structurally present but produces no retrieved context. Documented in `notes/023-triage-email-wiring.md`.
2. **Pre-existing HIGH CVE — `System.Security.Cryptography.Xml`** (transitive, csproj untouched — NOT introduced by r1). Surface for the wrap-up security review; not a blocker for any r1 task (no r1 task adds it).
