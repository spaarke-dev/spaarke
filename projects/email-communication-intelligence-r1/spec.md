...
12. **FR-12 (Regarding-vs-related intent)**: Classify intent `file-to-existing | update-existing | new-record` (a communication that *presents a new record while referencing an existing one*). When phrasing like "new filing based on X" / "new litigation matter related to LIT-123456" indicates a new record, **suppress the misfile onto the referenced record X** — demote X's explicit-ID match from `Resolved`/auto-file to `Suggested` so a human decides — **note the cross-reference in `sprk_triagesummary`** (the reference is informational, not a structural link), and **propose creating the new record** (gated `dataverse.create_record`, human-confirmed, nothing auto-finalizes). **No `related` field and no second regarding mechanism** — `regarding` is the one direct relationship (ADR-024 amended 2026-07-30; owner decision `notes/042-regarding-vs-related-owner-decision.md`). — Acceptance: "new filing based on PAT-908068" does NOT auto-file onto PAT-908068; it lands `Suggested`, the summary notes the PAT-908068 reference, and a "create new record" proposal is offered (file-onto-X / create-new / dismiss are the reviewer's choices).
...
| ADR-024 | regarding family expresses "regarding" only | The "new-filing-based-on-X" case *appeared* to need a distinct "related-to" relationship | **DISSOLVED (owner 2026-07-30)** | No related field / second mechanism. `regarding` is the one direct relationship; the cross-reference is noted in the LLM triage summary, not persisted as a link. FR-12 = **suppress** the misfile (demote the referenced record's explicit-ID match `Resolved`→`Suggested`) + **note** the reference in `sprk_triagesummary` + **propose** creating the new record (gated). ADR-024 amended 2026-07-30 to state this explicitly; see `notes/042-regarding-vs-related-owner-decision.md` |
