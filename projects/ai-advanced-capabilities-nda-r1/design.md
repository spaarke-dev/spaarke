...
| Sub-UC | Job | Output |
|---|---|---|
| **UC1 — Review & advise** | Upload an NDA; analyze it against the **company NDA standard** with full advisory reasoning; return an **overall risk rating**, a concise **flagged-section summary** (in the Assistant), **in-document advisory Comments** on each flagged term, and standards-based **rewrite suggestions** applied per-section by the user | Concise cited summary (Assistant + Analysis widget) · advisory Comments in Compose · a generated **Summary Page** in the exported doc |
...
**Inputs → Outputs**:
- UC1: NDA file (DOCX/PDF) → risk rating + flagged-section summary (Assistant/Analysis widget) + advisory Comments in Compose + user-driven per-section rewrites + Summary Page on export.
...
**Target surfaces**: **SpaarkeAi workspace** (host) · **SprkChat / Assistant** (entry + concise summary) · **Analysis widget** (advisory presentation — NEW, see below) · **Compose/Tiptap editor** (advisory Comments + per-section Draft Alternative + Word I/O) · **Context pane** (execution trace + citations). No DataGrid in r1 (single-doc).
...
User (Assistant): uploads Acme_NDA.docx
  ...
  → Analysis widget presents the fuller advisory: overall risk, the flagged-section list with
...
**Net-new UI (r1)**:
- **Analysis widget** (NEW — owner pre-authorized, 2026-07-25): a dedicated advisory-presentation widget that "affords more flexibility and doesn't interfere with Compose." Hosts the overall risk + flagged-section list + advisory reasoning; click-to-navigate into the Compose doc. Registration is a well-worn one-liner (`registerWorkspaceWidget`); the existing **`FindingsWidget`** (context pane) is the closest analogue and the reuse-first alternative to evaluate (§11).
...
| C7 | Surface the concise cited summary in the **Assistant** + fuller advisory in the **Analysis widget** | Assistant render + Analysis widget | ✅ |
...
| C6b Summary Page in DOCX | **BUILD — small, no new package** | `DocxAnnotationWriter.cs` uses OpenXML (`DocumentFormat.OpenXml` 3.4.1 already a dep); constructs `Paragraph/Run/Text`, `PrependChild`/`AppendChild` | **NEW** sibling method: prepend/append a summary section + `Break{Type=BreakValues.Page}` |
| C7 Assistant summary + Analysis widget | **ACTIVATE + BUILD (widget)** | Assistant render exists; `registerWorkspaceWidget` trivial; `FindingsWidget` closest analogue | Concise cited-bullet summary in Assistant; author `NdaAnalysisWidget` (or reuse `FindingsWidget`) |
...
4. **Analysis widget (C7)** — net-new component on a one-liner registration path (or reuse `FindingsWidget`). *Client.*
...
- `nda-review/default` → `NDA-REVIEW`, disposition surfaces to Assistant + Analysis widget + `compose_advisory_comments`; risk **ConfirmWhenUncertain**; surfaces workspace + chat; event `document_uploaded` (conditional on NDA classification) + chip.
...
**Analysis widget**: register `nda-analysis` via `registerWorkspaceWidget` (or reuse `FindingsWidget`) — evaluate reuse-first per §11.
...
<hot-path-declaration>
  ...
  <spaarkeai>Y</spaarkeai> <!-- "Review NDA" card, Analysis widget, advisory-Comments receiver, Compose/Context wiring -->
...
| Summary-Page DOCX writer | `DocxAnnotationWriter.cs` (OpenXML, no page/section insert) | **Add sibling method**; reuse OpenXML primitives | Without it the summary doesn't travel with the exported document (owner-requested deliverable) |
| `NdaAnalysisWidget` | `FindingsWidget` (context pane), `registerWorkspaceWidget` | **Evaluate reuse of `FindingsWidget` first**; else one-liner registration | Without an advisory surface the rich analysis has nowhere flexible to render without cluttering Compose |
...
- **Advisory surface**: an `NdaAnalysisWidget` is the presentation surface, unless the reuse-first evaluation shows `FindingsWidget` suffices.
...
- [ ] **Analysis widget vs. FindingsWidget reuse**: does `FindingsWidget` meet the advisory-presentation need, or is a dedicated `NdaAnalysisWidget` warranted? *Blocks: §11 component (evaluate reuse-first during pipeline).*
- [ ] **Advisory-Comments routing**: emit on the existing assistant→workspace channel (lighter, no ledger/disposition change) vs. a routable Compose disposition? *Blocks: C4 wiring choice.*
