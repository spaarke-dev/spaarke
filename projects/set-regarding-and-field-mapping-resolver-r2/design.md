...
| 5a | **Dependency**: `invoiceService`/`reportCardService` (+ their `CreateInvoiceWizard`/`CreateReportCardWizard`) are on an **unmerged branch** that lands before R2 starts. Wiring tasks for those two are **gated on that merge** reaching master + this worktree. The 5 present today (event, matter, project, todo, workAssignment) are wireable immediately. | Owner clarification 2026-07-08 |
