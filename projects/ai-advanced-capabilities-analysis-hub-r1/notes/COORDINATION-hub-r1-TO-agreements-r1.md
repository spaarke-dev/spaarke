...
| Ask | Answer | Detail |
|---|---|---|
| **A1** — agreement-type/sub-domain picker in the wizard | **Will build** (small extension) | The per-type wizard (040) exists and launches as a modal. Adding a step-2 sub-domain picker is an extension. **Values come from your C1 registry** (we render, you own the list). If your registry isn't ready at our build time, we seed the picker from the `sprk_agreementsubdomain` option set (Part D) and reconcile. |
| **A2** — persist sub-domain on `sprk_analysis` | **Done (built)** | Resolved as a **lookup**, not an option set (see Part D rationale). Created reference table **`sprk_agreementtype`** (the data-driven registry — new type = add a row, zero code); `sprk_analysis.sprk_agreementtypeid` lookup points to it. The **same table is reused by the new `sprk_agreement` entity**. Seed rows being loaded. **The option set + integer↔key map are dropped.** |
| **A3** — launch envelope carries `activeWorkType` + `subDomain` | **Mostly built; finishing** | `openSpaarkeAi` already carries `activeWorkType` (041/052) and we already dispatch a Compose `widget_load` on wizard-finish/analysis-open (Phase 1). **`subDomain` is the one field to add** to the envelope on both wizard-launch (2b) and open-existing (2d). |
...
```
The hub resolves `subDomain` (`sprk_key`) → the `sprk_agreementtypeid` lookup when writing `sprk_analysis`. **No integer↔key map** — the lookup deletes the need.
Worktype reference: `SprkAnalysisWorkType.AgreementAnalysis = 100000000` (the "NDA Analysis" card maps here today).

**C3 — Persistence**: `sprk_analysis.sprk_worktype = agreement-analysis` (hub) · **`sprk_analysis.sprk_agreementtypeid`** → `sprk_agreementtype` (hub, lookup, A2 — reused by `sprk_agreement`) · `sprk_analysisoutput` (hub KEEP, your memo target, A5).
