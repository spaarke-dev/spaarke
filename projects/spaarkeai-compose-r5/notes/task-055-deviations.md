...
## Prong 1 (keep-edits recovery) — DEFERRED (fast-follow)
Prong 2 fixes the root cause (edits now anchor correctly), so the best-effort-apply-and-surface recovery is a **safety net**, not a blocker. Deferred to keep the fix focused and unblock redeploy. Tracked as a fast-follow: on an anchor-refusal batch, apply resolvable ops + surface unresolvable (reuse `ReanchorStaleSaveAsync`'s AUTO/REVIEW/ORPHAN model), never silently applying a wrong edit.
