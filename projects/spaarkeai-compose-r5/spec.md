...
- `src/server/api/Sprk.Bff.Api/Services/Compose/ComposeShadowPatchEngine.cs` — clean-apply branch (G2); `setBlockAttr`→`w:pPrChange` applier (G3); full table applier (G4); hyperlink edit applier (G5); `acceptRevision`/`rejectRevision` handlers incl. batch (G12).
...
- **ADR-039 / ADR-040** — closed op catalog, engine frozen, no new AI dispatch endpoint. G4/G12 **extend** the `ComposeOperation` catalog under version control.
- **ADR-013** — AI facade; no AI internals in `Services/Compose/`. R5 appliers are pure OOXML.
