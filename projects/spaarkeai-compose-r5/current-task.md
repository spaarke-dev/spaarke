...
| **Active task** | none in-flight. **021 DONE** (Candidate A) — engine `Apply(trackChanges:false)` clean branch (plain runs, physical deletes, no pPrChange); SaveAsync reads durable marker → clean/tracked; client reopened-authored → op-log+server-clean. 3/3 clean seam + 807/807 Compose + 24/24 byte-diff; R4.5 green; publish +0.00. Bounded limit: clean table-structural/accept-reject refuse (see notes/task-021-deviations.md §3). Next: 022 (G7 save split). |
