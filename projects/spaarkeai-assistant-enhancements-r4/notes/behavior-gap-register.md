...
- **Endpoint-wiring guard (task 024, fixed not deferred)**: the `ChatEndpoints` typed-`suggestions` emission was extracted to the testable `ChatEndpoints.BuildTypedFollowups` and guarded by `ChatEndpointsTypedFollowupsTests` (order action → capability → question, null-binding-capability dropped, empty ⇒ no event) — closing the typed-shape guard end to end (Action-contract fact + service 021a + client-parse 021b + this endpoint guard). See `notes/defer-issues.md` D-024-01 (resolved in-project — no better owner).
