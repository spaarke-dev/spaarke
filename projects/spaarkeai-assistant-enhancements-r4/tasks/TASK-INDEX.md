...
| 023 | Follow-on cards, open-tab-gated (Briefing/SmartToDo) | 2 E2 | FR-06 | sonnet / high | FULL | ❌ | 022, 012 | ✅ (client launcher on `list-tasks` FR-01 surface_launch + additive WorkspacePane `workspace_tabs_snapshot` open-tab gate [owner §6.5: POML "reuse R3 open-tab awareness" was factually wrong]; 2 gated ProactiveCardStack slots; click→handleSurfaceLaunch [022 registry, no per-card branch]; WorkspacePane layoutId de-dupe guarantees no-dup; WorkspaceLauncherCard [ADR-021 tokens] + pure buildAgendaFollowOnSlots; 8 gating/collapse/dispatch tests; surface-gate clean on 5 prod files; NO new Dataverse data [080 scope unchanged]; Step 9.5 PASS. Deploy w/ SpaarkeAi at 080) |
| 024 | E2 eval cases (dead-end + card gating) | 2 E2 | FR-10 | sonnet / high | FULL | ✅ | 021b, 023 | ✅ (FR-04/FR-06 guards already authored WITH their features per ADR-038: 021a service off-catalog drop, 021b client untyped/unbacked drop, 023 card open-tab gating, 013 AR4-003 dispatch — NOT duplicated. Added the one gate-owed guard: FR-04 no-dead-end CONTRACT fact `SuggestFollowupsAction_IsGroundedTypedTwoKindProposer_NoDeadEndFreeString` (8/8 R4 eval pass, net10) + FR-10 coverage map in Eval/README + register P2✅. D-024-01 (typed-SSE endpoint guard) FIXED not deferred: extracted `ChatEndpoints.BuildTypedFollowups` + `ChatEndpointsTypedFollowupsTests` (order/null-drop/empty, via InternalsVisibleTo); full BFF suite green, no new CVE, publish unchanged. Step 9.5 PASS) |
...
| 080 | Deploy + verify (BFF + sprk_spaarkeai; owner-gated) | Deploy | — | sonnet / high | STANDARD | ❌ | 013, 024, 033, 040 | ✅ (2026-08-18 deploys EXECUTED to dev: BFF 44.96 MB hash-verified + healthy [routes 401]; advisory `list-tasks` Action seeded via REST PATCH from authored JSON [modeltier=Reasoning, temp=0.3, allow-list, prompt, schema — verified]; grounded tools are code-registered handlers not Dataverse rows; Binding=Surface Launch; SpaarkeAi code page built [R4 markers 023/040/021b verified in bundle] + deployed + published; compose-r7 identity-key guard=Active. Runbook: notes/deploy-verify.md. **Owner UAT of 7 DoDs pending.**) |
