...
| **1-D** | 015 | 🔲 next | — | — | — | Migrate `SessionSummarizeOrchestrator` to stable-ID lookup (FR-05 first — lowest blast radius). NOTE: POML needs language refresh from `*PlaybookCode`/`GetByCodeAsync` to `*PlaybookId`/`GetByIdAsync` per Q1 refactor. |
| **0-B** | 004 | 🔲 pending | — | — | — | |
