...
> **Last Updated**: 2026-06-25 (Task 117b complete — FE consumes `playbook_options` SSE event end-to-end. Added `playbook_options` to `ChatSseEventType` union + `IPlaybookOptionsPayload` + `IPlaybookOptionCandidate` shapes; `setOnPlaybookOptions` callback ref in `useSseStream` (mirrors `setOnPaneEvent` synchronous pattern); `onPlaybookOptions` + `onSelectPlaybook` + `onOpenLibraryModal` props on `ISprkChatProps`; new `playbook_options` card in `SprkChatMessageRenderer` renders Fluent v9 primary buttons per candidate + "Open Library" link (ADR-021 semantic tokens, dark-mode safe); `ConversationPane` synthesizes Assistant message via `injectLocalMessage`, POSTs `/api/ai/playbook-dispatch/execute` on candidate click (Xrm-Nav-style), launches `sprk_playbooklibrary` Code Page via Xrm.Navigation.navigateTo on Library click. **12 new tests** (8 renderer card + 3 SSE parser + 1 lib edge); Shared lib SprkChat suite 361/361 green; shared lib builds clean; SpaarkeAi vite build clean (47.92 MB compressed BFF baseline unchanged — pure FE change). **Known follow-up**: `/api/ai/playbook-dispatch/execute` endpoint does NOT yet exist in BFF; FE handler surfaces a friendly fallback message on 404. The 117a `PlaybookOptionsEventBuilder` is registered in DI but the SSE emit-point in `ChatEndpoints.cs` is still pending the future orchestrator task. Earlier: Task 114R `NodeType.DeliverComposite` complete (47.92 MB compressed); Task 116 dead-code removal; Task 117a SSE event contract; Task 111R hybrid intent reranker; Task 115 `intentHint` vector-query bias.)
> **Recovery**: READ "Quick Recovery" section FIRST. Tasks 112 + 113R + 115 + 111R + 117a + 116 + 114R + 117b ✅ shipped on `work/spaarke-ai-platform-chat-routing-redesign-r1`. Next active tasks: 114a (per-section SSE — consume `CompositeOutputPayload` + `CompositeSectionResult`), 114b (FE widget rework), 114c (ADR; main session only because `.claude/` touch). 117 (telemetry, blocked-by 117b) now unblocked.
