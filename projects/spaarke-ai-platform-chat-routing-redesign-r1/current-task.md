...
> **Last Updated**: 2026-06-25 (Task 116 complete — `SoftSlashIntentToCapabilityName` BE dict + `SOFT_SLASH_TO_INTENT` FE map removed; `CapabilityRouter.TryClassifySoftSlash` Layer 0.5 pre-pass deleted (~80 lines); `toCommandIntent` reduced to structural `command.slice(1)` derivation; 2 BFF test files deleted; FE 100 SoftSlash-related tests still green; BFF 7872 unit tests pass; BFF compressed 46.31 MB, no measurable Δ. FR-20 acceptance verified — slash + NL converge via task 115's PlaybookDispatcher Phase B bias. Earlier same-day: Task 117a `playbook_options` SSE event contract complete; Task 111R hybrid intent reranker complete; Task 115 `intentHint` vector-query bias.)
> **Recovery**: READ "Quick Recovery" section FIRST. Tasks 112 + 113R + 115 + 111R + 117a + 116 ✅ shipped on `work/spaarke-ai-platform-chat-routing-redesign-r1`. Next active tasks: 117b (FE consumes the `playbook_options` SSE event), and a future orchestrator task to wire `PlaybookOptionsEventBuilder.BuildAsync` + `CreatePlaybookOptionsEvent` into the chat-streaming flow (the 117a builder is registered in DI but intentionally NOT yet emitted from `ChatEndpoints` — emission point is the future orchestrator's decision).
