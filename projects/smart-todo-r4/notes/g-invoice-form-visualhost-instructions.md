# G — Add Visual Host "Upcoming To Dos" to the Invoice main form

> **Task**: R4-083 (smart-todo-r4)
> **Date**: 2026-06-11
...
> - R4-081 — Matter template + per-form substitution table this doc applies (§10)

This document is the maker checklist for mounting the `Spaarke.Visuals.VisualHost` virtual PCF on the `sprk_invoice` main form and binding it to the `Upcoming To Dos — Invoice` chart definition created in R4-080. The form-designer work itself is a live action performed in spaarkedev1 — this checklist captures the exact steps so a maker (Ralph or successor) can execute it without ambiguity.
...
This doc is the Invoice-form instance of the R4-081 template; per-form value substitutions are applied throughout (entity = `sprk_invoice`, `contextFieldName` = `sprk_invoiceid`, chart def name = `Upcoming To Dos — Invoice`, owning solution = `Spaarke.Invoice` — verify name). The companion tasks 082 / 084 mount the same control on Project / WorkAssignment forms via parallel docs.
...
|---|---|---|
| Invoice chart def record `Upcoming To Dos — Invoice` | (Dataverse `sprk_chartdefinition` row, deployed via R4-080 PS script) | Live record once script runs in spaarkedev1 |
| Form-mount checklist (this doc) | `projects/smart-todo-r4/notes/g-invoice-form-visualhost-instructions.md` | NEW (this task) |
| `sprk_invoice` main form change | Live form-designer mod on `sprk_invoice` main form | Live action — user executes per §1–§7 |
...
Per FR-31 and FR-36, Smart To Do R4 surfaces a curated card list of upcoming + pinned `sprk_todo` records on the 4 parent entity forms (Matter / Project / Invoice / WorkAssignment). The card list is rendered by the existing virtual PCF `Spaarke.Visuals.VisualHost` (v1.4.16), the same PCF used for every other VH-driven card in the Spaarke UX. The chart def is the env-portable contract layer: chart-def text fields (`sprk_contextfieldname`, `sprk_drillthroughtarget`, `sprk_fetchxmlquery`, `sprk_visualtype`) carry all the business logic; the form-designer step just binds one VH instance per form. Drill-through opens the SmartTodo Code Page modal (FR-34) pre-filtered to the current Invoice via the `useLaunchContext` translator (extended in R4-034 to consume VisualHost's auto-injected `entityName` / `filterField` / `filterValue` / `mode=dialog` keys).

A dedicated section makes the card visually distinct from the rest of the Invoice form (Related, Documents, etc.) and lets the form designer position it where users will see it on cold-open of the record.
...
| **Old `UPCOMING TASKS` chart visual instance to be replaced** (chart def `154bd4a4-f359-f111-a825-3833c5d9bcab`) | Open Invoice main form in form designer → check whether existing Visual Host control mounted with that GUID | Note the existing instance's position so the new instance can replace it (see §6). If the Invoice form never had a legacy UPCOMING TASKS visual, §6 is a no-op |
| **RegardingResolver PCF wired on `sprk_todo` form** | Verify `sprk_regardinginvoice` lookup is populated on test To Do records (the chart def's `sprk_contextfieldname = sprk_regardinginvoice` filter requires it) — see R4-051 instructions doc | Run R4-051 form-designer steps + R4-051 PCF CREATE-mode bridge in `RegardingResolverApp.tsx` |

---

## 1. Open the `sprk_invoice` main form in the form designer
...
3. Left nav → **Tables** → search **Invoice** → click `sprk_invoice`.
4. Tab strip → **Forms**.
...
> **Reference**: if a legacy `UPCOMING TASKS` visual driven by chart def `154bd4a4-...` is mounted on this form, plan to remove it as the final step in §6 (NOT before — verify the new card renders first). If no legacy visual is present, skip §6 entirely.
...
1. In the form designer left tree, navigate to the tab that should host the new section. Recommended: the main / general tab where Related / Activities and the existing Visual Host cards live. If unsure, ask the form owner. The default Invoice form has a "Summary" / "General" tab at the leftmost position — use that.
2. Right-click the target tab → **Add section** → choose **1-column section**.
3. The new section appears at the bottom of the tab. Drag it to the desired position. Recommended position: above any "Related" tab content and below the basic Invoice detail fields (Name, Status, Owner). If a `UPCOMING TASKS` visual already exists in this tab, place the new section ABOVE it (the legacy section will be removed in §6).
...
VisualHost is bound to a column. Per the established Spaarke pattern (see existing VH instances on Invoice form for KPI / Document counts / etc., if any), the binding column must be a field that exists on the host entity and is NOT used for any other purpose by the form. A common convention is to bind to a hidden text column the entity already exposes (e.g., `sprk_visualhostbindingfield` if it exists) — but for R4 we can bind to the entity's **primary name field** (`sprk_name`) and rely on the `chartDefinitionId` static input to drive the actual chart. (See §3.1 for rationale.)
...
- The Invoice form doesn't have a dedicated `sprk_chartdefinition` lookup field (and adding one just for VH binding would clutter the schema).
...
- The static-ID approach is what the existing `UPCOMING TASKS` instance uses on Matter (and was the convention for any pre-existing Invoice-form VH instance).

**Implication**: VH must still be added to a column (PCF rule). Any hidden text column on `sprk_invoice` works. The primary name field is universally present and the binding does NOT cause VH to read the column's value (the lookup branch wins precedence-wise, and we use the static-ID input).

If a dedicated hidden text column for VH bindings exists on `sprk_invoice` (e.g., one previously added by KPI cards), prefer that — it makes intent clear at form-designer reading time. Open the form tree and scan for an existing column with VH already bound to it; if present, mirror its binding pattern.
...
| `chartDefinitionId` (input) | **GUID of the `Upcoming To Dos — Invoice` chart def record** | Retrieve via the deploy-script output OR via a one-time query (see §3.3). Per NFR-03, this value is env-specific BUT the chart def name (`Upcoming To Dos — Invoice`) is portable; the GUID changes per env. Capture and treat as env config. |
| `contextFieldName` (input) | **`sprk_invoiceid`** | VisualHost auto-injects the current record's primary ID into the drill-through `filterValue` and uses this field to know WHICH parent ID to substitute. Per VisualHost manifest line 29–34, this is the field name on the **VIEWING entity** that holds the parent record ID — for the Invoice form, that's `sprk_invoiceid` (Invoice's primary key). When VH passes `filterValue=<current-invoice-id>`, the SmartTodo Code Page filters on the chart def's `sprk_contextfieldname=sprk_regardinginvoice`. |
...
### 3.3 Get the `Upcoming To Dos — Invoice` chart def GUID
...
$uri = "$env/api/data/v9.2/sprk_chartdefinitions?`$select=sprk_chartdefinitionid,sprk_name&`$filter=$([uri]::EscapeDataString("sprk_name eq 'Upcoming To Dos — Invoice'"))"
...
4. Top of the solution view (or the Tables > Invoice view) → click **Publish all customizations**. This publishes the form change AND ensures any cached chart def metadata in the runtime is refreshed.
...
> **Environment**: spaarkedev1. **Browser**: Edge or Chrome, signed in as a user with at least Read access to `sprk_invoice` and `sprk_todo`, and access to at least 1 published Invoice record that has 1+ regarding-Invoice `sprk_todo` records (i.e., `sprk_regardinginvoice` populated by the R4-051 RegardingResolver flow).

### Smoke A — section + card render

1. Open the spaarkedev1 model-driven app (whichever app surfaces Invoice; typically `Spaarke`).
2. Navigate to **Invoice** → open an Invoice record that has at least 1 `sprk_todo` regarding it.
3. **Expected**:
   - The "Upcoming To Dos" section is visible in the tab where it was added (typically the main / general tab).
   - Under the section header, the VisualHost card renders a list of due-date cards for matching `sprk_todo` records (state=Active, statuscode in {Open, In Progress}, and either duedate within next 5 days OR `sprk_todopinned=true`) per the chart def's FetchXML + the VH-injected `sprk_regardinginvoice eq <current-invoice-id>` filter.
   - If the Invoice has 0 matching `sprk_todo` records, the VH shows an empty-state placeholder (depending on VH version, may be "No items" or a blank well — not an error).
...
1. With the same Invoice open, click the **expand** icon (or whichever toolbar button on the VH instance triggers drill-through). The icon may appear as a maximize/full-screen button in the card's top-right corner.
2. **Expected**:
   ...
   - The SmartTodo Kanban is **pre-filtered** to only the current Invoice's `sprk_todo` records (via `useLaunchContext` decoding VH's `data` envelope: `entityName=sprk_todo&filterField=sprk_regardinginvoice&filterValue=<current-invoice-id>&mode=dialog`).
   - The Code Page header may show a small "Filtered: <Invoice name>" indicator (per R4-034's `openTodos` discriminator surface) OR the filter is implicit in the data shown.
3. **Console verification** (open DevTools BEFORE clicking expand):
   ...
   - Run:
     ...
     ```
   - **Expected**: `?data=entityName%3Dsprk_todo%26filterField%3Dsprk_regardinginvoice%26filterValue%3D<guid>%26mode%3Ddialog`
4. **PASS criterion**: Modal opens AND Kanban displays only the current Invoice's `sprk_todo` records.
...
### Smoke D — modal close returns control to Invoice form

1. Inside the modal, click the close button (X) or press ESC.
2. **Expected**: Modal closes, Invoice form remains on screen, no unsaved-changes prompt (because the modal is a read-only-by-default surface — though card actions inside the Kanban may have modified records).
3. **PASS criterion**: Clean close without errors.

### Smoke E — new Invoice (CREATE form) graceful behavior

1. Click **+ New Invoice** in the app.
2. **Expected**:
...
3. Fill in the Invoice name + Status; save. Open the saved record.
...
**Only after Smokes A–D PASS**. If a legacy `UPCOMING TASKS` visual driven by chart def `154bd4a4-f359-f111-a825-3833c5d9bcab` was mounted on the Invoice main form (it was historically scoped to Matter only, but verify per-env), it is superseded by the new "Upcoming To Dos" card.

**Steps**:

1. Re-open the `sprk_invoice` main form in the form designer.
...
> If the legacy `UPCOMING TASKS` visual is NOT present (the expected default for the Invoice form, since the legacy chart def was Matter-scoped), skip this section entirely.
...
1. In `make.powerapps.com` → **Solutions** → open the solution that owns the `sprk_invoice` form. For Spaarke, this is typically the `Spaarke.Invoice` solution (or `Sprk.Invoice.Forms`, depending on env-specific publisher conventions). **Verify the owning solution name before exporting.**
...
5. From the repo root, extract the solution to the appropriate solution dir using PAC:
   ...
   ls src/solutions/Spaarke.Invoice/
   ...
   pac solution unpack `
     ...
     --folder src/solutions/Spaarke.Invoice `
     --processCanvasApps `
     --allowDelete
   ```

6. `git diff src/solutions/Spaarke.Invoice/` should show the form-XML diff including:
   ...
   - The `contextFieldName` input value `sprk_invoiceid`
7. **Stage + commit** only the relevant form XML files (NOT the entire solution if other unrelated dirty entries are present). Example:
   ...
   ```powershell
   git add src/solutions/Spaarke.Invoice/SolutionPackage/src/Entities/sprk_invoice/FormXml/main/Information.xml
   # Verify
   git status
   ```
...
```
R4-083 deployed surfaces:
- MODIFIED — sprk_invoice main form "Information"
  ...
  - NEW VH INSTANCE: Spaarke.Visuals.VisualHost bound to <binding-column>
    * chartDefinitionId = <env-specific GUID for "Upcoming To Dos — Invoice" chart def>
    * contextFieldName = sprk_invoiceid
    * showToolbar = true, enableDrillThrough = true, showVersion = true
  - REMOVED: legacy UPCOMING TASKS VH instance (chart def 154bd4a4-...) — only if present prior to R4
- UNCHANGED: Spaarke.Visuals.VisualHost PCF v1.4.16 (no source change)
- UNCHANGED: Chart def "Upcoming To Dos — Invoice" (deployed via R4-080 PS script)
...
2. **Full rollback (form)**: re-import the previous version of the `Spaarke.Invoice` (or equivalent) solution to restore the prior form definition. The legacy `UPCOMING TASKS` visual (if it was removed in §6) returns automatically.
...
4. **Drill-through issue isolation**:
   ...
   - If drill-through opens the modal but with an UNFILTERED Kanban OR with the wrong parent's todos: suspect (a) the `contextFieldName` input on the VH instance isn't `sprk_invoiceid`, OR (b) the chart def's `sprk_contextfieldname` isn't `sprk_regardinginvoice`. Verify both per `infrastructure/dataverse/charts/upcoming-todos-invoice.json` and §3.2 step 6.

---

## 10. Per-form value substitutions for sibling tasks (081 / 082 / 084)

This Invoice doc was generated from the R4-081 Matter template via the §10 substitution table reproduced below. Sibling tasks 081 / 082 / 084 follow the EXACT same checklist with the per-form substitutions called out.
...
| **R4-083** (THIS) | `sprk_invoice` main form | `Upcoming To Dos — Invoice` | `sprk_invoiceid` | `Spaarke.Invoice` (verify name) |
...
**Per-form chart def JSON**: `infrastructure/dataverse/charts/upcoming-todos-invoice.json` is the contract for this task — `sprk_contextfieldname = sprk_regardinginvoice` (the field on the **child** `sprk_todo` entity that the chart def filters on; distinct from the VH input `contextFieldName = sprk_invoiceid` which names the field on the **viewing** `sprk_invoice` entity holding the parent ID).
...
- Verify the chart-def deploy script ran for all 4 parent types (Invoice + Matter + Project + WorkAssignment).
...
- Verify the R4-051 PCF + JS Web Resource is live so that `sprk_regardinginvoice` is populated on new `sprk_todo` records — otherwise the Invoice card stays empty even after this form change is deployed.

### For R4-093 (UI test suite — NFR validation)

- NFR-05 (modal nav latency < 500ms): measure the VH expand-click-to-modal-paint time on Invoice form. Acceptance: < 500ms cold, < 200ms warm.
...
- NFR-08 (orientation switch): irrelevant to R4-083 (this is the Kanban orientation toggle; tested per R4-070).

---

*End of G form-mount checklist (Invoice). Task R4-083 complete when this checklist has been executed in spaarkedev1 and all five smokes (A–E) pass; the solution XML for the form change is captured to the repo via §7.*
