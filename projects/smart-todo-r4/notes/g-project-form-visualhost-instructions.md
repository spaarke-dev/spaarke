# G — Add Visual Host "Upcoming To Dos" to the Project main form

> **Task**: R4-082 (smart-todo-r4)
> **Date**: 2026-06-10
...
This document is the maker checklist for mounting the `Spaarke.Visuals.VisualHost` virtual PCF on the `sprk_project` main form and binding it to the `Upcoming To Dos — Project` chart definition created in R4-080. The form-designer work itself is a live action performed in spaarkedev1 — this checklist captures the exact steps so a maker (Ralph or successor) can execute it without ambiguity.
...
The companion tasks 081 / 083 / 084 mount the same control on Matter / Invoice / WorkAssignment forms — those tasks reuse the same checklist with per-form value substitutions called out in §10 of the Matter instructions doc (`g-matter-form-visualhost-instructions.md`). This Project doc IS the substitution result for Project; tasks 083 / 084 still need their own per-form substitution.
...
|---|---|---|
| Project chart def record `Upcoming To Dos — Project` | (Dataverse `sprk_chartdefinition` row, deployed via R4-080 PS script) | Live record once script runs in spaarkedev1 |
| Form-mount checklist (this doc) | `projects/smart-todo-r4/notes/g-project-form-visualhost-instructions.md` | NEW (this task) |
| `sprk_project` main form change | Live form-designer mod on `sprk_project` main form | Live action — user executes per §1–§7 |
...
Per FR-31 and FR-36, Smart To Do R4 surfaces a curated card list of upcoming + pinned `sprk_todo` records on the 4 parent entity forms (Matter / Project / Invoice / WorkAssignment). The card list is rendered by the existing virtual PCF `Spaarke.Visuals.VisualHost` (v1.4.16), the same PCF used for every other VH-driven card in the Spaarke UX. The chart def is the env-portable contract layer: chart-def text fields (`sprk_contextfieldname`, `sprk_drillthroughtarget`, `sprk_fetchxmlquery`, `sprk_visualtype`) carry all the business logic; the form-designer step just binds one VH instance per form. Drill-through opens the SmartTodo Code Page modal (FR-34) pre-filtered to the current Project via the `useLaunchContext` translator (extended in R4-034 to consume VisualHost's auto-injected `entityName` / `filterField` / `filterValue` / `mode=dialog` keys).

A dedicated section makes the card visually distinct from the rest of the Project form (Related, Documents, etc.) and lets the form designer position it where users will see it on cold-open of the record.
...
| **Pre-existing chart visual instance on Project form** (if any) | Open Project main form in form designer → check whether existing Visual Host control mounted in any section | If a legacy "UPCOMING TASKS" or similar visual exists, note its position so the new instance can replace it (see §6). If no legacy instance, §6 is a no-op. |

---

## 1. Open the `sprk_project` main form in the form designer
...
3. Left nav → **Tables** → search **Project** → click `sprk_project`.
4. Tab strip → **Forms**.
...
> **Reference**: if a legacy `UPCOMING TASKS` visual (driven by chart def `154bd4a4-...`) is still mounted on the Project form, plan to remove it as the final step in §6 (NOT before — verify the new card renders first). If no legacy instance exists, §6 is a no-op.
...
1. In the form designer left tree, navigate to the tab that should host the new section. Recommended: the main / general tab where Related / Activities and the existing Visual Host cards live. If unsure, ask the form owner. The default Project form has a "Summary" / "General" tab at the leftmost position — use that.
2. Right-click the target tab → **Add section** → choose **1-column section**.
3. The new section appears at the bottom of the tab. Drag it to the desired position. Recommended position: above any "Related" tab content and below the basic Project detail fields (Name, Status, Owner). If a legacy `UPCOMING TASKS` visual already exists in this tab, place the new section ABOVE it (the legacy section will be removed in §6).
...
VisualHost is bound to a column. Per the established Spaarke pattern (see existing VH instances on Project form for KPI / Document counts / etc.), the binding column must be a field that exists on the host entity and is NOT used for any other purpose by the form. A common convention is to bind to a hidden text column the entity already exposes (e.g., `sprk_visualhostbindingfield` if it exists) — but for R4 we can bind to the entity's **primary name field** (`sprk_name`) and rely on the `chartDefinitionId` static input to drive the actual chart. (See §3.1 for rationale.)
...
- The Project form doesn't have a dedicated `sprk_chartdefinition` lookup field (and adding one just for VH binding would clutter the schema).
...
- The static-ID approach is what existing legacy chart visual instances on Matter / similar forms use today.

**Implication**: VH must still be added to a column (PCF rule). Any hidden text column on `sprk_project` works. The primary name field is universally present and the binding does NOT cause VH to read the column's value (the lookup branch wins precedence-wise, and we use the static-ID input).

If a dedicated hidden text column for VH bindings exists on `sprk_project` (e.g., one previously added by KPI cards), prefer that — it makes intent clear at form-designer reading time. Open the form tree and scan for an existing column with VH already bound to it; if present, mirror its binding pattern.
...
| `chartDefinitionId` (input) | **GUID of the `Upcoming To Dos — Project` chart def record** | Retrieve via the deploy-script output OR via a one-time query (see §3.3). Per NFR-03, this value is env-specific BUT the chart def name (`Upcoming To Dos — Project`) is portable; the GUID changes per env. Capture and treat as env config. |
| `contextFieldName` (input) | **`sprk_projectid`** | VisualHost auto-injects the current record's primary ID into the drill-through `filterValue` and uses this field to know WHICH parent ID to substitute. Per VisualHost manifest line 29–34, this is the field name on the **VIEWING entity** that holds the parent record ID — for the Project form, that's `sprk_projectid` (Project's primary key). When VH passes `filterValue=<current-project-id>`, the SmartTodo Code Page filters on the chart def's `sprk_contextfieldname=sprk_regardingproject`. |
...
### 3.3 Get the `Upcoming To Dos — Project` chart def GUID
...
$uri = "$env/api/data/v9.2/sprk_chartdefinitions?`$select=sprk_chartdefinitionid,sprk_name&`$filter=$([uri]::EscapeDataString("sprk_name eq 'Upcoming To Dos — Project'"))"
...
4. Top of the solution view (or the Tables > Project view) → click **Publish all customizations**. This publishes the form change AND ensures any cached chart def metadata in the runtime is refreshed.
...
> **Environment**: spaarkedev1. **Browser**: Edge or Chrome, signed in as a user with at least Read access to `sprk_project` and `sprk_todo`, and access to at least 1 published Project record that has 1+ regarding-Project `sprk_todo` records.

### Smoke A — section + card render

1. Open the spaarkedev1 model-driven app (whichever app surfaces Project; typically `Spaarke`).
2. Navigate to **Project** → open a Project record that has at least 1 `sprk_todo` regarding it.
3. **Expected**:
   - The "Upcoming To Dos" section is visible in the tab where it was added (typically the main / general tab).
   - Under the section header, the VisualHost card renders a list of due-date cards for matching `sprk_todo` records (state=Active, statuscode in {Open, In Progress}, and either duedate within next 5 days OR `sprk_todopinned=true`) per the chart def's FetchXML + the VH-injected `sprk_regardingproject eq <current-project-id>` filter.
   - If the Project has 0 matching `sprk_todo` records, the VH shows an empty-state placeholder (depending on VH version, may be "No items" or a blank well — not an error).
...
1. With the same Project open, click the **expand** icon (or whichever toolbar button on the VH instance triggers drill-through). The icon may appear as a maximize/full-screen button in the card's top-right corner.
2. **Expected**:
   ...
   - The SmartTodo Kanban is **pre-filtered** to only the current Project's `sprk_todo` records (via `useLaunchContext` decoding VH's `data` envelope: `entityName=sprk_todo&filterField=sprk_regardingproject&filterValue=<current-project-id>&mode=dialog`).
   - The Code Page header may show a small "Filtered: <Project name>" indicator (per R4-034's `openTodos` discriminator surface) OR the filter is implicit in the data shown.
3. **Console verification** (open DevTools BEFORE clicking expand):
   ...
   - Run:
     ...
     ```
   - **Expected**: `?data=entityName%3Dsprk_todo%26filterField%3Dsprk_regardingproject%26filterValue%3D<guid>%26mode%3Ddialog`
4. **PASS criterion**: Modal opens AND Kanban displays only the current Project's `sprk_todo` records.
...
### Smoke D — modal close returns control to Project form

1. Inside the modal, click the close button (X) or press ESC.
2. **Expected**: Modal closes, Project form remains on screen, no unsaved-changes prompt (because the modal is a read-only-by-default surface — though card actions inside the Kanban may have modified records).
3. **PASS criterion**: Clean close without errors.

### Smoke E — new Project (CREATE form) graceful behavior

1. Click **+ New Project** in the app.
2. **Expected**:
...
3. Fill in the Project name + Status; save. Open the saved record.
...
**Only after Smokes A–D PASS**. If a legacy `UPCOMING TASKS` visual driven by chart def `154bd4a4-f359-f111-a825-3833c5d9bcab` (or any other pre-R4 to-do card) is mounted on the Project form, it is superseded by the new "Upcoming To Dos" card.

**Steps**:

1. Re-open the `sprk_project` main form in the form designer.
...
> If the legacy `UPCOMING TASKS` visual is NOT present (e.g., it was never mounted on Project, or it was removed in a prior cleanup), skip this section. The Matter form is where the legacy visual is most likely to live; Project may not have it at all.
...
1. In `make.powerapps.com` → **Solutions** → open the solution that owns the `sprk_project` form. For Spaarke, this is typically the `Spaarke.Project` solution (or `Sprk.Project.Forms`, depending on env-specific publisher conventions). **Verify the actual owning solution name in spaarkedev1 before exporting.**
...
5. From the repo root, extract the solution to the appropriate solution dir using PAC:

   ```powershell
   # Verify the target solution dir exists
   ls src/solutions/Spaarke.Project/
   ...
   pac solution unpack `
     ...
     --folder src/solutions/Spaarke.Project `
     --processCanvasApps `
     --allowDelete
   ```

6. `git diff src/solutions/Spaarke.Project/` should show the form-XML diff including:
   ...
   - The `contextFieldName` input value `sprk_projectid`
7. **Stage + commit** only the relevant form XML files (NOT the entire solution if other unrelated dirty entries are present). Example:
   ...
   ```powershell
   git add src/solutions/Spaarke.Project/SolutionPackage/src/Entities/sprk_project/FormXml/main/Information.xml
   # Verify
   git status
   ```
...
```
R4-082 deployed surfaces:
- MODIFIED — sprk_project main form "Information"
  ...
  - NEW VH INSTANCE: Spaarke.Visuals.VisualHost bound to <binding-column>
    * chartDefinitionId = <env-specific GUID for "Upcoming To Dos — Project" chart def>
    * contextFieldName = sprk_projectid
    * showToolbar = true, enableDrillThrough = true, showVersion = true
  - REMOVED: legacy UPCOMING TASKS VH instance (chart def 154bd4a4-...) — only if present prior to R4 on Project form
- UNCHANGED: Spaarke.Visuals.VisualHost PCF v1.4.16 (no source change)
- UNCHANGED: Chart def "Upcoming To Dos — Project" (deployed via R4-080 PS script)
...
2. **Full rollback (form)**: re-import the previous version of the `Spaarke.Project` (or equivalent) solution to restore the prior form definition. Any legacy visual (if it was removed in §6) returns automatically.
...
4. **Drill-through issue isolation**:
   ...
   - **Project-specific**: if Kanban opens but is filtered by Matter / Invoice / WorkAssignment instead of Project, suspect the `contextFieldName` input is wrong on the VH instance. Verify it is exactly `sprk_projectid` per §3.2.

---

## 10. Per-form value substitutions — Project (this task) vs siblings 081 / 083 / 084

This Project doc IS the substitution result of the Matter template (per `g-matter-form-visualhost-instructions.md` §10). Tasks 083 + 084 still need to clone the Matter template and apply their own substitutions. For convenience, the full table is reproduced here:

| Task | Form | Chart def name | `contextFieldName` (VH input) | Owning solution (assume current; verify per env) |
|---|---|---|---|---|
| **R4-081** | `sprk_matter` main form | `Upcoming To Dos — Matter` | `sprk_matterid` | `Spaarke.Matter` |
| **R4-082** (THIS) | `sprk_project` main form | `Upcoming To Dos — Project` | `sprk_projectid` | `Spaarke.Project` (verify name) |
...
**Per-form chart def GUID**: each task pulls a different GUID from the chart def query in §3.3 (substitute the `sprk_name` filter — this Project doc already uses `Upcoming To Dos — Project`).

**Per-form smoke test target**: each form requires its own test record with at least 1 `sprk_todo` regarding-row of the respective parent type. For R4-082 specifically: verify a Project record exists in spaarkedev1 that has 1+ `sprk_todo` rows with `sprk_regardingproject` populated. Verify the resolver-set fields persisted from R4-051 work (i.e., on the test `sprk_todo` records, `sprk_regardingproject` is set to the Project's GUID — the resolver pre-save handler stages this when the user picks a Project via the regarding picker).

---

## Open notes for follow-up tasks

### For R4-083 / R4-084 (next-in-wave — DO NOT do in R4-082)

- Clone the Matter template (`g-matter-form-visualhost-instructions.md`) and apply substitutions per §10. Optionally use this Project doc as a worked reference.
- The smoke test set (A–E) applies identically per form.
- The deploy-trail content for §8 must be reproduced per form.
- If 083/084 are run in parallel by separate agents (alongside this task 082), EACH must capture solution XML to a DIFFERENT solution dir (per the table in §10). No file conflict expected.

### For R4-092 (deploy task)

- Verify the chart-def deploy script ran for all 4 parent types (Project is in R4-082's scope; the script deploys all 4 in one run per R4-080).
...
- NFR-05 (modal nav latency < 500ms): measure the VH expand-click-to-modal-paint time on Project form. Acceptance: < 500ms cold, < 200ms warm.
...
- NFR-08 (orientation switch): irrelevant to R4-082 (this is the Kanban orientation toggle; tested per R4-070).

---

*End of G form-mount checklist (Project). Task R4-082 complete when this checklist has been executed in spaarkedev1 and all five smokes (A–E) pass; the solution XML for the form change is captured to the repo via §7.*
