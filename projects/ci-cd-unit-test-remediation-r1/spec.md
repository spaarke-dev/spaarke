...
6. **FR-A06** — **CI Router stability gate (added 2026-06-26 after parallel-session discovery; UPDATED later 2026-06-26 after CICD-086 bisect)**. The router workflow (`ci-router.yml`, name `CI`, the FR-A01 deliverable) MUST achieve **≥2 consecutive successful master pushes** with `conclusion=success` and `latest_check_runs_count ≥ 1` BEFORE Phase 3 cutover (task 071) can flip branch protection to require `CI / Router`.

   **Resolution history (2026-06-26)**:
   - Initial state: workflow failed with `conclusion=startup_failure` on every push since merge (`cc305da98`); two parallel-session fix attempts (`fd451d39d` merge_group:branches:, `0f9922ea9` cancel-in-progress literal) did not resolve.
   - Task CICD-086 6-step bisect on PR #469 isolated the actual root cause: **permission-floor mismatch** — `ci-tier2-advisory.yml` declared `permissions: pull-requests: write` (for tier2-pr-comment job); router declared only `pull-requests: read`. GitHub rejects reusable workflows that request more permissions than caller granted, pre-job-creation, no logs. Fix: bumped router's `permissions.pull-requests` from `read` to `write` (commit `cb6208d29`, merged in PR #469 as `b54d7b528`).
   - Hardening: tier2's `run-*` inputs changed from `type: boolean` to `type: string` (commit `e05bb8d17`) to eliminate string-coercion ambiguity (was previously the rank-1 suspect; turned out to be defensible cleanup but not the bug). actionlint quote fix on router-side literals (commit `1f2b01e27`).

   **Remaining blockers for SC-12** (pre-existing ADR violations exposed when CI Router stopped startup_failure):
   - **ADR-007 violation**: 18 endpoint classes import `Microsoft.Graph` or `Microsoft.Graph.Models.ODataErrors` — task **CICD-088** fixes (4-6h; introduces `SpaarkeStorageException` in SpeFileStore; mechanical endpoint refactor).
   - **ADR-009 violation**: 3 endpoint classes inject `IMemoryCache` directly — task **CICD-087** fixes (2-3h; introduces `IEndpointResponseCache` wrapper service).

   Both violations pre-date this project and were silently swallowed by `sdap-ci.yml`'s `continue-on-error: true`. Project scope expanded 2026-06-26 to fix them (overrides project's `<hot-path-declaration><bff-api>NO</bff-api>` per owner direction: "if it is a real issue then we need to fix it"). **If 087 + 088 cannot land, cutover (071) is BLOCKED indefinitely** and the rollback path (re-disable router, keep `sdap-ci.yml` as gate) applies.
...
16. **FR-B10** — **Retroactive deep-cleanup pass (best-practice driven; clarified 2026-06-26)**. **The MUST rule is qualitative: surviving BFF unit tests are actually useful and required per build-vs-maintain best practices (ADR-038 §7).** Numeric reduction is a *signal*, not a *gate*. Process: (1) automated heuristic pass (CICD-082) catches grep-friendly antipatterns (B1-B5 wiring; B8/B11/B12/B16/B17 signature-detectable); (2) deeper-judgment pass (CICD-082b) reads each BFF.Api test file and classifies per Beck/Feathers/Google/DHH framing — *"would deleting this test allow a real production-behavior regression to ship undetected?"* If NO → delete. (3) Sliced PRs (CICD-083/084/085) execute deletions with path-check (FR-B06) and per-PR `dotnet build` + spot-check. Each PR is a forcing function for reviewer judgment; reviewers may extend the automated baseline at PR time.

   **Expected directional outcome** (not a gate): meaningful reduction toward ~50% as the heuristic + judgment passes compound. If the actual reduction falls short of ~50%, the spec is NOT violated — the binding artifact is the codified ADR-038 §7 criteria + `/test-diet` skill (FR-B09) that catches new scaffolding in every future project. The cultural reset is the long-term remediation; the count is a snapshot.

   Path-check at Step 9.5 enforces no KEEP-path file deleted without same-PR replacement (FR-B06 applies).
...
| SC-11 | **(BINDING)** ADR-038 §7 build-vs-maintain criteria codified with ≥12 concrete C# BAD/GOOD examples; `/test-diet` skill from FR-B09 exists and is wired into `task-execute` Step 11 wrap-up gate; CICD-082 automated pass + CICD-082b deeper-judgment pass + CICD-083/084/085 sliced deletion PRs have shipped per the spec's qualitative "actually useful and required tests" standard. **(DIRECTIONAL, non-binding)** BFF unit test count moves meaningfully toward ~3,500 over the project window; final count is reported in `notes/deep-cleanup-final-summary.md` with honest accounting of automated vs judgment-driven removals. | (binding) skill + ADR + summary doc existence checks; (directional) `grep -rE "^\s*\[(Fact\|Theory\|SkippableFact)\b" tests/unit/Sprk.Bff.Api.Tests \| wc -l` reported in summary |
| SC-12 | CI Router stability gate per FR-A06: `gh run list --workflow=CI --branch=master --limit=2 --json conclusion` shows both runs `conclusion=success`; `gh workflow view CI` shows ENABLED state; cutover (071) precondition satisfied. | `gh` API queries (link in `notes/ci-router-stability-proof.md`) |
