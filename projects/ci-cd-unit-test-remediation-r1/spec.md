...
6. **FR-A06** — **CI Router stability gate (added 2026-06-26 after parallel-session discovery)**. The router workflow (`ci-router.yml`, name `CI`, the FR-A01 deliverable) MUST achieve **≥2 consecutive successful master pushes** with `conclusion=success` and `latest_check_runs_count ≥ 1` BEFORE Phase 3 cutover (task 071) can flip branch protection to require `CI / Router`. As of 2026-06-26, the workflow fails with `conclusion=startup_failure` on every push since merge (`cc305da98`); two attempted fixes (`fd451d39d`, `0f9922ea9`) did not resolve; workflow is disabled via `gh workflow disable CI`. Task 086 remediates. Most-likely root cause: string-to-boolean coercion at reusable-workflow boundary — `ci-tier2-advisory.yml` declares inputs as `type: boolean` but router passes string-valued expressions. **If the router cannot be stabilized, cutover (071) is BLOCKED indefinitely** and the rollback path (re-disable router, keep `sdap-ci.yml` as gate) applies.
