...
- **2026-06-26** — **SCOPE EXPANSION (owner-directed)**: spec extended with FR-B08/B09/B10 + SC-11. Reason: post-Phase-2 review showed the narrow 9-file deletion (179 tests, 2.4% reduction) did not achieve the project's stated intent. Owner reframed the test problem as "build-vs-maintain" (scaffolding tests written for design/coverage vs regression-protecting tests with ongoing value) — a clearer judgment-based criterion than the project's original strict-signature pattern-matching. **Industry consensus** (Beck "delete the scaffolding", Feathers characterization-vs-behavior, Google test-sizes, DHH less-tests) supports this framing. New Phase 2.5 added: (A) codify ≥10 scaffolding-test bans in ADR-038/constraints/tests-CLAUDE; (B) build `/test-diet` skill invoked at project wrap-up to reconcile build vs maintain tests; (C) execute retroactive deep cleanup targeting BFF unit test count from ~6,700 to ≤3,500 (≥48% reduction). Phase 3 cutover (071) is GATED on Phase 2.5 completion — the architectural change is only meaningful if the surviving suite IS actually focused. Critical path extends from ~28 to ~35 elapsed days. Task count: 25 → 31.
- **2026-06-26 (later)** — **CI ROUTER FAILURE DISCOVERED via parallel session**: `ci-router.yml` (workflow name `CI`, the FR-A01 deliverable from task 040) has been failing with `conclusion=startup_failure` on every push since merge (`cc305da98`). Two attempted fixes by parallel sessions (`fd451d39d` for `merge_group: branches:`, `0f9922ea9` for `cancel-in-progress` literal) did NOT resolve. A parallel session ran `gh workflow disable CI` to stop the bleeding. **Most-likely root cause** (per parallel-session ranked suspects): string-to-boolean coercion at reusable-workflow boundary — `ci-tier2-advisory.yml` declares `run-*` inputs as `type: boolean` but router commit `bc3403705` passes them as expressions whose evaluated values are strings `"true"`/`"false"`. GitHub's strict reusable-workflow input validation rejects pre-job-creation, producing no logs and zero check-runs. Browser error message on the run page is the cheap diagnostic; otherwise deterministic bisect on `debug/ci-router-bisect` branch. New spec FR-A06 + SC-12 + task CICD-086 added. Task 040 status remains ✅ in the index but is annotated CRITICAL EXCEPTION — production deployment is broken. **Cutover (071) is now GATED on BOTH 085 (deep cleanup final) AND 086 (CI Router stability proof: ≥2 consecutive successful master pushes)**. If 086 cannot succeed, cutover is blocked indefinitely and rollback (re-disable router, keep `sdap-ci.yml` as gate) applies. Task count: 31 → 32 not-started becomes 11 → resolved stays 29; total POMLs 39 → 40.
