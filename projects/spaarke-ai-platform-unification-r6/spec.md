...
- **FR-21**: Add new `IInvokePlaybookAi` facade in `Services/Ai/PublicContracts/` per ADR-013 (per Q11 recommendation — cleaner than extending `IWorkspacePrefillAi`). Acceptance: facade wraps `IPlaybookOrchestrationService`; CRUD-side callers never inject `IPlaybookOrchestrationService` directly.
  ...
  **Additional binding acceptance criteria** (added 2026-06-08 per cross-project request from `spaarke-insights-engine-r2` audit; aligns with `.claude/patterns/ai/public-contracts-facade.md` + `endpoint-di-symmetry.md` + `bff-extensions.md` §F.1 — these patterns landed in master via PR #360 / `219bd230` on 2026-06-08; references DR-003 + DR-008 of the `bff-ai-architecture-audit-r1` project):

  1. **NullInvokePlaybookAi.cs exists** at `Services/Ai/PublicContracts/NullInvokePlaybookAi.cs` implementing `IInvokePlaybookAi`. Every method throws `FeatureDisabledException("ai.invoke-playbook.disabled", "<detail>")` per ADR-032 P3 Fail-Fast. Mirrors the canonical `NullBriefingAi` pattern.
  2. **Symmetric DI registration** — real `InvokePlaybookAi` registered in `AnalysisServicesModule.AddPublicContractsFacade` (compound-AI-ON gate); `NullInvokePlaybookAi` registered in `AnalysisServicesModule.AddNullObjectsForCompoundOff` for BOTH `DocumentIntelligence:Enabled=false` and `Analysis:Enabled=false` branches.
  3. **Transitive ctor-dep symmetry** verified per `bff-extensions.md` §F.1 static-scan recipe — every ctor dep of the real `InvokePlaybookAi` impl is available (real OR Null peer) in the compound-OFF branch. The transitive dependent `InvokePlaybookHandler` follows the same rule via its own ctor deps (`IPlaybookService` has a Null peer; `IServiceProvider` / framework primitives always resolve).
  4. **Unit test** — `InvokePlaybookAiTests.Null_InvokePlaybookAsync_ThrowsFeatureDisabledException` asserts the stable errorCode + message contract; sibling test `Null_InvokePlaybookAsync_ExceptionConvertsToProblemDetails503` verifies the `AsFeatureDisabled503()` helper produces a 503 ProblemDetails per ADR-019.
  5. **Integration test** — `InvokePlaybookHandlerTests.ExecuteChatAsync_WhenFacadeIsNullKillSwitchActive_ReturnsFailureToolResult_NotInvalidOperationException` exercises the chat-tool dispatch path: when the real `NullInvokePlaybookAi` is in the DI graph (compound-OFF), the handler converts the `FeatureDisabledException` into a `ToolResult.Failure` whose body carries the stable `ai.invoke-playbook.disabled` errorCode. This is the chat-tool path's equivalent of "503 ProblemDetails" — for non-streaming HTTP endpoints (M365 Copilot adapter, future R7+ endpoint consumers) the `AsFeatureDisabled503()` extension produces the literal 503 response.

  **Rationale**: closes the same anti-pattern that produced LATENT BUG #1 (per `bff-ai-architecture-audit-r1` audit DR-003 + DR-008). PR #351 fixed the existing 4 PublicContracts facades (`IBriefingAi`, `IInvoiceAi`, `IWorkspacePrefillAi`, `IRecordMatchingAi`) the same way; adding `IInvokePlaybookAi` without these criteria would reopen the bug class the audit closed. Coordinated with `bff-ai-architecture-audit-r1 migration-plan.md` §2.1 (canonical pattern source).
