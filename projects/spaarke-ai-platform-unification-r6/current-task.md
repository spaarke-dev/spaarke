...
| **Wave B-G3** | ✅ All 2 complete (uncommitted): 040 array dispatch (+268 LOC widget, +13 tests); 041 object dispatch (+390 LOC widget, +11 tests). Widget 1117 → 1775 LOC. Both R5 SC-18 Gap C bugs structurally fixed (tldr-as-JSON-tokens + entities-as-JSON-literal). Build 0 TS errors; ADR-021 semantic tokens throughout; 0 MB BFF delta; ~4 KB frontend bundle delta. |
| **Next Wave** | **B-G4 — CapabilityRouter dedup (task 042)**. Enforces "one user intent → one route → one playbook → one DeliverOutput" at the CapabilityRouter level (NOT action metadata). Depends on 041 + 025 (both ✅). FULL rigor. May touch DI in `Services/Ai/Chat/` → §F.1 static-scan + endpoint-di-symmetry pattern apply per Phase B sub-agent prompt alignment. |
