...
| **Wave B-G4** | ✅ Task 042 complete (uncommitted). Architectural insight: R5 Gap A duplicate-fire is NOT a CapabilityRouter bug — router is per-turn TOOL FILTER, not output router. Real duplicate-fire is LLM emitting inline text alongside tool call. Fix: CapabilityRouter publishes SelectedPlaybookId additive; SprkChatAgentFactory resolves terminal-node destination per-turn via INodeService + task 031 NodeRoutingConfig; non-chat destinations get system-prompt dedup directive (single-sentence ack — NFR-01 preserved). 25 new tests, 0 regressions, 6883/6992 pass. BFF -1.32 MB. |
| **Next Wave** | **B-G5 — Phase B integration test (task 048)**. End-to-end vertical-slice test: TL;DR + Entities + dedup + pre-fill regression. Depends on 042 + 034 + 035 (all ✅). STANDARD rigor. |
