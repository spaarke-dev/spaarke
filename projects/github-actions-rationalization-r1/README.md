...
- [ ] Every workflow in `.github/workflows/` either passes a clean run on its declared triggers OR is deleted with rationale (FR-01, FR-05, FR-06)
- [ ] Current master CI failure (run 26755019759) is root-caused and either fixed or documented (FR-02)
- [ ] `actionlint` is a required status check on master AND blocks a deliberate-fail PR (FR-07, FR-08, FR-13)
- [ ] Workflow count is rationalized to ≤8 (down from 13) (FR-06)
- [ ] `.github/WORKFLOWS.md` exists with per-workflow purpose, owner, SLA, trigger model (FR-09)
- [ ] `docs/procedures/workflow-incident-response.md` exists with runbook (FR-10)
- [ ] Weekly workflow-health report runs and creates/updates an issue (FR-11)
- [ ] Notification routing setup is documented; owner confirms `dev@spaarke.com` is receiving spaarke-dev notifications (FR-12)
- [ ] Rolling 7-day workflow success rate ≥90% across remaining workflows (FR-14)
