# Spaarke AI Widget — Summarize a Document (r1)

> **Project ID**: `spaarkeai-widget-summarize-document-r1`
> **Methodology**: Product-led — see [`docs/procedures/PRODUCT-LED-DEVELOPMENT.md`](../../docs/procedures/PRODUCT-LED-DEVELOPMENT.md)
> **Status**: Design — pre-pipeline
> **Renamed from**: `P1-summarize-document-v1` (2026-06-28 — naming convention finalized to `{surface}-{verb}-r{round}`)
> **Owner**: Product
> **Created**: 2026-06-26
...
this project is the first project in a **product-feature-led project sequence** (this project ships first; follow-on projects layered on top). Each project ships a coherent product feature that a salesperson can demo end-to-end. this project picks the closest-to-done verb — Summarize a Document — and **productizes it fully**: UAT-passed, demo-ready, documented, sales-trained.
...
3. **Multiplier seeding for the follow-on drafting project.** Drafting (follow-on project) shares 4 of the 5 build items with Summarize. Building them in this project with paragraph-structure + rich-text extensibility in mind means the follow-on drafting project inherits 60% of its scope from this round. Sequence: this project first, drafting next, intake after.

### 1.3 What this project explicitly does NOT ship

- **Open-ended drafting** (memo, brief, clause) — follow-on drafting project scope
- **NDA / agreement drafting** — follow-on drafting project scope
- **In-Word AI redline surface** — strategic bet, separate project (spaarkeai-word-redline-r1 (strategic bet — separate project))
...
- **E-signature integration** — out of scope (DocuSign/AdobeSign integration is follow-on project later)
- **Multi-document tabular review** — follow-on project later
- **Cross-session matter memory promotion workflow** — substrate inherited from R6, full workflow is post-shipment
- **Outside counsel work-product summary intake automation** — overlaps but is follow-on intake project scope; this project ships the summarize verb, not the auto-intake pipeline
...
- We have nothing better than competitors on general open-ended drafting (follow-on drafting project scope, partially addressable)
...
| Component | Location | What it does for this project |
|---|---|---|
...
| **Daily Briefing infrastructure** (recently rewritten) | `DailyBriefingEndpoints.cs` task-031 | Reference pattern for thin-dispatch endpoints via `IConsumerRoutingService` + `IInvokePlaybookAi`. this project endpoints follow this pattern. |
...
|---|---|---|
| **RAG retrieval** (verb #4 follow-up Q&A) | AI Search service deleted in dev — `spaarke-search-dev.search.windows.net` NXDOMAIN | Sister project: `spaarke-ai-azure-setup-dev-r1`. Hard dependency for this project graduation. Single-document Q&A (LLM with file in context) still works without AI Search; matter-RAG does not. |
...
### 4.3 Built but silently broken (R6 surface-completion gaps relevant to this project)
...
this project leverages this passively. We don't extend it; we surface its effect (pinned facts visibly influence summary output, with affordance to "show what's pinned that affected this answer").
...
### 5.2 The 4 multipliers (designed to serve follow-on drafting reuse too)

These are the components that, if built right, also unlock the follow-on drafting project (NDA pattern / general) with minimal additional work.
...
- **Designed for follow-on drafting reuse**: No design constraint — same handlers serve drafting tab updates.
...
- **Designed for follow-on drafting reuse**: **CRITICAL** — must handle paragraph structure, not just bullets, because follow-on drafting (NDAs) requires paragraph documents. Spec'd from day 1 with `EditableRichTextSection` variant for paragraph content. the follow-on drafting project reuses this with extended section schema for NDA clauses.
...
- **Designed for follow-on drafting reuse**: **CRITICAL** — must handle paragraph structure (sections, clauses) not just bullets / TL;DR. Spec'd day 1 with section/clause emit. the follow-on drafting project reuses this directly.
- **Effort**: ~2-3 weeks.
...
- **Designed for follow-on drafting reuse**: Same endpoint serves any future "email this" verb (NDAs, briefs, anything in workspace tab). Spec endpoint as generic "email artifact" with artifact-type discriminator.
- **Effort**: ~2-3 weeks.
...
this project picks up the following items from prior project backlogs because they're load-bearing for the user verbs:

### 6.1 From R1 Phase 4 deferred (chat-routing-redesign-r1 MVP cut)

| Item | What | Why this project needs it | Folded into |
|---|---|---|---|
...
NOT folded in: `WriteSessionMemoryHandler` (T2), promotion workflow (T3 → matter memory), enrichment pipeline (`FileClassificationService` etc.) — **the latter was misframed earlier; subject-area routing via playbook vector match makes a separate classifier unnecessary for this project.**
...
| **R6 Audit-item-01: JSON schema validation guardrails** | Defensive — if playbook output drifts from schema, fail soft not 500 | Already partly handled by `ProjectPlaybookResultToNarrateResponse` pattern (today's daily-briefing-r4 task-031 fix). Apply same pattern to this project summarize projection. | Built into Multiplier C design |

### 6.3 From R7-backlog (just filed this session)

| Item | What | Why this project needs it | Folded into |
|---|---|---|---|
| **DEF-003 `OrchestratorPromptContext.MatterName` always null** | Decision: prune dead code OR wire matter name into prompts | Matter-area-aware summary benefits from matter context in prompt. Decision needs to happen in this project. | Open question Q4 (see §13) |
| **ISS-001 user message verbatim embed** | LLM prompt construction doesn't escape `"` in user messages | If this project endpoint constructs any prompts, apply defensive escape | Spec section in Multiplier C / D code review |
| **DEF Chat ↔ workspace write-side unification** | Chat-tool tab creation feels slower (~500ms-2s poll) vs playbook-output tab (instant SSE) | Verb #2 + verb #5 latency. Affects perceived product quality. | **Conditional escalation gate** — if UAT surfaces summary-tab-mount latency as a quality complaint, escalate into this project scope. Otherwise stays in R7-backlog. |

### 6.4 From sister projects (hard dependencies)

| Sister project | What it delivers | this project dependency |
|---|---|---|
...
| **`spaarke-redis-cache-remediation-r1` (closeout)** + **`-r2` (just spec'd)** | Redis flip-on + IConnectionMultiplexer registration + tool-list cache + token cache | **SOFT** — this project functions on in-memory CacheModule fallback for single-instance dev. Multi-tenant production scale needs r2 to land. this project graduation criteria: "works in dev" — Redis-required scale is post-shipment. |
...
- ≤60 MB (CLAUDE.md §10 binding). Current baseline 46.67 MB. this project budget: +5 MB cumulative (multipliers C + D combined ≤+5 MB).
...
Spaarke approach: The workspace tab IS the artifact. Edit in place. Save to system as a real Dataverse record. Email with deep link back to the record. Tab persists across chat sessions for the same matter.
...
Spaarke approach: SPE container is OUR container, per matter. Save-to-matter is one click. No integration needed.
...
Spaarke approach: badge on summary tab "Influenced by N pinned facts." Click to see which pins applied. Closes the trust loop.
...
| `spaarke-redis-cache-remediation-r2` (spec just landed) | Spec done | Redis flip-on + IConnectionMultiplexer + cache layer | **SOFT** for this project dev graduation; HARD for prod scale |
...
|---|---|
| Follow-on drafting project (NDA pattern + general) | This round's Multipliers B + C designed for paragraph structure. The follow-on drafting project inherits, does not rewrite. |
| Follow-on projects after drafting | Pattern this project establishes (product-feature project framing, multiplier identification, deferred-item folding) becomes template. |
| R6 closeout (UAT) | Don't merge PRs until R6 closes — minor scope guard. |
...
|---|---|---|
| Open-ended drafting (memo, brief, clause) | Separate product verb. Reuses this project multipliers. |Follow-on drafting project|
| NDA / agreement drafting | Same. |Follow-on drafting project|
| In-Word AI redline surface | Strategic bet — biggest single competitive gap but biggest single effort. Separate project. | **spaarkeai-word-redline-r1 (strategic bet — separate project)** (strategic bet, not in numbered P sequence yet) |
| Outlook AI compose UX inside Outlook | this project ships outbound send FROM Spaarke. In-Outlook compose is a separate add-in scope. | **follow-on project later** |
| Teams send / Teams app | SMB segment uses Outlook; manual link works | **Deferred indefinitely** |
| E-signature integration (DocuSign/AdobeSign) | New external integration. Significant effort. Doesn't gate summarize feature. | **follow-on project later** |
| Multi-document tabular review | Distinct verb. Substantial UX surface. | **follow-on project later** |
...
this project graduates when ALL of the following are true:
...
- [ ] Multiplier C: `IDocumentArtifactService` ships; Word + PDF generation work for both summary (this round) and section-structured (follow-on drafting readiness) outputs
- [ ] Multiplier D: outbound email endpoint ships; OBO + MI paths both tested

### 12.2 Quality graduation
- [ ] BFF publish size ≤ 60 MB (binding); this project delta ≤ +5 MB cumulative
...
- [ ] Redis flip-on landed in dev (sister project complete) OR explicit acceptance of in-memory CacheModule for this project graduation

---

## 13. Resolved owner decisions (made during this design discussion)

These decisions are LOCKED for this project. Recording them here so `/design-to-spec` doesn't re-raise them and so the trail is auditable.

| Decision ID | Decision | Rationale | Date |
|---|---|---|---|
| D-1 | Adopt **product-led project framing** (descriptive surface+verb+round naming) replacing technical-project framing (Rn) | Sales-driven prioritization; productization in-scope by definition | 2026-06-26 |
| D-2 | **Summarize first**, then Drafting follow-on, then Intake/Outside-counsel | Closest to done; multipliers seed follow-on drafting; lowest infra risk | 2026-06-26 |
...
| D-4 | **4 area-specific playbooks + 1 generic refresh** for this project v1 (vendor-contract, employment-agreement, real-estate-lease, IP-agreement, generic) | Covers ~80% of corporate-SMB in-house work without combinatorial explosion | 2026-06-26 |
| D-5 | **Multipliers B + C designed for paragraph structure from day 1** | Required for follow-on drafting reuse; trivial to spec day-1, expensive to retrofit | 2026-06-26 |
| D-6 | **Editable widget = real rich text**, NOT plain textarea | Follow-on drafting paragraph editing depends on it; Fluent v9 RichTextEditor preferred, community lib fallback acceptable | 2026-06-26 |
...
| D-11 | **Graceful degradation when AI Search down**: single-doc Q&A works; matter-RAG returns clear unavailability message | Sister project ETA unknown; don't block this project demo | 2026-06-26 |
| D-12 | **Wire `OrchestratorPromptContext.MatterName`/`ActivePlaybookName`** in this project (R7-DEF-003 closed) | Small effort; real area-aware prompt quality benefit | 2026-06-26 |
| D-13 | **Out of scope**: drafting (separate project), in-Word AI surface (separate project), Teams send, e-signature, multi-doc tabular review, voice/mobile | Scope discipline; each is its own project | 2026-06-26 |

## 14. Applicable ADRs

Per the existing chat-routing-redesign-r1 project pattern, these ADRs are BINDING for this project. `/design-to-spec` and `/task-execute` will auto-load these as constraints.

| ADR | Why this project touches it |
|---|---|
| **ADR-001 Minimal API** | New endpoints `/api/ai/summaries/{tabId}/save` + `/api/ai/summaries/{tabId}/email` |
...
| **ADR-029 BFF Publish Hygiene** | Per-task `dotnet publish` measurement; this project cumulative budget ≤+5 MB; current baseline 46.67 MB; ceiling 60 MB |
...
| **ADR-033 Streaming chat-tool side channel** | Multiplier A handlers (update/send/close tab) respect existing side-channel pattern; no new SSE channel for this project |
...
| `src/widgets/EditableStructuredOutputWidget/EditableRichTextSection.tsx` | **NEW** | Rich-text field variant (designed for follow-on drafting paragraph support — D-5, D-6) |
...
| Path | Change type | Purpose |
|---|---|---|
| `docs/guides/SUMMARIZE-DOCUMENT-USER-GUIDE.md` | **NEW** | 1-page end-user help doc (graduation criterion) |
| `docs/guides/SUMMARIZE-DOCUMENT-DEMO-SCRIPT.md` | **NEW** | 15-minute sales demo script (Appendix promoted to file) |
...
- **MUST** measure publish size on every BFF-touching task; budget ≤+5 MB cumulative for this project
...
- **AI Search restoration** lands before this project graduation (sister project `spaarke-ai-azure-setup-dev-r1` — TBD). If not, D-11 graceful degradation applies; matter-RAG demo deferred until restoration.
- **Redis flip-on** is acceptable to defer past this project dev graduation; multi-instance prod scale requires sister project `spaarke-redis-cache-remediation-r2`.
- **R6 closeout completes** during this project design + early execution; PRs do NOT merge to master until R6 is closed (per R7 backlog coordination rule).
- **R1 (current branch) closeout** completes before PRs land — task 146 UAT regression + task 150 wrap-up.
- **JPS playbook framework + scope catalog** are stable through this project — no breaking changes to playbook schema during execution.
- **Spaarke Auth v2 (ADR-028)** is fully provisioned in spaarkedev1 — MI Graph permission grants in place, including `Mail.Send` for email feature (verify per `auth-deployment-setup.md` §5 before this project wave 2).
...
- **Fluent v9 RichTextEditor** is production-ready at our quality bar OR a community lib (TipTap / Slate) integrates cleanly with semantic tokens — spike required in this project wave 1.
...
|---|---|---|---|---|
| 1 | AI Search sister project slips → this project demo can't show matter-RAG | Medium | High | D-11 graceful degradation; demo with single-doc Q&A as primary, matter-RAG as v1.1 |
| 2 | Fluent v9 RichTextEditor doesn't exist / not production-ready → Multiplier B technology choice forced | Medium | High | Spike in this project wave 1; community lib (TipTap / Slate) fallback list pre-evaluated |
...
| 4 | Graph `Mail.Send` scope provisioning blocked by tenant policy | Low | High | Verify pre-this project wave 2; MI fallback path exists |
...
| 10 | Multiplier B widget scope creep absorbs Wave 2 (1-2 month component) | Medium | Medium | Strict scope: TL;DR + Summary + Key Takeaways + Entities sections only; defer extensibility hooks to follow-on drafting project |
...
| **W13** | Project wrap-up | `/repo-cleanup`, lessons-learned for the follow-on drafting project template, archive | <1 week | — |
...
These fields are always null today. this project area-aware summarize would benefit from matter name + active playbook name in prompt construction. Two options:
- (a) Wire them in `AgentRequest` → `BuildPromptContext` pathway (~1 day in this project scope)
- (b) Leave deferred to R7; document workaround (template parameter passing in playbook)

**Recommendation: (a)** wire it in this project — small effort, real prompt-quality benefit.
...
- (c) Plain `Textarea` per field (degrades follow-on drafting paragraph-edit experience)

**Recommendation: investigate (a) first, (b) as fallback. Do NOT pick (c)** — Follow-on drafting paragraph editing depends on this being real rich text.
...
### Q9: this project dependency on AI Search restoration — block or proceed?

- (a) Block this project until sister project lands
...
- **Close out** — wrap-up task; portfolio status updated; lessons captured for follow-on project templates
