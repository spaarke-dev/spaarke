...
| **Next Action** | Execute **cluster 1 (re-anchor)** exactly as specified in [`notes/070-composeservice-seam-map.md`](notes/070-composeservice-seam-map.md) § "Cluster 1 — executable extraction spec". Everything is pre-verified: source range **2593–3065**, **5 ctor deps** (named, with line numbers), 2 internal + 3 private members, and **only 2 call sites change** (`SaveAsync:1401`, `SaveAsync:1473` — prefix only, argument lists unchanged). Do NOT re-derive the analysis. Then build + run the Compose seam/op-log suites **before** starting cluster 4 (POML step 3: verify after EACH extraction, not once at the end). |
