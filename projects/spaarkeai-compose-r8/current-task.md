...
| **Next Action** | **ONE gate left: UAT section B — the `section-break-flattened` accept/decline** (five → six signed residual-loss rows). ✅ **Track A PASSES** (UAT round 2 #4: saved, reopened, edits held). ✅ Numbering round-trip CONFIRMED working (#3: Word shows the numbering) — round-1 item 4 is display-only. ✅ Round-2 items 5/6 + half of 2 fixed and deployed. 🔔 One owner question open: do we want TRUE server-side autosave? Then rewrite PR #924's title and merge. See `notes/uat/uat-round-2-findings.md`. |
