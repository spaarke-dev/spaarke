...
| 052b | **Stale-target DETECTION durability** · 052's *answer* is ledger-durable but the *question* is `sessionStorage`, so a cross-tab reopen / >200 suggestions / storage-disabled silently restores the pre-052 overwrite. Detection only — do NOT re-open O-1…O-6 or revive `target_text` | FR-C05 residual | FULL | opus/xhigh | ❌ | 052 | 🔲 |
| 053 | **Bounded confirmable fallback** + verify the dead-end is **unreachable**; no UAT-21 regression. 052 left the structural boundary: `resolveLegacyReplayedSpans`, pinned to `strict`, the sole remaining edit-path caller of `resolveTargetSpans` | FR-C06/07 | FULL | opus/xhigh | ❌ | 052 | 🔲 |
