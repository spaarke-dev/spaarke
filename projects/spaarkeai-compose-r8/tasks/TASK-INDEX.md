...
| 059 | **SECURITY — a request can no longer name its own tenant.** Filed for the `X-Tenant-Id` header; enumeration found **21 sites / 3 mechanisms**, and the filed one was the LEAST severe (latent — the claim short-circuits it). The live class was an unfiled **`?tenantId=` query string on 4 routes, 2 of them Compose's own** (document open/resume + annotations), 3 consulting **no claim at all**. Closed structurally: `TenantResolution` takes a `ClaimsPrincipal`, not an `HttpContext`, so it cannot reach the request. 4 non-contract fixtures repaired; 2 vacuous tests fixed. **Owner-signed-off 2026-08-28** (notes §9). Sign-off also DIRECTED the cross-user gap be fixed, not accepted — that is issue **#863** (session ownership), in progress; and RagEndpoints body-supplied tenant documented + deferred (different, lower class: an API-key principal has no tenant claim to bypass) | — | FULL | opus/xhigh | ❌ | 060 | ✅ |
...
| 063 | **Durable erasure** — enumerates by tenant/session **PREFIX** (proven: an orphan blob no manifest names IS erased; a manifest-driven break deleted 0 of 2), then **RE-ENUMERATES to verify**. Without that verify step the naive impl returned **204 for a session whose bytes still existed**. Tri-state: Erased / StoreDisabled / **Incomplete ⇒ 500, session left intact** (fail-closed is cheap *because* erasure is prefix-driven — the retry converges with no manifest). **0 new store surface, 0 new DI** — composes 062. ⚠️ **code gate closed, but see the arming warning** | FR-B06 | FULL | opus/xhigh | ⚠️ not parallel with 061/062 | 062 | ✅ |
