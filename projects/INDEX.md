...
|---|---|---|---|---|---|---|---|---|
| `spaarkeai-compose-r3` | `work/spaarkeai-compose-r3` | `C:/code_files/spaarke-wt-spaarkeai-compose-r3` | Y | Y | N | N | 2026-07-16 | **INITIALIZED 2026-07-16** (`project-pipeline`) — Word-feature **fidelity**: E1 retained-original delta save (`Docxodus` `WmlComparer`, SkiaSharp-excluded) + E2 `w14:paraId` identity + E3 grounding-tied confidence + editing toolset + import round-trip. **BFF=Y is NARROW**: extends `Services/Compose/*` (`Save`/`LoadAsync` delta-onto-original), REUSES `DocxAnnotationWriter`/`DocxAnnotationReader`/`AnnotationReanchorService`; adds `Docxodus` NuGet (+2.44 MB). **Successor to `spaarkeai-compose-r2` — confirm R2 merged/frozen before the E1 cutover (task 022).** **Engine frozen (ADR-039)**: no new dispatch endpoint, no catalog rows; E3 is server-derived. **⚠️ Consume `spaarke-ai-architecture-redesign-r2` `PublicContracts` seams — NO fork of `Services/Ai/`**; `/conflict-check` before each BFF PR. SpaarkeAi=Y: `Spaarke.Compose.Components` toolset + editor. Skill=N. |
