...
| ID | Title | Status | Dependencies | Blocks | Hours | Rigor | Parallel |
|---|---|---|---|---|---|---|---|
| 030 | Deploy `alerts.bicep` to dev + KQL verification (NFR-07) + publish-size delta (NFR-04) | 🟡 PARTIAL | All Phase 1-3 | 031 | 2 | FULL | serial — **Offline parts done 2026-06-26**: BFF publish-size delta = **+0.01 MB** apples-to-apples (R1 46.67 MB → R2 46.68 MB after master merge — initial pre-merge 49.66 MB was confounded by 50 master commits incl. AI-search size reductions). Runbook for live deploy + KQL verification in `notes/post-deploy-verification.md`. ⏸ OPERATOR: alerts deploy + BFF deploy + 10-min KQL propagation + force-reboot dry-run + workflow_dispatch dry-run. |
