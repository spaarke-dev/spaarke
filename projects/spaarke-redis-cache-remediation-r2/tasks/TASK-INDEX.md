...
| 003 | `cache.hits.by_resource` + `cache.misses.by_resource` at TenantCache | ✅ | 002 | 030 | 2 | FULL | Group B — **Done 2026-06-26**: 2 new Counters added to canonical `CacheMetrics` static class; emitted from `TenantCache.GetAsync<T>` + `GetStringAsync` hit/miss paths with `resource` dimension. Set/Remove/Refresh paths untouched per FR-03 scope. |
...
| 005 | Decorator regression integration test (`MetricsDistributedCacheRegistrationTests`) | ✅ | 002 | 030 | 2 | FULL (TEST-MODIFYING override) | Group B — **Done 2026-06-26**: NEW test class in `tests/integration/Sprk.Bff.Api.IntegrationTests/Cache/` (POML had stale project-name alias — actual is `Sprk.Bff.Api.IntegrationTests`). 2 tests pass: (a) decorator wraps inner + double-decoration guard, (b) MeterListener confirms exactly one `Meter("Sprk.Bff.Api.Cache")`. §F.2 fix: `RecallSessionFileHandlerResolvableTests.cs` IDistributedCache → ITenantCache (pre-existing chat-routing-redesign-r1 task 091 break). |
...
| ID | Title | Status | Dependencies | Blocks | Hours | Rigor | Parallel |
|---|---|---|---|---|---|---|---|
| 030 | Deploy `alerts.bicep` to dev + KQL verification (NFR-07) + publish-size delta (NFR-04) | ✅ | All Phase 1-3 | 031 | 2 | FULL | serial — **Done 2026-06-29 (live deploy)**: Created `ag-spaarke-oncall-dev` action group (dev@spaarke.com receiver). 4 alerts deployed + enabled. BFF redeployed: 46.71 MB = **+0.04 MB** vs R1 baseline (NFR-04 ✅). KQL acceptance MET with real traffic: FR-01 cache.failures = 29 rows (outcome=connection, op=set/get), FR-03 cache.hits.by_resource = 3 bounded resources (cmd-catalog/session/stored-session — within NFR-06). R1 instruments preserved. Full evidence in `notes/post-deploy-verification.md` §5-6. |
| 031 | Close GitHub Issues #462 / DEF-007 / DEF-008 / DEF-009 + flip R1 `defer-issues.md` | 🔲 | 030 | 032 | 1 | MINIMAL | serial |
| 032 | Project wrap-up (code-review + adr-check + repo-cleanup + README → Complete + lessons-learned) | ✅ | 031 | — | 2 | FULL | serial — **Done 2026-06-26**: code-review clean (0 critical / 0 warning / 2 deferred suggestions); adr-check 5/5 focus ADRs compliant; lessons-learned.md authored (3 sections + cross-refs); README.md status flipped to ✅ Complete (PR pending); current-task.md reset to PROJECT COMPLETE; `dotnet build` clean (0 err / 0 warn); draft-pr-description.md authored for operator. INDEX.md row stays in Active per R1 convention until PR merge. |
