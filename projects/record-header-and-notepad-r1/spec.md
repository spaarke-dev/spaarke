...
**FR-06** `useRelatedCount(entity, recordId, relatedEntity)` hook returns `{ count, loading, error }`; internally calls `Xrm.WebApi.retrieveMultipleRecords` with `$count=true&$top=0` OData query filtered to related rows for the given record.
   *Acceptance*: `count` is `number` when loaded; used for `sprk_todo` count and `sprk_memo` count. Filter query built correctly for the `sprk_memo` case: `$filter=sprk_regardingrecordid eq '{recordId}'` (see Owner Clarifications).
...
**FR-08 (REVISED per owner clarification O2)** Sparkle icon on click opens a **Fluent v9 popover** (not a modal) anchored to the sparkle button. Popover content:
   - If `sprk_recordsummary` exists for this record → render the summary body inline within the popover (scrollable if long).
   - If not found → render an empty-state message (e.g. "No summary yet").
   - Popover header/footer contains a **refresh icon** (see FR-08a).
   *Acceptance*: no `Xrm.Navigation.navigateTo` call for sparkle in R1; popover renders summary content when present; empty-state renders otherwise. Sparkle icon has no badge.
...
**FR-14** Notepad queries `sprk_memo` records where `sprk_regardingrecordid eq '{regardingId}'` (see Owner Clarifications for field decision), orders by `createdon desc`, and opens the most-recent record in the editor by default. If none exist, editor is empty and a note-list dropdown is empty.
   *Acceptance*: query correct; sort correct; empty state renders cleanly.

**FR-15** Notepad `+` (new memo) button creates a new empty `sprk_memo` row via `Xrm.WebApi.createRecord` with `sprk_regardingrecordid` set to the current `regardingId` value, `sprk_body` empty, and immediately switches the editor to the new record.
   *Acceptance*: create call is single Xrm.WebApi.createRecord; new record's id is captured and used for subsequent saves.

**FR-16** Notepad `list` icon opens a dropdown showing prior memos (most-recent first), each item showing a derived title (first non-empty line of `sprk_body`, truncated) + `createdon` date. Clicking an item switches the editor to that memo.
   *Acceptance*: dropdown lists all memos for this record; derived-title logic in `utils/deriveTitle.ts`; click switches editor and cancels any pending debounce.

**FR-17** Notepad saves the current `sprk_body` to the current `sprk_memo` via `Xrm.WebApi.updateRecord`:
...
**FR-19** Notepad launch contract (`?regardingEntity=<logical>&regardingId=<guid>`) MUST be entity-agnostic. Verified by a test-only launcher wiring a synthetic non-Matter entity + record during QA (e.g. launch Notepad from a test harness with `regardingEntity=sprk_project&regardingId=<synthetic-guid>`); Notepad must render and behave identically.
   *Acceptance*: QA test documented in the authoring guide; contract fields do NOT include Matter-specific assumptions.
...
- **ADR-024** — 11-entity regarding relationship. See §ADR Tensions below — `sprk_memo` does not participate in this pattern; uses a text field.
...
|---|---|---|---|---|
| **ADR-024** | "11-entity regarding relationship" — the Spaarke norm for `regarding` associations across entities like `sprk_todo`, `sprk_task`, etc. | `sprk_memo` uses a plain-text field `sprk_regardingrecordid` (GUID-only string) rather than participating in the 11-entity polymorphic `regardingobjectid` lookup pattern. This is a pre-existing schema decision of the `sprk_memo` entity — not introduced by this project — but it means R1 code queries and creates memos in a non-standard way. Consequences: no Dataverse-enforced referential integrity between memos and their regarding record (no cascade delete, no relationship navigation via `Xrm.WebApi.retrieveRelated`). Application-level linkage only. | **A (project-scoped exception)** | `sprk_memo` already ships with this schema; changing the entity is out of R1 scope and would break `MemoSection.tsx` in EventDetailSidePane. Path A documents the deviation for both R1 and any future consumer that adopts `useSprkMemoRepository`. A future ADR amendment (Path B) may be warranted if a broader review of Spaarke's regarding-relationship patterns finds the 11-entity model insufficient — but that reassessment is outside R1's charter. |

*No other ADR tensions surfaced at design time. ADR-006, ADR-012, ADR-021, ADR-022, ADR-028, ADR-032, ADR-038 all apply without exception.*
...
|---|---|---|---|
| **O1** `sprk_memo` regarding | How is a memo's regarding record modeled? | **Text field `sprk_regardingrecordid`** (not a lookup, not part of ADR-024's 11-entity pattern). | FR-06, FR-14, FR-15 all use `sprk_regardingrecordid eq '{guid}'` in queries and `sprk_regardingrecordid` in create payloads. Also drives the ADR-024 exception in ADR Tensions above. |
| **O1a** field format | What exact string is stored in `sprk_regardingrecordid`? | **GUID only** (e.g. `a1b2c3d4-...`), no entity prefix. | Queries and creates use `recordId` directly with no serialization. Cross-entity queries are not supported by this field alone (application-layer concern). |
