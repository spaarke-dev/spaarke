...
| **PA-2** Initial 3 practice areas (Wave A3 output) | Which 3 practice areas ship in the initial Wave D2 / D3 implementation? | **CTRNS (Commercial Transactions), IPPAT (IP Patents), BNKF (Banking & Finance)** — per Wave A3 selection (task 012, 2026-06-02). Rationale: three distinct legal substrates (transactional, technical, financial), high SME readiness, full breadth of the 2D taxonomy. Live `sprk_practicearea_ref` query confirmed all 6 rows active in Spaarke Dev. **Owner sign-off pending** — see `design-a3-2d-taxonomy.md` §3.2 + §8 for swap-cost analysis (swap is cheap until D7 fixtures seeded). | Wave D1 (task 030) seeds `sprk_documenttype_ref` with 5 doc types per area (15 rows); Wave D2 (031) authors 3 Layer 1 prompts; Wave D3 (032) authors 5 Layer 2 schemas for the high-value `(area, type)` pairs `CTRNS×CLOSING_STATEMENT`, `CTRNS×ASSET_PURCHASE_AGREEMENT`, `IPPAT×PATENT_APPLICATION`, `IPPAT×OFFICE_ACTION`, `BNKF×LOAN_AGREEMENT`. MA / IPTM / APPL deferred to post-Phase-1.5 customer-onboarding cadence. |
...
- **Initial Wave D2 practice area count**: Wave A3 design task confirms by Dataverse query; initial Wave D2 implementation targets **3 high-value practice areas** (selection deferred to A3 based on SME readiness + document variety). **Resolved 2026-06-02 — see PA-2 above**: CTRNS, IPPAT, BNKF (owner sign-off pending).
...
- [x] **Q-D2-1** — **Resolved by PA-2 (2026-06-02)**: initial 3 = CTRNS, IPPAT, BNKF (owner sign-off pending). See `design-a3-2d-taxonomy.md` §3.2 for selection rationale.
