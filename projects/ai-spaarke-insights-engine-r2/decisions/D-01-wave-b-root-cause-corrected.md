...
|---|---|---|---|
| Q1 | Is `sprk_actiontype` (int) writeable on `sprk_analysisaction`? | ✅ **Resolved** | **Yes (assumed writeable; pending empirical confirmation in task 002).** Per `docs/architecture/playbook-architecture.md` line 54: ActionType is canonically stored in `sprk_playbooknode.sprk_configjson.__actionType` (used when node has no `sprk_actionid` — the "structural node" path at `PlaybookOrchestrationService.cs:938`). `sprk_analysisaction.sprk_actiontype` (int) is the OTHER source — consulted at `PlaybookOrchestrationService.cs:929` when node has `sprk_actionid` FK. The C# code (`AnalysisActionService.cs:392`) maps it via `[JsonPropertyName("sprk_actiontype")]`. The MCP TSQL rejection on direct query is likely metadata-cache staleness or option-set exposure quirk. **Task 002 will set this field via `mcp__dataverse__create_record` — success/failure confirms writeability.** Both dispatch paths (FK→action.ActionType AND configjson.__actionType) must be aligned per node. |
| Q2 | What is the lookup target entity for `sprk_actiontypeid`? Does it have a numeric code field deriving the ActionType integer? | ✅ **Resolved (deferred — not load-bearing)** | **Less critical than originally framed.** Per `PlaybookOrchestrationService.cs:929`, dispatch reads `action.ActionType` (int) from `sprk_analysisaction.sprk_actiontype` field — NOT from the lookup. The `sprk_ActionTypeId` lookup is for display purposes (label like "02 - Classification" visible on the Action form). Wave B does NOT require populating the lookup. The 6 new sprk_analysisaction rows can leave `sprk_ActionTypeId` null; setting `sprk_actiontype` (int) is sufficient for runtime dispatch. (Future audit: if dispatch ever fails because lookup is required, revisit.) |
