...
| §7.3 ERD | Regenerate from `docs/data-model/entity-relationship-model.md` (~75–80 real `sprk_*` entities). Grep trap: `sprk_party` returns hits, but only as fabricated names inside two test fixtures (`FetchEndpointTests.cs`, `UpdateRecordNodeExecutorTests.cs`) — the entity does not exist; the other seven NOT FOUND entities have zero occurrences even as string literals. `sprk_organization` = Party; `sprk_billingevent` = invoice line; `sprk_servicerequest` exists and is wired into the Association Engine while Front Door r1 proposes a new `sprk_legalrequest` — **duplicate Request object to settle**. |
...
| D-i | Foundry IQ vs hand-built RAG | Inventory complete (item 9); repo already documents "hybrid, evaluate post-GA, V1 MUST NOT" (Insights §22.1) — D-i re-opens that gate; needs external research (§7) |
