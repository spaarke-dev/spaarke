...
- **Visual anchor (mirror verbatim in spirit)**: `ConsumerChips.tsx` `chip` style class — `tokens.borderRadiusMedium`, `tokens.colorNeutralBackground1` bg, `tokens.colorBrandForeground2`, 1px `tokens.colorBrandStroke2` border, `tokens.shadow2`, hover `colorBrandBackground2Hover`/`Pressed`, disabled `colorNeutralBackgroundDisabled`. Tokens ONLY (ADR-021 dark-mode-correct). Test-id convention `consumer-chip-{id}` → use `suggestion-card-{suggestionId}`.
- **Client contract (task 021)**: `getNotificationsClient().registerHandler("suggestion", cb)` from `@spaarke/notifications` (notificationsBootstrap.ts already registers a LOG-ONLY handler — 051 must register its OWN handler via the same singleton, NOT duplicate the client). The SignalR signal is `{outboxRowId, kind}` only (task 020); the client fetches the envelope. **TODO on resume**: read `@spaarke/notifications` package to confirm the exact `NotificationEvent`/envelope type delivered to handlers (does it carry the full 013 `SuggestionEnvelope`: suggestionId/source/regardingRecordId/title/actionHint/expiresAt? and the poll/re-fetch surface = task-022 `/api/notifications/pending`).
- **Dispatch/fail pattern**: `useConsumerChips.runBindingDispatch` — single-dispatch-per-click, `.catch()` → `inject(makeLocalAssistantMessage("Sorry — I couldn't run that action..."))` (ADR-019 stable local line, never raw error). The suggestion card FRONT-LOADS a re-fetch/re-ground BFF call before dispatch.
- **Design (3 files)**: (1) `SuggestionCard.tsx` — compact bordered card mirroring the `chip` class; (2) `useSuggestionCards.tsx` — subscribe via registerHandler, keep envelope list, **pre-mount expiry filter** (`expiresAt < now` → excluded, not disabled), click → **re-fetch/re-ground** (freshness+access re-check via task-022 `/pending` — confirm the outboxRowId is still pending) → on valid: invoke the shared dispatch; on stale/revoked: graceful local message, NO dispatch; (3) wire into `ConversationPane.tsx` as a NEW render slot (NOT the `consumerChipsSlot` transcript footer — a suggestion arrives independent of a dispatch turn).
- **⚠️ REAL COUPLING to task 052 (surfaced, not blocking)**: the `SuggestionEnvelope` carries `actionHint` (e.g. "review") + `regardingRecordId` — NOT a `bindingId`. The actionHint→dispatch mapping is **task 052's re-entry contract** (not yet built). Per POML notes, design 051's action to route through a HOST-PROVIDED `onSuggestionAction(envelope)` callback = the 052 plug-point (which wires `dispatchConsumer`/`launchSurface`). 051 owns: visual + expiry-filter + re-fetch-before-action + graceful-fail + dark-mode; 052 owns: actionHint→binding dispatch. This respects the escalation trigger (NO second dispatch pipeline — the card routes through the shared dispatch via the host callback).
- **Tests**: `SuggestionCard.test.tsx` — renders-from-envelope; expired-not-rendered (absence); click-re-fetches-before-dispatch (call-order, mocked BFF+dispatcher); stale-fails-gracefully; **dark-mode** (webLightTheme vs webDarkTheme, token-only). Run the SpaarkeAi vite suite (`npm run test`); confirm ConsumerChips/ConversationPane tests unmodified.
- **Next action on resume**: read `@spaarke/notifications` event type + `ConversationPane.tsx` slot wiring → implement the 3 files + tests → `npm run test`/`build:prod` (SpaarkeAi) → gates → notes → TASK-INDEX 051→✅. NOTE: SpaarkeAi uses `npm install --legacy-peer-deps` (not `npm ci`) per root CLAUDE.md §12.
