...
| ADR-026 (Full-page Custom Page standard) — **added 2026-07-15 (W4 pivot)** | The `sprk_communication` record surface should be a standalone full-page Code Page | For a **record-review** surface, replacing the OOB model-driven form discards native field layout, rich-text body editor, subgrids, ribbon, save, and audit — and departs from R1's shipped OOB-form base — with no proportional benefit; the OOB 66/34 accessories column already provides the "right rail" the design wanted, and the form already hosts a Spaarke PCF (RELATED RECORD v1.4.6). | **A (project-scoped exception)** | Keep the OOB form; enhance with PCFs (Connections PCF + Actions PCF). Lower-build, native, reversible, and already the deployed pattern here. Reserve Code Pages for genuinely bespoke full-page surfaces, not record forms. Full record: `notes/W4-architecture-pivot-oob-form-pcf.md`. Supersedes the Code-Page framing in tasks 040/041/042/043 (re-scoped). |

> No further ADR tensions surfaced at design time. All listed ADRs otherwise apply without exception. This section may be updated if tensions emerge during implementation. **(Updated 2026-07-15: ADR-026 Path-A exception added for the W4 OOB-form-+-PCFs pivot.)**
...
6. [ ] **(Re-scoped 2026-07-15 — W4 pivot)** The OOB `sprk_communication` form is enhanced with a **Connections PCF** (multi-association review in the accessories column) + an **Actions PCF** (Reply/Send/Save); reviewer confirms/overrides suggestions across typed slots in ≤1 click per slot (or Accept-all). — Verify: manual review-loop walkthrough on the OOB form. *(Supersedes "Code Page replaces the OOB form" per ADR-026 Path-A.)*
