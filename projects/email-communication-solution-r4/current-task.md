...
| **W13 / R5 (`04e0e3741` → `f0be7cf9c`, on branch — NOT yet merged/imported/deployed)** | Task 131: CommunicationConnections card → RegardingResolver parity (Number Link / Name grid, OOB labels, NON-clickable title, refresh + lookup icons, reduced padding). Task 132 (R5 fix): **inbound regarding name/number was backwards** — server `IncomingAssociationResolver.PopulateResolverFieldsAsync` set name=EntityReference.Name (a number rung puts the NUMBER there) + never wrote number. FIX A (client): card sources number/name from the primary FILED ASSOCIATION (fallback denorm) → correct for inbound+outbound, existing+new. FIX B (server): retrieve real name+number from the matched record. **Import `CommunicationConnectionsSolution_v1.6.1.zip`** + **DEPLOY BFF** (132 touched IncomingAssociationResolver). 41 client + 552 Communication tests. Open Q: lookup opens the rich modal (could swap to record-picker). |
| **Also on branch unmerged** | CommunicationActions **v1.3.1** (`9cb8af79c` recipient-picker blur fix + search-by-email). | 
| **KNOWN CRITICAL (separate, pre-existing — NOT this feature work)** | Login popup + MSAL errors: `@spaarke/auth` `BrowserMsalStrategy` ssoSilent hidden-iframe times out in the MDA PCF sandbox (`monitor_window_timeout`) → falls back to `acquireTokenPopup`. Prior hotfix precedent: `811e98b54 fix(auth-v2): popup-on-startup hotfix`. Likely third-party-cookie blocking / iframe sandbox. Needs a dedicated auth-infra investigation; do NOT bundle into Communication PCF work. |
