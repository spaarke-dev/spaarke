...
| Field | Value |
|-------|-------|
| **Phase** | W9+W10 shipped+deployed (BFF on dev, master `010678595`). **W11 (round 3) + W12 (round 4) COMPLETE on branch — NOT yet merged/imported/deployed.** W12 includes a BFF change (124 reply/forward inheritance) → needs a BFF deploy, plus shared-lib EmailComposer/FilePreview changes. |
| **Active (NEXT STEP)** | Owner decision: merge W11+W12 to master + deploy BFF (for 124) + **import the updated PCF ZIPs** (W12 paths below; W11 Connections/Attachments/Actions superseded by W12 versions — import the W12 ones). Then UAT re-verify R3+R4. When UAT closes → **090 wrap-up** (HELD). |
