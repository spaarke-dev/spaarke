...
| **W13 / R5 (`04e0e3741`, on branch — NOT yet merged/imported)** | Task 131: CommunicationConnections on-form card now matches RegardingResolver — Regarding Number (Link) / Regarding Name grid with OOB labels, OOB-styled NON-clickable title, refresh icon (save+refresh; also surfaces sent-email regarding), lookup icon opens the existing modal, reduced top padding. Number/name from denorm regarding (not provenance). Modal + data contract unchanged. 39 tests. **Import `CommunicationConnectionsSolution_v1.6.0.zip`** (`src/client/pcf/CommunicationConnections/Solution/bin/`). Open Q for owner: lookup icon opens the rich modal (could swap to a record-picker like RegardingResolver if preferred). |
