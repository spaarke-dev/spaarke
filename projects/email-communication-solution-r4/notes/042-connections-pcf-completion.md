...
**Owner check**: if you intended `sprk_communication` to have a SINGLE regarding parent (not multi), this is the decision point — say so and I'll switch to single-select + clear-and-set. Everything else in W4 (the multi-slot ConnectionsEditor UX you approved) assumes multi.
