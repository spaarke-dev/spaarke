...
| ID | Test | Steps | Expected | Pass/Fail | Notes |
|----|------|-------|----------|:---:|-------|
| F-1 | Archive creates docs | `POST …/{id}/archive` on an un-archived communication with attachments | 200; `.eml` Document + one Document per attachment; `attachmentDocumentsCreated` reflects count | ⚠️ | **INCONCLUSIVE — 403 `graph_error` "Access denied" with the az-CLI TEST TOKEN; likely a test-identity artifact, needs owner H-8 to confirm.** Investigation 2026-07-19: (1) ruled out mailbox re-fetch (archive reconstructs `.eml` from the stored record; outbound send archival also 403s with no mailbox). (2) Ruled out the container itself — **retargeted `Communication__ArchiveContainerId` to the writable `DefaultContainerId` (`b!vzGDfDpd…`) and it STILL 403'd**, then reverted. The MI/app demonstrably WRITES `DefaultContainerId` (attachment doc `994c4307` is a real 17 KB SPE file). (3) Same `_speFileStore.UploadSmallAsync` + same container succeeds for attachments (BACKGROUND job → app-only) but fails for archive (HTTP endpoint → **my token in context**). Most consistent theory: the archive upload resolves **OBO with the caller token** and the **az-CLI test token lacks the SPE `FileStorageContainer` delegated scope**. **DEFINITIVE TEST = owner clicks H-8 "Save to SharePoint" in the Actions PCF (real user OBO):** succeeds ⇒ F-1 passes for real users (my 403 was a test-token artifact); 403s ⇒ real gap (OBO SPE scope not granted, or upload should be app-only — code/log-level fix). Get the exact failing Graph request (App Insights / log trace for the H-8 correlationId) to settle it. |
| F-2 | Idempotent | Archive the same communication again | 200 `alreadyArchived: true`; no duplicate Documents | ⬚ | Gated on F-1 (owner H-8) |
