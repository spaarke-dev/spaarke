...
(async () => {
  try {
    // Walk to the top frame — the console may be scoped to the PCF iframe where Xrm is undefined.
    const X = window.Xrm || (window.top && window.top.Xrm) || (window.parent && window.parent.Xrm);
    if (!X || !X.Page) {
      console.error("Xrm not found. In the Console tab's context dropdown (top-left), pick the top frame showing your CRM org URL, then re-run.");
      return;
    }
    const id = X.Page.data.entity.getId().replace(/[{}]/g, "");
    const provenance = {
      version: 1,
      direction: "inbound",
      decision: {
    ...
    await X.WebApi.updateRecord("sprk_communication", id, {
    ...
    console.log("mock provenance written OK");
  } catch (e) {
    console.error("mock failed:", (e && e.message) ? e.message : e);
  }
})();
```
