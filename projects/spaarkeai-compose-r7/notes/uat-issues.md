...
### UAT-10 — SignalR notifications 401 on negotiate — ✅ ROOT-CAUSED = ENV/CONFIG (2026-08-18)
- **Rounds**: R2 item 6 (console).
...
- **Root cause (CONFIRMED via full trace, 2026-08-18)**: the 401 is **NOT** at the BFF's `POST /api/notifications/negotiate` (that returns **200** — REST auth works, proven by the working `GET /api/notifications/pending` poll-fallback). The 401 is thrown by the `@microsoft/signalr` SDK's OWN negotiate against the **Azure SignalR *service*** URL: the BFF mints a client access token LOCALLY from `Notifications:SignalR:ConnectionString` (serverless Management SDK, `ServiceTransportType.Transient` — there is NO hosted hub in the BFF, and correctly no `MapHub`/`JwtBearerEvents.OnMessageReceived` query-string handling; that classic gotcha does not apply to serverless topology), and the Azure SignalR **service** rejects that token's signature/audience. This is a **connection-string / resource mismatch in the UAT env**, not a code defect.
- **Evidence**: `notificationsBootstrap.ts:62-100` (bootstrap + the logged warning), `Spaarke.Notifications/src/negotiate.ts:60-143` (BFF negotiate 200 → SDK connect to `info.url` via `accessTokenFactory`), `SignalRDeliveryService.cs:44,207-247` (serverless, no hosted hub, transient transport), `NotificationsEndpoints.cs:37-105` (REST negotiate returns url+token under `RequireAuthorization()`), `pollFallback.ts` (working fallback isolates the fault to the hub-token path).
- **Resolution = OPS (ENV/CONFIG), NOT code** — remediate the UAT Azure SignalR resource, in priority order: (1) verify `Notifications__SignalR__ConnectionString` `AccessKey=` matches the resource's current primary/secondary key (rotated key not re-copied is the single most likely cause and produces exactly this "BFF 200 + service 401"); (2) confirm the resource is in **Serverless** service mode (the transient-transport client-negotiate requires it); (3) confirm `Endpoint=` targets the correct UAT resource (the token's audience embeds the endpoint host).
- **Code done (hardening only, not the fix)**: added an Information-level diagnostic in `SignalRDeliveryService.NegotiateAsync` logging the RESOLVED endpoint HOST (never the key/token) so this drift is diagnosable from server logs instead of only as a client-side 401. BFF builds 0 errors.
- **Disposition**: R7 code portion DONE (diagnostic). The actual fix is an operator action on the UAT environment — no further Compose/SpaarkeAi code change warranted.
...
| R-5 | History stale (last shows Aug 7) | **DEFINITIVELY root-caused + FIXED (2026-08-18).** NOT env, NOT fire-and-forget (both earlier hypotheses superseded). **Ground truth**: dev `sessions` container's newest doc = `2026-08-07T04:01Z`, **zero writes since** despite active UAT + two post-Aug-7 BFF deploys → the failure is LIVE in current code, not a transient outage. Cosmos infra is 100% healthy (MI `mi-bff-api-dev` holds Data Contributor; keys on; network open; RU *low* not throttled; reads succeed → History shows the frozen pre-Aug-7 list). **Live App-Service logs (Aug 17–18)**: `CosmosException BadRequest (400): "The input ttl 'null' is invalid…"` on every unfiled-session `UpsertItemAsync`. **Root cause**: FR-D10 (task 033) added `StoredSession.Ttl` (`int?`) with `[JsonIgnore(WhenWritingNull)]`, but the CosmosClient used the SDK **default Newtonsoft serializer** (`WithSerializerOptions(CamelCase)`), which **ignores STJ attributes** and wrote `"ttl": null`. The `sessions` container has TTL enabled (`defaultTtl 7776000`), so Cosmos **rejects `ttl:null` with 400** → swallowed at Warning (`UpsertToCosmosAsync`) → 11-day silent write outage. Filed sessions (`ttl:-1`, valid) still wrote, which is why the last surviving doc had `ttl:-1`. **Memory was collaterally broken by the SAME bug**: `MemoryItemDocument.Ttl` (no `[JsonIgnore]` at all) on the TTL-enabled `memory-items` container → `memory-items` + `memory` both hold **0 docs**. **FIX (this round, owner-approved "full fix + observability")**: (1) CosmosClient now uses `WithSystemTextJsonSerializerOptions` with `DefaultIgnoreCondition = WhenWritingNull` (`AiPersistenceModule.CosmosJsonSerializerOptions`) → null optional fields OMITTED, honoring the models' intent, fixing sessions AND memory AND every future nullable Cosmos field; (2) new alertable metric `cosmos.write_failures{container}` (`CosmosPersistenceTelemetry`) at the session + memory swallow points so a silent total outage can't recur; (3) regression test binds to the PRODUCTION serializer options (closes the test-used-raw-STJ vs prod-used-Newtonsoft gap). No packages added; 442 persistence/memory tests green. **Note**: the Aug 7–18 gap is unrecoverable (those sessions lived only in Redis, 24h TTL); History resumes from deploy forward. **Separate concern flagged**: FR-D10 put `sessions` on a 90-day container TTL — unfiled History now expires at 90d (oldest May-21 docs begin expiring ~Aug 19); owner to confirm that's intended. | ✅ FIXED (deploying) |
...
**Redeploy**: R-2/R-3/R-6 + the revert are code — needs BFF (R-3 memo copy) + `sprk_spaarkeai` redeploy together (NFR-05). ~~R-5 is ENV (no code this round)~~ **[SUPERSEDED by session 4 below — R-5 was a CODE fix (ttl serializer), now deployed + confirmed holding].**

---

## Follow-up UAT round (2026-08-19, session 4) — R-5 deploy/confirm, banner unify, file re-attach, close-out

Owner UAT on the redeployed build. R-5 confirmed holding; two new UX items shipped; two owner decisions logged; UAT-10 assigned.

| # | Item | Resolution | Status |
|---|------|-----------|--------|
| S4-01 (R-5 confirm) | Deploy the R-5 ttl-serializer fix + verify it holds | Deployed; **clobbered once** by a concurrent project's BFF deploy from pre-merge master (writes reverted to `ttl:null` 400 on Aug 19) → **merged R-5 to master + redeployed** → confirmed **Aug 19 sessions landing (5 docs)**. Fix is in master, protected from future clobbers. Deployed BFF+`sprk_spaarkeai` together from current master. | ✅ Confirmed holding |
| S4-02 (banner unify) | Compose notices split above/below the toolbar with inconsistent look | Unified all PASSIVE notices into the single `ComposeBannerStack` rail (Fluent MessageBar): hoisted the redline "couldn't be placed" notice out of `ComposeEditor` (was a hand-rolled div below the toolbar) via new `onRedlineErrorChange` prop + `clearRedlineError()` handle; folded in `composeDraftError` + `memoActionMessage`. Reanchor/external-change left as-is (already MessageBars, same zone; folding risked shared usages for no visual change). 121 banner/redline + 80 editor tests. Deployed. | ✅ |
| S4-03 (1b re-attach) | Reopening a History session doesn't reload its files | **Recall already works on reopen** (server scopes by the persisted session manifest — no client re-arm needed). Gap was UI-only. Client-only best-effort **24h** re-attach: reopen fetches `/restore`, stages the manifest, re-renders the chip for the reopened session, computes 24h availability from last-message age, shows a dimmed **"no longer available"** chip beyond 24h. Surface gate 0; 9 restore + 6 new availability tests. Deployed. | ✅ (best-effort 24h) |

**Owner decisions (2026-08-19)**:
- **90-day History expiry** (FR-D10 container TTL) — **accepted as-is**; History older than 90 days ages out by design.
- **Durable session files** (lift the 24h → 90-day content window; needs a durable SPE/blob copy) — **moved to `spaarkeai-compose-r8`** as a scope track: [`../../spaarkeai-compose-r8/notes/durable-session-files.md`](../../spaarkeai-compose-r8/notes/durable-session-files.md).

**File-storage clarification (owner Q)**: Assistant session uploads live in **Cosmos** (metadata, 90d) + **Azure AI Search** (content, ~24h) — **NOT SPE**; no durable byte copy. SPE holds Compose/DMS documents (a different path).
...
- **Faithful-fidelity + durable-files** work → **`spaarkeai-compose-r8`** (worktree created 2026-08-19 on `work/spaarkeai-compose-r8`, off master).
