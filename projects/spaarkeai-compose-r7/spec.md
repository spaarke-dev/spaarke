...
- **NFR-01 (publish size)** — BFF-touching tasks (FR-06, FR-07, possibly FR-12) MUST verify compressed publish size ≤ **60 MB** and report absolute + delta vs the **net10 baseline (~44.96 MB incl PDBs)** established empirically in task 001 (supersedes the net8 ~46.94 MB R6/task-014 figure; master is net10 as of 2026-08-14). ≥+5 MB single-task delta → explicit justification; ≥55 MB cumulative → architecture review; ≥60 MB → HARD STOP. Per root §10.
...
9. [ ] Publish size ≤60 MB (report delta vs the net10 ~44.96 MB baseline from task 001); no new HIGH CVE; placement/component justifications recorded; `/conflict-check` clean; BFF + `sprk_spaarkeai` deployed together. — Verify: publish measure + CVE scan + conflict-check.

## Dependencies

### Prerequisites
- R6 engine assumptions confirmed: append-only version model; PDF DI compound gate ON in the target env; publish-size baseline (net10 ~44.96 MB; re-measured in task 001).
