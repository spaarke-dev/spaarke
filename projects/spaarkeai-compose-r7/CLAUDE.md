...
- **BFF hygiene (root §10)**: BFF work stays in `Services/Compose/`; Placement Justification per BFF task; publish ≤60 MB — **net10 baseline ~44.96 MB incl PDBs** (per completed `dotnet-10-upgrade-r1` + `code-quality-and-assurance-r3`; re-measured empirically in task 001; supersedes the net8 ~46.94 MB R6 figure); no new HIGH CVE; `/conflict-check` before every BFF PR.
...
- **2026-08-15 re-sync (code-quality-and-assurance-r3 + dotnet-10 COMPLETE merged to master)**: R7 anchors re-verified against the new base. **Only drift**: `AnalysisServicesModule.cs` DI gate moved (`:154`→~L145/165, real reg→~L229-230, `NullComposePdfIntakeSource`→~L510-511) — updated in spec/tasks 001/051/073. `ComposeService.cs` (all anchors), `Api/ComposeEndpoints.cs` (Endpoints→Api migration doesn't affect us — already Api/), `ComposePdfIntakeSource.cs` (FR-11 target — collapsed `return null` @~L51 confirmed present), and ALL client anchors are **intact**. Publish baseline is now **net10 ~44.96 MB**. code-quality-r3's `UnwrapServiceClient` downcast-consolidation + AI-facade compliance don't change R7's approach.
