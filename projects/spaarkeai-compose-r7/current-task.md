...
| **Step** | Owner approved **Honest-now (R7) / Faithful-next (compose-r8)** split. R7 batch = make Compose *never lie* (surface losses, no mis-placement). ✅ Done: UAT-01(container), 02, 03, 05, 07b. **8 commits ahead of master, UNPUSHED + UNDEPLOYED** (batching per owner — do NOT deploy piecemeal). |
| **Next Action** | **CONTINUE THE R7 HONEST/SAFE BATCH** (all code, no deploy yet). Priority order: **UAT-21** (highest — AI redline mis-places to wrong selection + reports success, `usePendingRedline.ts:667-690`), **UAT-24** (tolerant-but-SURFACED resolver — propose-don't-auto-place; fixes 06/09/21/22 at root), **UAT-22/23** (surface silently-dropped comments/edits), **UAT-11** (container residual — honest signal + retry, not one-shot swallow), **UAT-12** (surface dropped tracked-changes/comments on read failure), **UAT-13** (association orphan signal), warned-but-cryptic **copy gap** (friendly copy for footnote/field/content-control/numbering codes in `ComposeBannerStack.tsx`), then **UAT-04** (progress indicator in Compose top-banner area), **UAT-08** (enable Promote-to-Analysis + auto-create Analysis on analyze), **UAT-09** (comment-anchor signal), **UAT-10** (notifications 401), **UAT-25/26** (honest concurrency guard on the ContentModel save path). Then merge master in, push, `/worktree-sync`, and redeploy **BFF + `sprk_spaarkeai` together** (NFR-05) when the batch is complete. R7 closes only when every UAT row is Fixed or Deferred-with-link. |
