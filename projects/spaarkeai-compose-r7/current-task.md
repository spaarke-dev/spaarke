...
| Field | Value |
|-------|-------|
| **Task** | 🔬 **R7 IN UAT — NOT closed.** Resolving UAT findings. **Single source of truth: [`notes/uat-issues.md`](notes/uat-issues.md)** (UAT-01..26 + proactive audit). |
| **Step** | Owner approved **Honest-now (R7) / Faithful-next (compose-r8)** split. R7 batch = make Compose *never lie*. **BATCH IN PROGRESS (2026-08-18 session 2)** — ✅ Fixed this session: **UAT-21** (redline wrong-selection fallback removed), **UAT-22** (comment save-drops surfaced), **UAT-23** (anchor-lost edits surfaced), **UAT-11** (honest container resolution + retry), **UAT-13** (association-orphan banner + retry), **copy-gap** (~18 cryptic degradation codes → friendly copy), **UAT-12** (unreadable tracked-changes/comments → honest "don't treat as clean" banner, full-stack BFF+client), **UAT-04** (top-area progress indicator, owner-requested), **UAT-09** (verified-surfaced, no change). Earlier: UAT-01/02/03/05/07b. **6 new commits this session** (`92529616d`→`48481ac04`), 8 ahead of origin, UNPUSHED + UNDEPLOYED (batching per owner). All tsc-clean; BFF builds 0 errors; targeted jest green (usePendingRedline 80/80, interceptor+comment 64/64, ComposeBannerStack 38/38). 3 pre-existing FR-13 "Element type is invalid → ComposeSaveNameDialog/FormModal" test failures confirmed ENV-only (reproduce with changes stashed). |
| **Next Action** | **4 items remain — ALL need an owner decision or lean r8 (NOT just-implement); escalate before coding:** (1) **UAT-25/26** (ContentModel save path skips stale-base check → silent lost-update; `ComposeService.cs:1222` gates the guard on `ContentModel is null`). Honest fix = detect staleness on the model path (via `BaselineVersionId`) and **refuse-with-reload 409** (the model path re-authors the whole body — can't re-anchor like the op-log path). **HIGH blast-radius on the MAINSTREAM save path** → owner design call: refuse-vs-merge UX + guard aggressiveness + first-save behavior. (2) **UAT-08** (promote + auto-create Analysis): the Promote affordance was **deliberately removed**, rewiring owned by **task 034 / FR-D9 "Set related record"** (another scope); + auto-create-on-analyze needs design → owner call on both facets. (3) **UAT-10** (notifications SignalR 401 on negotiate): investigate — likely hub-auth/token-audience **env/config**, not a clean code fix; low user impact (poll-fallback active). (4) **UAT-24 tolerant/fuzzy resolver**: the SURFACE half is DONE (UAT-21); the tolerant-match half is **fidelity → leans compose-r8**. When the batch closes: `/worktree-sync`, run BFF §10 publish-size gate (net10 baseline ~44.96 MB — batched, only UAT-12 touched BFF: additive bool, ~0 delta), then redeploy **BFF + `sprk_spaarkeai` together** (NFR-05, never from a net8 tree). R7 closes only when every UAT row is Fixed / Verified / Deferred-with-owner-decision. |

### 🔬 SESSION 2026-08-18 (context-handoff — READ for recovery)
