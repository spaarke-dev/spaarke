...
| 031 | Deploy to slot + §6.1 OBO checklist (**COMPLETE 2026-08-24 — MI-FIC OBO PROVEN live; rollback verified. Office add-in UAT PASSED on the secret-free build: Outlook saved + the created .eml opens; Word created the Document record + file profile. Word's "no extractable content due to unsupported file type" is a content-extraction defect, NOT auth — owner routed it to a focused project. UAT also surfaced 3 pre-existing non-auth add-in defects, fixed in `77f61574b`. Open owner decision: §6.1 task-030 carry-forward**) | 3 Rollout | FULL | sonnet/high | 022 | — | ❌ | FR-C1 | ✅ |
...
| 090 | Project wrap-up (**✅ COMPLETE 2026-08-24.** /test-diet (0 deletions) · **criterion 12 EXERCISED** — guards fired on a seeded 9th secret client · lessons-learned · **graduation walk: 14/16 verified with evidence, 1 waived (Power BI — deferral re-verified VISIBLE in both guards), 2 NOT MET and carried forward with owners (#9 local-dev OBO, #15 provisioning §5.1)** · seam/Auth adjudicated into `tests/integration/seam/README.md` (auth/** is uncompiled — a test authored there would run never) · KV ServiceBus+AiSearch rollback secrets deleted (soft, to 2026-11-23) · grid_overview + daily-briefing-overview rows seeded — root cause was an UNREGISTERED JSON in $RowFiles, so the file on disk made the work look done · publish 45.04 MB (+0.08 vs 44.96) · README + projects/INDEX.md + portfolio #800) | 9 Wrap | STANDARD | sonnet/high | all active | — | ❌ | — | ✅ |
