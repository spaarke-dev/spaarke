...
| **Task** | **033 ✅ COMPLETE (all 7 steps).** 🎉 **The project's central deliverable is DONE — the BFF identity is secret-free, including OBO. ADR-028 E-3 is CLOSED.** Record: [`notes/decisions/033-secret-removal.md`](notes/decisions/033-secret-removal.md) |
...
| **Next Action** | **090 wrap-up** — the last active task. Carries obligations `031-A/B/C` **plus three new ones from 033** (see below) and the `/test-diet` gate. Also still pending cutover: **051 🔄 / 053 🔄**. |
...
| **🔴 NEW — booked to 090 by 033** | **(1) Partial secret VALUES are in git history** — `auth-azure-resources.md` published 7 and 12 chars of live Entra secrets under a "First 5 chars" caption (commit `c1803e99a`, 2026-03-09). Redacted in the working tree; **history untouched**. `Dataverse-Checkout-20251218` is still valid until 2027-12-18. **Recommended: rotate/delete it — cheap now precisely because nothing reads it.** **(2) Local-dev OBO has no credential path for a fresh setup** — the code calls the user-secret fallback *"the legitimate — and only — way to run OBO locally"*, and the KV copy was the only **readable** copy. Owner may want a deliberate replacement. **(3) `AZURE_CLIENT_ID` still set**, logs an ERROR every boot — 031 hygiene, deliberately not bundled (the Azure Identity SDK reads that variable itself) |
