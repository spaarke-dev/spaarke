...
| **Task** | **033 🔄 — steps 1–4 of 7 done.** The BFF has no secret to resolve, and none exists in Key Vault. Record: [`notes/decisions/033-secret-removal.md`](notes/decisions/033-secret-removal.md) |
| **Status** | `Graph__Credentials__Order__0=ManagedIdentityFederated` (**no fallback beneath it**) · `RequireSecretFreeIdentity=true` · **4 secret app settings DELETED** 16:50:25Z · **both KV copies DELETED** 17:14:40Z (26→24) · `PowerBi__ClientSecret` + `spe-owning-app-secret` correctly untouched · **7 scripts modified, 8 documented** · `-SkipClientSecret` added (acceptance criterion) · OBO green at every rung incl. byte-exact SPE round-trip, 15/15 + 15/15 · `dotnet build` 0 errors · all 10 `.ps1` parse |
| **Next Action** | **Step 5 — the doc sweep. 33 files reference the secret, not the POML's ~25/13.** Classify BEFORE editing: live operational guides MUST be corrected; **historical records (`.claude/CHANGELOG.md`, `docs/assessments/*`, `.claude/AUDIT-FINDINGS-AUTH-SYSTEM.md`) MUST NOT be rewritten** — they are point-in-time. Priorities: (a) the **false add-in claim** (§0.1) wherever it appears — check `office-addins-*.md`; (b) `SPE.BFF.API-SECRETS-SETUP.md`, which is **doubly stale** and today produces a local env that silently cannot authenticate (§3.3); (c) `appsettings.template.json` — a real artifact that would re-seed the key. Then **6** (ADR-028 E-3 — 🛑 still contends with **PR #812** on `.claude/constraints/auth.md`) and **7**. |
| **Rollback** | Rungs 1 and 2 retired. Live path: `az keyvault secret recover --vault-name spaarke-spekvcert --name BFF-API-ClientSecret` (**until 2026-11-22**) → restore the 4 app settings (fingerprint `b09a140a603e`) → remove the order override. **Nothing was purged** — deliberately |
| **Owner decision booked to 090** | 🔔 **Local-dev OBO now has no credential path for a fresh setup.** The code states the user-secret fallback is *"the legitimate — and only — way to run OBO locally"*, and the KV copy was the only **readable** copy (Entra won't disclose a secret post-creation). Developers who already have it set are unaffected; the app-reg secret lives to 2027-12-19. Recoverable in one command. The owner may want a deliberate replacement (dev-only app reg, or `AzureCliCredential`) rather than inheriting this |
