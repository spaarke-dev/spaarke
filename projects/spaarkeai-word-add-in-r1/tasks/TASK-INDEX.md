...
| 012 | FR-01 server: document-identity resolver extending `/api/documents` — **CODE COMPLETE 2026-09-10; live criteria pending.** `POST /api/documents/resolve-identity`. The Step 9.5 review was applied (notes/012 §9): no duplicate-tolerant fallback, Graph 401/403 → 503, `identity_conflict`, and `#`/`%` encoding. 61 unit tests pass; publish size +0.016 MB; no CVE. **Not optional under Spike-1 §7** (notes/012 §1). **Still needed to close:** a dev BFF deploy (operator sign-off) to run the live checklist in notes/012 §8, and the Word-desktop `document.url` capture (operator). | 🔄 | FULL | opus / high | — | 002 ✅ |
