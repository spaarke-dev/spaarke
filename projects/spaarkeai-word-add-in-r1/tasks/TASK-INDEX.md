...
| 046 | 🔴 **Email/Attachment saves must never overwrite or delete another document's file** — **(a) DONE** (`9b58c6d9b`, merged `dbcb4c18f`): the dedup cleanup deletes an upload only when no document points at it (and deletes nothing if that lookup fails); it also fixed a second case — a hash-linked copy's file; full suite 12,245/0/56; +1,854 B. **(b) IN PROGRESS**: owner decision B2 (2026-09-15) — a unique suffix only on system-derived `.eml` names (including emails arriving through the Exchange/Graph integration), never on a name the user typed; a clash on a typed name → 025's prompt | 🔄 | FULL | opus / high | — | 039 ✅ |
