...
| 046 | 🔴 **Email/Attachment saves must never overwrite or delete another document's file** — (a) the immutable suppress branch can DELETE a canonical document's own SPE file (the dedup probe does not exclude the probed item; under `Replace` the "transient blob" IS the canonical's file — `ContentDedupDetector.cs:127-142`, `OfficeService.cs:550-578`); **live on master**, deterministic for a direct `ContentType=Attachment` save. (b) Owner 2026-09-14: generated `.eml` names get a short unique suffix. Found by the 025 analysis (R3/M9) — owned here | 🔲 | FULL | opus / high | — | 039 ✅ |
