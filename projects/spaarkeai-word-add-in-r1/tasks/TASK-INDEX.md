...
| 012 | FR-01 server: document-identity resolver extending `/api/documents` — **IN PROGRESS 2026-09-10.** `POST /api/documents/resolve-identity` is implemented; the build is clean, 38 new unit tests pass, and publish size is +0.016 MB. Step 9.5 review is running. **Not optional under Spike-1 §7**: 014 stamps only the Office save path, so documents uploaded elsewhere arrive unstamped and only this path identifies them (notes/012 §1). The live Spike-1 links 2-3 need a dev BFF deploy (operator sign-off), and the Word-desktop capture is an operator item. | 🔄 | FULL | opus / high | — | 002 ✅ |
