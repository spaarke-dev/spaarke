...
| 012 | FR-01 server: document-identity resolver extending `/api/documents` — **DEPLOYED TO DEV and LIVE-VERIFIED 2026-09-10.** `POST /api/documents/resolve-identity` resolves the Spaarke document from both the raw and the `%20` URL, and **Spike-1 links 2 and 3 are GREEN** (notes/012 §8, spike-1 §22). The live run showed Graph uses 403 for missing items, so 403 → `not_resolvable` (notes/012 §4). Full suite 12,139/0; ArchTests 191; no CVE; publish +0.016 MB. **Still needed to close:** (1) the Word-desktop `document.url` capture (operator); (2) optionally, the URL of any OneDrive or SharePoint file with no Spaarke record, to exercise the Dataverse not-found shape live. | 🔄 | FULL | opus / high | — | 002 ✅ |
