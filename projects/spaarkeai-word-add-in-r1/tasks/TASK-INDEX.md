...
| 053 | **The pane silently swallows quick-create failures — including 031's new actionable 403** — added 2026-09-17 from task 031's closing report, confirmed in code by the main session: `SaveFlow.tsx` ~:962 does `if (!res.ok) return null` on `POST /api/office/quickcreate/{type}`, so the 403 `owner_unresolved` and its message vanish and the user sees a silent no-op after clicking Create. Same class at ~:908 (`return []` on entity search renders a failure identically to "no matches"). The server did the right thing — refused before writing, with a reason — and the pane throws the reason away. Now actionable because 050 shipped typed `problem+json` errors. Owned here, not deferred. `parallel-safe=false` — `SaveFlow.tsx` is shared with 025 and 045 | 🔲 | FULL | sonnet / high | — | 031 ✅, 050 ✅ |
