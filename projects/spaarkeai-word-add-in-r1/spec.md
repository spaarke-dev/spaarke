...
| **ADR-044** | Use the canonical `cleanGuid` from `@spaarke/ui-components` | This package cannot import `@spaarke/ui-components` (the ADR-012 row above). | **A — continuation of the ADR-012 exception** | A local `shared/taskpane/utils/cleanGuid.ts`, pinned by its own gated test; `todoChoices.ts` is the approved precedent (2026-09-11, task 013). |
...
> The ADR-012 and ADR-050 rows were identified at design time; the other five were raised and resolved during execution (each is also in project `CLAUDE.md` Decisions). All other listed ADRs apply without exception.
