...
> **Last Updated**: 2026-09-15 (context-handoff before /compact — 027, 034 and 035 ready to merge; local HEAD NOT pushed)
...
| Field | Value |
|---|---|
| **State (2026-09-15, context-handoff before /compact)** | `origin/work/spaarkeai-word-add-in-r1` is at `ed776f915` (026 + 046(a) + 033 + docs). **Local HEAD = this checkpoint commit on top of `eda1e2f75` (the 046(b) merge) — NOT pushed yet** (see Next Action 1). PR #960's live description reflects `ed776f915`; the updated draft is `scratchpad/pr-960-body.md` in the session scratchpad — if it is gone, rebuild from `gh pr view 960 --json body`. CI on `ed776f915`: 32 pass / 1 fail — the fail is in Tier 2 Full Unit Tests, expected to be the known flaky AI-chat test (confirm). Done this session: 013, 016, 021, 022, 023, 024, 026, 030, 033, 038, 039, **046 ✅** (a `9b58c6d9b`, b `5bbdbacca`). `tasks/TASK-INDEX.md` is authoritative for counts. |
| **Agent work waiting to merge** | **027 ready** — branch `worktree-agent-aad4e56e3ed274112`, commit `d660a5906`: client-only; `openRecordLauncher.ts` opens `${ORG_URL}/main.aspx?etn=…&id=…&pagetype=entityrecord` via `Office.context.ui.openBrowserWindow` (unset `ORG_URL` = safe no-op), new `canOpenBrowserWindow` capability (runtime `isSetSupported`), focus/visibility refetch reusing 033's `useDocumentProfile.refetch()`; gates green. **035 ready** — branch `worktree-agent-abd79ab3b571aadeb`, commit `ae84e00b1`: `CreateTodoAsync` widened + client; a pane To Do carries the document/communication AND the record; resolver fields describe the record; owner ratified the ADR-024 exception 2026-09-15; +0.05 MB; gates green. **034 ready** — branch `worktree-agent-ac164237e1e38afd0`, commits `437c12ae0` (F-c = documents-only; the route cannot page) + `2f35d97f0` (Path 2): new `useLazyResults.ts` (reveals rows already in memory, 20 at a time; `hasMore` = unrevealed rows only; no network calls) + `FindResultsList.tsx` ("Most similar documents"; hub nodes in a separate "not a similarity match" section; no pager; `onOpenResult` left unwired); `FindView.tsx` wiring only; `PARTIAL_RESULTS` bar unchanged; no BFF files; typecheck 111/0; gated 24/24 (356 tests) on its base. **On merge, add `hooks/__tests__/useLazyResults.test.tsx` and `components/__tests__/FindResultsList.test.tsx` to `ci-gated-suites.txt`** (the agent did not). The live-host UI tests are unverified (no Office host). |
| **Owner decisions this session (2026-09-15)** | (1) Email `.eml` names: B2 — suffix only system-derived names, never typed names (in project CLAUDE.md). (2) **ADR-051 exception for the Find list**: a ranked top-50 list — progressive reveal of the single bounded response, no pager, no fake pages, the `PARTIAL_RESULTS` warning kept. (3) **ADR-024 exception for pane To Dos**: record lookup + a document/communication source lookup together; resolver fields describe the record. (2) and (3) are in project CLAUDE.md Decisions; Recorded in `spec.md` ADR Tensions 2026-09-15; **still to cite in the PR description (§6.5).** |
| **Waiting on the owner** | (1) **How Word documents are matched to avoid duplicates** — 014/FR-02 (an invisible id inside the file) vs a user-confirmed match via the 025 clash prompt [Save as a new version of it] [Use a different name] [Cancel]; decides 014, 025, 045 and 020. (2) Dev BFF deploy sign-off. (3) Owners for the unowned repo-wide items: #975; the idempotency filter's no-header path is a no-op; the flaky AI-chat Tier-2 test (verified not ours — do not re-investigate). (4) 031: does Project numbering belong to the separate numbering project? (5) Word manifest 1.0.8.0 re-upload (closes 011). |
| **Next Action** | **1) Identify the extra failing jest suite.** The full jest run on `eda1e2f75` showed **11 failing suites / 86 failing tests vs the 10 / 84 baseline** (the gated list was 25/25 green, 355 tests). Run `npx jest --ci` in `src/client/office-addins`, list the FAIL suites, and compare with the known ten (OutlookAdapter, ApiClient, EntityPicker, SaveFlow, TaskPaneNavigation, TaskPaneShell, SaveView, ShareView, useEntitySearch, useSaveFlow). Evidence for a flake: 034's agent saw the same kind of blip (11 failed suites / 85 failed tests), and an immediate re-run gave the stable 10/84. Still confirm by name; anything real gets fixed before pushing. **2)** ~~ADR exceptions into ADR Tensions~~ done (`spec.md`). **3)** Merge 027 and 035 (`git merge --no-ff <branch>`), run gates (BFF build + Office/Documents/dedup/idempotency/Email/Rag/Visualization tests + ArchTests; add-in typecheck/build/full jest/gated list), add 027's new suites to `ci-gated-suites.txt` (`openRecordLauncher.test.ts`, `SaveFlow.openRecord.test.tsx`), commit, **push**, publish the PR draft (add 027/035 rows, 035's placement note, the two ADR exceptions), watch CI. **4)** Dispatch **036** (Send Email; reuse 027's `openRecordLauncher` URL builder; its capability sits next to `canOpenBrowserWindow`) and **029** (per-version analysis key — the save spine is free now). 020 stays held (014's fate). **5)** Merge 034 together with 027/035 in step 3 (same gates). **6)** Remove merged agent worktrees — `agent-a76319be262ff6297` (046), then 027/035 after their merge. NEVER touch `agent-a97210f38cd331cfb` (another session's). |
| **Dispatch rules (learned the hard way)** | Before any Agent dispatch, run a `Set-Location 'C:\code_files\spaarke-wt-spaarkeai-word-add-in-r1'` command and confirm the environment says "is a git worktree" — the lowercase `c:` path once placed an agent worktree INSIDE this worktree. One worktree creation per turn. Every brief: confirm `git rev-parse --show-toplevel` is its own worktree; rebase onto the LOCAL `work/spaarkeai-word-add-in-r1`; never `git stash`; run long jobs in the foreground and never wait on a background job (agents stall); contract tests go in NEW files when agents run in parallel. |
