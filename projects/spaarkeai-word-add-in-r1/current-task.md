...
| **Next Action** | **ONE live check, then carry on.** In Word, create a NEW document and open the Save tab. **Expected: no "Couldn't check this document" banner** — the fix for that is deployed. If clean → criterion 2 goes PARTIAL → PASS, **close [#997](https://github.com/spaarke-dev/spaarke/issues/997)**, and continue the remaining UAT in `notes/042-uat-results.md` §7 (the 14 parity rows + the 4 POML `<ui-tests>`). If the banner still appears → re-open, pull the App Insights fault, and compare its `ErrorCode` against `0x80060891`. **Then** 090. |
