...
> **Last Updated**: 2026-09-15 evening (029/036/047 merged locally; 048 created; BFF full suite running; NOT pushed)
...
| Field | Value |
|---|---|
| **State (2026-09-15, evening)** | Pushed: `e819d199d` (027/034/035/046b + chat-test fix). **Local, NOT pushed**: 036 (`e6e38422e`), 047 (`b07e4772e`), 029 (`68b3af5b6`) merged, plus docs and the matter-type suite hardening. 37 of 49 tasks ✅ (048 added). CI on `e819d199d`: Tier 1 all green (32 pass); Tier 2 Full Unit Tests hit its 30-minute job limit (cancelled, advisory; CI project's file, reported in the PR). Owner decisions today: 029 = upload-then-trim, no dev backfill, close the revert case; 048 = trim every re-index path, owned here; Send Email hidden in Word for r1. |
| **Gates on the merged tree (029+036+047)** | Add-in: typecheck 111 / production 0; build exit 0; full jest = the ten known failing suites; 32 gated suites — `SaveFlow.matterTypeQuickCreate` and `RelatedToPicker.matterType` failed once under heavy load (passed alone every time; ~33 s/test vs a 20 s budget) → budgets widened to 60 s per test / 10 s per wait (test-only), 12/12 after. BFF: build + ArchTests + chat tests + the FULL `Sprk.Bff.Api.Tests` were still running when this was written — **read that result before pushing** (the agents each measured 12,287 / 0 / 56 on their own branch). |
| **In flight (agents, isolated worktrees)** | None. Next to dispatch: **048** (sonnet) — turn 029's trim on for Compose save-back, manual Run Index and any other path that re-indexes an existing item; POML written. |
| **Waiting on the owner** | (1) **How Word documents are matched to avoid duplicates** — 014/FR-02 vs a user-confirmed match through the 025 clash prompt; decides 014, 025 (now also carrying 047's create B → A → B case), 045 and 020. (2) Dev BFF deploy sign-off, and an add-in redeploy. (3) Owners for #975 and the idempotency filter's no-header no-op; CI owners for the Tier 2 30-minute limit. (4) 031: Project numbering → the numbering project? (5) Word manifest 1.0.8.0 re-upload (closes 011). |
| **Next Action** | **1)** Read the BFF full-suite result (background run). If green: push, publish `scratchpad/pr-960-body.md` (rows for 036/047/029, placements, publish size, owner decisions, open items and Tier 2 note are in; add a Testing row with the merged-tree numbers), watch CI. **2)** Dispatch 048 (Set-Location check first; one worktree per turn). **3)** Remove merged worktrees `agent-aa138408f12a07cd2` (036), `agent-a4c6c7214269897df` (047), `agent-a96e6f6f75d84dc3e` (029) after checking each is clean and merged. NEVER touch `agent-a97210f38cd331cfb`. **4)** Held: 020, 025, 045 (matching discussion), 031, 037. |
