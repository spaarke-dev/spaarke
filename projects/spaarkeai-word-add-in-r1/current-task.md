...
> **Last Updated**: 2026-09-18 (task 042 — deploys done, **011 CLOSED**, one UAT defect found → **fixed, tested 26/26, redeployed**; awaiting ONE live re-test, then 090)
...
| **Next Action** | **Diagnose the 2026-09-18 UAT finding — `notes/042-uat-findings-2026-09-18.md` §4, in order.** A pane save put bytes in SPE with `_sprk_matter_value = NULL` (document not associated to the matter), and a later save of the same file was blocked by a name collision against what is probably that orphan. **Run §4.1 first** (does an `Untitled Document.docx` row exist?) — it decides whether the collision message is correct or is itself a defect. Then §4.2 (App Insights: was `TargetEntity` sent?). **Do not start 090 until this is triaged.** |
...
| 🔴 **NEW UAT FINDING (2026-09-18) — save produces an ORPHAN** | Full triage: **`notes/042-uat-findings-2026-09-18.md`**. Symptom: file lands in SPE, `sprk_document` row has `_sprk_matter_value` NULL, matter still shows only its original document. **CONFIRMED in code** (`OfficeDocumentPersistence.cs:256-267`): association runs **only** `if (request.TargetEntity != null)`, and a null target creates an unassociated row **silently** — no warning on that branch; `sprk_matter` is NOT in the known-gaps list, so a real matter target would associate or log. **Leading hypothesis**: the pane sent no `TargetEntity` (or the collision-retry path drops it). **Blast radius if real**: every targetless save orphans bytes in SPE AND its name then blocks the next legitimate save of that file. ⚠️ Task 026 records **16 association-shaped lookups** on `sprk_document` — check all 4 direct slots before declaring it unassociated. |
