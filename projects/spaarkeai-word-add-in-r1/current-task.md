...
| **In flight** | **2026-09-15 — PUSHED through 033**: 026 ✅ (`a7bb22bea`), 046(a) merged (`dbcb4c18f`), 033 ✅ (`4754a6af2`); merged-tree gates green (BFF 350/0/10, ArchTests 191, add-in gated 24/24 with 352 tests, full jest 10 failing suites = the pre-existing baseline). **046(b) in flight** (owner decision B2: a unique suffix only on system-derived `.eml` names, including emails arriving through the Exchange/Graph integration; never on a name the user typed). **Next: dispatch 027** — open the related record and the document record: Spike-2 option (3), details in the pane + an `Office.context.ui.openBrowserWindow` escape hatch + a focus/visibility refetch through 033's `useDocumentProfile.refetch()`; deep links from `ORG_URL` (unset = a safe no-op); 026's `onOpenRecord` seam already exists. CI on the new push being watched. |
