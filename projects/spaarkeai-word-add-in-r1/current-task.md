...
> **Last Updated**: 2026-09-19 (by context-handoff) — **task 055 IN PROGRESS: code COMPLETE, targeted suites GREEN, full-suite verification OUTSTANDING.**
...
| Field | Value |
|---|---|
| 🔴 **HANDOFF 2026-09-19 — READ THIS ROW FIRST** | **Task 055 code is COMPLETE and the targeted suites are GREEN; what remains is whole-suite verification + Step 9.5 gates + commit.** Nothing is committed yet for the implementation — see "Files modified, UNCOMMITTED" below. **Do NOT re-derive the design**: it is settled and written up in `notes/055-collision-names-its-target.md` (§1 the trigger-(b) authorization answer, §2 why the gate is at the ENDPOINT not the service, §3 the one-projection mechanics, §4 why half (2) withholds rather than re-associates, §5 the stale `constraints/auth.md`, §5A three fixture gaps). |
| **055 — NEXT ACTIONS, in order** | **(1)** **Run the full BFF suite — it has NOT been run.** An earlier background attempt produced **no output at all** (87-byte file, header only) before its timeout, so there is nothing to read and no result to trust: `dotnet build tests/unit/Sprk.Bff.Api.Tests/Sprk.Bff.Api.Tests.csproj -c Debug -m:1` (check exit code), then `dotnet test … --no-build`. Expect baseline **12,375 passed / 0 failed / 56 skipped**. It exceeds 10 minutes, so expect it to background — capture the task id and read its summary when it completes. **(2)** Gated jest by path — from `src/client/office-addins`: `npx jest --runTestsByPath $(grep -vE '^\s*(#\|$)' ci-gated-suites.txt)`; expect **46 suites green**. (Full `npx jest` gives 10 failed / 46 passed of 56 — the 10 are PRE-EXISTING non-gated reds, stated as such in `ci-gated-suites.txt`'s own header; do not chase them.) **(3)** ArchTests: `dotnet test tests/Spaarke.ArchTests/Spaarke.ArchTests.csproj` → expect **191/191**. **(4)** BFF publish size vs a **FRESH `origin/master` build** with `Compress-Archive -Optimal` (CLAUDE.md §10 bullet 4 — never vs the recorded number) + `dotnet list package --vulnerable --include-transitive`. **(5)** Step 9.5: `/code-review` + `/adr-check` (unconditional at FULL rigor). **(6)** POML `<status>` → completed, TASK-INDEX 055 → ✅, finish the note, commit + push, comment on [#1005](https://github.com/spaarke-dev/spaarke/issues/1005) — but do NOT close it until verified on a deployed build (the ISS-005 precedent). |
