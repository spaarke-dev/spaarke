...
| **Next Action** | **1)** 031 is DISPATCHED (agent, own worktree) — merge + gate it when it reports, then run the FULL `Sprk.Bff.Api.Tests` suite ONCE covering both 031 and 050 (050's full-suite run was on its own branch: 12,316/0/56). **025 is blocked from dispatch until 031 merges** — 031 has modified `OfficeService.cs`, which 025 also modifies, and 025 is `parallel-safe=false` on that spine. Queue after 031: **025 → 014 (version-path first) → 051 → 045**, plus **052** whenever `OfficeEndpoints.cs` is free. **2)** ~~041~~ DONE — no Entra write needed, trigger stays `workflow_dispatch`, workflow file untouched. **3)** Then 042: `/bff-deploy` for the BFF, `workflow_dispatch` for the add-in, and turn every "pending live check" in `notes/parity-checklist.md` into a real verification. **4)** 090 closes the project. **5)** ✅ **Matching DECIDED 2026-09-17 (A + C, B supporting).** Newly dispatchable: **025** (build the collision prompt — do it FIRST; it gates 014's create-path half and unblocks 045), then **014** (version-path stamping now, create-path after 025), then **051** (client reader, deps 014), then **045**. |
