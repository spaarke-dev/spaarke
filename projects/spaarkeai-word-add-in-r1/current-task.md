...
> **Last Updated**: 2026-09-15 late (040 merged + pushed `1e9e63147`; 048 and 049 in flight; PR body published)
...
| Field | Value |
|---|---|
| **State (2026-09-15, late)** | Pushed `1e9e63147` — everything merged so far is on origin: 027, 034, 035, 046(b), the chat-test fix, **036** (Send Email, Outlook-only), **047** (a version save repeating an earlier version's content is written again), **029** (profile + index refresh after a version save) and **040** (parity audit). 38 of 50 tasks ✅. CI on the previous head `7652fb3ae`: Tier 1 fully green (28 pass); Tier 2 Full Unit Tests hit its 30-minute job limit (cancelled, advisory, CI project's file — reported in the PR). CI on `1e9e63147` was still running. PR #960's description is published and current. |
| **Gates on the merged tree (029+036+040+047)** | BFF (`68b3af5b6`): build 0/0; full `Sprk.Bff.Api.Tests` **12,301 / 0 / 56**; ArchTests 191/191; `ChatEndpointsTests` 20/20. Add-in after the 040 merge (`cc2edbbea`): typecheck 111 / production 0; build exit 0; full jest = the ten known failing suites (84 tests); **33/33 gated suites, 417 tests**. |
| **In flight (agents, isolated worktrees)** | **048** (sonnet, BFF-only) — turn 029's trim on for Compose save-back, manual Run Index and any other path that re-indexes an existing item. **049** (sonnet, client-only) — the Create To Do tab is still Outlook-only although 035 made To Do work from both hosts and the spec lists it as shared; the fix changes the tab gate, the test that pins the old behaviour, and the "file this email first" copy together. Neither edits TASK-INDEX / current-task / CLAUDE.md / `ci-gated-suites.txt`. |
...
| **Next Action** | **1)** When 048 and 049 report: review each diff yourself, merge with `--no-ff`, run the gates (BFF build + targeted + ArchTests + full suite for 048; add-in typecheck + build + full jest + the gated list for 049), add any new suites to `ci-gated-suites.txt`, mark ✅ in TASK-INDEX + POML, push, refresh the PR body. **2)** Watch CI on `1e9e63147` (`gh pr checks 960`; Tier 2 is advisory and currently times out). **3)** Remove the merged worktrees for 040 (`agent-a706862704e0cd768`) and then 048/049 after checking each is clean and merged — NEVER `agent-a97210f38cd331cfb`. **4)** Held: 020, 025, 045 (document-matching discussion), 031 (numbering), 037 (manifest), 041 (owner sign-off — live Entra writes), 042 + 090 (deploy, wrap-up). |
