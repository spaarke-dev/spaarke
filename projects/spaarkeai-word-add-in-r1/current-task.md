...
> **Last Updated**: 2026-09-16 (020 dispatched — hold lifted with the naming question left to the owner; CI green on `4c7af1c21`)
...
| Field | Value |
|---|---|
| **State (2026-09-15, end of day)** | On origin after this push: 027, 034, 035, 046(b), the chat-test fix, **036**, **047**, **029**, **040**, **049** and **048**. **40 of 50 tasks ✅.** Everything not done is either held on an owner decision or is 042 (deploy + UAT) / 090 (wrap-up). PR #960's description is published and current. |
| **Gates on the merged tree (through 048)** | BFF (`25c747d5a`): build 0 warnings / 0 errors; **full `Sprk.Bff.Api.Tests` 12,311 passed / 0 failed / 56 skipped** (12,367; 18 min); ArchTests 191/191. Add-in (`a4b429423`, 049): typecheck 111 / production 0; build exit 0; full jest = the ten known failing suites (84 tests); **33/33 gated suites, 417 tests**. |
| **In flight (agents, isolated worktrees)** | **020** (sonnet, client + one BFF mapping) — FR-06: the pane's Document Name defaults to the open document's file name minus its extension, is editable behind a pencil affordance, and the edited value finally lands in `sprk_documentname` instead of `sprk_documentdescription` (the columns are inverted today). **Hold lifted 2026-09-16**: 020's only dependency is 015 ✅, its file conflicts (021/022/023/026) are all merged, and its escalation triggers are about Email/Attachment naming and the 850-char bound — none of them needs the document-matching decision. The one link to that discussion, `notes/025-residual-collision-surface.md` open question 8 ("should FR-06 default to something MORE specific than the file name, since `Untitled Document.docx` multiplies collisions?"), is explicitly OUT of the agent's scope and stays with the owner. |
| **Waiting on the owner** | (1) **How Word documents are matched to avoid duplicates** — 014/FR-02 vs a user-confirmed match through the 025 clash prompt; decides 014, 025 (which also carries 047's create B → A → B case) and 045. Includes 025 open question 8: should the Document Name default to something more specific than the file name? (020 ships FR-06 as written and does not decide it.) (2) Dev BFF deploy sign-off, and an add-in redeploy. (3) **041 needs explicit sign-off**: it writes SPA redirect URIs to LIVE Entra app registrations per environment (a wrong or missing redirect locks every add-in user out — AADSTS7000471) and depends on 011 (the manifest re-upload); not dispatched to an agent. (4) Owners for #975 and the idempotency filter's no-header no-op; CI owners for the Tier 2 30-minute limit. (5) 031: Project numbering → the numbering project? (6) Word manifest 1.0.8.0 re-upload (closes 011). |
| **Next Action** | **1)** When 020 reports: review the diff yourself, merge `--no-ff`, run both gate chains (BFF build + Office/Documents tests + ArchTests + full suite; add-in typecheck + build + full jest + the 33 gated suites), add any new suites to `ci-gated-suites.txt`, mark ✅ in TASK-INDEX + POML, push, refresh the PR body, remove its worktree. **2)** CI on `4c7af1c21` finished green: 32 pass, all blocking Tier 1; only Tier 2 Full Unit Tests cancelled on its 30-minute limit (advisory, CI project's file). **3)** After 020, nothing else can start without the owner — see the row above. NEVER touch `agent-a97210f38cd331cfb`. |
