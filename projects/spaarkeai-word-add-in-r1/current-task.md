...
| **Owner decisions this session (2026-09-15)** | (1) Email `.eml` names: B2 — suffix only system-derived names, never typed names (in project CLAUDE.md). (2) **ADR-051 exception for the Find list**: a ranked top-50 list — progressive reveal of the single bounded response, no pager, no fake pages, the `PARTIAL_RESULTS` warning kept. (3) **ADR-024 exception for pane To Dos**: record lookup + a document/communication source lookup together; resolver fields describe the record. (2) and (3) are in project CLAUDE.md Decisions; **still to mirror into `design.md` ADR Tensions and the PR (§6.5).** |
