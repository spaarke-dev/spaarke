...
> **Last Updated**: 2026-09-15 (027/034/035 merged + pushed; 029 re-dispatched with owner decisions; 047 created + dispatched; 036 running; chat-test flake fixed)
...
| **In flight (agents, isolated worktrees)** | **029** (opus, re-dispatched with the owner decisions) — per-version profile + index refresh: upload-then-trim re-index for version saves only, one new `IRagService` delete method, a per-save discriminator (job id or written version id) so B → A → B refreshes three times; must NOT touch the save key (that is 047). **047** (opus) — NEW 🔴: a version save repeating an earlier version's content is answered "Duplicate" and never written (pane key `existingDocumentId+fileName+contentSha256+failedAttempts`; `CheckForExistingJobAsync` = newest row, no time window); must cover the `IdempotencyFilter` response cache too; options (a) server checks the document is still at the version the job wrote [leaning], (b) base version in the key, (c) per-click id. **036** (sonnet) — Send Email; client-only; Word may come back as an owner question. None of them edits TASK-INDEX / current-task / CLAUDE.md / `ci-gated-suites.txt`. |
| **Waiting on the owner** | (1) **How Word documents are matched to avoid duplicates** — 014/FR-02 vs a user-confirmed match through the 025 clash prompt; decides 014, 025, 045 and 020. (2) Dev BFF deploy sign-off, and an add-in redeploy (027/034 client changes, 046(b)'s name flag, the new `ORG_URL`). (3) Owners for #975 and the idempotency filter's no-header no-op. (4) 031: Project numbering → the numbering project? (5) Word manifest 1.0.8.0 re-upload (closes 011). (6) Possibly from 036: Send Email hidden in Word for r1? (7) **Clarify**: for 029 the owner answered "this is dev data — do not need to fix old data" to the question of who fixes Compose save-back / manual Run Index, which keep CREATING stale tail chunks going forward; recorded as "no backfill"; the forward fix for those paths is still unassigned. |
| **Next Action** | **1)** Pushed through `0f96c51cf`; its CI hit only the flaky chat test, now fixed here (test-only) and pushed with 029/047 docs. Publish the PR draft (`scratchpad/pr-960-body.md` — already has the 027/034/035 rows, 035's placement statement, an ADR-exceptions table and the new owner decisions; add a Testing row with these gate numbers and mention the `ORG_URL` fix), and watch CI (`gh pr checks 960`; Tier 2 is advisory). If the draft is gone, rebuild from `gh pr view 960 --json body`. **2)** When 029 / 036 report: review the diffs yourself (not just the report), merge with `--no-ff`, run the same gates, add new suites to `ci-gated-suites.txt`, mark ✅ in TASK-INDEX + POML, push. **3)** Held: 020 (014's fate), 025 and 045 (the matching discussion), 031 (numbering answer), 037 (manifest churn). Remaining 🔲 after 029/036: check TASK-INDEX. **4)** Worktree cleanup of 046/027/035/034 was started; confirm with `git worktree list`. NEVER touch `agent-a97210f38cd331cfb` (another session's). |
