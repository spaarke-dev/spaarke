...
> **Last Updated**: 2026-09-15 (context-handoff before /compact — 034 in flight; 027 + 035 ready to merge; local HEAD NOT pushed)
...
| **Agent work waiting to merge** | **027 ready** — branch `worktree-agent-aad4e56e3ed274112`, commit `d660a5906`: client-only; `openRecordLauncher.ts` opens `${ORG_URL}/main.aspx?etn=…&id=…&pagetype=entityrecord` via `Office.context.ui.openBrowserWindow` (unset `ORG_URL` = safe no-op), new `canOpenBrowserWindow` capability (runtime `isSetSupported`), focus/visibility refetch reusing 033's `useDocumentProfile.refetch()`; gates green. **035 ready** — branch `worktree-agent-abd79ab3b571aadeb`, commit `ae84e00b1`: `CreateTodoAsync` widened + client; a pane To Do carries the document/communication AND the record; resolver fields describe the record; owner ratified the ADR-024 exception 2026-09-15; +0.05 MB; gates green. **034 IN FLIGHT** — branch `worktree-agent-ac164237e1e38afd0` (notes commit `437c12ae0`: F-c = documents-only; the similarity route cannot page) — resumed to build Path 2 (owner decision). |
