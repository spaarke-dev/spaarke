...
- **Phase**: Phase 0 complete; Phase 1 nearly complete (011 awaits the 1.0.8 manifest re-upload; 014 not started); Phases 2–3 under way. **26 of 45 tasks ✅ as of 2026-09-12** — 012, 013, 016, 021 (Profile), 022 (Generate Profile), 023 + 024 (version save, server + pane), 030 (Matter quick-create, numbering moved out) done. 🔴 039 (save-spine defects incl. silent lost edits) must land before any production deploy of the save path
...
| 2026-09-14 | **Outlook email saves get a short unique suffix on their generated `.eml` file name — like attachments already get a unique prefix — so two same-subject/same-date emails never collide** (owner decision; task 025 input) | The `.eml` name is system-generated from subject + date and never typed by the user, so a suffix changes no user choice. Owner principle recorded alongside: **a file name the user typed is never changed automatically** (no auto-rename, no overwrite). How Word-document name clashes and round-tripped copies are handled is **under owner discussion** (014/FR-02 + 025) — see `current-task.md`. |
