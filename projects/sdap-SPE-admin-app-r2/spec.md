...
| **FR-C08** | Show **replication state** ("pending, up to 24h") on container-type settings, and show which settings a consuming tenant **is permitted to override**. ~~Overridden settings are visibly marked.~~ 🔴 **AMENDED 2026-08-27 (task 026, operator decision — drop cross-tenant).** The *effective* override lives on a `fileStorageContainerTypeRegistration`, which hangs off `fileStorage` and is **scoped to the calling tenant** — so an owning tenant can read its own registration and never a remote consuming tenant's. Showing "this setting is overridden" is a claim this screen **cannot make on this platform**, and in Spaarke Dev the owning tenant *is* the consuming tenant, so a cross-tenant override cannot even be produced to test against. Cross-tenant override display is **dropped, not deferred** — the honest surface is the *permission* list plus an explicit statement of the limit. | After a settings save, the UI indicates replication is pending with the 24h expectation. The overridable-settings permission list renders, and the screen states plainly that overrides applied in another tenant are not visible from it. ✅ **Met as amended.** |
...
| **FR-E02** | **Real quota management** — `maxStoragePerContainerInBytes` as a **ceiling** control, cleanly separated from consumption reporting per FR-C05/FR-C06. ~~Gives legal customers per-matter storage caps.~~ 🔴 **AMENDED 2026-08-27 (task 051, operator decision — option A).** Graph has **no per-container ceiling**: the property exists only on `fileStorageContainerTypeSettings` (the container TYPE) and is absent from `fileStorageContainerSettings` on both API versions; a container-scope PATCH returns **200 and silently discards it** (measured live). Per-matter *differentiated* caps are therefore **not achievable on this platform**. Delivered instead: the type-scope ceiling (one value governing every container of the type, labelled as such) **plus** a per-container read-only quota surface (`total`/`used`/`remaining`/`deleted`/`state`) from the container drive. | A ceiling can be set and read back **with the write verified against Graph's own response, not a 200**; it is presented distinctly from usage; the UI states that the limit is type-wide. ✅ **Met as amended.** |
