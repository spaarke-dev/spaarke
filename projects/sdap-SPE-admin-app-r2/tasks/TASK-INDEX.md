...
| 025 | [Full 9-property settings surface](025-full-settings-surface.poml) | 3 C | C07 | FULL | sonnet | high | W9 | ❌ | 023, 040 | ✅ **complete 2026-08-27 — the deferred FORM is delivered, AC-1 met.** 🔴 FR-C07 listed a property that **does not exist** (`agent.chatEmbedAllowedHosts`) and omitted one that does (`sharingCapability`). 🔴 **The rebind exposed a 4th fabrication defect**: `extractSettingsFromConfig` read the *Dataverse* config and invented every missing value — `?? "disabled"`, `?? false`, `?? 100`, `?? 1 GB`, and **`isSearchEnabled: true` hard-coded** with a comment asserting an unmeasured Graph default. An admin looking at a container type with search OFF saw the switch ON. Invisible until now because the server never sent settings at all, so nothing could disagree with the guess. Now seeded from `containerType.settings` (Graph is the authority); initial state `{}`, not defaulted. All nine render (+ beta `isOfficeRestricted`); `consumingTenantOverridables` and `isOfficeRestricted` deliberately **read-only** (override PERMISSION metadata whose live flags exceed the SDK enum — a free text box would let a typo silently widen what a consuming tenant may override). New `TriStateSwitch`: `<Switch checked={undefined}>` renders identically to `false`, so a **Not reported** badge keeps the third state visible and the value is **omitted from the save** rather than written as `false` — an unknown must not become a write. Save re-seeds from the PUT **response** (FR-C04 read-back), and MERGES rather than replaces: the response DTO is narrower (no `owningAppId`/`expiryDateTime`/`region`) so assigning it wholesale would have made a successful save look like data loss — caught before shipping. 🔴 **Stale-caveat correction**: this row previously carried *"every PATCH returns 400, nothing is writable"* — superseded on 2026-08-25 by [`../notes/patch-400-resolution.md`](../notes/patch-400-resolution.md) (`etag` is a REQUIRED **body** property; fix is in the code, proven live). Two notes disagreed for two days; 051 was right. ⏳ **UAT**: one save against Spaarke Dev to confirm each of the nine persists (051's read-back verification would *report* a silent discard as 502 + `unwrittenFields`). See [`../notes/task-025-schema-verification.md`](../notes/task-025-schema-verification.md) §6 |
| 026 | [Replication + override state](026-replication-and-override-state.poml) | 3 C | C08 | FULL | sonnet | high | W10 | ✅ | 025 | ✅ **complete as AMENDED 2026-08-27 — escalation answered (operator: DROP cross-tenant).** 🔴 `consumingTenantOverridables` is a **permission**, not a state. The *effective* override lives on a `fileStorageContainerTypeRegistration`, which hangs off `fileStorage` and is **scoped to the calling tenant** — an owning tenant can read its own registration and never a remote consuming tenant's, so *"this setting is overridden"* is a claim this screen cannot make on this platform. In Spaarke Dev the owning tenant IS the consuming tenant, so a cross-tenant override could not even be produced to test against. **Dropped, not deferred** (spec FR-C08 amended). ⚠️ Graph's published override enum is **narrower than the live tenant** — `sharingCapability` and `isOfficeRestricted` come back as flags that are NOT members of the SDK's generated enum, which is why the value is carried as the raw comma-delimited **string**: parsing through the typed enum would drop or reject real data. **No code change was needed for the amendment** — the shipped UI already renders the permission list and states plainly that *"overrides applied in another tenant are not visible from this screen"*, which is exactly the honest surface the drop calls for. See [`../notes/task-026-findings.md`](../notes/task-026-findings.md) §6 |
...
| 027 | [Container-type owner management](027-container-type-owner-management.poml) | 3 C | C09 | FULL | sonnet | high | W11 | ❌ | 011, 020, 040 | ✅ **COMPLETE 2026-08-25 — AC-1 verified live** (list 0 → add **201** → list 1 → delete **204** → list 0, reverted). 🔴 POML premise FALSE — the new relationship supersedes **nothing** (`applicationPermissions` = which APPS may access; `permissions` = which PEOPLE own). Nothing retired; AC-4 vacuous. 🔴 **delegated+beta never existed** and is the only combination Graph serves (v1.0 → 400 *"Resource not found for the segment 'permissions'"*, beta → 403 = route exists). ⚠️ the 3-owner limit is **unsourced** — stated as admin-center UX, not Graph-enforced |
| 028 | [Container URL + Purview deep-link](028-container-url-and-purview.poml) | 3 C | C10, C11 | FULL | sonnet | medium | W12 | ❌ | 020, 040 | ✅ **done 2026-08-24.** 🔴 The container entity has **no URL property in either version** — and on the COLLECTION Graph accepts the `drive` expand, returns **200**, echoes it in `@odata.context`, and **omits it from every row**. Escalation evaluated, did NOT fire (GET-single returns it). ⚠️ Grid resolves per row on demand — see notes §3 |
| 041 | [LiveIntegration suite + throwaway fixture](041-live-integration-suite.poml) | 4 D | D02 | FULL | sonnet | high | W13 | ✅ | 011, 040 | ✅ **completed 2026-08-27** — fixture + guard + 6 tests, green live and 6/6 no-op in 10 ms without credentials (CI-exclusion verified independently). Teardown-on-failure and the foreign-container guard both PROVEN live, not just implemented. 🔴 Found a real production defect: `RegisterConsumingTenantAsync` POST returns **400 apiNotFound on both API versions** while GET on the identical URL succeeds — captured as a characterization test that fails loudly if fixed. Role flow gated behind `SPE_LIVE_DELEGATED_TOKEN` (delegated+beta-only, needs interactive sign-in) |
| 042 | [Retire scaffolding tests (ADR-038)](042-retire-scaffolding-tests.poml) | 4 D | D03 | FULL | sonnet | high | W14 | ✅ | 040, 041 | 🔄 **partial 2026-08-27 — 722 → 356 SpeAdmin cases, suite green.** 8 files deleted whole; keepers **relocated, not deleted** (4 mappers → `unit/domain/`, 5 ad-hoc ADR-007 reflection tests **generalised into 2 ArchTest rules**). 🔑 Structural key: `tests/unit/Sprk.Bff.Api.Tests/**` is **NOT a KEEP path**, and all 8 KEEP categories compile into that same assembly via csproj globs — so relocation needs no csproj change. 🔴 `MultiAppSupportTests.cs` + 10 Phase3 tests exercised code with **zero callers** (`GetClientForOwningAppAsync`, `ValidateOwningAppSecretsAsync`, `FetchOwningAppSecretAsync` — grep-verified): **34 green tests against a feature that cannot execute**. 🔴 2nd instance of *tests defending a defect* — 2 skip-token tests pinned a **numeric** offset scheme production had replaced with an **opaque** token, while their comments claimed to mirror it. 🔴 Found a real `ADR007_GraphIsolationTests` gap: its allowlist exempts **any** namespace containing `Infrastructure.Graph`, so every nested domain record was unguarded — new rule closes it (return-type rule narrowed to exempt `GraphServiceClient`, reason documented inline). ⚠️ POML counts were stale (**29** files not 14; **722** cases not 359). 🔔 **`SecurityEndpointTests.cs` (20) ESCALATED, not deleted** — all match a ban but no Security contract test exists anywhere. ✅ **`SecurityEndpointTests` escalation RESOLVED 2026-08-27** — the replacement now exists: `tests/integration/contract/SpeAdmin/SpeAdminSecurityContractTests.cs`, **11 contract tests** on a KEEP path exercising real Graph request+response. Real coverage of `/security/alerts` + `/security/score` went **0 → 11** (the 20 banned tests had count 20, coverage 0). Load-bearing cases are about ABSENCE: a swallowed **403** would render *"No active alerts"* to an admin who cannot read alerts at all, and a defaulted secure score fabricates a posture (0 = catastrophic, max = perfect; neither measured). The 20 are **still not deleted** — that is `/test-diet`'s call at 090 — but the *reason for holding them* is gone. 🔴 **New evidence on `SearchItemsTests`**: `SearchItems_WithToken_ValidConfigIdNotFound_Returns400` makes a **real outbound Dataverse call from `tests/unit/**`** — passed twice then failed with `TaskCanceledException` after ~100 s in the same session. Proven pre-existing by `git stash -u`. Non-deterministic by construction and its ~2-min timeout holds the suite hostage; for /test-diet the choice is an offline Dataverse double or removal, NOT merely tightening the assertion. ⏳ ~104 scaffolding methods across 6 files classified but **not yet removed**; AMBIGUOUS held for `/test-diet` at 090. See [`../notes/test-retirement-inventory.md`](../notes/test-retirement-inventory.md) |
| 050 | [Container archival](050-container-archival.poml) | 5 E | E01 | FULL | sonnet | high | W15 | ❌ | 020, 040 | 🔄 **built + contract-tested 2026-08-27; AC-1/AC-2 NOT live-verified — 🔔 escalation FIRED.** Live probe on a throwaway container (provisioned→activated→probed→torn down 204/204, NFR-07): `POST …/archive` → **403 notAllowed "…does not currently support archiving"** — *semantic, not routing*, so the beta action exists and the container type simply has not opted in. Enabling it is an operator action on a **shared** container type + needs an SPO module upgrade. 🔴 **The documented remediation does not exist**: POML, spec FR-E01, design §4.3 and the knowledge corpus all said `Set-SPOContainerType -IsArchiveEnabled` — **no such parameter on that cmdlet in any version** (verified by reflecting the cmdlet types). Real one is `Set-SPOContainerTypeConfiguration -ContainerTypeId <guid> -IsArchiveEnabled $true`, SPO ≥ **16.0.27515.12000**; all 5 docs corrected. Following AC-4 literally would have shipped an error naming a command that does not exist. 🔴 **`status` was fabricated as `"active"` for 100% of responses on every path** — it is a *typed* SDK property, so the `AdditionalData` lookup behind all four mapping sites could never match and `: "active"` always fired (a just-created container Graph reported as `inactive` was reported active); client had a second `?? "active"`. Both fixed; `Status` now nullable → "Not reported". 🔴 3rd instance of *tests defending a defect* — `ListContainers_WhenGraphOmitsStatus_**DefaultsToActive**` asserted the fabrication in its own name; inverted. ⚠️ archive/unarchive are **beta-only** (absent from v1.0 CSDL) — no ADR conflict, the container surface is already beta-pinned by task 020. ⚠️ Async: `recentlyArchived`→`fullyArchived`, `reactivating`→active; endpoints return **202 + `pending:true`**, never "archived". ⚠️ `archivalDetails` has **never been seen on the wire** even with an explicit `$select` that `@odata.context` echoes — open question in [`../notes/task-050-findings.md`](../notes/task-050-findings.md) §7. Kiota shape (`UntypedObject`) guessed wrong twice, caught by a test asserting the mapped value. See [`../notes/task-050-findings.md`](../notes/task-050-findings.md) |
| 051 | [Per-container quota ceiling](051-quota-ceiling.poml) | 5 E | E02 | FULL | sonnet | high | W16 | ❌ | 023, 024, 040 | ✅ **complete 2026-08-27 as AMENDED — 🔔 escalation fired and was answered (operator: option A).** 🔴 **There is no per-container ceiling in Graph**: `maxStoragePerContainerInBytes` exists only on `fileStorageContainerTypeSettings`, absent from `fileStorageContainerSettings` on **both** versions, and no bound action sets a quota. So FR-E02's "per-matter storage caps" is **not achievable** — one value governs every container of a type. 🔴 **Worse: `PATCH /containers/{id}` with the ceiling returns 200 and SILENTLY DISCARDS it** — nested *and* top-level, read-back carries no such property (measured on a throwaway container, NFR-07). 3rd instance of accepted-200-silently-dropped (028 `$expand=drive`, 050 `$select=status`). **The POML's own "confirm by read-back, not by a 200" constraint is what caught it.** Shipped: type-scope ceiling labelled type-wide (warning MessageBar on the field) + **per-container read-only quota** (`total`/`used`/`remaining`/`deleted`/`state`) from the container drive + **write verification** → `SettingsNotPersistedException` → **502** with `unwrittenFields`. 🔑 **Bonus: `quota.used` is available on GET-single**, closing the gap tasks 020/024 documented (`storageUsedInBytes` is beta-only AND LIST-only) — live 868,906,006 B on `Spaarke Inc`, consistent with 024's measured 861 MB. ⚠️ Scope was smaller than the POML assumed — 023/025 had already built the type-scope write. ⚠️ `remaining` is Graph's figure, never `total − used` (deleted items still count); a contract test asserts they differ. See [`../notes/task-051-findings.md`](../notes/task-051-findings.md) |
| 052 | [Item recycle bin (207 handled)](052-item-recycle-bin.poml) | 5 E | E03 | FULL | sonnet | high | W17 | ❌ | 022, 040, 041 | ✅ **complete + live-verified 2026-08-27** (18/18 checks on a throwaway container it provisioned and tore down; teardown confirmed 0 remaining — NFR-07). Shipped as a **Recycle Bin tab on the container detail pane** (not a top-level screen: a deleted *file* only has meaning relative to its container, and a sibling top-level screen would invite exactly the conflation spec D3 forbids). 🔴 **Restore and permanent delete fail in OPPOSITE ways** — restore: `207` listing only the ids that SUCCEEDED (partial failure = `requested − returned`, no per-item error object) and **atomic** on rejection; delete: **`204` regardless of what it did**, empty body, **non-atomic** (a batch with one bad id still purges the valid ones and reports none of it). For an irreversible op that is the worst reporting shape found in this project, so `PermanentDeleteRecycleBinItemsAsync` **lists the bin BEFORE and AFTER and diffs** — the *before* list is load-bearing, without it an id that was never in the bin reads as "purged" (a fabricated success on an irreversible op). Unverifiable re-read → **207 + `verified:false`**, never 5xx (the delete WAS issued; an error status would assert the opposite unestablished thing). 🔴 **NEW: Graph's error `code` for a rejected restore is NOT STABLE** — `badArgument` and `invalidRequest` both observed for the identical condition hours apart on 2026-08-27. The detector keys on the **400 status**, not the code string; had it matched the code (as `IsArchivalNotEnabled` must for `notAllowed`) it would have stopped detecting within a week, silently. Contract test is now a `[Theory]` over both payloads. 🔑 **Trap #3 dissolved, not handled** — raw JSON via `SendGraphJsonAsync` means `deletedBy` never becomes an `UntypedObject`. Restore rejection → **409** with `remediation`; batch cap 200. **No `Program.cs` edit, no new DI, no new NuGet**; publish **45.12 MB compressed** (+0.16 vs baseline); ArchTests identical to the stash-proven baseline. Also corrected the knowledge corpus's *"item recycle bin — GA v1.0"* claim (the **actions are beta-only**; entity types are on v1.0). See [`../notes/task-052-findings.md`](../notes/task-052-findings.md) §5 |
...
| 061 | [Refresh SPE knowledge corpus](061-knowledge-corpus-refresh.poml) | 6 | X01 | MINIMAL | sonnet | medium | W18 | ✅ | 025 | ✅ **completed 2026-08-27** — 3 corpus docs + REFRESH-LOG (+525 lines). 🔴 The 2026-05-14 snapshot said container-type CREATE needs an *application permission* — `containerTypes` supports **no** application permissions at all (403 live, task 010). Also flagged `agent.chatEmbedAllowedHosts` as **fictional** (a prior R2 requirement doc invented it). Create-role doc-vs-doc conflict recorded with **both** sides, unresolved |
| 062 | [Billing-attach cross-project handoff](062-billing-attach-handoff.poml) | 6 | X02 | MINIMAL | sonnet | low | W18 | ✅ | 029 | ✅ **completed 2026-08-27** — requirement filed on `customer-provisioning-orchestration-r1` + issue **#831**. Read/write boundary stated in both docs: SPE Admin **reads** billing and warns; provisioning is the **sole writer** |
