...
## Verification Addendum — 2026-08-06 (live-code reconciliation, Fable)

> A read-only Fable adversarial-verification pass re-checked every §3 finding against the live `src/server/api/Sprk.Bff.Api/` on `work/code-quality-and-assurance-r3`. **All §3 findings HOLD** (LOC match the design exactly; KEEPs confirmed wired; nothing became load-bearing). Corrections + new findings below are integrated into r3 tasks 020–029. This addendum is authoritative for exact line numbers.

### Verified anchors (exact, current)
- **KEEPs (MUST NOT delete)**: `StubInsightGraph` wired `Infrastructure/DI/InsightsModule.cs:53`. Todo double-layer: flag read `TodoSyncModule.cs:47`; NullObject regs `:64-71`; NotImplemented regs `:73-76`; factory ternaries `:84-98`.
- **Bug-1 casts**: `FinanceRollupService.cs:230` (helper 228-236; live call :85); `Services/Finance/Tools/FinancialCalculationToolHandler.cs:140` and `:206`. Sole `IDataverseService` reg `GraphModule.cs:46-50` → `DataverseServiceClientImpl` (`Spaarke.Dataverse/DataverseServiceClientImpl.cs:17`, not a `ServiceClient`). Live via `InvoiceExtractionJobHandler.cs:253`; handler reg `FinanceModule.cs:162`.
- **Bug-2**: dead half `EmailToEmlConverter.cs:59` (`ConvertToEmlAsync`) + `:115`; live `ExtractAttachments :681` ← `UploadFinalizationWorker.cs:1047`. Dual reg `EmailServicesModule.cs:44-46` (scoped, **wins by Program.cs order :193 > :125**) vs `OfficeWorkersModule.cs:31` (singleton).
- **13 downcasts / 17 sites (exact)**: impls at `TodoGenerationService.cs:882`, `SavedQueryService.cs:217`, `RecordService.cs:206`, `UserPrivilegeChecker.cs:126` (inline), `SpendSnapshotService.cs:822`, `MetadataService.cs:298`, `GridConfigurationService.cs:166`, `FinanceSummaryService.cs:444`, `FetchService.cs:83` (inline), `MembershipFieldDiscoveryService.cs:558`, + the 3 BROKEN (`FinanceRollupService.cs:228`, `FinancialCalculationToolHandler.cs:140,206`). `DataverseServiceExtensions.cs` = DI-only, no unwrap helper.
...
- **DC-2 (task 020)**: Safety "2 stale crefs" → live count is **3 CS1574 cref sites in 2 files**: `SideEffectGateAIFunction.cs:153`, `PromptShieldChatMiddleware.cs:30` and `:67`. (5 `<c>` prose mentions elsewhere + `AiSafetyModule.cs:119` + `ConfirmationPolicyGateLiveDecisionTests.cs:219` are non-breaking but merit a comment sweep.)
- **DC-3 (task 021)**: Bug-1 cast is at `FinanceRollupService.cs:230` (design said :228; helper spans 228-236).

### Missed findings (integrate)
- **MF-1 HIGH (→ task 023, scope expansion)**: the Scorecard siblings' web-resource callers are **already broken today** — `ScorecardCalculatorEndpoints.cs:28,46` require auth but callers send NO Authorization header (repo-wide grep of `src/solutions/webresources` for `Authorization|Bearer|token` = 0 hits): `sprk_matter_kpi_refresh.js:269`, `sprk_kpi_subgrid_refresh.js:294`, `sprk_kpiassessment_quickcreate.js:54-56,262-263` → swallowed 401s. Fold into 023's `@spaarke/auth` web-resource migration (all 4 callers) or route explicitly to 030/FR-17.
- **MF-2 MEDIUM doc-drift (→ task 023 + 034)**: `.claude/patterns/webresource/subgrid-parent-rollup.md:16` **mandates the anti-pattern** ("MUST use `.AllowAnonymous()` because web resources cannot acquire Azure AD tokens", cites ADR-001). MUST be rewritten when 023 lands, or the next agent reintroduces anonymous endpoints.
- **MF-3 MEDIUM (→ task 023)**: the "hardened" sibling `/healthz/dataverse/doc/{id}` ALSO leaks — echoes `ex.Message` + `ex.InnerException?.Message` at `EndpointMappingExtensions.cs:93`. Add to 023's ex.Message-scrub scope (design only scoped the two unhardened probes).
- **MF-4 LOW (→ task 022)**: captive-dependency edge — last-wins `IEmailToEmlConverter` reg is the **Scoped** one, captured by the **singleton** hosted `UploadFinalizationWorker`. Task 022 acceptance must require the final single registration lifetime to be safe for the singleton hosted worker.
- **MF-5 LOW (scorecard evidence, not a task here)**: oversized-file census — `SpeAdminGraphService.cs` 4,910 LOC, `ChatEndpoints.cs` 3,587, `ComposeShadowPatchEngine.cs` 2,999 (intentional ADR-049), `ComposeService.cs` 2,915, `CommunicationService.cs` 2,567, `PlaybookOrchestrationService.cs` 2,528. **OfficeService.cs re-measured 1,983 LOC** (`Services/Office/OfficeService.cs`; archived R3 #1 said 1,951) — still God-class; backlog/scorecard evidence.
- **MF-6 INFO (→ task 024)**: `InvoiceExtractionJobHandler.cs:33,55` also injects `IPlaybookLookupService` from a non-`Ai/` namespace — cleared by A-4's relocation; include in 024's final grep acceptance.
- **MF-7 INFO (→ task 041)**: Release publish emits 3× CS0618 (`DemoExpirationService.cs:347-348` re `DemoProvisioningOptions`) — matches design NG4; note for the analyzers-as-errors baseline.

### Data-driven dispatch — STRONGER than design stated (→ task 026)
`IToolHandler` dispatch keys on `sprk_analysistool.sprk_handlerclass` (`AnalysisToolService.cs:108,186,257,616`). MUST-NOT-CHANGE HandlerIds: `"FinancialCalculationToolHandler"` (`Ai/Handlers/FinancialCalculationToolHandler.cs:51`) + `"FinancialCalculatorHandler"` (`FinancialCalculatorHandler.cs:67`). **Additionally** the Finance/Tools handler is `IAiToolHandler`, discovered by **CLASS NAME** via `/api/ai/tools/handlers` (`HandlerEndpoints.cs:9,22-34`; dual reg `FinanceModule.cs:136-137`) — the rename pre-check must verify no playbook references the class name, not just the two `sprk_handlerclass` rows.

### SCORECARD BFF row (D1–D11, verified)
D1 Architecture **B** · D2 Correctness **C+** · D3 Security **B–** · D4 Perf/size **A–** · D5 DRY/dead-code **C** · D6 Consistency **B–** · D7 Test quality **B** · D8 Dependency hygiene **A–** (design-claimed) · D9 Observability **B** · D10 ALM/build hygiene **D+** · D11 Doc accuracy **C+**. (Evidence in the sections above; feeds `notes/SCORECARD.md`.)
