...
- **Prescriptive ordering when 011 decides to do it**: MI-verify (r1 trap T2 — register the **UAMI** SP as Dataverse Application User in both envs: dev `mi-bff-api-dev` clientId `5967251e…`/principalId `9fd47efb…`, demo `mi-bff-api-demo` clientId `b0ce4ca4…`; **NOT** the stale `56ae2188…` named in `appsettings.template.json:79` — that is a RETIRED Phase-C-era principal per the BFF Auth Surface Map §C/§H) → migrate code → relax `DataverseOptions [Required]` → remove the **config key** `Dataverse:ClientSecret` (≠ removing the KV secret `BFF-API-ClientSecret`, which stays — one KV secret, 5 config keys, 9 consumers incl. OBO). Also: `DataverseWebApiClient.cs:42` prefers secret-over-MI (code change) + `prvActOnBehalfOfAnotherUser` re-grant. Full detail: `notes/bff-auth-surface-map.md`.
