...
28. **FR-28 (BFF Auth Surface Map — auth de-risking, owner-requested 2026-08-13)**: Produce a verified, consolidated **BFF Auth Surface Map** — inbound schemes/policies/fallback/filters, outbound credentials per service (Graph, Dataverse both stacks, Cosmos, KV, Content Safety, OpenAI, SPE), the identity model (system-assigned MI vs UAMI; which SP holds Graph roles), the secrets inventory, and — the centerpiece — a **secret/identity → consumer dependency graph** with a safe-to-remove verdict per secret. Read-only ⇒ conflict-free. It **gates every auth-touching task** (023, 060, 061, 062) and is the verified input to task 011's #3b credential-migration decision, so all auth work reasons from one complete picture rather than piecemeal slices (auth failures are cross-slice — e.g. `API_CLIENT_SECRET` is shared by OBO *and* the shared-lib Dataverse path). — Acceptance: `notes/bff-auth-surface-map.md` exists (sections A–H, file:line-cited); the dependency graph enumerates every secret/identity's full consumer set; GAP #2 (CIAM-scheme vs `RequireAuthorization`) and GAP #4 (system-vs-UAMI + Graph-role SP) are resolved or flagged for owner confirmation; per-task implications recorded. (Task 019.)

29. **FR-29 (Resource/secret naming standardization for productization — owner directive 2026-08-13)**: As deployments replicate to more environments, resource + KV-secret names must be **standard, accurate, env-agnostic** (name stable across environments; env-specific *value* only). r3 owns the **standard** (refresh `AZURE-RESOURCE-NAMING-CONVENTION.md` — which already mandates env-agnostic KV names at :329 but is unenforced + drifted) and the **forcing-function** (a naming-conformance CI gate that fails on an env token baked into a replicated name, casing/prefix drift, or orphan/duplicate secrets); the deployment project `customer-provisioning-orchestration-r1` owns **applying** the names in provisioning + **remediating** live-environment drift. Assessed by task 017 (naming-drift census → current→canonical target-name map); standard+gate by task 063; r1 handoff recorded. Evidence: the dev-vault `spaarke-spekvcert` census (`SPRK-DEV-DATAVERSE-URL` env-token-in-name, `BFF-API-ClientSecret` vs `bff-api-client-secret` casing, orphaned `Graph-API-ClientSecret`). — Acceptance: convention refreshed with the env-agnostic KV/resource standard; the conformance gate fails a seeded env-token/casing violation; r1 handoff (rename map + apply/remediate ownership) recorded. Same r3-owns-standard / r1-owns-apply seam as FR-27 (#4). (Tasks 017, 063.)
