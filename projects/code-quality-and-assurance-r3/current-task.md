...
| **Next Action** | **RED-4 B COMPLETE + deployed + DEF-2 live-verified** (master `08cd5b5b7`). **#3b MI migration: 2 attempts 2026-08-17 → BLOCKED on an Azure/MI issue (not code).** Attempt 1 (eager connect) crashed boot; Attempt 2 (lazy connect + sync token, commit `d40f6c24a` + `notes/3b-mi-migration.patch`) is **startup-safe and correct** — boot stayed up — but unmasked the real blocker: **the App Service MI gets HTTP 400 (`managed_identity_request_failed`) requesting a *Dataverse* token** (Graph works; identical scope/credential to the app's canonical pattern). Dev restored to ClientSecret (healthz 200, Dataverse paths 200); secrets untouched; nothing MI on master. **#3b now = an Azure MI-enablement investigation** (why can't the UAMI mint a Dataverse token — MI corr id `f8b7a8ce-4606-4f38-8ba4-f025c1c3392c`), NOT a coding task; the code is ready to apply once the token issue is fixed. Full writeup: `notes/task-011-ng1-3b-mi-migration.md` (Attempts 1 & 2). |
