...
**Absorbs stale PR #508 (owner-confirmed 2026-08-15)**: PR #508 (`fix/events-smarttodo-components-package-boundary`, +196/-13, CONFLICTING, abandoned 2026-06-29) restored the `@spaarke/ui-components` package boundary for `Spaarke.SmartTodo.Components` (rewrote relative `../../../src/...` imports → `@spaarke/ui-components/...` package specifiers + package.json/tsconfig `paths`, per the PR #506 pattern). The hoist MUST land on that restored boundary, so **FR-01 re-applies #508's fix on current master first, then hoists** — and #508 is closed as superseded by R5 (not revived separately).
