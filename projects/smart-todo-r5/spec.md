...
| ADR-006 | "PCF over webresources" | FR-02/03 (task 011) adds a `sprk_todo` **form OnChange webresource** (`sprk_todo_score_onchange.js`) that carries the priority/effort→score mapping tables | **A (project-scoped exception)** | ADR-006 governs custom **UI controls** (fields/grids/ribbon), where PCF is preferred. A Dataverse **form OnChange event handler** is form scripting — inherently JavaScript, not expressible as a PCF control. The webresource literal-mirrors the canonical TS mapping (`@spaarke/ui-components` `todoScoreMappings.ts`) with a keep-in-sync cross-reference comment, following the existing `sprk_todo_regarding_presave.js` precedent (same entity, same pattern). Not a control violation; sanctioned for this project. Cited at task-011 code review. |

> All other listed ADRs apply without exception. FR-15 explicitly complies with ADR-050 (uses `BrowseModal`, adds no forked chrome). The ADR-050 tension above is narrow (one surface, OOB-by-owner-requirement) and does not weaken the shell elsewhere. The ADR-006 tension is likewise narrow — one form script, mirroring an existing sanctioned precedent — and does not license webresource-based UI controls anywhere.
