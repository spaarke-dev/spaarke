...
3. **#3 hide "General" tab title on To Do main form** — ✅ DONE + published. Operator confirmed no maker toggle exists (only "hide tab" which hides content). Patched the **"To Do main form"** (`eca59df4-1364-f111-ab0c-7ced8ddc4cc6`) formxml via Web API: added `showlabel="false"` to the General `<tab>`, PATCHed `systemforms`, PublishXml `sprk_todo`. Backup: `scratchpad/todo-mainform.bak.xml`. (Distinct from the won't-do 032 form-HEADER hide.)
