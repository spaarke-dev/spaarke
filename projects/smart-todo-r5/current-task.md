...
- **#1 Dialog chrome title → "Smart To Do Item"** (uniform, not the record's `sprk_name`): via the `navigateTo` `title` param at all 3 To Do launch sites (`openTodoLauncher`, `newTaskLauncher` — was "New To Do", `todo.registration` widget open). Deployed `sprk_smarttodo` + `sprk_spaarkeai`.
- **#2 Hide header entity name "To Do"** (`entity_name_span`): the SUPPORTED UCI Client API `formContext.ui.setFormEntityName(" ")` (researcher-confirmed; DOM manipulation NOT needed). Extended the OnLoad script `sprk_todo_hide_tabnav.js` (v1.1.0) → redeployed web resource + published.
- **#3 Modal size: one standard size down** — `fullCover`(100%) → `record`(85%). Added optional `size?: OobModalSize` param to `navigateToEntityRecordSurfaceAsync` (**default stays fullCover** for the other generic consumers — surfaceHandoff/Communication launchers); the 3 To Do sites pass `getOobModalSize('record')`. **Import gotcha**: SmartTodo vite can't resolve the deep subpath `@spaarke/ui-components/utils/adapters/oobModalSizes` (its `/utils` alias points at `index.ts`) → import `getOobModalSize` from the **barrel** `@spaarke/ui-components` (already re-exported via adapters/index). Tests updated + green (SmartTodo 27, wizardLaunchers 14).
- **Researcher findings memoized**: `setTabNavigatorVisible` + `setFormEntityName` are the two supported UCI header-chrome levers (both must run from a FORM OnLoad handler to fire inside the `navigateTo` dialog).
