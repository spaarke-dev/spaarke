...
| R-8 | Bug 2 / ADR-012 cross-cutting limitation — SpaarkeAi Home tab shows structural placeholders only because section infrastructure lives solution-local in LegalWorkspace | **RESOLVED (2026-05-20 / task 067)** | Medium | **Architecturally resolved**: builder + types hoisted to `@spaarke/ui-components`. Future Code Pages consume the canonical `buildDynamicWorkspaceConfig`. Section content render remains placeholder in SpaarkeAi by design — hoisting legal-domain factories would violate ADR-012 "MUST NOT hard-code Dataverse entity names". Full SpaarkeAi section content render is scoped follow-on work (requires context-agnostic section catalog design). See [`notes/067-shared-lib-hoist-summary.md`](notes/067-shared-lib-hoist-summary.md). |
