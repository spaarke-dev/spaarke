...
| Row | GUID | Key values |
|---|---|---|
| `sprk_analysisaction` **CREATE-TASK@v1** | **`b66c8dda-8279-f111-ab0e-7ced8ddc4cc6`** | `sprk_actioncode=CREATE-TASK@v1`, `sprk_name=Create Task for Chat`, `sprk_kind=100000000 (Prompted)`, `sprk_modeltier=100000001 (Standard)`, `sprk_inputschema` = `{fileIds?, due_date! (elicitation_prompt "What's the due date for this task?"), assign_to! (elicitation_prompt "Should I assign it to you or someone else?")}` (originally a standard JSON-schema `required` array + per-property `required:true` dual declaration — the per-property half was INVALID JSON Schema and 400-failed every text-path turn at G-P3; removed 2026-07-07, object-level `required` array retained), `sprk_systemprompt` = CREATE-TASK@v1 JPS (artifact: [`notes/jps/CREATE-TASK-v1.jps.json`](jps/CREATE-TASK-v1.jps.json); jps-action-create Step 4 checks passed), `sprk_outputschemajson` = strict 4-field schema (mirrored at `infra/dataverse/outputschemas/create-task-v1.schema.json`) |
...
// ⚠️ CORRECTED by the G-P3 UAT round-1 fix wave (2026-07-07): the ORIGINAL seed additionally put
// "required": true INSIDE the due_date/assign_to property definitions ("dual declaration"). That is
// INVALID JSON Schema — Azure OpenAI rejects the ENTIRE request (invalid_function_parameters:
// "True is not of type 'array'"), which 400-failed EVERY text-path turn at G-P3 UAT round 1.
// Required-ness is declared ONLY via the object-level required array. Canonical mirror (CI-validated):
// infra/dataverse/inputschemas/create-task-v1.input.schema.json. The live spaarkedev1 row was fixed in place.
