...
| **Next Action (096)** | Implement the builder + fault describer in `DataverseServiceClientImpl.cs`; XML docs on interface + impl; tests; perturbation (revert to `ExecuteMultipleRequest` → a test must fail); full suite; publish size (fresh master vs fresh branch, short paths, matching file counts); ISS-005 + #970; TASK-INDEX. |
