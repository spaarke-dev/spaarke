...
> **Last Updated**: **2026-09-12, session 9** (checkpoint: tasks 102/103 created; task 100 paused at `e7bd02189`). Prior stamp: 2026-09-11, session 8 END (by `context-handoff`) — task 098 closed; everything through
...
| Field | Value |
|---|---|
| **Task** | **SESSION 9, NOW: task 102 (governance, new ADR-052) → 103 (scheduler lease) → 100 (finish).** Task 100 is PAUSED at `e7bd02189` (committed, not pushed) and waits on 103. Owner decisions D1–D7 and all evidence: `notes/decisions/workload-placement-policy-evaluation.md`. Earlier state, kept for context: **100 — FR-33 expiry reminders** · 🔔 **ESCALATED at step 0** (RESOLVED 2026-09-12) (session 9, 2026-09-11) — awaiting owner: (1) recipient when `sprk_grantedby` is empty, (2) channel. Evidence (live dev, read-only): 20/28 active grants have NO `sprk_grantedby` (every grant created 2026-08-11 → 08-25; grants from 09-09 have it); grant `createdby`/`ownerid` = BFF service identities; 10/11 root records owned by team "Spaarke Business Unit 1" (BU default team). Outbox channel is invisible without client work: `Spaarke.Notifications` `KindRouter` SKIPS unknown kinds; SpaarkeAi registers only 3 ACTIVE kinds. Alternative channel: existing `Services/NotificationService.cs` (`appnotification`, MDA bell) — 0 appnotification rows exist in dev, per-app enablement unverified. Scheduler candidate: `Spaarke.Scheduling` `IScheduledJob` (PlaybookSchedulerJob / MembershipReconciliationJob pattern). POML premise wrong ×13 (outbox has no consumer for a new kind). No code written. |
...
| **Next Action** | **TASK 102 IN PROGRESS (FULL, opus@high).** Step 0 ✅: ADR-052 is free (other worktrees use it only as an example). **Soft coordination**: `code-quality-and-assurance-r4` (PR #935) edits `.claude/adr/INDEX.md` (ADR-012 row) and plans `AdrClassificationCensusTests` (its task 013, not started), which will require ADR-052 to carry a classification once it lands; it doesn't touch ADR-001/004/013/036. Step 1 ✅: drafts in `notes/drafts/` — `ADR-052-workload-placement.full.md`, `.concise.md`, `adr-amendments-and-key-directives.draft.md` (ADR-001 A1, ADR-036 A1, ADR-004 A1, ADR-013 pointers incl. "MUST use Job Contract for background AI work" fix, bff-extensions §D/§E/table rewrite). Step 2 ✅ (Fable review done: direction holds, escalation NOT fired, v1 not ready → **v2 drafts** fix every finding + `notes/drafts/drift-guard-design.draft.md`; POMLs 102/103 and evaluation note §4.4 updated). **NEXT = WAIT FOR OWNER APPROVAL of the v2 drafts and four points: (1) apply v2; (2) identity refinement: a Function reuses the managed identity APP-ONLY, never as the BFF app registration / no OBO; SPE needs an explicit grant; (3) transitional path-A exception, per-instance dispatch until 103 merges, ≤2026-11-10; (4) scope growth: 3 more ArchTests + ~20 more docs. Then step 3 (apply).** Superseded instruction: invoke task-execute on `tasks/102-workload-placement-governance.poml`. Steps 0–2: draft ADR-052 and the amendments in `notes/drafts/`, run the Fable-tier adversarial review, and **show the owner BEFORE applying** (owner asked for plan-before-changes twice this session). Then 103, then finish 100. **Task 100's outstanding review fixes**: retry by throwing after the heartbeat · wording "ends after {date}" (access holds THROUGH the expiry date) · re-check after the lock · person = accessmode 0/1/2 · cancellation counts as cancelled, not failed · paging cap · correct the doc claim about pausing · new tests (DaysLeft in the key via a 16-day advance, write failure then re-run, cancellation, project/work-assignment roots, disabled owner → creator, support user excluded, paging 5001, ResultJson) · re-run perturbation P1 (its first build hit MSB3030). Worktrees `C:\wt100m`, `C:\wt100b`, `C:\wt100c`, `C:\wt100p` can be removed (results recorded: master 45.35 MB, e7bd02189 45.42 MB, both 214 files; suite 12,327/0/58; ArchTests 197/197). Previous text: **Task 100 — OWNER ANSWERED (2026-09-12): recipient = grantedby → record owninguser → record createdby (first ENABLED, NON-APPLICATION systemuser), else "unroutable" (heartbeat count + Error log per grant); channel = MDA bell via existing `NotificationService` (`appnotification`).** **CHECKPOINT 2026-09-12**: code + 23 tests COMMITTED `e7bd02189` (not pushed); targeted tests 23/23 green; master publish `e0a6f87c4` = 214 files / **45.35 MB** (C:\wt100m); IN FLIGHT: perturbations P1–P9 (script in scratchpad `perturb-100.ps1`, rewrites the job file then `git checkout`s it — if interrupted run `git checkout -- src/server/api/Sprk.Bff.Api/Services/ExternalAccess/GrantExpiryReminderJob.cs`), branch publish+CVE (C:\wt100b), full suite+ArchTests (C:\wt100c), code-review + adr-check agents (read-only, from the commit). Notes drafted: `notes/task-100-expiry-reminders.md` (§9 verification to fill). Remove worktrees C:\wt100m/b/c when done. Step 1 was: NEW `Services/ExternalAccess/GrantExpiryReminderJob.cs` (`IScheduledJob`, cron `0 6 * * *` UTC, ONE FetchXML query w/ outer joins — verified live 2026-09-12: 25 rows, aliases `gb.*`/`mt.*`/`mtc.*`/`ct.fullname` as read), `ExternalAccessModule.cs` (singleton + forwarded IScheduledJob + nested `GrantExpiryReminderBootstrapHostedService`). NEXT: build → tests `tests/integration/data-mutation/ExternalAccess/GrantExpiryReminderJobTests.cs` (strict FetchXML fake) → perturbations → full suite + ArchTests + publish size (fresh short-path worktrees) → notes `task-100-expiry-reminders.md` → Step 9.5. Previous text: get the owner's answer to the two escalation questions, then implement. Previous handoff text follows, for context: **Owner's call on the next task** (session 8's work is pushed). FR-33 chain: **100** (reminders — 🔴 **must be live by 2026-11-10**; deps 097 ✅, ready now) · **099** (Manage Access Expiration picker — deps 097 ✅ 098 ✅, but **needs task 065's M8 first**; see TASK-INDEX sequencing; consumes the 098 contract in `notes/task-098-record-share-expiry.md` §6) · **101** (expiry views, operator-applied). Also ready: **043** (chain C), **063** (chain B), and the C-items in § NEXT SESSION. ⚠️ **036 carries a MANUAL pre-merge canary gate.** Verify every POML's premises against the code first — see the header counter. |
