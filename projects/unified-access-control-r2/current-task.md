...
> **Last Updated**: **2026-09-18, session 15 — task 106, V1 FOUND AND FIXED** (commit `e17a4c1c9`). Step 9.5 ran BOTH gates and both independently found the same Critical defect **this task had introduced**: the survivor election was ranked by a per-request value, so it was not a total order two callers share — two concurrent grants could elect different survivors and deactivate each other, leaving **ZERO active rows while both received HTTP 200**. Pre-106 that was impossible, so it was strictly worse than the false 409 it replaced (that one was inert). Fixed by ranking on `ConferralRank(row, today)` and **removing the request parameter entirely**, so request-independence is structural. Verified: build **0/0** (`--no-incremental`) · the two classes **52/52** (expected 52 derived from source attributes independently of the run). **13 commits, ALL UNPUSHED.** ⚠️ **Remaining**: P1–P5 perturbations (**P5 is the V1 regression — none of P1–P4 could ever have caught V1, since all four are single-request**), ArchTests, full suite, publish size vs a FRESH master, then notes §14 + POML + TASK-INDEX. Full record: `notes/task-106-regrant-expired-survivor.md` §16.
...
| Field | Value |
|---|---|
| **Task** | **106 🔄 IN PROGRESS 2026-09-18 — code COMPLETE, verification unfinished** — ISS-008 / #973 / FR-09. Commits `3d1ce86e6` (original fix) · `b849cf6e7` (checkpoint + record) · `e17a4c1c9` (**V1 fix**). **What it does**: a re-grant judges "confers access" across every active row on the key, so the 409 `sdap.grant.expired_not_restored` fires only when NO active row confers — previously an expired lowest-id row produced that 409 while a live duplicate meant the grantee DID have access. The fix is the ELECTION, not the check: the survivor is the row conferring access longest, which makes three properties structural — an expired survivor proves every row is expired; the collapse cannot deactivate the conferring row (it IS the survivor); and 🚨 the naive fix (suppress the 409 and fall through) would have REVOKED that live access, turning an inert bug into a destructive one. **Plus V1**: the first version ranked by `EffectiveExpiry(request.ExpiryDate, …)` — request-DEPENDENT, so two racers could elect different survivors and mutually deactivate (zero rows, two 200s). Now `ConferralRank(rowExpiry, today) = rowExpiry ?? DefaultExpiry(today)`, and `ElectSurvivor` **takes no request value at all**. ⚠️ **NOT** `?? DateOnly.MaxValue` (the reviewer's own suggestion): that elects unbounded rows, FR-33 bounds them to +90, and a +200 sibling is collapsed — SHORTENING access. Also fixed from review: F6 stale comment 5 lines from my own edit, F2 my own FALSE "must not merge" claim, F4 one name for two values, F9 two tests missing the precondition my notes claimed all had, F3 a second in-memory expiry copy folded in, F11 an unenforced precondition. |
...
| **Next Action** | **Verification, in this order — nothing here is optional.** (1) **P1–P5** via `scratchpad/p106.js` (V2: multi-file perturbations; P1/P4 anchors were re-pointed after the fix moved the code, and all 8 anchors re-verified =1 after the format hook). **P5 restores V1 in full** (signature + both call sites) because request-independence is structural — no single-line edit can reintroduce it. (2) ArchTests — ⚠️ `ExternalAccessQueryIntegrityGuardTests` guards the grant filters and names `ExpiryPredicate`. (3) Full `Sprk.Bff.Api.Tests` (the earlier run was stopped as superseded; its testhost was confirmed gone — a survivor silently runs OLD DLLs). (4) **Publish size vs a FRESH `origin/master`** — `scratchpad/publish106.ps1`; pwsh 7.6.6 confirmed; short paths both sides; file-count parity asserted; repo-wide `worktree prune` deliberately removed (~12 other agent worktrees share this `.git`). (5) Fill notes §14 + add P5 to the §11 table. (6) POML `<status>completed</status>` + TASK-INDEX ✅ in ONE transaction, then `pwsh scripts/check-task-status-drift.ps1`. (7) Step 11 gate: next in wave 1 is **107** (ISS-009 / #974), needs the owner's go-ahead. 🔔 **Owner items**: **F5** (stale-snapshot election can shorten access — the ranking key is now a mutable column; NEW shape, pre-existing class) and **F10** (a collapse can lower the effective LEVEL, since the read path takes `Max(level)`) both want register entries rather than a paragraph in a task note. **W5**: ADR-038 ban B8 formally covers `InternalsVisibleTo` (verified verbatim) while the ADR itself files B8 as "blocked on a production refactor" → path A now, **path B needs the ADR owner**. Plus: ISS-019/020/021 registered with `{URL}` placeholders, **issues NOT filed**; ISS-020 needs a decision; 063's `user_not_internal` refusal — product intent?; ISS-003 / #964 blocks 054; owners for #988, #971, #993. **Push only when asked**; pushing ≠ merging. |
| **Task status** | **74 done · 27 open (106 in progress) · 3 escalated / blocked.** Status-drift last verified green 2026-09-17 (105 POMLs / 105 index rows); **must be re-run after 106's bookkeeping write**, against a baseline taken before it, or the green is inherited rather than measured. |
