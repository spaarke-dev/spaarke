...
> **Last Updated**: **2026-09-18, session 15 HANDOFF — task 106 ✅ COMPLETE** (by `context-handoff`; ready for `/compact`). ISS-008 / #973: a re-grant now judges "confers access" across the WHOLE key. 🔴 **The headline is not the fix — it is that Step 9.5 caught a privilege-loss regression THIS TASK INTRODUCED**, which I had asserted was safe in three separate comments. **16 commits, ALL UNPUSHED.** Verified: build **0/0** · affected classes **52/52** · perturbations **5/5 CAUGHT** (0 MISSED, 0 INVALID; spread 7/5/1/1/2) · ArchTests **323/323** · `Sprk.Bff.Api.Tests` **12,501/0/58** · `Spaarke.Core.Tests` **64/64** · `Spaarke.Scheduling.Tests` **75/75** · IntegrationTests **compiles** · publish **45.46 vs re-measured master 45.35 = +0.11 MB** (214 files each side, 14.54 MB headroom) · no CVEs. Gates: adr-check **1 violation / 9 warnings**, code-review **1 Critical / 6 Warning / 8 Suggestion** — the Critical fixed, six more fixed, two filed as ISS-022/023, the rest dispositioned. Drift **rc=0** (105/105). **NEXT = 107** (wave 1), which needs the owner's go-ahead per Step 11.
...
| Field | Value |
|---|---|
| **Task** | **106 ✅ COMPLETE 2026-09-18** — ISS-008 / #973 / FR-09. Commits `3d1ce86e6` · `b849cf6e7` · `e17a4c1c9` · `97481c917` · `3750bf269` · `daab55a72`. **What it does**: the 409 `sdap.grant.expired_not_restored` now fires only when NO active row on the key confers access — before, an expired lowest-id row produced it while a live duplicate meant the grantee DID have access. The fix is the **ELECTION, not the check**: the survivor is the row conferring longest (`ConferralRank`), so an expired survivor PROVES every row is expired and the collapse cannot deactivate the conferring row — it IS the survivor. 🚨 The naive fix (suppress the 409 and fall through) would have **REVOKED** that live access: an inert bug turned destructive, which is why the register said "NOT collapse first". 🔴 **V1 — the regression this task introduced**: the first version ranked by `EffectiveExpiry(request.ExpiryDate, …)`, a PER-REQUEST value, so the election was not a total order two callers share. Rows A(+10, lower id) and B(+200): a date-less request elects B; one naming +30 ties both ranks so the id tie-break elects A. Both reading before either writes ⇒ each deactivates the other's row ⇒ **ZERO active rows, two HTTP 200s naming deactivated rows**. Pre-106 that was impossible. Fixed by removing the request parameter entirely, so request-independence is **structural**. ⚠️ **NOT** `?? DateOnly.MaxValue` (the reviewer's own suggestion) — it elects unbounded rows, FR-33 bounds them to +90, a +200 sibling is collapsed, access SHORTENS; **P4 pins it**. Record: `notes/task-106-regrant-expired-survivor.md` (§16 = V1). |
| **Branch** | `work/unified-access-control-r2` · PR **#950** (not a draft) · **pushed through `7a6f1c43d`** (2026-09-17; `7e60089e2..7a6f1c43d`, 14 commits). CI: **Tier 1 fully GREEN at `7a6f1c43d`** — all 8 blocking jobs pass (Classify, Compile, Arch Tests full suite, Auth Smoke, Tenant Isolation, Eval Gate, Compose Fidelity, Changed-Surface Integration Smoke). Required `Router` context: **completed / success** on `7a6f1c43d`, verified via the check-runs API (the master ruleset confirms the required context is literally `Router`). The 5 `cancelled` runs on that SHA are the two advisory/legacy jobs still pending when the handoff commit was pushed seconds later. Head is now `597882f7c` (docs-only) with its own run in flight — docs-only pushes skip Tier 1 and `Router` still reports success on that shape, but **confirm `Router` on the head SHA before merging**. Two NON-blocking jobs were still running at handoff (Tier 2 Full Unit Tests, legacy Build & Test Debug) — Tier 2 is advisory by design, so a red there is not a merge blocker. **Trivy is red and NOT a required check**: 15 of its 16 alerts are pre-existing on master (→ #992, handed off) and 1 is TipTap 2.x from the session-8 LegalWorkspace build repair (→ #991, Compose owns it). Re-check with `gh pr checks 950`. **Before merge — three items, none of them optional**: (1) 🚨 **real-Dataverse DOWNGRADE check** for task 063 — seed Full Access on a scratch record, `share-user` at View Only, read `accessrightsmask` and assert **1**. Everything offline pins our end of the wire only; if `ModifyAccess` were additive, the read-back would refuse loudly while the user silently KEPT Write and Delete (notes `task-063-…` §9.4). **Not run — no live-Dataverse access in session 13.** (2) real-Dataverse smoke of the reminder `appnotification` write (task-100 notes §7). (3) a real-ARM check of H9's slot guard (#987). Trivy remains red and is **not** a required check: 15 of 16 alerts are pre-existing on master (→ **#992**, handed off), 1 is TipTap 2.x from the session-8 LegalWorkspace build repair (→ **#991**, Compose owns it). Portfolio sync skipped — the `gh` token lacks `read:project` (`gh auth refresh -s read:project,project`). |
| **Next Action** | **Step 11 transition gate — next in wave 1 is `107`**, and task-execute requires the owner's go-ahead before starting it. 107 = ISS-009 / #974, FR-33: grants written OUTSIDE the BFF get the default expiry — Dataverse-side, every write path; deps **097** ✅. Sequence: 104 ✅ → 063 ✅ → 043 ✅ → 106 ✅ → **107 → 093 → 082 → 094 → 095**. ⚠️ **Verify 107's POML premises against the code FIRST** — this project's task files have been wrong **19** times, though 106's was RIGHT on all five checked points, so verify rather than assume in either direction. 🔔 **Owner items open**: (1) **FIVE** register entries carry `{URL}` placeholders and the **GitHub issues are NOT filed** — ISS-019/020/021 (043) plus **ISS-022** (a stale snapshot can SHORTEN access: 106 moved the election key from an immutable id to a MUTABLE column, and V1's fix does NOT cover it) and **ISS-023** (a collapse can lower the effective LEVEL — pre-existing per task 010, and 106 provably does not change level outcomes; needs a **product** answer: does "grant X" mean *set to* X or *raise to at least* X?). `push-to-github` Step 1.6 audits for exactly this. (2) **W5** — ADR-038 ban B8 formally covers `InternalsVisibleTo` (verified verbatim) while the ADR itself files B8 as "blocked on a production refactor"; path A now, **path B needs the ADR owner**. (3) **ISS-020** needs a decision on its merits. (4) 063's external-**licensed**-user refusal (`user_not_internal`) — product intent? (5) ISS-003 / #964 blocks **054**; owners needed for **#988**, **#971**, **#993**. **Push**: 16 commits unpushed — **push only when asked**, and pushing ≠ merging (`/merge-to-master` is separate). Portfolio sync still blocked: token scopes are `gist, read:org, repo, workflow` → `gh auth refresh -s read:project,project`. Also noted, deliberately untouched: `projects/INDEX.md`'s row for this project still narrates "session 7, 66 of 98 tasks" — its hot-path declaration (BFF=Y) is correct, which is what Step 0.5 requires, so the stale prose is cosmetic on a file shared by ~12 worktrees. |
| **Task status** | **75 done · 26 open (next = 107) · 3 escalated / blocked.** Status-drift **rc=0** 2026-09-18 (105 POMLs / 105 index rows), re-run AFTER 106's bookkeeping write — so the green is measured, not inherited. |
| **⚠️ ENVIRONMENT** | **The connected system is DEV and its records are TEST records** (owner, 2026-09-17). Read every live-data risk in this file through that lens: task 042's backfill touched **two test contacts**; ISS-020's `sprk_enddate` fail-open currently has **zero** real exposure (no organisation holds a standing grant, and the org rows are test data); and the three **before-merge real-Dataverse checks** (063's DOWNGRADE check, task 100's reminder `appnotification` write, H9's ARM slot guard) are verification gaps rather than production incidents. ⚠️ This does NOT retire them — each becomes materially riskier the moment a real customer environment is provisioned, and 063's DOWNGRADE check in particular is the ONLY evidence that `ModifyAccess` replaces rather than adds, which the whole share write-path assumes. It changes urgency, not validity. |
| **Session 15 record** | **Closed 106** (`3d1ce86e6`, `e17a4c1c9`, + bookkeeping). **The lesson is the task**: build, 51/51 tests and 4/4 perturbations all passed over a real privilege-loss defect, because **every one of those instruments was single-request and the defect lived between two**. Both Step 9.5 gates found it independently, by READING. A test suite can be green, perturbation-proven, and still blind to an entire dimension. **Other lessons**: (a) one of my own tests ENCODED the defect as an invariant — it asserted the request-dependent tie that *was* the bug, so it could only ever go green on broken behaviour; (b) I wrote three comments asserting convergence and a fourth asserting a non-hazard (F2) — a comment that guards nothing teaches the next reader to discount its true neighbours; (c) a stale comment sat **five lines** from my own edit (F6) while these notes congratulated themselves on repairing the same drift elsewhere; (d) my notes claimed all seven duplicate tests asserted a precondition when two did not (F9) — accurate about the design, false about the code, which is the worse failure because a spot-check confirms it; (e) **seven perturbation non-results** came from the harness racing itself — `CS2012` (a lingering `testhost` owning the output DLL) and `CS2001` (no settle window between writing a file and compiling it) are DIFFERENT faults and conflating them wasted a cycle; labelling them INVALID rather than MISSED is the only reason the record does not now say my V1 regression test is decorative; (f) my first conflict-check used `git diff HEAD origin/master`, which is BIDIRECTIONAL and returned my own 457 files as "master touched our file" — the right question is `HEAD..origin/master`. Every one of (e) and (f) is the same root error as session 14's three: **an observation taken outside the thing being observed**. |
