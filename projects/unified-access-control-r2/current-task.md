...
> **Last Updated**: **2026-09-18, session 15 — task 106 IN PROGRESS** (mid-task checkpoint by `context-handoff`, Step 8.5: 8 files modified). ISS-008 / #973: the grant upsert now judges "confers access" across the WHOLE key. Code is COMMITTED (`3d1ce86e6`) and verified; bookkeeping and the publish measurement are not yet done. **10 commits, ALL UNPUSHED.** Verified so far: build **0/0** · GrantLifecycle+GrantExpiry **51/51** (expected count 51 derived from source attributes INDEPENDENTLY of the run, then matched — a filter selecting nothing reports identically to one selecting everything) · perturbations **4/4 CAUGHT** (0 MISSED, 0 INVALID) · no vulnerable packages. **In flight**: ArchTests + full `Sprk.Bff.Api.Tests`, and the Step 9.5 gates (code-review + adr-check, both mandatory here — FULL rigor AND test-modifying). **Not yet run**: publish size vs a FRESH master. Task 043 ✅ closed in session 14 (its record is in `notes/task-043-org-expansion-term.md`, POML and TASK-INDEX).
...
| Field | Value |
|---|---|
| **Task** | **106 🔄 IN PROGRESS 2026-09-18** — ISS-008 / #973 / FR-09: a re-grant over an EXPIRED survivor judges "confers access" across every active row on the key. Commit `3d1ce86e6`. **The defect**: `CreateGrantAsync` elected the survivor as the **lowest id** and task 023's "confers no access" check judged only that row — but the read path unions EVERY active unexpired row, so an expired lowest-id row returned 409 `sdap.grant.expired_not_restored` while a live duplicate meant the grantee DID have access. Duplicates are real: one dev contact holds **five** active rows on one matter. 🚨 **The naive fix is WORSE than the bug**: suppressing the 409 lets control reach `CollapseDuplicatesAsync`, which deactivates every row but the expired survivor — **revoking the live access it just detected**. The bug's current form is inert; that fix is destructive (register: "NOT collapse first"). **The fix is the ELECTION, not the check**: `ElectSurvivor` ranks by the expiry a row would CARRY after this request (`requested ?? row ?? DefaultExpiry`), ties broken by ascending id. Then an expired survivor PROVES every row is expired, the collapse cannot drop the conferring row (it IS the survivor), and an explicit expiry ties every row so the election is byte-for-byte pre-106. **Rejected**: ranking the RAW column — `null` sorts as "never expires", FR-33 bounds that winner to +90 and collapses a sibling dated +200, so access leaves SHORTER than it arrived. Escalation trigger did **not** fire (200 + the surviving row id is already in the contract). Record: `notes/task-106-regrant-expired-survivor.md`. |
...
| **Next Action** | **Finish task 106's verification + bookkeeping.** In flight when this was written: (a) ArchTests + full `Sprk.Bff.Api.Tests` — ⚠️ `ExternalAccessQueryIntegrityGuardTests` guards the grant filters and names `ExpiryPredicate`, so it is exactly what the `ExternalParticipationService` edit could trip; (b) **Step 9.5 gates** code-review + adr-check (MANDATORY — FULL rigor AND test-modifying; the usual skip does not apply). Then, in order: (1) **publish size vs a FRESH `origin/master`** — CLAUDE.md §10 hazards 1–4: re-measure master (never the recorded number), `Compress-Archive`, fresh worktrees at SHORT paths on **both** sides (`C:\wt106m` / `C:\wt106b`), and **assert equal file counts** or the delta is meaningless; (2) fill `notes/task-106-…md` §14/§15, which are deliberately marked _pending_ rather than pre-filled; (3) POML `<status>completed</status>` + TASK-INDEX ✅ **in one transaction**, then `pwsh scripts/check-task-status-drift.ps1` — both artifacts drift, in both directions; (4) Step 11 gate: next in wave 1 is **107** (ISS-009 / #974, grants written outside the BFF get the default expiry), which needs the owner's go-ahead. 🔔 **Owner items still open**: ISS-019/020/021 registered with `{URL}` placeholders — **GitHub issues NOT filed**; ISS-020 (`sprk_enddate` fail-open) needs a decision on its merits; 063's external-**licensed**-user refusal (`user_not_internal`) — product intent?; ISS-003 / #964 blocks 054; owners needed for #988, #971, #993. **Push**: 10 commits unpushed — **push only when asked**, and pushing ≠ merging (`/merge-to-master` is separate). Portfolio sync still blocked: token scopes are `gist, read:org, repo, workflow` → `gh auth refresh -s read:project,project`. |
