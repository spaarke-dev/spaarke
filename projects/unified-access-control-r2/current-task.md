...
> **Last Updated**: **2026-09-17, session 14 HANDOFF** (by `context-handoff`) — task **043 is CODE-COMPLETE and VERIFIED but deliberately NOT marked complete**: Step 9.5 code-review returned a **Critical that is an OWNER decision**, and task-execute puts Step 9.5 before Step 10. Four commits, **all unpushed**. Everything else is done: build 0/0 · affected classes **109/109** · namespaces 328/328 · seam+auth 197/197 · ArchTests **323/323** · perturbations **11/11 CAUGHT** · publish **45.45 vs re-measured master 45.35 = +0.10 MB** (214 files each side) · no CVEs. Gates: adr-check **0 violations / 11 warnings**; code-review **1 Critical / 2 High / 7 Medium / 6 Low**.
...
> ⚠️ **One live block. Replace it; never append.** Row headings (`| **Next Action** | **1. Get the C-1 answer — A / B / C** (options + recommendation in `notes/task-043-org-expansion-term.md` §14.1). **A (recommended)** = revert the one line in `ComposeForSystemUserAsync` to `accessConferringOnly: false` until 036 lands (A-8 stays open, as it has since the system was built), and flip the `owner`-absent assertion in `ResolveAsync_AccessConferringOnly_DoesNotShareACacheEntryWithTheUnfilteredCall`. **B** = accept the window — internal staff lose owner/team/BU access on every BFF read path until 036. **C** = extend the registry to express SystemUser/Team/BusinessUnit conferring columns (ADR-034 A1 amendment + a change to `FilterToAccessConferringRoles`); arguably the right end state, filed as follow-up under A. **2. THEN** run `node <scratchpad>/ct043b.js "11/11 CAUGHT" "45.45 vs master 45.35 = +0.10 MB, 214 files each side"` — it writes the POML `<status>` **and** the TASK-INDEX row in ONE transaction, deliberately: a 2026-09-03 audit of this project found 17 disagreements across 92 tasks. **3.** `pwsh scripts/check-task-status-drift.ps1` — the baseline was green (105/105) BEFORE any bookkeeping edit, so a red result is this task's. **4.** Step 11 transition gate; next in wave 1 is **106**. **Also awaiting a decision**: ISS-019/020/021 sit in `notes/defer-issues.md` with `{URL}` placeholders — **the GitHub issues are NOT filed** (three outward-facing writes; `push-to-github` Step 1.6 audits for exactly this). **Push only when asked.** |
...
| Field | Value |
|---|---|
| **Task** | **043 BLOCKED on one owner decision** — org-expansion membership term + registry filter on the systemuser membership path (FR-24/25/22). Commits `28f833a0e` (impl+tests), `e8f57dec9` (notes), `47a63314c` (P5 matcher fix), `62e5d5d0c` (Step 9.5 review fixes). **Delivered**: a contact derives access to records referencing — via a REGISTRY-LISTED org-typed lookup — an organisation it actively belongs to, at **that organisation's** standing-grant baseline; one walk per DISTINCT baseline (at most 3); Secure-suppressed; a provenance flag added. 🚨 **The prerequisite the brief did not name**: `MembershipResolverService.HashOptions` omitted `AccessConferringOnly`, so flipping it on would have made the AUTHORIZATION and SCOPING callers share one 5-minute cache entry — a scoping call landing first hands the gate the **unfiltered** descriptor set (register A-8 restored through Redis). Fixed, and pinned by two tests that run the calls in the dangerous order. 🔴 **THE BLOCKER (code-review C-1; verified in source, escalated NOT decided)**: `accessConferringOnly: true` **also deletes owner/team/business-unit membership** from the systemuser plane. The conferring registry can only express Contact/Organization columns — `MembershipResolverService.cs:592-596` drops anything else as *malformed* — so `ownerid` is **inexpressible**, not merely unregistered (`:782-784` is the lost binding). The compensating term is design §4.5 term 1 = task **036**, open behind a blocked **034**, and `IImpersonatedRootSetSource` is **not injected** into the evaluator. An internal Type-1 user who owns matters but is named in no `sprk_assigned*` column now composes to nothing from membership. Fail-CLOSED (availability, not disclosure) and confined to **one line**. ⚠️ **This task's own test asserted the regression as correct** (it seeds `ownerid` and expects roles == `{assignedAttorney}`) — that is how it got past review-by-author. |
...
| **Task status** | **73 done · 28 open (043 code-complete but BLOCKED on C-1, NOT marked ✅) · 3 escalated / blocked.** Status-drift green 2026-09-17 (105 POMLs / 105 index rows) — taken deliberately BEFORE any bookkeeping edit, so it is a usable reference point rather than a formality. |
...
| _(this commit)_ | session-14 handoff + registers ISS-019/020/021 |
