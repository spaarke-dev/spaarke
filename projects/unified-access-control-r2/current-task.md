...
> **Last Updated**: **2026-09-13, session 10 END** (by `context-handoff`) — task **102 ✅ COMPLETE** (ADR-052 Accepted; ArchTests 288/288; issues #976–#986). Next = task **103**, then finish **100**.
...
| Field | Value |
|---|---|
| **Task** | **103 — Scheduler lease + slot guard + `AddScheduledJob<TJob>` helper** · FULL · opus@high · `tasks/103-scheduled-jobs-exactly-once.poml` · **IN PROGRESS (session 11, 2026-09-14)** — **COMMITTED `a9c538c72`** (not pushed; on top of `0cf6f0c92` POML-drift cherry-pick). Running at checkpoint: perturbations P1–P8 (`/tmp/p103.sh`, each restores its file), branch publish + CVE in `C:\wt103b` (master `C:\wt103m` = **45.35 MB / 214 files** @ `e0a6f87c4`), code-review + adr-check agents (read the commit via `git show`). Notes drafted: `notes/task-103-scheduled-jobs-exactly-once.md` (§6/§7 PENDING). **After them**: fill notes §6–7, apply review fixes, full suite, POML 103 + TASK-INDEX ✅, drift check, remove temp worktrees `C:\wt103m`/`C:\wt103b` + the POML agent's worktree `C:\code_files\spaarke\.claude\worktrees\agent-a98fde6d556b49fed` (branches `uac-r2-adr052-poml-drift`, `worktree-agent-a98fde6d556b49fed` — content already cherry-picked). Then task 100 (fix 7 DONE by 103; fix 8 gains the Q2 catch-up). Earlier detail — code + tests written: Spaarke.Scheduling (`IScheduledJobLease`, `ProcessLocalScheduledJobLease`, `ScheduledJobLeaseHold`, `ScheduledJobRegistration` + `AddScheduledJob`, host rewrite, registry/store DI ctors, `JobRunResult.Skipped`, options `RunScheduledJobs`/`LeaseDuration`); BFF (`Infrastructure/Scheduling/RedisScheduledJobLease.cs`, `SchedulingModule` rewrite, Membership + ExternalAccess on the helper, both bootstraps deleted, trigger 409/503); `scripts/Deploy-BffApi.ps1` sets `Scheduling__RunScheduledJobs=false` slot-sticky; tests (`ScheduledJobLeaseTests` 10, `AddScheduledJobTests` 3, `RedisScheduledJobLeaseTests` 8, ArchTest rule 2b + controls, two back-to-back-trigger tests now wait). Scheduling.Tests **69/69**. POML-drift commit cherry-picked = `0cf6f0c92`. Impersonation proposal written → `notes/decisions/function-impersonation-proposal.md` (AWAITING SIGN-OFF; found SAS still enabled on Service Bus). **Remaining**: ArchTests + BFF tests green; 409 contract test; docs (ADR-036 concise+full A1.1 decision, `scheduled-jobs.md`, `jobs.md`, CHANGELOG); perturbations; full suite; publish size; notes/task-103; Step 9.5. **Owner decisions 2026-09-14**: (Q1) lease store configured but DOWN → retry the acquire briefly (JobRetryPolicy backoff, ~15 s), then do NOT dispatch; record the tick **FAILED** ("lease store unavailable") + LogError (not skipped — every instance loses it together, so it is a real miss). (Q2, task 100) the expiry reminder **catches up** a missed threshold (send the most urgent unsent threshold whose day has passed, once per threshold) — fold into task 100 fix 8. (Q3) fix the 8 stale POMLs + widen the drift guard to not-started POMLs — delegated to a background agent in an isolated worktree; cherry-pick its commit. (Q4) Function impersonation: owner asked for "the correct approach that allows the functions to work; the best technical approach" → researcher agent running; bring a proposed ADR-052 §6 rewording back for sign-off (auth ADR → explicit sign-off, §6.5). **Design**: `IScheduledJobLease` in Spaarke.Scheduling (`ProcessLocalScheduledJobLease` default = not distributed → dispatch + warn once; `RedisScheduledJobLease` in BFF `Infrastructure/Scheduling`, SET NX PX via `LockTakeAsync`, renew at TTL/3, compare-and-delete release); lease renewed for the whole run incl. retries; lost to another holder → cancel run; `ScheduledJobHostOptions.RunScheduledJobs` (slot guard, `Scheduling__RunScheduledJobs` sticky=false on staging); `AddScheduledJob<TJob>(cron, enabled)` adds a `ScheduledJobRegistration` that the registry + in-memory store consume in their DI constructors → NO bootstrap hosted services, no `Insert(0)`. Then **100** (finish; paused at `e7bd02189`, needs 103; must be live by **2026-11-10**). **102 ✅ COMPLETE 2026-09-13** — full record `notes/task-102-workload-placement-governance.md`. |
...
| **Next Action** | **task-execute 103.** FIRST its open escalation: ask the owner what the scheduler does when a lease store IS configured but UNAVAILABLE (skip the tick / dispatch anyway / fail) — ADR-036 A1 rule 1 deliberately leaves it to 103. Then implement ADR-036 A1 rules 1 (distributed lease that outlives every retry; manual trigger and scheduled tick mutually exclusive; no store configured → dispatch + warn once; a host without the lease records **skipped**), 2 (non-production slot guard, slot-sticky setting) and 6 (`services.AddScheduledJob<TJob>(cron, enabled)`); migrate `PlaybookSchedulerJob`, `MembershipReconciliationJob`, `GrantExpiryReminderJob` onto it and delete the per-job bootstraps (incl. the nested one in `ExternalAccessModule`). Keep `WorkloadPlacementGuardTests` (host-neutrality, timer ratchet) green. Then **task 100**: the 10 review fixes in § Task 100 below. **Owner decisions carried from 102 (non-blocking — raise at session start)**: (1) 8 not-started POMLs in ACTIVE projects (`ai-spaarke-action-engine-r1` 001/014/022/030/040/042/070; `sdap-SPE-admin-app-r2` 050) still carry the old flat ban — recommend fixing them + widening the drift guard to not-started POMLs; (2) confirm the Function "no Dataverse impersonation" MUST NOT (ADR-052 §6); (3) Insights Engine D-20. **Push only when asked.** |
...
### Commits session 10 (2026-09-13) — NOT PUSHED
