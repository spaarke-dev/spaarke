...
> **Last Updated**: **2026-09-18, session 15 HANDOFF — task 106 ✅ COMPLETE, EVERYTHING PUSHED** (by `context-handoff`; ready for `/compact`). Head **`3b409bfc1`**, tree clean, **0 unpushed**. 🔴 **The headline is not the fix — Step 9.5 caught a privilege-loss regression THIS TASK INTRODUCED**, which I had asserted was safe in three separate comments. Verified: build **0/0** · affected classes **52/52** · perturbations **5/5 CAUGHT** (0 MISSED, 0 INVALID) · ArchTests **323/323** · `Sprk.Bff.Api.Tests` **12,501/0/58** · `Spaarke.Core.Tests` **64/64** · `Spaarke.Scheduling.Tests` **75/75** · IntegrationTests **compiles** · publish **45.46 vs re-measured master 45.35 = +0.11 MB** (214 files each side) · no CVEs · drift **rc=0**. ⚠️ **CI NOT YET GREEN — do not assume**: on `3b409bfc1` two checks were pending and **`Router` (the ONLY required context) had not reported at all**; no failures. The last push was docs-only, and docs-only shapes skip Tier 1 while `Router` still succeeds — but **confirm `Router` on the head SHA before merging**, and count pending explicitly (`gh pr checks 950 | grep -c pending` must be 0). Tier 2 is advisory; Trivy is red and NOT required (→ #992, #991).
...
| **Next Action** | **Step 11 transition gate — and the audit changed what is safe to start.** ⚠️ **Do NOT start 107 or 093 as written** (`notes/task-accuracy-audit-2026-09-18.md`): **107** needs an 🔔 **owner ADR-002 plugin exception** first — a business rule provably cannot work (client-form-only, no date arithmetic), so a pre-op plugin is the only in-transaction mechanism, ADR-002 requires explicit exception approval, and **none has ever been granted in this repo**; its reference impl `BaseProxyPlugin.cs` is `[Obsolete]` *for violating ADR-002*; and its test plan cannot build (net462, and adding a net4x project to `Spaarke.sln` is a deliberate Tier-1 CI failure). **093** has **four false premises** — chiefly "upload → create → link", which task **076 inverted**, so an implementer obeying it would **redo shipped work**. ✅ **Genuinely clean runnable set: 108, 065, 044, 094, 095** (recommend **108** first — small, deps met, ISS-018/#995). ⚠️ **044 and 065 are needlessly deferred** behind owner-blocked 036; pulling 065 forward unblocks 066→067→099 **and** 069 (099's escalation already fired waiting on it). 🔔 **Owner items**: (1) **ISS-020 / #999 is UNDECIDED** — should a date-ended but state-active membership stop conferring? The same query feeds an additive term (over-inclusion = over-GRANT) and the deny veto (over-inclusion = a stricter wall), so one shape cannot be right for both; recommendation on file is to bound the additive callers only. (2) The **ADR-002 exception** for 107. (3) **Before merging #950**: 063's real-Dataverse DOWNGRADE check (the ONLY evidence `ModifyAccess` replaces rather than adds), task 100's reminder `appnotification` write, and a real-ARM check of H9's slot guard. (4) ISS-003 / #964 blocks **054**; owners for #988, #971, #993. **`/merge-to-master` is a separate, unmade decision** — pushing ≠ merging. Portfolio sync still blocked: `gh auth refresh -s read:project,project`. |
| **Task status** | **75 done · 32 open · 3 escalated / blocked.** Open grew 26 → 32: tasks **109–114** queued 2026-09-18 per the owner's "tracked AND queued, not deferred" directive. ⚠️ Those six are **STUBS** — each says so at the top, `<status>pending</status>` + a **PREMISES NOT YET VERIFIED** marker. Author them with `/task-create` so each gets its own premise check; six task files written from unverified premises is exactly what the audit found eight times over. Drift **rc=0** (111 POMLs / 111 index rows), re-run after every bookkeeping write. ⚠️ **ISS-025 / #1004**: that script pairs from the POML side, so an index row with **no POML is never examined** — I created that condition by accident and it still printed "No drift". Do not read its rc=0 as proof of orphan-freedom; count both sides yourself. |
