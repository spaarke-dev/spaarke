...
| Field | Value |
|---|---|
| **Task** | **104 — `DataverseImpersonation` fails closed in the shared helper (ISS-013 / #990)** · FULL · opus@high · `tasks/104-iss013-impersonation-helper-fail-closed.poml` · **IN PROGRESS (started 2026-09-15)**. FULL rigor. Now at Step 0: inventorying the callers and running `/conflict-check` on the hot path (`Spaarke.Dataverse`). Finding so far: `CreateAuthenticatedRequestAsync` (`DataverseWebApiService.cs:173`) applies the helper to every request with an optional id, so the PATCH write path can go app-only on `Guid.Empty`. **Checkpoint (Step 8, implementation written, not yet built):**
- **Escalation trigger 1: does NOT fire.** The only impersonating writers, the Communication apply services, reject `Guid.Empty` (`CALLER_NOT_RESOLVED`) before passing an id.
- **Trigger 2: does NOT fire.** The tenant check gets its data via parameters. The environment check is not knowable in the helper; the reason is documented in its XML docs.
- **Conflict-check:** soft pass.
- **Researcher (MS Learn):** `CallerObjectId` + Entra oid is *preferred*; `MSCRMCallerID` + systemuserid is *legacy*. Behaviour with both headers is undocumented, as is behaviour for an unknown, disabled or foreign-tenant user.

Files changed:
- `DataverseImpersonation.cs`: `ApplyAsSystemUser` / `ApplyAsEntraUser` replace `Apply(Guid?)`.
- `DataverseWebApiService.cs`:
  - the helper runs before the token is acquired;
  - `Guid.Empty` is refused;
  - optional `TokenCredential` ctor param, mirroring `DataverseWebApiClient`. Safe: registered via the GraphModule lambda.
  - `UpdateRecordFieldsAsync` has an early guard.
- `IFieldMappingDataverseService.cs`: doc.
- Tests:
  - new `tests/integration/auth/UnifiedAccessControl/DataverseImpersonationHelperTests.cs`, replacing `tests/unit/.../DataverseImpersonationTests.cs` (removed via `git rm`);
  - new `DataverseWebApiServiceImpersonationTests.cs`;
  - canary Test 4 (helper path) and the canary remark;
  - ArchTest sample string.

Next: build; run the auth + canary + ArchTests; then perturbations; full suite; publish size.

**Side finding to verify and register:** `DataverseWebApiClient` is registered `AddSingleton<DataverseWebApiClient>()` (`SpeAdminModule.cs:71`) with an optional `TokenCredential` ctor param. Container activation may therefore inject the singleton `TokenCredential` (`Program.cs:48`), bypassing its credential selection. It is first in the dependency-ordered sequence (owner 2026-09-15: *"sequence the tasks so that they have full knowledge of dependent task builds"*; see TASK-INDEX § Execution sequence). **099 ⏸**: its escalation fired 2026-09-15 before any code was written, because M8 has not landed (`AccessGrantModal.tsx:334` has no `res.ok` check). The owner chose to wait for 065, so 099 now runs after 065 **and** 066. **101** now runs after 107. **100 ✅ COMPLETE 2026-09-15** (`e7bd02189` + `521ab1b9a` + `46656864a`) — full record `notes/task-100-expiry-reminders.md`. **103 ✅ 2026-09-14.** |
