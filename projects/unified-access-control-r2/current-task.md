...
> **Last Updated**: **2026-09-17, session 14 HANDOFF — task 043 ✅ COMPLETE** (by `context-handoff`; ready for `/compact`). Org-expansion term + registry filter + **platform-ownership conferral (C-1, owner-decided)**. **9 commits, ALL UNPUSHED.** Verified: build 0/0 · edited classes **111/111** · namespaces **331/331** · seam+auth **197/197** · ArchTests **323/323** · perturbations **13/13 CAUGHT** · publish **45.45 vs re-measured master 45.35 = +0.10 MB** (214 files each side) · no CVEs · drift green (105/105). Gates: adr-check **0 violations / 11 warnings**; code-review **1 Critical / 2 High / 7 Medium / 6 Low** — C-1 implemented, H-2 corrected, 6 fixed, rest accepted with reasons. ADR-034 **A1.1** amended (§6.5 path B). **NEXT = 106** (wave 1), which needs the owner's go-ahead per Step 11.
...
| Field | Value |
|---|---|
| **Task** | **043 ✅ COMPLETE 2026-09-17** — org-expansion membership term (design §4.5 term 4, FR-24/25/22) + FR-24's registry filter on the systemuser membership path + **platform-ownership conferral**. Commits `28f833a0e` · `e8f57dec9` · `47a63314c` · `62e5d5d0c` · `69e9e39a2` · `ad41dcc84` · `d51370559` · `1a5b28f9d`. **What it does**: a contact derives access to records referencing — via a REGISTRY-LISTED org-typed lookup — an organisation it actively belongs to, at **that organisation's** standing-grant baseline; one walk per DISTINCT baseline (≤3, so a View Only firm's record can never inherit a Full Access firm's level); Secure-suppressed; own provenance flag. **Plus C-1**: `ownerid`/`owningteam`/`owningbusinessunit` confer **structurally** ahead of the registry, gated to the systemuser plane — FR-24 guards *maker-authored* lookups from conferring by naming accident, but these are platform-maintained and unrenameable, and being the owner IS Dataverse access; requiring an entry had made them **inexpressible** (the validator rejects any type outside `{Contact, Organization}`). **`owningteam` is the load-bearing column** — discovery binds the first target matching `IncludedIdentityTables`, which starts at `systemuser`, so a polymorphic Owner column always resolves to SystemUser; on a team-owned record `ownerid` holds the TEAM's id and never matches, so keying on it alone would have compiled, passed and conferred nothing. 🚨 **The finding that gated the task**: `HashOptions` omitted `AccessConferringOnly`, so flipping it on would have made the AUTHORIZATION and SCOPING callers share one 5-minute cache entry — a scoping call landing first hands the gate the **unfiltered** descriptor set (A-8 restored through Redis). Fixed; pinned by two tests that run the calls in the dangerous order. Record: `notes/task-043-org-expansion-term.md` (§15 = C-1). |
...
| **Next Action** | **Step 11 transition gate — next in wave 1 is `106`**, and task-execute requires the owner's go-ahead before starting it. Sequence: 104 ✅ → 063 ✅ → 043 ✅ → **106 → 107 → 093 → 082 → 094 → 095**. Verify 106's POML premises against the code FIRST — this project's task files have been wrong **19** times (043 found two: a renamed reader, and a "flag-off branch of 036" that does not exist). 🔔 **Owner items open**: (1) **ISS-019/020/021** are registered in `notes/defer-issues.md` with `{URL}` placeholders — **the GitHub issues are NOT filed** (three outward-facing writes; `push-to-github` Step 1.6 audits for exactly this); (2) **ISS-020** needs a decision on its merits — the `sprk_enddate` fail-open, whose only safeguard today is that no organisation holds a standing grant; (3) still open from 063 — sharing with an external **licensed** system user is refused (`user_not_internal`): product intent? (4) ISS-003 / #964 blocks **054**; (5) owners needed for **#988**, **#971**, **#993**. **Push**: 9 commits unpushed — **push only when asked**, and note pushing ≠ merging (`/merge-to-master` is separate). Portfolio sync blocked: token scopes are `gist, read:org, repo, workflow` → `gh auth refresh -s read:project,project`. |
| **Task status** | **74 done · 27 open (next = 106) · 3 escalated / blocked.** Status-drift green 2026-09-17 (105 POMLs / 105 index rows), re-run AFTER the 043 bookkeeping write against a baseline taken before it — so the green is meaningful, not inherited. |
