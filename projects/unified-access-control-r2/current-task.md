...
> **Last Updated**: **2026-09-17, session 14** — **C-1 ANSWERED AND IMPLEMENTED**. The owner chose the CORRECT option over the expedient one, and supplied the deciding fact: records are owned primarily at **team / business-unit** level. Platform ownership (`ownerid` / `owningteam` / `owningbusinessunit`) now confers **structurally**, gated to the systemuser plane; ADR-034 **A1.1** amended (§6.5 path B). Six commits, **all unpushed**. Verified: build 0/0 · edited classes **111/111** · namespaces **331/331** · seam+auth **197/197** · ArchTests **323/323** — every filter proven non-empty before being trusted. **Remaining**: 2 perturbations for the C-1 fix, a publish re-measure at the final commit, then `ct043b.js` → drift check → Step 11.
...
| Field | Value |
|---|---|
| **Task** | **043 — C-1 resolved; finishing verification.** Org-expansion term + registry filter + platform ownership. Commits `28f833a0e`, `e8f57dec9`, `47a63314c`, `62e5d5d0c`, `69e9e39a2`, **`ad41dcc84`** (C-1). **C-1 outcome**: `ownerid`/`owningteam`/`owningbusinessunit` are admitted ahead of the registry WITHOUT an entry — FR-24 exists so a *maker-authored* lookup cannot confer by naming accident, but these are platform-maintained and fixed by the Dataverse data model, so there is nothing for a review to protect; requiring an entry made them **inexpressible** (the validator rejects any type outside `{Contact, Organization}` as malformed). 🔴 **This regression already happened in production** — R7 W12 task 130: *"sprk_matter resolved rows=0 for a user who owns 44 matters via ownerid, verified via raw SQL"*. ⚠️ **`owningteam` is load-bearing, `ownerid` alone would have been a FALSE fix**: discovery binds the first target matching `IncludedIdentityTables`, which starts at `systemuser`, so a polymorphic Owner column always resolves to SystemUser and binds the caller's own id — on a team-owned record `ownerid` holds the TEAM's id and never matches. **NARROW by construction**: keyed on the three platform NAMES, not the identity type, so a maker-authored `sprk_reviewer → systemuser` still needs a reviewed entry (else A-8 returns through another door). ⚠️ **I over-widened it first and a PRE-EXISTING test caught me** — applied unconditionally it reached the contact plane, where `ResolveByContactAsync_AllowlistedAssignedContactRole_ReturnsMatchingRecords` failed with `ByRole {["owner"] = {empty}, …}` against its own NFR-05 reason; fixed by threading `includePlatformOwnership` per call site (contact plane stays byte-identical). **Its expectations were not touched.** Notes §15. |
...
| **Next Action** | **1.** Two perturbations for the C-1 fix — (a) strip `PlatformOwnershipColumns` → the ownership tests must FAIL; (b) strip only the plane gate (`includePlatformOwnership` always true) → the pre-existing contact-plane test must FAIL. (b) matters more: it is the mistake actually made, so it is the one worth pinning. Harness `p043.js` requires a CLEAN TREE. **2.** Re-measure publish at the final commit — the +0.10 MB figure was taken at `47a63314c`, five commits back; use `publish043b.ps1` (ASCII-only, `pwsh`, exit code captured before any pipe). **3.** `node <scratchpad>/ct043b.js "<perturbation verdict>" "<publish verdict>"` — POML `<status>` + TASK-INDEX row in ONE transaction. **4.** `pwsh scripts/check-task-status-drift.ps1` (baseline was green 105/105). **5.** Step 11 gate; next in wave 1 is **106**. **Open, needs a decision**: ISS-019/020/021 carry `{URL}` placeholders — GitHub issues NOT filed. Portfolio sync blocked: token scopes are `gist, read:org, repo, workflow` (needs `gh auth refresh -s read:project,project`). **Push only when asked.** |
