...
| Field | Value |
|---|---|
| **Task** | **097 — every external grant carries an expiry (IN PROGRESS, session 7, 2026-09-10).** FULL rigor. Owner decision "Server fills +90" applied to the POML before start: absent `ExpiryDate` → keep the grant's existing expiry, else today + 90 (Date Only, one shared TimeProvider-based computation); past date → 400 `sdap.access.grant.expiry_in_past`; clients untouched; then the dev backfill (record before-state → write → re-query). Dataverse MCP is DOWN this session — backfill goes via the Web API (`az` token works). **Checkpoint (steps 0–2 written, UNCOMMITTED)**: step 0 found `/invite` writes NO grant (POML premise wrong — nothing to default there) and `DelegationRuleFilter:223` is target-resolution only (trigger 1 does NOT fire). Code: `ExternalGrantLifecycle` (DefaultExpiryDays/TodayUtc/DefaultExpiry), `GrantExternalAccessEndpoint` (TimeProvider param, `ValidateRequestedExpiry`, `CreateGrantAsync(today)` keep-else-+90), `InviteAndGrantExternalUserEndpoint` (validate before onboarding), both DTO docs, `ExternalAccessModule` TryAdd TimeProvider. Tests: GrantLifecycle (fixed Today; +3), ExternalAccessContract (FakeTimeProvider + payload capture; +6), DelegationRule (+2 pin). **Next**: build/run → commit → perturb → backfill (before-state → write → re-query) → notes → gates. |
