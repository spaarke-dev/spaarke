...
> **Last Updated**: **2026-09-18, session 15 — task 106: code COMPLETE + V1 fixed, verification nearly done**. Step 9.5 ran both gates; **both independently found a Critical defect this task had introduced** (V1/F1 — a request-dependent survivor election, so two concurrent grants could deactivate each other and leave **ZERO active rows while both got HTTP 200**). Fixed in `e17a4c1c9` by ranking on `ConferralRank` and **removing the request parameter entirely**, making request-independence structural. **14 commits, ALL UNPUSHED.** Verified: build **0/0** · the two classes **52/52** (expected 52 derived from source, independently of the run) · **perturbations 5/5 CAUGHT, 0 MISSED, 0 INVALID** (spread 7/5/1/1/2 = five separable guarantees; **P5 proves the V1 regression test actually bites**) · **ArchTests 323/323** (count unchanged → no endpoint-census shift) · no CVEs, no package added. **Still owed**: full `Sprk.Bff.Api.Tests` (IN FLIGHT) and publish size vs a FRESH master — acceptance criterion 7 is unmet until both land, so the POML and TASK-INDEX are deliberately UNTOUCHED. Record: `notes/task-106-regrant-expired-survivor.md` (§16 = V1).
...
| Field | Value |
|---|---|
| **Task** | **106 🔄 code COMPLETE, verification unfinished (2026-09-18)** — ISS-008 / #973 / FR-09. Commits `3d1ce86e6` · `b849cf6e7` · **`e17a4c1c9` (V1 fix)** · `97481c917`. **What it does**: a re-grant judges "confers access" across every active row on the key, so the 409 `sdap.grant.expired_not_restored` fires only when NO active row confers — before, an expired lowest-id row produced that 409 while a live duplicate meant the grantee DID have access. The fix is the **ELECTION, not the check**: the survivor is the row conferring longest, which makes three properties structural — an expired survivor proves every row is expired; the collapse cannot deactivate the conferring row (it IS the survivor); and 🚨 the naive fix (suppress the 409 and fall through) would have REVOKED that live access, turning an inert bug destructive. **Plus V1**: the first version ranked by `EffectiveExpiry(request.ExpiryDate, …)` — request-DEPENDENT, so racers could mutually deactivate. Now `ConferralRank(rowExpiry, today) = rowExpiry ?? DefaultExpiry(today)` and `ElectSurvivor` takes no request value at all. ⚠️ **NOT** `?? DateOnly.MaxValue` (the reviewer's own suggestion) — that elects unbounded rows, FR-33 bounds them to +90, a +200 sibling is collapsed, and access SHORTENS; **P4 pins it**. Also fixed from review: **F6** stale comment 5 lines from my own edit · **F2** my own FALSE "must not merge" claim (the `!=` guard already made a merge safe) · **F4** one name for two values · **F9** two tests missing the precondition my notes claimed all had · **F3** a second in-memory expiry copy folded through `ConfersAccessOn` · **F11** an unenforced precondition. |
...
| **Next Action** | **(1) Full `Sprk.Bff.Api.Tests` — IN FLIGHT** when this was written (an earlier run was stopped as superseded; its testhost was confirmed gone, since a survivor makes the next run silently execute OLD DLLs). **(2) Publish size vs a FRESH `origin/master`** — `scratchpad/publish106.ps1`; pwsh 7.6.6 confirmed; short paths BOTH sides; **file-count parity asserted or the delta is meaningless**; repo-wide `worktree prune` deliberately removed (~12 other agent worktrees share this `.git`). ⚠️ Run it ALONE — build contention produced seven non-results this session. **(3)** Fill notes §14's last two rows. **(4)** POML `<status>completed</status>` + TASK-INDEX ✅ in ONE transaction, then `pwsh scripts/check-task-status-drift.ps1` — both artifacts have drifted before, in both directions. **(5)** Step 11 gate: next in wave 1 is **107** (ISS-009 / #974), which needs the owner's go-ahead. 🔔 **Owner items**: **FIVE** register entries now carry `{URL}` placeholders and are **NOT filed as GitHub issues** — ISS-019/020/021 (task 043) plus **ISS-022** (stale-snapshot collapse can SHORTEN access; the election key is now a mutable column — V1's fix does NOT cover it) and **ISS-023** (a collapse can lower the effective LEVEL; pre-existing per task 010, and 106 provably does not change level outcomes — needs a **product** decision: does "grant X" mean *set to* X or *raise to at least* X?). `push-to-github` Step 1.6 audits for exactly this. Also: **W5** — ADR-038 ban B8 formally covers `InternalsVisibleTo` (verified verbatim) while the ADR itself files B8 as "blocked on a production refactor" → path A now, **path B needs the ADR owner**; **ISS-020** needs a decision on its merits; 063's `user_not_internal` refusal — product intent?; ISS-003 / #964 blocks 054; owners for #988, #971, #993. **Push only when asked**; pushing ≠ merging. |
