...
| **Next Action** | ✅ **076 IS NOT BLOCKED — I was wrong to say so.** The owner answered on **2026-08-28** (`notes/SESSION-STATUS-2026-08-28.md` §6.5 Q1): for content with no record yet, the **SERVER** derives the **acting user's BU** container. Not an upload ticket, not wizard reordering. §5 of the 076 notes was stale and is now corrected in place. **The record-LESS route is BUILT** — `PUT /api/obo/me/files/{*path}` (`756e089cb`), ArchTests 182/182, solution build clean, additive/safe to deploy alone. **NEXT = THE CLIENT CUTOVER** (076 steps 4–11): 6 `uploadFilesToSpe` sites → record-keyed route; the 3 parentless sites → the new record-less route; then delete the container-keyed route + its Pending waiver, delete W1/W2, classify the 12 suppliers, route the 7 Communication sites, absence-grep. 🔴 **The cutover IS ship-together client+BFF** — no compatibility window. |
