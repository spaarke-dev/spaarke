...
> **Last Updated**: **2026-09-13, session 10** (task 102 COMPLETE) — ADR-052 Accepted; ADRs, directives and ~70 docs aligned; drift guard + ratchet + Functions guard live (ArchTests 288/288); issues #976–#986 filed. Next = task **103**. Task 100 paused at `e7bd02189`.
> Session-9 + session-10 work is COMMITTED but NOT PUSHED (tip = `git log -1`). 0 behind master at the 2026-09-13 fetch.
...
> ⚠️ **This project's task files have been WRONG THIRTEEN times now** (session 9: task 100 named `OutboxService`, whose client skips unknown kinds, and assumed the granter is on the grant row — 20/28 grants lack it; session 8: task 098 listed
...
| Field | Value |
|---|---|
| **Task** | **103 — Scheduler lease + slot guard + `AddScheduledJob<TJob>` helper** · `tasks/103-scheduled-jobs-exactly-once.poml` · **NOT STARTED (next)**. Then **100** (finish; paused at `e7bd02189`, waits on 103). **102 ✅ COMPLETE 2026-09-13** (record: `notes/task-102-workload-placement-governance.md`). History of 102 follows: `tasks/102-workload-placement-governance.poml`. **Owner APPROVED the v2 drafts 2026-09-13.** Identity = option 1, "consistent with the existing approach": a Function reuses the stamp UAMI **app-only** (ADR-028 A4's app-only row, the provisioning control plane's precedent). Two corrections were applied: (a) the CIAM Graph (`CiamGraphClientFactory`, cert) and Power BI (`ReportingEmbedService`, secret) confidential clients are named as NOT inherited; (b) the Functions-project ArchTest also bans MSAL confidential-client/assertion types in `src/server/functions/**`. **Steps 0–3 of 7 DONE** (step 3 = ADRs + indexes applied, session 10). After it: **103** (scheduler lease + `AddScheduledJob` helper) → **100** (finish; paused at `e7bd02189`, waits on 103). |
| **Branch** | `work/unified-access-control-r2` · PR **#950** · **session-9 AND session-10 commits are NOT pushed** (`e7bd02189` … tip = `git log -1`). CI last checked on `45e6814ba` (session 8: all Tier 1 green; Trivy red, pre-existing). Temp worktrees `C:\wt100*` removed. |
| **Next Action** | **task-execute 103** (`tasks/103-scheduled-jobs-exactly-once.poml`): ADR-036 A1 rules 1, 2 and 6 — the distributed lease (FIRST resolve its open escalation: behaviour when a CONFIGURED lease store is unavailable), the non-production slot guard, and `AddScheduledJob<TJob>`; migrate the 3 jobs onto the helper. Then task 100's 10 review fixes (§ Task 100 — outstanding review fixes). **Owner decisions carried from 102 (non-blocking)**: (1) 8 not-started POMLs in active projects (`ai-spaarke-action-engine-r1` 001/014/022/030/040/042/070, `sdap-SPE-admin-app-r2` 050) still carry the old flat ban — fix them and widen the drift guard to not-started POMLs, or record the exclusion; (2) confirm the Function "no Dataverse impersonation" MUST NOT (ADR-052 §6); (3) Insights D-20. **(Task 102 history follows.)** ~~Task 102: steps 3–6 DONE~~ (ADRs applied · 47 docs + all `.claude` directives aligned · ArchTests `ADR001` fixed + `WorkloadPlacementDocDriftTests` + `WorkloadPlacementGuardTests` · issues **#976–#986** filed). **NOW**: step 7 verify (full ArchTests suite running) → Step 9.5 code-review + adr-check → Step 10 (POML status, TASK-INDEX, drift check) → commit. Record: `notes/task-102-workload-placement-governance.md`. **(Earlier step-4 plan follows, done.)** ~~Task 102 STEP 4 — align every in-scope document~~ (evaluation note §4.1 + POML 102 `<notes>` list). `.claude/**` in the MAIN session: constraints (api, ai, plugins, jobs, bff-extensions §A.1/§D/§E/decision table per `notes/drafts/adr-amendments-and-key-directives.draft.md` §6), pattern `api/background-workers.md`, skills (code-review + its `references/review-checklist.md`, adr-check + `references/adr-validation-rules.md`, adr-aware, task-create, design-to-spec, task-execute tag maps), the project-setup template. Sub-agents may do `docs/**`, `src/**` comments and `infra*/**` headers. Also: evaluation note §4.4 Q1 gets the CIAM/Power BI carve-out; extend the `.claude/CHANGELOG.md` 2026-09-13 entry. Then step 5 ArchTests — **the drift guard MUST handle an inline marker pair on ONE line** (ADR-036 A1 §1 uses one) → step 6 issues → step 7 verify → Step 9.5 gates. **Step-3 lessons**: (1) `\bno Azure Functions\b` also hits LEGITIMATE BFF-scoped wording ("no Azure Functions hosting BFF endpoints"). Rephrase such lines ("never hosted in Azure Functions"); don't marker them. (2) ADR-032 L242's stale item was `IJobHandler<T>`, as the POML said — fixed. **Coordinate**: PR #935 edits `.claude/adr/INDEX.md` (ADR-012 row only), a soft overlap. **(Superseded session-9 text follows.)** ~~Get the owner's approval to apply the v2 drafts~~ in `notes/drafts/`: `ADR-052-workload-placement.full.md` + `.concise.md`, `adr-amendments-and-key-directives.draft.md`, `drift-guard-design.draft.md`. Owner answers at session-9 end: **Q1 identity**: my "a Function needs its own SPE grant" was WRONG and is corrected in the drafts. The BFF's app-only Graph/SPE calls already run as the managed identity (`GraphClientFactory`, `Graph:ManagedIdentity:Enabled=true`), so a Function reusing it needs no new grant. The rule stays: no acting as the BFF app registration, no OBO, no BFF-endpoint calls. **Confirm the owner accepts the corrected wording.** **Q2**: dev only, so no formal exception (applied: one line in ADR-036 A1; POML AC updated). **Q3**: bigger scope APPROVED; the ~20 extra docs are listed in POML 102 `<notes>`. **Q4**: synopsis given (§ Session 9 record). **On approval**: task-execute 102 from **step 3**: apply the ADRs + indexes (`.claude/` main-session only; `.claude/CHANGELOG.md` entry; ai-procedure-maintenance Checklist A) → step 4: docs (evaluation note §4.1 + POML `<notes>` list) → step 5: ArchTests (ADR001 message + METHOD-level attribute scan; drift guard with reasoned region markers; timer-service ratchet; `IScheduledJob` host-neutrality; Functions-project guard) → step 6: 8 GitHub issues → step 7: verify → Step 9.5 gates. **Coordinate**: `code-quality-and-assurance-r4` PR #935 also edits `.claude/adr/INDEX.md` (the ADR-012 row); its future `AdrClassificationCensusTests` will need ADR-052 classified. |
| **Task status** | **69 done · 28 open (incl. 100 paused, 103 next) · 3 escalated · 1 blocked-shipped · 100 total** (102 ✅ 2026-09-13). Drift gate green (100 POMLs = 100 index rows). |
...
### Commits session 9 (2026-09-12) — NOT PUSHED
...
| _(next)_ | handoff — owner Q1/Q2 corrections to the drafts + this file |
...
1. **Retry**: throw after the heartbeat when the run can't make progress, or when units failed transiently and no later tick will revisit them (exact-day job ⇒ throw on write failures), so `JobRetryPolicy` applies (ADR-036 A1 rule 4).
...
7. Move registration onto `AddScheduledJob<TJob>` (from task 103); delete the nested bootstrap; fix the doc comment claiming operators can pause the job (the admin disable is per-instance and not durable).
8. **Tests**: DaysLeft in the idempotency key (advance 16 days, expect the 14-day reminder) · write failure then re-run sends once · cancellation · project + work-assignment roots · disabled owner → creator · support user excluded · paging past 5,000 · ResultJson breakdown.
...
> ⚠️ **Session 10: Quick Recovery → Next Action wins over this section.** First task 102 (from step 3, once the
> owner approves the v2 drafts), then 103, then finish 100. The chains and C-items below remain the valid backlog
> after that, with 102 and 103 added to the FR-33 chain's prerequisites.
