...
> **Last Updated**: **2026-09-19, session 16 — 13 OWNER DECISIONS RECEIVED, COMMITTED AND PUSHED.** Head **`d8c139697`** (`cb9ec2ea2..d8c139697`), **0 unpushed**, tree clean. PR **#950** (no second PR created). 📖 **Read `notes/owner-decision-brief-2026-09-18.md` + `notes/remediation-and-sequencing-plan-2026-09-18.md` FIRST — they supersede this row.** 🔴 **SIX handoff/plan claims corrected — two of them TODAY, by falsified prediction**: (1) `Router` DID report on `3b409bfc1`/`597882f7c`, as a **failure** (not "not at all", not "no failures"); (2) cause is **cancellation** — each push cancels an **in-flight** run and `alls-green` counts a cancelled dependency as a hard fail; (3) ⚠️ **but a red on a superseded SHA is expected ONLY IF that run was still in flight** — `cb9ec2ea2` COMPLETED before the session-16 push and kept a genuine **`success`**, so do NOT wave away a superseded red without checking it completed; (4) `gh pr checks` **NEVER lists `Router`** — probe it **by name** via the check-runs API on the head SHA; (5) 🔴 **"docs-only pushes skip Tier 1" is FALSE FOR THIS PR** — `d8c139697` touches only `projects/**` yet **both tier jobs dispatched**; they are gated `if docs_only != 'true'`, the run `event` is `pull_request`, and paths-filter classifies against the **PR BASE** (`ci-router.yml:140-142`), so since #950 touches BFF throughout, **no push on this PR is ever docs_only and Tier 1 ALWAYS runs**; (6) the split is 75 done / 32 open / **3 escalated + 1 blocked** = 111. **Merge gate: Tier 1 green + `Router` green on the head SHA, probed by name. Do not poll it; probe once at the decision point.** `/merge-to-master` remains a separate, unmade decision.
...
| **Next Action** | ✅ **ALL THIRTEEN DECISIONS RECEIVED (2026-09-18/19) — nothing is blocked.** Full record: `notes/owner-decision-brief-2026-09-18.md`; sequencing: `notes/remediation-and-sequencing-plan-2026-09-18.md`. **D-1 = A + B, and C is IN SCOPE, not deferred** (owner: *"for C if this is needed for the best solution then do not defer"*): **A** invert the read default so a null `sprk_expiresdate` confers **NOTHING** (drop the `eq null` branch from `ExpiryPredicate`, `ExternalParticipationService.cs:99-100`; blast radius **zero** today — task 097 backfill took dev 25 undated to 0, but COUNT before deploy); **B** ONE **scheduled reconciliation job** on ADR-036 reusing task 103 lease + slot guard; **C** remove `Create` from human roles — 🔴 **prerequisite**: re-point `TrackingFieldTrio:43` off `hasEntityPrivilege(...Create...)` onto the server real rule (**Write-on-record**, task 008 delegation) FIRST, or Manage Access vanishes for every user while the BFF app-only write keeps working. 🔴 **Plugins are OFF the table repo-wide; task 107 is RESCOPED off every Dataverse-side mechanism; no ADR-002 exception is sought.** **D-2 fully decided**: part 1 bound the ADDITIVE path; **part 2 = NO — an ethical wall KEEPS binding a former member**, veto stays `statecode`-only and over-matches; part 3 count date-ended-but-active rows before deploy; part 4 writer = D-1 option B. **109 + 110 ship as ONE change** (one read projecting `sprk_enddate`, two named sets, preserving task 043 single-snapshot invariant). 🔑 **ONE MECHANISM SERVES FOUR PROBLEMS** — D-1 B, ISS-026 write half, ISS-020 part 4, rescoped 107. Design it once. **Now authorised**: author the POMLs (115 nine-file repair, 116 ISS-025, ISS-026/027/028, rescoped 107, the C prerequisite) via `/task-create`, each with its own premise check. Then wave 1: **108 → 065 → 044**. |
| **Task status** | **75 done · 32 open · 3 escalated · 1 blocked = 111.** Recomputed independently 2026-09-18: **111 POMLs / 111 index rows, ZERO orphans in BOTH directions** — the both-sides comparison ISS-025 says `check-task-status-drift.ps1` structurally CANNOT perform. Register now **28 entries**, verified 28 disposition rows = 28 entry headings, id sets equal both ways, no duplicates, **zero** placeholders. **5 tasks still to author** (all HELD on D-1): **115** the accuracy repair — ⚠️ **NINE files, not eight**: task **089** carries pre-renumbering refs and its `<gate>` blocks on **082**, a live UNRELATED task — **116** ISS-025, plus ISS-026/027/028. ⚠️ **082 and 093 are RESCOPES, not patches** and are excluded from 115: 093 **closes as delivered** (create-first is COMPILER-enforced), and 082 reduces to ONE deliverable, the CLAUDE.md §11 four-primitive memo (re-tier opus/xhigh → sonnet/medium). |
