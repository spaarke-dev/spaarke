...
> **Last Updated**: **2026-09-17, task 043 IN PROGRESS** (by `task-execute`) — 063 ✅ closed, amended and **pushed** (`c3553730c`; 0 unpushed, tree clean). Now executing **043 — org-expansion membership term (FR-24/25/22) + the registry filter on the systemuser membership path**. RIGOR **FULL** (POML-authored; tags bff-api/auth/security). 🔴 **POML premise errors found before coding — #18 and #19**: (18) it cites `ContactStandingGrantReader.cs` as a canonical reference, but task 042 RENAMED it `SubjectStandingGrantReader.cs` and the old file does not exist; (19) step 3 says wire the flag into "the flag-off branch of **036**", but 036 is `🔲 open` (deps 032/**034**/035/**104**, and 034 is 🟡 blocked) — `ComposeForSystemUserAsync` has **no flag and no branches** today, so its single `_membership.ResolveAsync` call IS the path to filter. Its three `<dependency status="pending">` attributes are also stale (037/041/042 are all ✅).
...
> ⚠️ **One live block. Replace it; never append.** Row headings (`| **Next Action** | Implement 043. Order: (1) add `AccessConferringOnly` **and** the new org-binding option to `HashOptions`; (2) extend `ResolveByContactAsync` to bind supplied org ids into `PersonIdentity.OrganizationIds` behind an explicit option so registry-filtered org-typed descriptors emit conditions; (3) wire the evaluator's org-expansion term at each org's 042 baseline, Secure-suppressed; (4) pass `AccessConferringOnly: true` on `ComposeForSystemUserAsync`'s `ResolveAsync`; (5) INVERT `ComposeAsync_NeverReadsTheOrganizationStandingGrant_OrgTermIsTask043` (`AccessibleRecordSetServiceTests.cs:1391`) — it asserts `Times.Never` on `ReadForOrganizationAsync` and 042 wrote it as a scope boundary for THIS task; (6) closed-set tests; (7) build + affected suites + perturbations; (8) publish size per CLAUDE.md §10 measured from FRESH short-path worktrees on BOTH sides. **Push only when asked.** 🔔 **Owner item still open** (063 notes §9.3): sharing with an external LICENSED system user is refused (`user_not_internal`) — is that the product intent? **Before merge of #950**: the real-Dataverse DOWNGRADE check, the reminder `appnotification` write, and a real-ARM check of H9's slot guard (#987). |
...
| Field | Value |
|---|---|
| **Task** | **063 ✅ COMPLETE 2026-09-15, AMENDED 2026-09-16** — internal system-user share endpoints, delegation-gated (FR-29): `POST /share-user`, `POST /unshare-user`, `GET /user-shares` on the `/api/v1/external-access` group, so they inherit the FR-07 Write-on-the-record gate. Commits `d47b586eb` (implementation + tests), `5e84c6b9f` (record, register, task 108), `1d016f1ed` (publish + Step 9.5 records), plus the Step 9.5 fix commit. **AMENDED 2026-09-16** (`c28c1d0ac`) on the owner's answers: a share now carries the requested level **∩ the caller's own rights** on the record (`RecordShareLevels.Intersect`), so a caller cannot hand anyone — themselves included — a right they do not hold; a narrowed grant is written and reported (`narrowed: true`, `accessLevel` null when the intersection matches no level), and an empty intersection is 403 `…caller_cannot_grant`, which also catches a probe that cannot answer. The owner also ruled that a Write-holder revoking any share — the creating attorney's included — is **not policed** (not a use case). **Design**: `ModifyAccessAsync` (new) for an existing share — Microsoft Learn documents it as REPLACING the mask, while GrantAccess on an existing share is undocumented and reported additive, so a downgrade through it could silently keep Write and Delete; `GetPrincipalAccessOrThrowAsync` (new) because the soft read's empty answer cannot distinguish "no share" from "the read failed"; every write confirmed by reading the stored mask back; unshare with no share answers `removed=false` without calling Revoke. One level table (`RecordShareLevels`) that the provisioning constants now derive from; no level carries Share or Assign (owner, "No re-share at any level"). **Verification** (after the amendment): perturbations **12/12** · affected suites **164/164** · ArchTests **323/323** · publish 45.45 vs master 45.35 MB (214 files each) · no CVEs. Full suite on the FINAL tree: green, exit 0 (BFF assembly **12,475 / 0 / 58**; SPE integration 403/0/25). The BFF count is +12 on the 12,463 at `d47b586eb` — exactly the twelve tests the Step 9.5 fixes and the amendment added, which is the check that the run covered the right tree. **Step 9.5**: adr-check 0 violations / 8 warnings; code-review approve-with-changes, 22 findings — 11 fixed, 3 escalated, the rest accepted with reasons (notes §7.3). Record: `notes/task-063-internal-user-share-endpoints.md`. ✅ **Both owner decisions answered 2026-09-16**: intersect with the caller's own rights (implemented — see AMENDED above), and a Write-holder revoking any share is not policed. 🔲 **One question still open** (notes §9.3): sharing with an external LICENSED system user is refused (`user_not_internal`) — is that the product intent? 🔔 **Before merge**: the live Dataverse check must exercise a **DOWNGRADE**, not a create (notes §9.4). **NEXT: 043.** **099 ⏸** waits for 065 + 066. **101** runs after 107. **104 ✅**, **100 ✅**, **103 ✅ 2026-09-14.** |
...
| **Next Action** | **task-execute 043**, continuing TASK-INDEX § Execution sequence (dependency-ordered, owner 2026-09-15). Wave 1: 104 ✅ → 063 ✅ → **043 → 106 → 107 → 093 → 082 → 094 → 095**. Verify every POML's premises against the code first — this project's task files have now been wrong **17** times (063 found two: the POA seam had moved to `Services/Access`, and the "060 seam mapping" from level to rights did not exist at all). 🔔 **Owner items**: (1) ✅ answered 2026-09-16 — intersect with the caller's own rights; implemented in `c28c1d0ac`; (2) ✅ answered — a Write-holder revoking any share is not policed; (3) 🔲 **still open** (notes §9.3) — sharing with an external LICENSED system user is refused (`user_not_internal`); is that the product intent? (4) ISS-003 / #964 blocks **054** (055–058 wait with it); (5) owners for **#988**, **#971**, **#993**. **Before merge of #950**: the real-Dataverse DOWNGRADE check (notes §9.4), the reminder `appnotification` write, and a real-ARM check of H9's slot guard (#987). **Push only when asked.** |
| **Task status** | **73 done · 28 open (043 next; 063 ✅ 2026-09-15, task 108 added) · 3 escalated / blocked** (counts read from TASK-INDEX 2026-09-15). Status-drift check green 2026-09-16 (105 POMLs / 105 index rows). |
