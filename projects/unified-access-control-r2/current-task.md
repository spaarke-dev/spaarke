...
| **Next Action** | ✅ **PR #933 MERGED** → master `399651fff` (31 checks green, 0 pending/failing; worktree + main repo synced). N-1, item 7 and item 5 all DONE. **Item 5 filed 093/094/095** — ⚠️ **094 was deliberately NARROWED**: most of it shipped 2026-09-02, so re-scope before implementing. **NEXT: item 6 — execute 076** (the container contract; ship-together client+BFF; its own session). Item 4 needs an ENVIRONMENT query (org-side ribbon). 🔔 **OWNER IS ADDING `sprk_account` + `sprk_contact` lookups to `sprk_document`** — when they exist, add the two cases to `DocumentAssociationMap` + `DataverseServiceClientImpl` and the client `ENTITY_CONFIGS`; do NOT wire them before the columns exist (that converts a silent drop into a hard write error). |
