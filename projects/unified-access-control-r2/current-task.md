...
> ### ⏱️ 30-SECOND SUMMARY — session 18, 2026-09-19 (by `context-handoff`). This supersedes every longer row below.
>
> | Field | Value |
> |---|---|
> | **Task** | **108** — ISS-018 / **#995**, "unsecure-project must not report a failed share read as 0 shares revoked" |
> | **Status** | 🟡 **BLOCKED on the build host — code complete, both Step 9.5 gates passed, verification partially done.** Task is deliberately still `pending`; do NOT mark it complete. |
> | **Step** | 9.5 done (both gates, all findings applied) → Step 10 **not** taken, on purpose |
> | **Next Action** | **1)** Clear the build host: `dotnet build-server shutdown`, or terminate the orphaned `dotnet.exe` processes (there were **10**, ~1.2 GB, where there is normally 1 — this is the root cause of every build failure). **2)** Re-run: ArchTests (expect 323), the perturbation (expect **Failed 2 / Passed 17 / Total 19**), and the `wt108b` republish. **3)** Then mark 108 complete: POML `<status>` + TASK-INDEX row `:717` + `pwsh scripts/check-task-status-drift.ps1`, comment #995, reset this file, `git worktree remove C:/wt108m C:/wt108b`. |
...
> **NOT verified (pre-review code only — the gap):** ArchTests 323 · perturbation criterion 4 · publish delta 0.00 MB.
>
...
>
> ⚠️ **Do not re-derive the build failures as a code problem.** Ten builds failed across six error codes; none was about the code. Several were self-inflicted (an `rm -rf obj bin` that destroyed `project.assets.json`, then property overrides that moved the failure rather than fixing it). Disk is fine; path casing was investigated and exonerated.
