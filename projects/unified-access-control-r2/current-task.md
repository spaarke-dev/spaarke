...
> **Last Updated**: **2026-09-14, session 11** (by `task-execute` Step 11) — task **103 ✅ COMPLETE** (lease + slot guard + `AddScheduledJob`; ADR-036 A1.1; full suite 14,540/0/86). Next = finish task **100**.
> Sessions 9–11 are COMMITTED but NOT PUSHED (tip = `git log -1`). 0 behind master at the 2026-09-14 fetch.
...
| Field | Value |
|---|---|
| **Task** | **100 — FR-33 expiry reminders (finish)** · **IN PROGRESS (session 12, 2026-09-15)** — Step 0.5 conflict-check clean; knowledge loaded. **Design (fixes 1–6, 8)**: query window `today ≤ expires ≤ today+30` (one FetchXML, `ge`/`le`); send the MOST URGENT unsent threshold T = min{30,14,7,3,1 ≥ daysLeft} (catch-up; day 0 = expiry day still active → 1-day threshold sent "ends today"); key = (grant, expiry date, T); claim = processed? → lock → re-check processed → send → mark (CancellationToken.None; mark failure = sent+warning) → release; person = enabled ∧ no applicationid ∧ accessmode ∈ {0,1,2}; wording by actual daysLeft ("ends in N days / tomorrow / today", body "Access for {grantee} to the {label} "{name}" ends after {date}."); cancellation by our token → cancelled, other OCE (timeouts) → failed; paging cap 20 pages; throw AFTER the heartbeat on query failure or any failed write at daysLeft 0 (no later tick revisits) — other failures partial; `JobRunContext.Attempt` added (host sets it per retry) and logged in the heartbeat. POML criterion "20 days → nothing" is superseded by the catch-up decision (a grant whose 30-day reminder was sent gets nothing at 20). Separately: #987 H9 fix running in a background agent (isolated worktree). **Checkpoint 2 (session 12)**: committed `521ab1b9a` (task-100 fixes round 1) + cherry-picked `6149edecf` (#987 H9 slot guard, agent; provisioning 1567/0/1). Step 9.5 ran on `521ab1b9a`: adr-check 1 violation (notes §8 cited ADR-001 → rewritten to ADR-052) + 8 warnings; code-review 0 High / 3 Medium / 16 Low. **Round 2 (uncommitted, testing now)**: job — transient-only last-day retry (M2), no throw when cancelled (L1), marker read-back → `markFailed` (L2), held claim counted `claimHeld` + last-day retry (L3/W2), per-row try/catch (L4), combined problems + all counts in ResultJson (L5), invariant key (L7), wording (L8), FetchXML `<order>`; tests — cache on the fake clock (M1), +11 tests (L9), host test renamed (L10); `SystemCacheKeys` rationale (W1); ADR-036 key types both copies (W4/L12); new ArchTest `SchedulingSlotGuardKeyTests` (the key in 5 places); POML 100 criteria amended (M3); notes §4/§5 (L11, accepted L6/L13–L16). Perturbation script `scratchpad/p100.sh` now 13 breaks. Earlier: committed `17ce03546` (impersonation accepted; P1–P3 = #988/#989/#990; 103 path-A ratified). **Uncommitted task-100 work**: `GrantExpiryReminderJob.cs` (rewritten — fixes 1–6, 8), `tests/integration/data-mutation/ExternalAccess/GrantExpiryReminderJobTests.cs` (rewritten, 39 tests), `Spaarke.Scheduling/JobRunContext.cs` (+`Attempt`), `ScheduledJobHost.cs` (passes the attempt), notes/task-100 (premise 16: `NotificationService` wraps cancellation). Live read-only FetchXML check passed (window shape + `accessmode` aliased; 0 rows in today's window, 28 in a wide one). 38/39 → cancellation catch fixed, re-running. **Next**: tests green → full suite + ArchTests → perturbations (idempotency off, grantee addressed, re-check removed, last-day throw removed, catch-up off, accessmode ignored) → publish size → Step 9.5 → notes §9 + POML + TASK-INDEX. · `tasks/100-fr33-expiry-reminder-job.poml` · **PAUSED → RESUME NEXT** (implemented at `e7bd02189`; the review fixes in § Task 100 below — fix 7 DONE by 103, fix 8 now carries the owner's catch-up decision). **Must be live by 2026-11-10.** **103 ✅ COMPLETE 2026-09-14** (`a9c538c72` + `0f1414c37`) — full record `notes/task-103-scheduled-jobs-exactly-once.md`. **102 ✅ 2026-09-13.** |
| **Branch** | `work/unified-access-control-r2` · PR **#950** · **sessions 9, 10 AND 11 are NOT pushed** (`e7bd02189` … tip = `git log -1`). CI last checked on `45e6814ba` (session 8). Temp worktrees `C:\wt103m`, `C:\wt103b` and the POML sub-agent's worktree + branches removed (its commit was patch-id-identical to the cherry-pick). |
| **Next Action** | **task-execute 100** (resume). Read § Task 100 below — 9 open fixes (7 is done). Fix 8 carries the owner's catch-up decision (a missed threshold is sent later, once per threshold); re-judge fix 1's throw rule against it, and remember ADR-036 A1.1 — if Redis is down at 06:00 the tick is not dispatched at all, and the catch-up is what recovers it. **Open owner decisions (raise at session start)**: (1) 🔔 **Function impersonation** — `notes/decisions/function-impersonation-proposal.md`: replace ADR-052 §6's blanket ban with a conditional permission (BFF-initiated work only; the caller id from a BFF-written job message on an Entra-only queue; one fail-closed helper). Its prerequisites P1–P3 are not met, and P1 is a standing risk on its own: **the Service Bus namespace still accepts SAS** (`SpaarkeAppAccess` rule + connection-string fallback). Needs accept / refine / keep-the-ban. (2) Ratify or reject task 103's two path-A test exceptions (tests co-located in `tests/unit/Spaarke.Scheduling.Tests`; the existing wall-clock `WaitUntilAsync` helper). (3) Insights Engine D-20 (carried). **Push only when asked.** |
| **Task status** | **70 done · 27 open (incl. 100 next) · 3 escalated · 1 blocked-shipped · 100 total** (103 ✅ 2026-09-14). Status-drift check green at the session-11 close. |
...
### Commits session 11 (2026-09-14) — NOT PUSHED
