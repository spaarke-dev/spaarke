...
| `OfficeAuthFilter`, `OfficeRateLimitFilter`, `ResourceAccessHandler` | sequential oid-first, early returns; `NameIdentifier`/`sub` used only as an **opaque partition/cache key** | Correct by design — the legitimate use of `sub`, and what `ResolveOpaqueCallerKey` exists to name. |
