...
| Can it be blank? | 🔴 **No. Required to grant, and the picker DEFAULTS to +90 days.** ⚠️ **Amended 2026-09-10 (session 7, owner: "Server fills +90")**: required in the **stored data**, not in the request — when a request carries no expiry the **server** keeps the grant's existing expiry, else stores today + 90; a past date is still rejected. | This is what preserves FR-33's actual point — expiry **mandatory and bounded**. A blank-means-never field would silently restore unbounded access and undo the whole FR. Defaulting the picker means the path of least resistance is bounded; requiring it means nobody grants by accident. **Why amended**: task 097 found no client sends an expiry (`AccessGrantModal`, `TrackingFieldTrio` PCF, external SPA `InviteUserDialog`) — a request-level 400 would break every sharing UI, and only the modal is scheduled to gain a picker. A server default bounds every grant regardless of which surface — current or future — wrote it |
