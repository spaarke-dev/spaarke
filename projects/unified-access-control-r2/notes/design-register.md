...
| I-4 | **The `sdap:auth:*` cache keys carry no tenant segment** — unlike the `DistributedCacheExtensions.CreateKey` / `ITenantCache` pattern used elsewhere in the BFF (and documented in `.claude/constraints/data.md`). Surfaced by task 014 while adding the auth-mode segment; **out of scope there** (that task changed only the auth-mode discriminator) and likely moot while each BFF environment is single-tenant. Becomes load-bearing if a BFF instance ever serves more than one tenant, and is directly relevant to **task 035**, which adds a per-user impersonated root-set cache — that key should be designed tenant-aware from the start rather than retrofitted. Verified 2026-08-21: `ITenantCache` exists and is used by `ExternalParticipationService` / `GrantExternalAccessEndpoint`, so the abstraction is available; the auth cache simply predates it |
