...
33. **FR-33** (High, security control — **ACCEPTED IN PRINCIPLE 2026-09-08, NOT SCHEDULED**): **every external access grant is time-bounded.** `sprk_expiresdate` becomes **mandatory** on `/grant`, bounded by a **tenant-configurable maximum lifetime**, with the granting internal user notified ahead of lapse so access is renewed deliberately rather than expiring by surprise.
