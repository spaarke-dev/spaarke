...
| # | Task | FR | Deps | Group | Safe | Tier | Effort |
|---|---|---|---|---|---|---|---|
| 🔲 [open] 115 | Repair the **nine** open task files carrying a wrong load-bearing sentence (036, 054, 055, 056, 087, 088, 089, 094, 095). ⚠️ **055 and 056 belong** — they inherit 054's retired fourth-root premise in **binding** elements (`<gate>`, `<dependency>`, `<constraint>`, `<goal>`, steps), not merely in notes. Both source notes **under-count**: the audit says 8, the plan's table has 9 rows but two are the excluded rescopes, and its "Proposed shape" names only 7. Per-file site counts are understated too (~8 in 087 vs 3 claimed). Verification is a **zero-residual grep per file**, not a count. **082 and 093 excluded** — rescopes, not sentence patches | — | none | — | ❌ | sonnet | high |
