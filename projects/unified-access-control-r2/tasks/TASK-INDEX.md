...
| 🔲 [open] 107 | **ISS-009 option A — invert the read default: a grant with NO `sprk_expiresdate` confers NOTHING** (#974). 🔴 **RESCOPED 2026-09-21** to owner decision **D-1** (2026-09-19): *"We do not use Dataverse plugins"* — plugins off the table **repo-wide**, no ADR-002 exception sought. The prior POML specified a Dataverse-side default and required criterion 5 that *"the read filter is unchanged (the `eq null` clause is still there)"* — **the exact opposite of what was decided**. This file was NOT in task 115’s nine, so its premise rot survived that pass: it is the **tenth** instance. 🔴 **The mirror is the risk** — `ConfersAccessOn` (`ExternalParticipationService.cs:124-125`) is the in-memory twin of `ExpiryPredicate` (`:99-100`); change one without the other and `/grant` reports a grant live while the reader denies it. The old POML never mentioned it. ⚠️ `parallel-safe` flipped **true → false**: the rescope moved it out of `src/dataverse/**` into the **ExternalAccess exclusive zone**. Siblings: **117** (option B job) · **118** (option C). Pre-deploy gate: **COUNT undated active grants** — they all lose access on deploy | FR-33 | 097 | — | ❌ | sonnet | high |
