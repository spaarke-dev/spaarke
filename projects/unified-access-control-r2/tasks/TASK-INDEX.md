...
| 🔲 [open] 116 | The drift check must compare POMLs and index rows as **SETS**, equal in both directions (ISS-025 / **#1004**). Root cause is one line: the loop is keyed on the POML side and does `if (-not $idx.ContainsKey($id)) { continue }` (`check-task-status-drift.ps1:135`), so an index row with no POML is never visited. ⚠️ **A parser decision comes first**: the row regex matches **119 lines for 111 distinct ids** — eight ids match twice (036, 064, 082, 088, 093, 094, 095, 107) from the dependency and accuracy-audit tables, and `$map[$id] = …` keeps the **last** hit, whose marker `**` reads as *open*. Green today **by luck, not construction** | — | none | — | ❌ | sonnet | high |
