...
| 🔲 [open] **093** | **Reorder the 7 Create wizards** — collect IsSecure → create record → provision-if-secure → upload → link | 3 | **FILED 2026-09-03.** The reorder is what closes the REAL isolation gap: today a secure Matter created WITH documents puts them in the shared BU container, because at upload time no matter exists yet to be secure — server-side container derivation alone cannot reach it. 🔴 **MUST NOT author a new Secure Project UI**: it exists in TWO implementations (`CreateProjectWizard/SecureProjectSection.tsx` + the LegalWorkspace copy) and pending task **068** already owns reworking it; this task owns only MOVING the IsSecure collection point earlier (owner: before the Info step) and REUSING the existing component. Which of the two copies is live per wizard is an open question to SURFACE, not silently pick — editing the wrong one produces a fix that never renders. ⚠️ Provisioning REQUIRES the record to exist (task 008), so IsSecure is *collected* early and *acted on* after creation — the order cannot be inverted; this is also why abandoning the wizard orphans no container. `provision-project` is **project-only** (owner: matters later). Safe w.r.t. AI pre-fill: pre-fill uploads to a server-configured STAGING container and the final upload is a separate call. 🔴 **SHIP-TOGETHER** client+BFF | **076** | ❌ | **opus** | xhigh |
