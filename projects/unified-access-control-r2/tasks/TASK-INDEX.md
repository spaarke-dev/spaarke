...
| 🔲 [open] 117 | **ONE scheduled reconciliation job** so a row's own `statecode`/`sprk_expiresdate` IS the truth — D-1 option **B** + ISS-026 (**#1006**) write half + ISS-020 (**#999**) part 4, consolidated because all three need the identical mechanism. Three rules: stamp undated grants (reusing `ExternalGrantLifecycle.DefaultExpiry`, **not** a second `90`), deactivate grants under an inactive organization, deactivate date-ended memberships — `statecode=1` **and** `statuscode=2`. Reads via `IGenericEntityService` (paged) because `DataverseWebApiClient.QueryAsync` reads ONE page and never reads the `@odata.nextLink` it declares (`:227-246`, `:296-297`); writes via `BulkUpdateAsync`. 🔴 **Ships DISABLED with report-only mode — R2/R3 REMOVE live access** | ISS-026 / ISS-020 | 103 | — | ❌ | **opus** | high |
