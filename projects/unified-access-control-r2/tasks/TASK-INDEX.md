...
| 🔲 [open] 044 | Unified-evaluator seam suite (Phase 1–2 contract) | FR-19…25 | 039,041–043 | **P2-A** | ✅ | sonnet | high |
...
| 🔲 [open] 119 | The Create To Do wizard **silently discards uploaded files** (ISS-027 / **#1007**) — it shows a files step, captions it *"Upload documents to associate with this to do"* (`TodoWizardDialog.tsx:178`), and its `onFinish` never reads `context.uploadedFiles`. Fix: `createTodo` → `uploadFilesToSpe('sprk_todo', todoId, files)` → `createDocumentRecords`; create-first is **compiler-enforced** (`EntityCreationService.ts:467-484`). Link column is **`sprk_relatedtodo`** (`Models.cs:214`) — check the `sprk_related*` family, never a bare `sprk_todo` column (three prior records got this wrong). Partial failure is a **warning**, never a discarded to-do. ⚠️ The **409 "No storage container is configured"** path is live-reachable: 3 of 6 BUs have `sprk_containerid` unset | — | none | — | ❌ | sonnet | high |
