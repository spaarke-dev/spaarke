...
| 🔲 [open] **083** | **Container-selection authorization sweep** — every SPE write path must have the SERVER choose the container from a record it authorized | 9 | 🔴 **OWNER-DIRECTED 2026-08-27.** One defect class, ≥5 instances, **2 LIVE** (`PUT /api/drives/{driveId}/upload` is **app-only MI**, `DELETE /api/drives/{driveId}/items/{itemId}` is a **destroy**) + 2 **UNTRACED** + Compose (#858, behind PR #806). Owner rejected offloading: acceptance criteria FORBID handing any row to another project | **075, 076** | ❌ | **fable** | **xhigh** | ⚠️ **2026-09-02 — MUST NOT BE CLOSED YET, verified against the build-enforced guard.** A parse of `SpeWriteSinkContainerProvenanceGuardTests.AllowList` reports **7 sinks still `ClientSupplied`**: `Api/DocumentsEndpoints.cs` UploadSmallAsync#1 + DeleteFileAsync#1, `Api/OBOEndpoints.cs` UploadSmallAsUserAsync#1, `ComposeService` ReplaceFileContentAsUserAsync#1, and `ComposeSaveStorageCoordinator` ReplaceFileContentAsUserAsync#1/#2/#3 (the Compose REPLACE trio — #858 converted only the MINT). Totals: 7 ClientSupplied / 14 ServerDerivedRecord / 5 ServerDerivedConfig / 3 AdministrativeRoleScoped / 1 Dead. The OBO row is gated on task **076**, so 083 cannot close before 076 executes. A session handoff listed "close 083" as bookkeeping — that was **wrong**, and closing it would have marked a security sweep complete with live client-supplied container sinks in it.
