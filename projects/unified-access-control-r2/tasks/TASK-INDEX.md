...
| ✅ 041 | **Access-conferring column registry (contact + org).** `ConventionPrefix` retired — conferral is registry-membership only, so a RENAME can no longer grant/revoke. 48 seed entries enumerated from LIVE metadata; 9 org-typed. Found + reported a cross-boundary test failure (raw unseeded `MembershipOptions` → empty registry → fail-closed), fixed in main session. Notes: [`notes/task-041-access-conferring-registry-seed.md`](../notes/task-041-access-conferring-registry-seed.md) | FR-24 | 040 | **P2-A** | ✅ | sonnet | high |
