...
| 🔲 [open] 065 | `AccessGrantModal` "+ User" picker — 🟡 **CLIENT HALF DONE 2026-09-21 (Wave 1), TASK STAYS OPEN FOR M2.** Shipped: the system-user share path + `pickUser`/`fetchSecureOwnerInfo` wired through `TrackingFieldTrio`; **16 new tests, 42/42**; PCF **1.0.29→1.0.30** across all four locations; gates 0 Critical / 0 violations (file-size warning acted on — `buildGrantBatchNotice` extracted). **M8 done**: `postJson`/`getJson` now check `res.ok` and throw a structured error instead of reading a failed response as success. Owner’s 2026-09-10 message rule met client-side (fully-revoked / SPE-may-remain / nothing-revoked, `deactivatedCount` never discarded) from fields `RevokeAccessResponse` already returns. 🔴 **REMAINING — M2**: align `/revoke`’s HTTP status with `/close-project`’s on an SPE failure. Needs `RevokeExternalAccessEndpoint.cs`, outside the agent’s lane, so the agent correctly STOPPED rather than editing it. ⚠️ Must be reconciled with the standing owner rule *“a failed revoke gives a message, not a bare 500”* before any status change | FR-29 | **063** | **P4-W4** | ✅ | sonnet | high |
