...
| 🔲 **085** | **`POST /api/office/save` — server-derive the container**; delete `SaveRequest.ContainerId` | 2 | **Row 9 of 083's census.** The route IS gated — `.AddEntityAccessFilter()` authorizes against `SaveRequest.TargetEntity` — but the bytes go to `SaveRequest.ContainerId`, **a different client-supplied field on the same body**. Authorized against one thing, writing into another: **option (B)**, which 083 explicitly rejected, live in shipped code on an **app-only MI write** (no SPE ACL backstop). Compounding: `TargetEntity` is *optional* and `EntityAccessFilter.cs:148-159` calls `next()` when it is absent, so a save can run on baseline Office auth alone. The client-supplied value also **persists** into the ProcessingJob payload (`OfficeService.cs:169`). ⚠️ `RouteAuthorizationGuardTests` scores this route **gated** — which is exactly why 083's second guard asks *where did the container id come from?* rather than *is this route gated?*. No shipped client sends `ContainerId`, so **the hole is the contract, not the traffic**. Owner model (§6.5 Q1/Q5): record → its container; no record → **acting user's BU**, server-side; returning the chosen `driveId` in the response is required, accepting one is the vulnerability. ⚠️ Shares `EntityAccessFilter.EntitySetByType` with the Q4 widening — re-verify after it lands | **076, 083** | ❌ | **opus** | high |
