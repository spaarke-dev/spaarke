...
| 🔲 [open] 118 | Make the Manage Access gate ask **the server's question**: re-point `computeCanGrantAccess` off table-level `hasEntityPrivilege(…Create, Global)` onto **Write-on-record** (FR-07, `DelegationRuleFilter`), and **fail CLOSED** — today it `return true`s at `index.ts:472`. D-1 option **C**. 🔴 **Binding order: code before config.** Removing `Create` from `SDAP User (Core)` while the client still gates on it hides Manage Access for **every** user while the BFF's app-only write keeps working. Adds a caller-rights endpoint (no client-callable one exists; `CallerRecordAccessProbe` is server-only). ⚠️ The fail-open's stated reason is **false** — `index.ts:459-460` and `types.ts:144` cite a "SECOND, defense-in-depth gate" that reads **the same prop**. Supersedes `code-quality-and-assurance-r3` pcf-controls **D7-01** | FR-07 / FR-29 | none | — | ❌ | **opus** | high |
