...
if (-not $SkipCveCheck) {
    ...
    $highLines = $cveOutput -split "`n" | Where-Object { $_ -match '\bHigh\b' }
    if ($highLines.Count -gt 0) {
        $cveSummary = "HIGH SEVERITY FOUND ($($highLines.Count) line(s)) -- review before merge"
        Write-Host "  $cveSummary" -ForegroundColor Yellow
        $highLines | ForEach-Object { Write-Host "    $_" }
    } else {
        $cveSummary = 'No new HIGH-severity vulnerabilities found'
...
if ($classifications.Count -eq 0) {
    [void]$classifications.Add('OK: within all ADR-029 / NFR-01 thresholds.')
}
...
$reportLine = "BFF Hygiene section 10 + NFR-01 verified: publish size = $compressedInclPdbMb MB (incl. PDBs; $compressedExclPdbMb MB excl.), delta = $deltaStr MB vs prior ($priorSource), CVE check: $cveSummary."
