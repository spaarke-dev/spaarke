...
- **PE-D7 — AuditLog full-suite parallelism flake (from compose-r2 handoff).** *Failing behavior:* `AuditLogServiceTests.LogInteractionAsync_PartitionsByTenantId` passes in isolation but fails under full-suite parallelism (shared-state/ordering race in the test, not the product). Pre-existing, unrelated to compose/E-20. *Owner/path:* core test-hygiene backlog — isolate the shared state or mark the test non-parallel. *Severity:* Low (test-only flake; no product defect).
