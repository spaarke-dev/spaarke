...
> **Status**: DRAFT v0.2 — 2026-07-07, for operator review → `/design-to-spec` (v0.1 superseded same day per operator direction 2026-07-07: Resourcefulness Doctrine added as D-F0; delivery re-cut to platform-core + satellites; industry grounding added; backlog + open questions refreshed against G-P3 rounds 4–6)
...
| **Daily Briefing hallucination remediation** | **Immediate FIX WAVE, not a project** — grounding/citation repair on the BRIEF-NARRATE prompts + collector | Runs as enumerated fix-wave tasks at r2 entry (default: core Wave 0); no new architecture; uses the r1 catalog-data + eval-case fix pattern proven across G-P3 |
...
**Decision**: Confirmation becomes a deterministic **gate-engine policy over (risk tier × request origin × argument completeness)**, replacing blanket declared-class gating:

| Request origin | Args | Risk tier (assessment model) | Behavior |
|---|---|---|---|
| **User-explicit** (typed/clicked request naming the action) | Complete | Tier 0 Read / Tier 1 Draft / Tier 2 Create-records | **EXECUTE immediately** — no dialog, no chat re-ask; ✅ + record link on completion |
| User-explicit | Incomplete | any | ONE elicitation turn (existing 032 machinery), then execute per row above |
| **Inferred / ambiguous / model-initiated** | any | Tier ≥ 2 | Confirm — **ONE dialog** (`ActionConfirmationDialog`), never a chat-loop re-ask |
| **Injection-suspect** (`dispatchUncertain`, content-safety flags, untrusted-doc-origin) | any | any write | Confirm via dialog + suspicion surfaced in the dialog copy |
...
**Mechanism sketch** (coarse): request-origin determined deterministically — Click path always user-explicit by construction; Text path marks origin from turn structure (user's utterance names the capability's action verb + invocation in that same turn ⇒ explicit; model-initiated calls in later turns or from document content ⇒ inferred); fail-closed default to *inferred*. Confirmation state is a Gate-ledger property (ADR-040 `Gate` status transitions). Undo affordance for Tier-2 auto-executes: the ✅ card carries an "Undo" chip where the tool declares a compensating action. **Scope addition (R5-E residual)**: gate **pre-suspend validation** — run the handler's `ValidateChat` BEFORE suspending into a dialog, so a doomed call renders an honest ❌ (with D-F0(d) affordance) instead of Confirm→❌ (§10 row 16).
...
Memory items are **structured objects, not embeddings**, each carrying the full governance envelope: `source, owner, confidence, scope, expiration, sensitivity, deletion_policy, created, updated`. Storage: Cosmos (assessment ruling — Dataverse stays business records).
...
**Decision**: Memory writes go through the SAME closed-catalog machinery as every other side effect: a `memory.write` typed tool with declared `side_effect_class`, subject to Policy v2 (explicit "remember that I prefer X" ⇒ execute + ✅; model-inferred capture ⇒ lightweight confirm or queue-for-review). **Untrusted content (uploaded-document text, tool results) can NEVER originate a memory write.** Memory reads surface provenance; users get a view-and-delete surface (G-R2-B). Eval-suite growth: memory-poisoning injection families. (Unchanged from v0.1.)
...
| 10 | **Test debt**: AnalysisWorkspace jest ESM (r1 /defer) + 3 SpaarkeAi failing suites (round-3) + **8 AI.Widgets failing suites (round-4, verified pre-existing at HEAD via git-stash A/B)** | AnalysisWorkspace debt clears via retire-at-parity (Compose r2, D-C5); SpaarkeAi + AI.Widgets suites = test-repair task in core Phase A |
...
3. **Policy v2 auto-execute tier line (D-F1)**: Tier ≤2 (create records) auto-executes on explicit-complete requests with an Undo chip — or Tiers 0–1 only at first? *Recommend: Tier ≤2 — it is the exact friction named, twice (charter + round-5 re-confirmation); Undo covers the risk.*
...
- **D-F0 as the FIRST wave** (doctrine block + directive-pin audit/fold-in + refusal-affordance plumbing + resourcefulness eval family), followed by the seam-publication tasks (§3.1) that unblock Compose r2.
...
- New NFRs: memory governance envelope completeness (all 9 fields); user memory view/delete surface; UI-action ack coverage for every UI-claiming tool; OutcomeCard coverage for every side-effect path; **refusal-affordance coverage — no refusal/block ships without an actionable affordance**; resourcefulness eval family as merge gate.
...
*DRAFT v0.2 for operator review. On approval (and §12 answers), run `/design-to-spec projects/spaarke-ai-architecture-redesign-r2`.*
