...
Overlays (precedence order): **injection-suspect always wins** (`dispatchUncertain`, content-safety flags, untrusted-doc-origin ⇒ dialog + suspicion surfaced) → incomplete args ⇒ ONE elicitation turn (existing 032 machinery) then re-evaluate → origin (explicit/inferred) → tier row. Inferred/model-initiated at Tier ≥ 2 always confirms — ONE dialog (`ActionConfirmationDialog`), never a chat-loop re-ask.
