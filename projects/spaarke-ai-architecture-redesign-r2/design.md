...
### 0.3 v0.4 correction round (2026-07-08, operator-ratified)

Two operator corrections landed after v0.3, both about **Compose** — the v0.1–v0.3 charter described the Compose satellite from stale information. Corrected here so `/design-to-spec` does not inherit the error:

- **C-1 (Compose r1 factual correction)**: the charter (D-C4, §8, former §12 Q1) asserted `spaarkeai-compose-r1` was *"never started"* and should be *absorbed/archived* with its *"spikes becoming Compose r2 Phase 0."* **This is wrong.** `spaarkeai-compose-r1` was a **full project, fully executed, and is now closed** (code on master via PRs #534/#538/#544; the R1 AI-dispatch endpoint was retired in #544). It is NOT absorbed and NOT archived-as-vestigial.
- **C-2 (Compose r2 is a separate, already-designed, parallel project)**: `spaarkeai-compose-r2` **already exists with its own mature design** (`projects/spaarkeai-compose-r2/design.md`, 2026-07-03) and is being set up to run **in parallel with this core**. The charter's framing of §8 as a *"handoff package that seeds Compose r2's design.md"* is therefore void — Compose r2 has its own baseline. §8 is reframed (below) as the **seam-obligation + coordination record** the core owes the existing Compose project line (r1 shipped/closed; r2 active with its own design; r3 scaffolding). **Coordination between this core and Compose r2 is CRITICAL** (both declare BFF=Y + SpaarkeAi=Y; Risk row 4).
- **C-3 (seam ruling — core keeps the FULL seam set)**: **Operator ruling — the core keeps the full seam set**: `compose` disposition member + OutcomeCard + ContextEnvelope + Policy v2 tiers + ledger provenance (§3.1); Compose consumes these, never forks a parallel AI-dispatch path. **Status (verified 2026-07-08): Compose r2 has ALREADY re-based onto these seams** in its own 2026-07-08 alignment revision (`projects/spaarkeai-compose-r2/design.md`) — dispatch = Action + Binding resolved via `IConsumerRoutingService` (the canonical ADR-039 Binding resolver) inside the shipped session-dispatch seam; draft-into-editor is ledger-first via the core's `compose` disposition; it consumes `ComposeDisposition v1`/`OutcomeCard v1`/`JobAwareCompletionState v1`/`TraceEvent v1`/`GateDecision v2` as **core A0 contracts, "never forked"**, with the ADR-013 facade boundary enforced as a Tier-1 CI-blocking NetArchTest rule. The prior worry (a LinearConsumer-only parallel path) is **retracted in Compose r2's §7 as "wrong… binding platform law"**: `ActionRunner`/`PromptSchemaRenderer` ("LinearConsumers") survives only as the *prompted executor reached through a Binding*, never by string action key. **Standing obligation** is therefore alignment-maintenance + PR-time verification (hot-path registry + `/conflict-check`), NOT a from-scratch re-base; the core still publishes the seams in Phase A FIRST so Compose r2 is never blocked on unstabilized contracts. *(Note: the redesign-r2 branch's checked-out copy of Compose r2's design lags at the 2026-07-03 revision until the compose-r2 branch merges — read the compose-r2 worktree for ground truth.)*
- **§12 answers recorded**: Q1 CLOSED (keep separate — per C-1/C-2) · Q2 = freeze + retire-at-parity · Q4 = Cosmos · Q5 = provider interface **+ researcher spike**. See §12.
...
|---|---|---|
| **Compose r2** — the operator's #1 feature | **Separate project with its OWN mature design** (`projects/spaarkeai-compose-r2/design.md`, 2026-07-08 alignment revision); runs in **parallel from day one** in its own worktree (built on the closed `spaarkeai-compose-r1`, NOT seeded by this charter — see §0.3 C-1/C-2) | **Consumes the core's FULL seam set** (per §0.3 C-3 ruling: `compose` disposition member, dispatch, ledger provenance, Policy v2 tiers, OutcomeCard, ContextEnvelope) — **already re-based** onto Action + Binding via `IConsumerRoutingService` + the `compose` disposition; consumes the A0 contracts without forking (ADR-013 facade enforced by a Tier-1 CI rule). §8 records the seam obligations + the full-ingestion-parity capability it implements. Carries its own browser-UAT gate — **the former G-R2-C lives in that project**. |
| **Daily Briefing hallucination remediation** | **v0.4 (operator ruling 2026-07-08): handled in a SEPARATE project, NOT core Wave 0** — this core spec notates it as a downstream consumer of the GroundednessCheck threshold→action pattern but enumerates no Briefing tasks. (Original v0.3 framing retained below for context.) Grounding/citation repair on the BRIEF-NARRATE prompts + collector. **v0.3 (AI-ARCHITECTURE assessment rec 4)**: the fix wave also installs the PERMANENT mechanism — `GroundednessCheckService` gains a **policy, not just a score**: below threshold X the rendered output carries a visible grounding warning; below Y the disposition routes to a review state instead of rendering as authoritative; low-groundedness events feed the eval suite as regression candidates. Annotation-without-consequence was how briefing hallucinations reached users despite the service existing. | Runs as enumerated fix-wave tasks at r2 entry (default: core Wave 0); no new architecture beyond the threshold→action policy; uses the r1 catalog-data + eval-case fix pattern proven across G-P3 |
...
## 8. Compose r2 — seam obligations + coordination record (Area 3 lives in the Compose r2 project)

Compose remains the operator's **#1 most important feature** — which is exactly why it ships as its **own parallel project** (§3.2). **v0.4 correction (§0.3 C-1/C-2)**: Compose r2 is NOT seeded by this charter — it **already has its own mature design** (`projects/spaarkeai-compose-r2/design.md`, 2026-07-03), built on the closed `spaarkeai-compose-r1`. This section is therefore **not a "handoff that populates Compose r2's design"** (the v0.1–v0.3 framing, now void); it is the **record of the seams the core owes Compose r2 and the cross-project coordination obligations both ways**. **This project does not implement the editor/lifecycle** — it publishes the seams (§3.1) and enforces the ingestion-parity invariant.

**Binding coordination obligation (§0.3 C-3)**: Per operator ruling the **core keeps the full seam set** (`compose` disposition member, OutcomeCard, ContextEnvelope, Policy v2 tiers, ledger provenance) and Compose consumes them without forking. **Already satisfied**: Compose r2's 2026-07-08 alignment revision re-based its AI dispatch onto Action + Binding via `IConsumerRoutingService` (canonical ADR-039 resolver) inside the shipped session-dispatch seam, made draft-into-editor ledger-first via the `compose` disposition, and consumes the A0 contracts "never forked" (ADR-013 facade enforced by a Tier-1 CI-blocking NetArchTest rule). "LinearConsumers" (`ActionRunner`/`PromptSchemaRenderer`) survive only as the prompted executor reached *through* a Binding. The standing obligation is alignment-maintenance + PR-time verification via `/conflict-check` + the hot-path registry; the core publishes the seams in Phase A FIRST.

**Reference decisions the Compose r2 design owns** (the D-C series below is retained for continuity + traceability; the authoritative version now lives in Compose r2's own design.md — full v0.1 text preserved in git history at commit `14e0c8762`):
...
- **D-C4 — `spaarkeai-compose-r1` disposition** (**CORRECTED v0.4, §0.3 C-1**): `spaarkeai-compose-r1` was a **full project, fully executed, now CLOSED** (code on master via PRs #534/#538/#544). It is **kept separate** — NOT absorbed, NOT archived-as-never-started (the prior "never started / spikes become Compose r2 Phase 0" text was factually wrong). Compose r2 is its **successor**, building on r1's shipped foundation with its own design; r1's AI-dispatch vocabulary (`IConsumerRoutingService`/`IInvokePlaybookAi`/consumer types) was already retired in Compose r1 PR #544 and by the redesign-r1 platform work. §12 Q1 CLOSED = keep separate.
...
| 2 | **Document-creation capability** — bar raised by R5-E to **FULL ingestion parity** (SPE storage + document profile analysis + indexing — everything the Document Upload wizard drives; a bare `sprk_document` row is the defect, not the feature) | **Implemented by the existing Compose r2 project** (§8 save-back/creation leg — NOT transferred/seeded by this charter, per §0.3 C-1/C-2); **the core owns + verifies the ingestion-parity invariant** (§8 R-2) via gate policy + `JobAwareCompletionState` |
| 3 | **Compose document pre-seeding** — **SHIPPED** in the G-P3 round-4 fix wave (R4-2) for real `sprk_document` rows | Residual (session-uploaded-file pre-seed) owned by Compose r2 with row 2 |
...
1. ~~**Absorb-compose-r1 formality (D-C4)**~~ — **CLOSED 2026-07-08 (operator, §0.3 C-1/C-2): KEEP SEPARATE.** `spaarkeai-compose-r1` was a full project, fully executed, now closed — NOT absorbed, NOT archived-as-never-started. Compose r2 is its successor, already a separate active project with its own design. The prior "absorb + archive" recommendation was based on the wrong premise that r1 never started.
2. **AnalysisWorkspace retire-at-parity (D-C5)** — **CLOSED 2026-07-08 (operator): YES** — freeze-now / retire-at-Compose-parity. Core enforces the freeze (catalog governance); Compose r2 executes the hard-cutover deletion at parity.
...
4. **Cosmos memory store (D-M1)** — **CLOSED 2026-07-08 (operator): YES** — new Cosmos container in the existing account for user/workspace memory; nothing memory-shaped in Dataverse.
5. **Work IQ / Foundry IQ research spike** — **CLOSED 2026-07-08 (operator): interface + spike** — define the provider interface inside the core (shapes D-M1) AND run the researcher-subagent spike on Work IQ / Foundry IQ. (Runtime integration stays out of scope per §9.)
...
- ~~**Daily Briefing hallucination remediation as the entry fix wave** (Wave 0)~~ — **v0.4: routed to a SEPARATE project (operator ruling 2026-07-08)**; NOT a core wave. The core spec references it as a downstream consumer of the GroundednessCheck threshold→action pattern only; no Briefing tasks in this project.
...
*DRAFT v0.4 — Compose framing corrected + §12 Q1/Q2/Q4/Q5 answered (§0.3), 2026-07-08. All open questions CLOSED (Q1 keep-separate, Q2 freeze+retire, Q3 sub-tier split, Q4 Cosmos, Q5 interface+spike). The compose-r2 cross-project seam obligation (§0.3 C-3) is already satisfied — Compose r2's 2026-07-08 alignment revision consumes the core seams without forking. Both pre-spec notes are AUTHORED (2026-07-08): [`notes/d-f0-eval-family-spec.md`](notes/d-f0-eval-family-spec.md) + [`notes/policy-v2-origin-classification-decision-tree.md`](notes/policy-v2-origin-classification-decision-tree.md). **All pre-`/design-to-spec` gates cleared** — ready to run `/design-to-spec projects/spaarke-ai-architecture-redesign-r2`.*
