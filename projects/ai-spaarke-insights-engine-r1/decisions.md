...
- **Do not** create five `insight-*` indexes in Phase 1 (D-53 revised 2026-05-28 — one `insights-index` with `artifactType` discriminator suffices; revisit splitting if Phase 1.5+ extraction layers produce signals that don't share the schema)
...
- **Do not** persist Observations below the per-field confidence threshold (D-63 — fields below threshold are dropped; may be logged for review but must not pollute `insights-index` cohorts that synthesis queries)
