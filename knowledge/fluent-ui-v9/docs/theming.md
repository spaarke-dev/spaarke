...
fetched: 2026-05-26
summary: Full theming reference — FluentProvider, tokens-not-CSS-vars rule, 5 built-in themes, custom brand ramps, overriding tokens, extending with new tokens.
loadWhen: deep drill-down on theme construction OR designing a Spaarke brand theme. Routine theming work covered by patterns/ui/fluent-v9-theming.md.
---
...
You can browse all the available tokens in the **Theme** section of the docs.
...
Place a `<FluentProvider />` at the root of your app and pass a theme to the `theme` prop. The provider will render a `div` and set all tokens as CSS variables on that element. The provider also propagates CSS variables to React portals created with the `Portal` component.
...
For more details, see [styling-griffel.md](./styling-griffel.md).

### ⚠ Do not use CSS variables directly

**Never use theme CSS variables directly.** The CSS variables implementation of the theme is internal to the library. The names may change, be hashed, or even be replaced with direct values for some variables. Always use the `tokens` to access the theme.

## Available themes

Fluent UI currently exports the following themes:
...
The brand ramp is a color ramp going from dark to light colors. A theme is derived from a brand ramp. To use a theme with a custom brand ramp, instead of importing a predefined theme, you can use theme factory functions:
...
> ⚠ Adding more tokens adds more CSS variables which can affect run-time performance as each DOM node carries all the tokens.
...
// Note: If you do it via themeToTokensObject you might see a negative effect on tree-shaking since bundles won't know the shape of the output.
