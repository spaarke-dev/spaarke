...
**Decision recommended**: Phase 2b should accept **either** `## Gotchas` **or** `## Failure Modes to Avoid` (or `## Failure Modes & Recovery`) as canonical-equivalent headings. The Failure Modes framing is structurally better (prescriptive recovery, not just warnings). Update spec F-15 to allow either heading.
...
| 12 | `dataverse-create-schema` | 777 | 4 | **SPLIT** | **needs-substantive-rewrite** | **3 of 4 reference scripts MISSING (archived).** Lowest cross-ref count in batch despite largest size. Possibly already superseded by direct MCP tools. **Human review needed**: rewrite vs archive vs merge into MCP-tool guidance. Do NOT bulk-refine | ☐ |
...
| 23 | `jps-action-create` | 282 | 17 | O-T | **needs-substantive-rewrite** | **5 broken paths to renamed `x-ai-json-prompt-schema-system/`**. Step 6 duplicated. Per F-15.3 do NOT bulk-refine — dedicated session needed | ☐ |
...
The audit surfaced 4 questions where the recommended action is conditional on human judgment:

1. **`add-reference-to-index` orphan**: 0 inbound references. Audit recommends `refine-in-place` (keep) because the workflow is genuinely useful even if low-frequency. Reviewer can override → `remove` if the RAG-index seeding workflow is no longer expected.

2. **`dataverse-create-schema` rewrite vs archive**: 3 of 4 reference scripts missing AND only 4 inbound references AND possibly superseded by direct MCP tools (per root CLAUDE.md MCP integration section). Audit cannot decide; reviewer call needed.

3. **`jps-playbook-audit` usage check**: 3 inbound refs, no see_also from other skills. Audit recommends `refine-in-place` BUT flags an optional usage-audit — if no developer has invoked this skill since creation, consider archival under NF-1 (reversibility).

4. **Canonical "failure modes" heading**: Audit recommends accepting **either** `## Gotchas` OR `## Failure Modes to Avoid`/`## Failure Modes & Recovery` as canonical-equivalent. Reviewer can endorse this or mandate one form.
...
| Override notes (if any) | _________________________________ |
| Special handling for Open Questions §6 (1-4) | _________________________________ |

After sign-off, the project advances to Phase 2b (skills refinement) per the wave sequencing in §5.
