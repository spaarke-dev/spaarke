...
| Auth bootstrap fails silently — page renders but `useAuth` returns null | Code Page missing `await initAuth({...})` from `@spaarke/auth` before `createRoot(...).render(...)` (v2 contract per [ADR-028](../../adr/ADR-028-spaarke-auth-architecture.md)) | Follow the v2 contract: `await initAuth({clientId, tenantId, bffBaseUrl, bffApiScope})` → `createRoot(...).render(...)`. The `DocumentRelationshipViewer` Code Page exemplar shows this verbatim. See [`.claude/patterns/auth/spaarke-sso-binding.md`](../../patterns/auth/spaarke-sso-binding.md) for INV-1..INV-8 and [`docs/guides/auth-deployment-setup.md`](../../../docs/guides/auth-deployment-setup.md) for env-var setup. |
