...
| Auth bootstrap fails silently — page renders but `useAuth` returns null | Code Page missing the `@spaarke/auth` resolve+set+ensure sequence before render. See [.claude/patterns/auth/DEPRECATED-spaarke-auth-initialization.md](../../patterns/auth/DEPRECATED-spaarke-auth-initialization.md) (⛔ deprecated — Spaarke Auth v2 supersedes; consult [AUDIT-FINDINGS-AUTH-SYSTEM.md](../../AUDIT-FINDINGS-AUTH-SYSTEM.md) for current `useAuth()` flow) | Follow the canonical bootstrap: `resolveRuntimeConfig()` → `setRuntimeConfig()` → `ensureAuthInitialized()` → `createRoot(...).render()`. The `DocumentRelationshipViewer` exemplar shows this verbatim. |
