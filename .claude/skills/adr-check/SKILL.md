...
| ADR-028 **A4** | Secret-free confidential credential | `.WithClientSecret(...)` on any BFF-identity client (use MI-FIC assertion or KV certificate); credentials constructed per call site instead of via the shared provider; `IConfidentialClientApplication` built per-request instead of singleton-cached. **Cite exception E-3** (do not report as a violation) for the transitional sites it enumerates; **still flag any NEW site** — E-3 does not license expansion. `DefaultAzureCredential` is **not** a valid fix on OBO paths — it cannot perform the exchange. |
