...
> **Exemplar rationale**: `projects/ai-procedure-quality-r1/spec.md` was generated by this skill from `design.md` — 20 FRs across 7 phases with cross-cutting NFRs, exactly the prescribed structure. A live, recently-validated reference output.
>
> **Phase 2a audit note**: This skill has ~80-100 lines of redundancy from an embedded spec.md template (lines ~570-660). Future improvement is to extract that template to a separate `.tmpl` file. Not applied in Wave 2b-A — flagged as candidate content-extraction for follow-up.
...
1. RESOURCE TYPES → ADR CONSTRAINTS
   ...
   - Record invariants / create-update paths / any plugin idea → Reference ADR-002 (no plugins; server-side write path WP-1…WP-8)
...
|---|---|---|
| Generated spec.md is too vague — agent can't decompose it into tasks | Source design.md left ambiguities that the skill didn't surface as "Unresolved Questions" | Always include an "Unresolved Questions" section in the generated spec. Force the human to answer them BEFORE running `/project-pipeline`. Vague spec → vague plan → flailing tasks. |
| Spec includes implementation details copied from design.md (e.g., specific class names, file paths) | Author of design.md anticipated implementation; skill preserved verbatim | Spec should be technology-neutral where possible. Move concrete file paths and class names to the `plan.md` (which is generated post-spec by `/project-pipeline`). |
| Original design.md gets lost or overwritten | Author opens design.md, generates spec, then accidentally edits or deletes the design | Preserve `design.md` verbatim as a project artifact. Acceptance criteria explicitly require this. If lost, retrieve from git history. |
| Spec lists requirements that contradict an existing ADR | Author of design didn't know about the ADR; skill didn't load adr-aware | Run `/adr-check` against the generated spec before publishing. If conflict, EITHER amend the spec to align OR open an RFC to update the ADR. Don't proceed silently. |
