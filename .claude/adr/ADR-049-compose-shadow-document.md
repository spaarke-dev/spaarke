...
**Full reasoning**: `projects/spaarkeai-compose-fidelity-r4.5/{design,spec}.md` + `notes/` (WS-1..WS-5, incl. the numbering-engine + citation-resolver notes and the WS-5 pagination decision). Consumer wiring of `CitationResolver` (review-note citations) continues in `ai-advanced-capabilities-agreements-r1` (see its inbound handoff note).
