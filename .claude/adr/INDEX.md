...
| ADR-046 | ACS Messaging Channel — transport, threads, ingestor seam | 🔒 **RESERVED / PLACEHOLDER** (2026-07-16, `messaging-communication-app-r1`) — ACS Chat as transport + Dataverse `sprk_communication` (type=`Message`=100000004) as record; server-side token minting; new `ICommunicationChannelIngestor` seam over ADR-045; first-class `sprk_communicationthread` + `IThreadResolver` (email reply-chains + chat); access via ADR-034 (open) + record-sharing grant (private); retention-minimized per-customer ACS resource. Full ADR authored during the project. Sibling ADR-047 to be claimed by `notification-spine-r1`. **Do not reuse 046.** | Placeholder (2026-07-16) |
