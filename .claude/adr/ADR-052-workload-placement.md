...
> **Last Updated**: 2026-09-15 (conditional Dataverse impersonation, owner-accepted; §5 MUST NOT narrowed to the caller's token)
...
- **MUST NOT** let the caller's **token** (OBO, user tokens) leave the BFF request that holds it. Acting for a user outside that request is only the conditional impersonation in the Identity row below (ADR-028 A5)
...
| Identity | **Reuse the stamp's user-assigned managed identity**, app-only (`DefaultAzureCredential` pinned to its client ID) — ADR-028 A4's **app-only** row: the BFF's managed-identity access, no new grants, no user sign-in. **MUST NOT** use A4's **confidential-client** row (no MSAL confidential client, no client-assertion or certificate credential, no OBO, no user tokens) and **MUST NOT** call BFF endpoints. **MAY impersonate a Dataverse user only under ADR-028 A5's conditions** (owner-accepted 2026-09-15): for work that user started through an authenticated BFF request — never timer, webhook or system-triggered work; the caller id from a typed requester field the BFF writes from the validated token (#989), on an Entra-only channel only the stamp identity can write (#988) — never from a client payload, webhook body or Dataverse data; the impersonated user is the one the output is delivered or attributed to; only through the shared fail-closed `Spaarke.Dataverse` helper (#990). **Not usable until #988–#990 land.** No secrets — including identity-based host storage. Dedicated identity only for isolation or for access bound to another app registration (e.g. CIAM Graph, Power BI), with owner approval |
...
| No new hand-rolled timer `BackgroundService` (ratchet) · `IScheduledJob` host-neutrality · Functions projects under `src/server/functions/`, not referencing `Sprk.Bff.Api`, no confidential-client / assertion / OBO types, impersonation only through the shared helper | `WorkloadPlacementGuardTests` |
...
| [ADR-028](ADR-028-spaarke-auth-architecture.md) | A4: the app-only row a Function uses; the confidential-client row it must not. A5: the conditional impersonation (scope extended 2026-09-15 to BFF-initiated jobs) |
